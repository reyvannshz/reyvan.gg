...
local API_URL = "https://majesty-obfuscate-v4-production.up.railway.app"
local KEY = _G.KEY

if not KEY or KEY == "" then
    player:Kick("Key tidak ditemukan! Pakai script dari bot Discord.")
    return
end
...
local ok, response = pcall(function()
    return HttpService:JSONDecode(
        game:HttpGet(API_URL .. "/verify?key=" .. KEY .. "&hwid=" .. hwid)
    )
end)

if not ok or not response then
    player:Kick("Gagal konek ke server. Coba lagi nanti.")
    return
end

if not response.valid then
    player:Kick("" .. (response.reason or "Key tidak valid!"))
    return
end
...
local patRemotes       = game:GetService("ReplicatedStorage"):WaitForChild("RemoteEvents")
local patStorePurchase = patRemotes:FindFirstChild("StorePurchase")
local patRpcRE         = patRemotes:FindFirstChild("RPC")
