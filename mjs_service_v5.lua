...
-- Coba jual satu item: equip → interact → tunggu konfirmasi inventory turun
local function trySellOne(msName, setStatus2)
    -- Snapshot sebelum aksi jual
    ...
    task.wait(0.4)   -- beri waktu server mengenali equip

    setStatus2("Jual: "..msName.."...", Color3.fromRGB(50,210,110))

    -- Aksi jual: E + fireProximity sebagai backup
    pcall(function()
        VIM:SendKeyEvent(true,  Enum.KeyCode.E, false, game)
        task.wait(0.18)
        VIM:SendKeyEvent(false, Enum.KeyCode.E, false, game)
    ...
    task.wait(0.15)
    -- Second attempt backup (beberapa game butuh double-fire)
    firePromptNearby(CFG.SELL_RADIUS)

    -- Tunggu konfirmasi: inventory MS berkurang
    local elapsed = 0
    local sold    = false
    while elapsed < CFG.SELL_TIMEOUT do
        ...
        -- Re-fire tiap 1 detik jika belum terjual
        if math.floor(elapsed * 10) % 10 == 0 and elapsed > 0.9 then
            firePromptNearby(CFG.SELL_RADIUS)
            pcall(function()
                VIM:SendKeyEvent(true,  Enum.KeyCode.E, false, game)
                task.wait(0.18)
...
print("=== MAJESTY STORE v8.4.0 LOADED ===")
