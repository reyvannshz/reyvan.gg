...
local function onCharacterAdded(newChar)
    if pendingTP then
        ...
        if hrp then task.wait(0.5); hrp.CFrame = target end
    end
end
LocalPlayer.CharacterAdded:Connect(onCharacterAdded)

-- ========== TELEPORT FIX TOTAL ==========
-- Metode: paksa posisi via RenderStepped loop selama ~0.5 detik
-- Ini override server reset karena dijalankan setiap frame render
-- Terbukti bekerja di game dengan anti-teleport server-side
...
local function doTeleport(targetCFrame)
    task.spawn(function()
        ...
        local numWP     = math.max(1, math.ceil(totalDist / 60))

        -- ── FASE 1: perjalanan waypoint ──
        for wp = 1, numWP do
            ...
            local isLast   = (wp == numWP)
            local targetWP = isLast
                and targetCFrame
                or CFrame.new(wPos + Vector3.new(0, 3, 0))

            local lockDone = false
            local conn = RunService.RenderStepped:Connect(function()
                if lockDone then return end
            ...
            if not isLast then task.wait(0.1) end
        end

        -- ── FASE 2: ANCHOR karakter di tujuan (cara paling reliable) ──
        -- Anchored = true mencegah physics engine menarik balik karakter
        char = LocalPlayer.Character
        hrp  = char and char:FindFirstChild("HumanoidRootPart")
        if not hrp then return end

        -- Kumpulkan semua BasePart di karakter, anchor semuanya
        local anchoredParts = {}
        ...
        -- Set posisi tepat sambil di-anchor
        hrp.CFrame = targetCFrame
        pcall(function() hrp.AssemblyLinearVelocity  = Vector3.zero end)
        pcall(function() hrp.AssemblyAngularVelocity = Vector3.zero end)

        -- Tahan anchor 0.8 detik — server "menerima" posisi baru
        task.wait(0.8)

        -- Set ulang posisi sebelum unanchor, biar tidak jatuh
        hrp.CFrame = targetCFrame
        pcall(function() hrp.AssemblyLinearVelocity  = Vector3.zero end)
        pcall(function() hrp.AssemblyAngularVelocity = Vector3.zero end)

        -- Unanchor semua part yang tadi di-anchor
        ...
        -- ── FASE 3: Stabilkan 0.3 detik setelah unanchor ──
        -- Paksa posisi lagi sebentar biar tidak langsung jatuh/terbang
        local stabilDone = false
        local stabilConn = RunService.RenderStepped:Connect(function()
            if stabilDone then return end
            local c2 = LocalPlayer.Character
            local h2 = c2 and c2:FindFirstChild("HumanoidRootPart")
            if not h2 then return end
            h2.CFrame = targetCFrame
            pcall(function() h2.AssemblyLinearVelocity  = Vector3.zero end)
            pcall(function() h2.AssemblyAngularVelocity = Vector3.zero end)
        end)
        task.wait(0.3)
        stabilDone = true
        stabilConn:Disconnect()
    end)
end
...
local function safeTeleport(targetCFrame, statusLabel)
    ...
    pcall(function()
        ...
        if hrp then dist = (hrp.Position - targetCFrame.Position).Magnitude end
    end)
    local numWP   = math.max(1, math.ceil(dist / 60))
    local estTime = numWP * 0.55 + 1.5   -- waypoints + anchor phase + stabilize

    doTeleport(targetCFrame)

    if statusLabel then
        task.delay(estTime, function()
