...
do
    ...
    local sellTogBtn=Instance.new("TextButton"); sellTogBtn.Size=UDim2.new(1,-20,0,36); sellTogBtn.Position=UDim2.new(0,10,0,548)
    sellTogBtn.BackgroundColor3=C.card; sellTogBtn.Text="AUTO SELL : OFF"; sellTogBtn.TextColor3=C.red
    sellTogBtn.Font=Enum.Font.Gotham; sellTogBtn.TextSize=13; sellTogBtn.BorderSizePixel=0; sellTogBtn.Parent=pageAuto
    ...
    local sellStatusLbl_ref=makeLabel(ssc,"OFF",90,0,200,44,13,C.red)
    local sic=makeCard(pageAuto,644,44); makeLabel(sic,"ITEM",12,0,80,44,10,C.subtext)
    local sellItemLbl_ref=makeLabel(sic,"-",90,0,280,44,13,C.text)
...
task.spawn(function()
    while true do
        task.wait(0.3)
        if not autoSell_UI then continue end
        local char = LocalPlayer.Character
        local hum  = char and char:FindFirstChildOfClass("Humanoid")
        ...
        if not targetItem then
            sellItemLbl_ref.Text = "Tidak ada MS Bag"
        ...
        holdItem(targetItem)
        task.wait(0.2)
        -- Cari ProximityPrompt terdekat untuk sell
