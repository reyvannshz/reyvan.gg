...
local function equipTool(name)
    for _att = 1, 8 do
        local ch = LocalPlayer.Character
        if not ch then task.wait(0.3); continue end
        local hum = ch:FindFirstChildOfClass("Humanoid")
        if not hum then task.wait(0.3); continue end
        -- Jika sudah ter-equip, langsung sukses
        if ch:FindFirstChild(name) then return true end
        -- Paksa unequip dulu jika ada SeatPart (duduk di kendaraan)
        if hum.SeatPart then
            pcall(function() hum:UnequipTools() end)
            task.wait(0.5); continue
        end
        local tool = LocalPlayer.Backpack:FindFirstChild(name)
        if not tool then return false end  -- item tidak ada
        pcall(function() hum:EquipTool(tool) end)
        task.wait(0.35)
        -- Verifikasi: tool harus sudah di karakter
        if ch:FindFirstChild(name) then return true end
        -- Gagal, unequip dan coba lagi
        pcall(function() hum:UnequipTools() end)
        task.wait(0.25)
    end
    return false
...
-- cookInteract: ROBUST VERSION
-- Equip tool, verifikasi ada di tangan, interact kompor, verifikasi item berkurang.
-- Retry hingga 5x sampai item benar-benar masuk kompor.
local function cookInteract(toolName, radius)
    local r = radius or 8

    if not toolName then
        -- Tidak ada tool (e.g. ambil hasil saja) — langsung interact
        firePromptNearby(r)
        task.wait(0.12)
        pcall(function()
            VIM:SendKeyEvent(true,  Enum.KeyCode.E, false, game)
    ...
    if countBefore == 0 then return false end

    for _try = 1, 5 do
        -- 1. Equip tool dengan verifikasi
        local ok = equipTool(toolName)
        if not ok then
            task.wait(0.4); continue
        end
        task.wait(0.3)

        -- 2. Pastikan tool benar-benar ada di tangan karakter
        local ch = LocalPlayer.Character
        if not ch or not ch:FindFirstChild(toolName) then
            -- UnequipTools dan retry
            local hum = ch and ch:FindFirstChildOfClass("Humanoid")
            if hum then pcall(function() hum:UnequipTools() end) end
            task.wait(0.3); continue
        end

        -- 3. Interact: firePrompt + VIM E (2x untuk antisipasi lag)
        for _press = 1, 2 do
            firePromptNearby(r)
            task.wait(0.1)
            pcall(function()
        ...
        -- 4. Verifikasi: item harus berkurang dari inventory
        task.wait(0.25)
        if countItem(toolName) < countBefore then
        ...
        -- Belum berkurang — unequip dan retry
    ...
    -- Semua retry habis — unequip dan return false
    local humF = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
    if humF then pcall(function() humF:UnequipTools() end) end
    task.wait(0.15)
    return false
end
...
local function doAutoSell(setStatus2)
    ...
    while countAllMS() > 0 do
        ...
        task.wait(0.2)
        -- backup: fire proximity kalau E tidak cukup
        firePromptNearby(CFG.SELL_RADIUS)

        -- 3. Tunggu inventory berubah (item terjual), max SELL_TIMEOUT detik
        local elapsed = 0
        local terjual = false
        ...
        -- 4. Unequip dulu sebelum iterasi berikutnya
        unequipAll()
        task.wait(0.3)
...
local function doOneCook()
    ...
    for _w = 1, 5 do
        ...
        task.wait(0.4)
        if countItem(CFG.ITEM_WATER) < wb then waterOK = true; break end
        _setStatus("Retry Water ".._w.."/5...", Color3.fromRGB(100,180,255))
        task.wait(0.5)
    end
    if not waterOK then
        _setStatus("Water gagal masuk kompor!", Color3.fromRGB(255,60,90))
        isBusy = false; return false
    end
    ...
    for _s = 1, 5 do
        ...
        task.wait(0.4)
        if countItem(CFG.ITEM_SUGAR) < sb then sugarOK = true; break end
        _setStatus("Retry Sugar ".._s.."/5...", Color3.fromRGB(255,220,100))
        task.wait(0.5)
    end
    if not sugarOK then
        _setStatus("Sugar gagal masuk kompor!", Color3.fromRGB(255,60,90))
        isBusy = false; return false
    end
    ...
    for _g = 1, 5 do
        ...
        task.wait(0.4)
        if countItem(CFG.ITEM_GEL) < gb then gelOK = true; break end
        _setStatus("Retry Gelatin ".._g.."/5...", Color3.fromRGB(255,200,50))
        task.wait(0.5)
    end
    if not gelOK then
        _setStatus("Gelatin gagal masuk kompor!", Color3.fromRGB(255,60,90))
        isBusy = false; return false
    end
...
local function doAutoFully(setFullyStatus)
    ...
    while fullyRunning do
        ...
        if not fullySavedPos then
            setFullyStatus("APART BELUM DIPILIH!", Color3.fromRGB(255,60,90))
            task.wait(3); break
        end
        ...
        -- Stabilisasi: unequip + tunggu HRP settle
        unequipAll()
        task.wait(1.5)

        -- Validasi posisi karakter sudah dekat apart
        do
            local _ch  = LocalPlayer.Character
            local _hrp = _ch and _ch:FindFirstChild("HumanoidRootPart")
            if _hrp and (_hrp.Position - fullySavedPos).Magnitude > 12 then
                setFullyStatus("Posisi meleset, teleport ulang...", Color3.fromRGB(255,160,40))
                task.wait(0.5)
                fullyTeleport(fullySavedPos)
                task.wait(1.5)
                unequipAll()
        ...
        if not hasAllIngredients() then
            setFullyStatus("Bahan tidak lengkap, ulangi beli...", Color3.fromRGB(255,160,40))
        ...
        while fullyRunning and hasAllIngredients() do
            ...
            if ok and result == true then
                cooked += 1
            elseif not ok then
                _setStatus("ERROR masak: "..tostring(result), Color3.fromRGB(255,60,90))
                task.wait(1)
            end
        ...
        -- JUAL
        setFullyStatus(cooked.." MS selesai! Teleport ke NPC...", Color3.fromRGB(52,210,110))
        task.wait(0.3)
        ...
        repeat
            ...
            if not _h or not _h.SeatPart then break end
            task.wait(0.3); _sw += 0.3
        until _sw >= 3
        task.wait(0.4)

        -- Validasi MS ada sebelum jual
        local msToSell = countAllMS()
        if msToSell == 0 then
            setFullyStatus("Tidak ada MS untuk dijual.", Color3.fromRGB(255,160,40))
            task.wait(1)
        else
            setFullyStatus("Jual "..msToSell.." MS...", Color3.fromRGB(52,210,110))
