...
-- ========== TELEPORT - METODE TERBUKTI WORK ==========
-- Dari diagnostic: M1 BERHASIL
-- CheckCharacter di ReplicatedStorage.RemoteEvents adalah anticheat posisi
-- Cara: rename CheckCharacter → set CFrame → rename balik
...
local function doTeleport(targetCFrame)
    task.spawn(function()
        ...
        -- Cari CheckCharacter (anticheat posisi)
        local checkChar = nil
        local RS = game:GetService("ReplicatedStorage")
        pcall(function()
            checkChar = RS.RemoteEvents.CheckCharacter
        end)

        -- Step 1: Disable anticheat dengan rename
        if checkChar then
            pcall(function() checkChar.Name = "CC_OFF" end)
        end

        task.wait(0.1)

        -- Step 2: Set CFrame ke tujuan
        ...
        -- Step 3: Restore nama CheckCharacter
        if checkChar then
            pcall(function() checkChar.Name = "CheckCharacter" end)
        end
    end)
end
