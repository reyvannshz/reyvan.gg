...
-- ========== FLY CAR VARIABLES ==========
local flyCarEnabled     = false   -- toggle utama
local flyCarSpeed       = 80      -- kecepatan horizontal (studs/s)
local flyCarVertSpeed   = 40      -- kecepatan naik/turun (studs/s)
local flyCarSmooth      = 0.18    -- lerp factor (0.05 lambat – 1 instan)
local flyCarVehicle     = nil     -- VehicleSeat yang sedang dinaiki
local flyCarBodyVel     = nil     -- BodyVelocity instance
local flyCarBodyGyro    = nil     -- BodyGyro instance
local flyCarConn        = nil     -- RenderStepped connection
local flyCarStatusLbl   = nil     -- label status di UI
local flyCarCurrentVel  = Vector3.zero  -- smooth velocity accumulator
...
do
    -- ── helpers lokal ──
    local function mkRow2(parent, yPos, h)
        local f = Instance.new("Frame")
    ...
    local function mkSep2(parent, yPos, txt)
        local l = Instance.new("TextLabel")
    ...
    local function mkSlider2(parent, yPos, label, minV, maxV, defV, suffix, callback)
        local row = mkRow2(parent, yPos, 44)
        local lbl = Instance.new("TextLabel")
        ...
        valLbl.Text = tostring(defV) .. suffix
    ...
    -- ── JUDUL ──
    mkSep2(pageFlyCar, y, "FLY VEHICLE")
    y = y + 20

    -- ── STATUS ROW ──
    local statRow = mkRow2(pageFlyCar, y, 44)
    local statLblLeft = Instance.new("TextLabel")
    statLblLeft.Size = UDim2.new(0.45, 0, 1, 0)
    statLblLeft.Position = UDim2.new(0, 10, 0, 0)
    statLblLeft.BackgroundTransparency = 1
    statLblLeft.Text = "Enable Fly Car"
    statLblLeft.TextColor3 = C.text
    statLblLeft.Font = Enum.Font.GothamBold
    statLblLeft.TextSize = 11
    statLblLeft.TextXAlignment = Enum.TextXAlignment.Left
    statLblLeft.Parent = statRow

    flyCarStatusLbl = Instance.new("TextLabel")
    flyCarStatusLbl.Size = UDim2.new(0, 44, 0, 18)
    flyCarStatusLbl.Position = UDim2.new(1, -90, 0.5, -9)
    flyCarStatusLbl.BackgroundTransparency = 1
    flyCarStatusLbl.Text = "OFF"
    flyCarStatusLbl.TextColor3 = C.red
    flyCarStatusLbl.Font = Enum.Font.GothamBlack
    flyCarStatusLbl.TextSize = 11
    flyCarStatusLbl.TextXAlignment = Enum.TextXAlignment.Right
    flyCarStatusLbl.Parent = statRow

    -- toggle switch
    local toggleBg = Instance.new("Frame")
    toggleBg.Size = UDim2.new(0, 34, 0, 18)
    toggleBg.Position = UDim2.new(1, -42, 0.5, -9)
    toggleBg.BackgroundColor3 = C.border
    toggleBg.BorderSizePixel = 0
    toggleBg.Parent = statRow
    mkCorner(toggleBg, 9)
    ...
    mkCorner(toggleKnob, 6)
    local toggleBtn2 = Instance.new("TextButton")
    toggleBtn2.Size = UDim2.new(1, 0, 1, 0)
    toggleBtn2.BackgroundTransparency = 1
    toggleBtn2.Text = ""
    toggleBtn2.Parent = toggleBg
    y = y + 50

    -- ── INFO BOX ──
    local infoRow = mkRow2(pageFlyCar, y, 60)
    local infoTxt = Instance.new("TextLabel")
    infoTxt.Size = UDim2.new(1, -16, 1, 0)
    infoTxt.Position = UDim2.new(0, 8, 0, 0)
    infoTxt.BackgroundTransparency = 1
    infoTxt.RichText = true
    infoTxt.Text = '<font color="rgb(120,120,120)">Naik kendaraan dulu, lalu aktifkan.\n'
        .. '<font color="rgb(220,38,38)">W/A/S/D</font> = Terbang  '
        .. '<font color="rgb(220,38,38)">E</font> = Naik  '
        .. '<font color="rgb(220,38,38)">Q</font> = Turun</font>'
    ...
    -- ── SLIDERS ──
    mkSep2(pageFlyCar, y, "PENGATURAN")
    y = y + 20

    mkSlider2(pageFlyCar, y, "Kecepatan Terbang", 10, 300, flyCarSpeed, " st/s", function(v)
        flyCarSpeed = v
    end)
    y = y + 50

    mkSlider2(pageFlyCar, y, "Kecepatan Naik / Turun", 5, 150, flyCarVertSpeed, " st/s", function(v)
        flyCarVertSpeed = v
    end)
    y = y + 50

    mkSlider2(pageFlyCar, y, "Kelancaran (Smooth)", 1, 20, math.floor(flyCarSmooth * 20), "", function(v)
        flyCarSmooth = v / 20
    end)
    y = y + 50

    -- ── KONTROL MANUAL (tombol tekan) ──
    mkSep2(pageFlyCar, y, "KONTROL MANUAL")
    y = y + 20

    local ctrlInfo = mkRow2(pageFlyCar, y, 70)
    local function ctrlLine(txt, xPos, yOff, col)
        local l = Instance.new("TextLabel")
        l.Size = UDim2.new(0.5, 0, 0, 14)
        l.Position = UDim2.new(xPos, 6, 0, yOff)
        l.BackgroundTransparency = 1
        l.Text = txt
    ...
    ctrlLine("W  — Maju", 0, 6, C.text)
    ctrlLine("S  — Mundur", 0, 22, C.text)
    ctrlLine("A  — Kiri", 0, 38, C.text)
    ctrlLine("D  — Kanan", 0, 54, C.text)
    ctrlLine("E  — Naik", 0.5, 6, C.green)
    ctrlLine("Q  — Turun", 0.5, 22, C.green)
    ctrlLine("Kamera arahkan", 0.5, 38, C.subtext)
    ctrlLine("kemudi otomatis", 0.5, 54, C.subtext)
    y = y + 76

    pageFlyCar.CanvasSize = UDim2.new(0, 0, 0, y + 20)

    -- ========== LOGIKA FLY CAR ==========

    -- Cari semua parts dalam kendaraan (untuk nonaktifkan gravity)
    local function getVehicleParts(seat)
        local parts = {}
        local model = seat:FindFirstAncestorOfClass("Model")
        if not model then
            -- fallback: pakai seat sendiri
            parts[#parts + 1] = seat
            return parts
        end
    ...
    -- Pasang BodyVelocity + BodyGyro ke primary part kendaraan
    local function attachFlyObjects(seat)
        -- cari root part: PrimaryPart model > seat itu sendiri
        local model = seat:FindFirstAncestorOfClass("Model")
        local root  = (model and model.PrimaryPart) or seat

        -- Hapus yang lama kalau ada
        if flyCarBodyVel  then pcall(function() flyCarBodyVel:Destroy() end) end
        if flyCarBodyGyro then pcall(function() flyCarBodyGyro:Destroy() end) end

        local bv = Instance.new("BodyVelocity")
        bv.Velocity        = Vector3.zero
        bv.MaxForce        = Vector3.new(1e6, 1e6, 1e6)
        bv.P               = 1e4
        bv.Name            = "_MajestyFlyVel"
        bv.Parent          = root
    ...
    -- Bersihkan semua fly objects
    local function detachFlyObjects()
        if flyCarBodyVel  then pcall(function() flyCarBodyVel:Destroy() end)  flyCarBodyVel  = nil end
        if flyCarBodyGyro then pcall(function() flyCarBodyGyro:Destroy() end) flyCarBodyGyro = nil end
    end
    ...
    local function stopFly()
        ...
        detachFlyObjects()
        flyCarCurrentVel = Vector3.zero
        flyCarVehicle    = nil
    end

    -- Start fly loop
    local function startFly(seat)
        stopFly()
        flyCarVehicle = seat
        local root = attachFlyObjects(seat)

        flyCarConn = RunService.RenderStepped:Connect(function(dt)
            -- Cek masih naik kendaraan
            local char = LocalPlayer.Character
            local hum  = char and char:FindFirstChildOfClass("Humanoid")
            if not hum or hum.SeatPart ~= seat then
                -- Player turun dari kendaraan → matikan fly
                stopFly()
                flyCarEnabled = false
                flyCarStatusLbl.Text = "OFF"
                flyCarStatusLbl.TextColor3 = C.red
            ...
            -- Hitung arah berdasarkan kamera (horizontal plane)
            local camCF   = Camera.CFrame
            local forward = Vector3.new(camCF.LookVector.X, 0, camCF.LookVector.Z).Unit
            local right   = Vector3.new(camCF.RightVector.X, 0, camCF.RightVector.Z).Unit
            local up      = Vector3.new(0, 1, 0)

            -- Input W/A/S/D (gerakan), E (naik), Q (turun)
            local UIS = UserInputService
            local moveVec = Vector3.zero

            if UIS:IsKeyDown(Enum.KeyCode.W) then
                moveVec = moveVec + forward * flyCarSpeed
            end
            if UIS:IsKeyDown(Enum.KeyCode.S) then
                moveVec = moveVec - forward * flyCarSpeed
            end
            if UIS:IsKeyDown(Enum.KeyCode.D) then
                moveVec = moveVec + right * flyCarSpeed
            end
            if UIS:IsKeyDown(Enum.KeyCode.A) then
                moveVec = moveVec - right * flyCarSpeed
            end
            if UIS:IsKeyDown(Enum.KeyCode.E) then
                moveVec = moveVec + up * flyCarVertSpeed
            end
            ...
            local lerpT = math.clamp(flyCarSmooth * 60 * dt, 0.01, 1)
            flyCarCurrentVel = flyCarCurrentVel:Lerp(moveVec, lerpT)

            -- Apply ke BodyVelocity
            flyCarBodyVel.Velocity = flyCarCurrentVel

            -- Gyro: hadapkan kendaraan sesuai arah kamera (yaw only)
            local yaw = math.atan2(-camCF.LookVector.X, -camCF.LookVector.Z)
            flyCarBodyGyro.CFrame = CFrame.new(root.Position)
    ...
    toggleBtn2.MouseButton1Click:Connect(function()
        ...
        if flyCarEnabled then
            -- Cek apakah sedang naik kendaraan
            local char = LocalPlayer.Character
            local hum  = char and char:FindFirstChildOfClass("Humanoid")
            local seat = hum and hum.SeatPart

            if seat and seat:IsA("VehicleSeat") then
                flyCarStatusLbl.Text      = "ON"
                flyCarStatusLbl.TextColor3 = C.green
                ...
                flyCarEnabled = false
                flyCarStatusLbl.Text       = "NO VEHICLE"
                flyCarStatusLbl.TextColor3  = C.yellow
                toggleBg.BackgroundColor3   = C.border
                toggleKnob.Position         = UDim2.new(0, 3, 0.5, -6)
...
local tabDefs = {
    ...
    {icon="✈️", label="Fly Car",   page=pageFlyCar},
}
