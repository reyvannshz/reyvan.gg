...
-- ========== FUNGSI TELEPORT V6 - ANTI KICK DENGAN JEDA DIAM ==========

-- Freeze karakter di tempat (tidak bergerak sama sekali)
local function freezeChar(freeze)
...
local function doTeleport(targetCFrame)
    ...
    -- ===== FASE 1: DIAM DULU 15-20 DETIK SEBELUM TELEPORT =====
    -- Supaya server tidak curiga ada aktivitas mencurigakan
    freezeChar(true)
    local waitBefore = math.random(15, 20)
    for i = waitBefore, 1, -1 do
        if not char or not char:FindFirstChild("HumanoidRootPart") then break end
        -- Update status countdown di UI (diakses lewat tpStatusValue global)
        if tpStatusValue then
            tpStatusValue.Text = "SIAP " .. i .. "s..."
            tpStatusValue.TextColor3 = Color3.fromRGB(234, 179, 8)
        end
        -- Tetap freeze tiap detik
    ...
    if not char then freezeChar(false) return false end
    hrp      = char:FindFirstChild("HumanoidRootPart")
    humanoid = char:FindFirstChildOfClass("Humanoid")
    if not hrp or not humanoid then freezeChar(false) return false end

    -- Metode 1: sethiddenproperty
    ...
    if ok1 and (hrp.Position - targetPos).Magnitude < 10 then
        -- ===== FASE 3: DIAM LAGI 15-20 DETIK SETELAH TELEPORT =====
        local waitAfter = math.random(15, 20)
        for i = waitAfter, 1, -1 do
            if tpStatusValue then
                tpStatusValue.Text = "SETTLING " .. i .. "s"
                tpStatusValue.TextColor3 = Color3.fromRGB(99, 102, 241)
            end
            hrp = char:FindFirstChild("HumanoidRootPart")
            if hrp then
                hrp.AssemblyLinearVelocity  = Vector3.zero
    ...
    if ok2 and (hrp.Position - targetPos).Magnitude < 10 then
        local waitAfter = math.random(15, 20)
        for i = waitAfter, 1, -1 do
            if tpStatusValue then
                tpStatusValue.Text = "SETTLING " .. i .. "s"
...
do
    ...
    for i, loc in ipairs(savedLocations) do
        ...
        locBtn.MouseButton1Click:Connect(function()
            ...
            task.spawn(function()
                local target = CFrame.new(l.x, l.y + 3, l.z)
    ...
    -- ===== AUTO LOOP TELEPORT =====
    sectionTitle(pageTP, "AUTO LOOP TELEPORT", 290)

    -- Toggle loop mode
    local loopToggle = Instance.new("TextButton")
    loopToggle.Size = UDim2.new(1, -20, 0, 38)
    loopToggle.Position = UDim2.new(0, 10, 0, 316)
    ...
    -- Interval info
    local intervalCard = makeCard(pageTP, 362, 30)
    ...
    sectionTitle(pageTP, "TELEPORT KE PLAYER", 404)

    local playerListFrame = Instance.new("ScrollingFrame")
    playerListFrame.Size = UDim2.new(1, -20, 0, 100)
    playerListFrame.Position = UDim2.new(0, 10, 0, 428)
    ...
    refreshBtn.Position = UDim2.new(0, 10, 0, 536)
    ...
    pageTP.CanvasSize = UDim2.new(0, 0, 0, 580)
end
