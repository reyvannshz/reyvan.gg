...
versionLabel.BackgroundTransparency = 1; versionLabel.Text = "v8.5.0"
...
-- ── Teleport cepat ke safe spot (support kendaraan) ──
local function tpToSafe()
    ...
    if seat then
        ...
        if vModel and vModel.PrimaryPart then
            vModel:SetPrimaryPartCFrame(CFrame.new(SAFE_X, SAFE_Y + 2, SAFE_Z)); return
        end
    end
    pcall(function() hrp.CFrame = CFrame.new(SAFE_X, SAFE_Y + 2, SAFE_Z) end)
end
...
local function triggerSafeEscape(newHP)
    ...
    -- Guard 2: tidak boleh trigger saat sudah aktif
    if safeModeActive then return end
    -- Guard 3: cooldown antar trigger (cegah spam dari multi-hit)
    local now = tick()
    if now - _safeLastTrigger < SAFE_COOLDOWN then return end
    -- Guard 4: pastikan ini benar-benar damage (HP turun)
    ...
    -- ── Teleport instan ke safe spot ──
    tpToSafe()

    -- ── Spawn goroutine recovery (tidak block HealthChanged) ──
    task.spawn(function()
        task.wait(0.6)   -- stabilisasi posisi

        -- ═══════════════════════════════════════
        -- FASE 1: SCAN MUSUH DI SAFE SPOT
        -- ═══════════════════════════════════════
        setSafeLbl("DI SAFE SPOT – SCAN MUSUH...", Color3.fromRGB(255,200,0))

        local safePos    = SAFE_POS_VEC
        local scanRadius = FULLY_ENEMY_RADIUS
        local waitStart  = tick()
        local MAX_WAIT   = 30   -- maksimal tunggu 30 detik di safe
        local clearCount = 0    -- butuh 3x scan bersih berturut-turut sebelum balik

        while tick() - waitStart < MAX_WAIT do
            -- Cek posisi player saat ini untuk scan radius yang akurat
            local myCh  = LocalPlayer.Character
            local myHRP = myCh and myCh:FindFirstChild("HumanoidRootPart")
            local scanCenter = myHRP and myHRP.Position or safePos

            local enemyFound, enemyName, enemyDist = hasEnemyNear(scanCenter, scanRadius)

            if enemyFound then
                clearCount = 0   -- reset counter jika musuh muncul lagi
                setSafeLbl(
                    string.format("⚠ MUSUH: %s (%.0fm) — TAHAN...", enemyName, enemyDist),
                    Color3.fromRGB(255,60,60)
                )
                -- Jika musuh terlalu dekat safe spot, re-teleport ke safe
                if enemyDist < 15 then
                    tpToSafe(); task.wait(0.5)
                end
            else
                clearCount += 1
                setSafeLbl(
                    string.format("Scan bersih %d/3 — tunggu konfirmasi...", clearCount),
                    Color3.fromRGB(255,220,80)
                )
                if clearCount >= 3 then break end  -- 3x bersih berturut → aman
            end
            task.wait(1.2)
        end
        ...
        if fullySavedPos then
            ...
            -- Verifikasi posisi sudah di apart
            ...
            if myHRP2 and (myHRP2.Position - fullySavedPos).Magnitude > 8 then
                -- Coba teleport sekali lagi
                fullyTeleport(fullySavedPos)
                task.wait(1.2)
            end
        else
            setSafeLbl("AMAN – Tidak ada posisi apart tersimpan.", Color3.fromRGB(160,160,160))
            task.wait(1)
        end

        -- ═══════════════════════════════════════
        -- FASE 3: SCAN ULANG SETELAH BALIK APART
        -- ═══════════════════════════════════════
        -- Cek musuh sekali lagi di posisi apart sebelum resume masak
        local apartPos = fullySavedPos or SAFE_POS_VEC
        local enemyStillNear, en2, ed2 = hasEnemyNear(apartPos, scanRadius)
        if enemyStillNear then
            setSafeLbl(
                string.format("MUSUH MASIH ADA DI APART: %s (%.0fm) – BALIK SAFE", en2, ed2),
                Color3.fromRGB(255,80,80)
            )
            tpToSafe(); task.wait(2)
            -- Satu kali retry scan lagi
            local st2 = tick()
            while tick() - st2 < 15 do
                local ef, en3, ed3 = hasEnemyNear(apartPos, scanRadius)
        ...
        -- ═══════════════════════════════════════
        -- FASE 5: RESUME / RESTART AUTO FULLY
        -- ═══════════════════════════════════════
        if not SM.wasFullyRunning or not safeMode then
            -- Auto Fully tidak sedang jalan → hanya update label
            setSafeLbl("STANDBY", Color3.fromRGB(0,255,136))
            return
        end

        -- Deteksi ulang fase setelah balik ke apart
        ...
        -- Aktifkan kembali state running
        isRunning    = true
        fullyRunning = true
        ...
        task.spawn(function()
            ...
            -- Ada bahan di inventory atau masih di fase masak → resume dari fase
            if hasW or hasS or hasG or phase == "need_bag" or phase == "cooking_progress" then
                dualStatus("Resume masak fase: "..phase, Color3.fromRGB(82,130,255))
                pcall(resumeCookFromPhase)

                -- Lanjut masak siklus normal jika bahan masih ada
                while fullyRunning and isRunning and hasAllIngredients() do
                    pcall(doOneCook)
                    if fullyRunning then task.wait(0.3) end
                end
                isRunning = false

                -- Lanjut ke siklus fully berikutnya (jual → beli → masak)
                if fullyRunning then
                    fullyRunning = false
                    task.wait(0.2)
                    task.spawn(function() doAutoFully(dualStatus) end)
                end
            else
                -- Semua bahan sudah masuk ke kompor / habis → restart fully loop
                isRunning    = false
                fullyRunning = false
                task.wait(0.3)
                task.spawn(function() doAutoFully(dualStatus) end)
...
print("=== MAJESTY STORE v8.5.0 LOADED ===")
