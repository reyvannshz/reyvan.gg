...
-- Saat karakter spawn setelah void drop, set posisi ke tujuan
local function onCharacterAdded(newChar)
    if not pendingTP then return end
    local target = pendingTP
    local sLbl   = pendingStatus
    pendingTP     = nil
    pendingStatus = nil
    ...
    task.spawn(function()
        ...
        -- Tunggu 1 frame physics siap
        RunService.Stepped:Wait()
        RunService.Stepped:Wait()

        -- Anchor, set posisi, tahan 0.5s, lepas
        hrp.Anchored = true
        hrp.CFrame   = target
        ...
        if sLbl and sLbl.Parent then
            sLbl.Text       = "ARRIVED ✓"
            sLbl.TextColor3 = Color3.fromRGB(34, 197, 94)
            task.wait(2)
            if sLbl and sLbl.Parent then
                sLbl.Text       = "STANDBY"
                sLbl.TextColor3 = Color3.fromRGB(234, 179, 8)
            end
        end
    end)
end
...
-- ========== VOID TELEPORT ==========
-- Cara kerja:
-- 1. Simpan tujuan ke pendingTP
-- 2. Jatuhkan karakter ke void (Y = -5000) → karakter mati/respawn otomatis
-- 3. onCharacterAdded menangkap spawn baru → set posisi ke tujuan
-- Server tidak bisa reset karena posisi diset di awal spawn
...
local function doTeleport(targetCFrame)
    task.spawn(function()
        ...
        -- Simpan tujuan untuk dipakai saat respawn
        pendingTP = targetCFrame

        -- Anchor sementara biar tidak kena physics saat jatuh
        hrp.Anchored = true
        pcall(function() hrp.AssemblyLinearVelocity  = Vector3.zero end)
        pcall(function() hrp.AssemblyAngularVelocity = Vector3.zero end)

        -- Pindah ke void (jauh di bawah map)
        hrp.CFrame = CFrame.new(hrp.Position.X, -5000, hrp.Position.Z)
        task.wait(0.1)
        hrp.Anchored = false

        -- Bunuh karakter → trigger respawn → onCharacterAdded akan set posisi
        hum.Health = 0
    end)
end
