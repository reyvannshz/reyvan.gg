...
local killAuraRadius     = 50      -- default 50 studs (1 stud ≈ ~1 meter di Roblox)
local killAuraInterval   = 0.1     -- detik antar pengecekan
local killAuraKillCount  = 0
local killAuraStatusLbl  = nil
local killAuraRadiusLbl  = nil
local killAuraCountLbl   = nil
local kaRadiusInput      = nil
...
local espInvColor        = Color3.fromRGB(255, 230, 100)
...
titleLabel.Text = '<font color="rgb(220,38,38)">Majesty Store</font>  |  <font color="rgb(170,170,170)">https://discord.gg/CF6ZMvgd23</font>'
...
do
    ...
    pageAuto.CanvasSize = UDim2.new(0,0,0,620)
end
...
do
    ...
    espInfoLbl.Text = "Box  ·  Username  ·  HP Bar  ·  Item  ·  Distance"
    ...
    -- 3 swatch rows, tiap 36px tinggi: y=110, 146, 182 → total sampai y=218
    ...
    mkEspSwatchRow(pageEsp, 182, "🖐  Warna Hold Inventory", function(c) espInvColor  = c end)

    -- ── Whitelist section (geser ke bawah 120px) ─────────────────────
    local WO = 120  -- whitelist offset
...
do
    -- ── Section Header ──────────────────────────────────────────────
    sectionTitle(pageKillAura, "☠  KILL AURA", 8)

    -- ── Toggle Button ───────────────────────────────────────────────
    local kaToggleBtn = Instance.new("TextButton")
    kaToggleBtn.Size = UDim2.new(1, -20, 0, 38)
    kaToggleBtn.Position = UDim2.new(0, 10, 0, 36)
    kaToggleBtn.BackgroundColor3 = Color3.fromRGB(35, 20, 20)
    kaToggleBtn.Text = "☠  KILL AURA : OFF"
    kaToggleBtn.TextColor3 = C.red
    kaToggleBtn.Font = Enum.Font.GothamBlack
    kaToggleBtn.TextSize = 14
    kaToggleBtn.BorderSizePixel = 0
    kaToggleBtn.Parent = pageKillAura
    mkCorner(kaToggleBtn, 6)
    mkStroke(kaToggleBtn, 1, C.red)

    -- ── Status Card ─────────────────────────────────────────────────
    sectionTitle(pageKillAura, "STATUS", 84)
    local kaStatCard = makeCard(pageKillAura, 110, 44)
    makeLabel(kaStatCard, "STATUS", 12, 0, 80, 44, 11, C.subtext, Enum.Font.GothamBold)
    killAuraStatusLbl = makeLabel(kaStatCard, "OFF", 90, 0, 260, 44, 16, C.red, Enum.Font.GothamBlack)

    local kaKillCard = makeCard(pageKillAura, 162, 44)
    makeLabel(kaKillCard, "KILLED", 12, 0, 80, 44, 11, C.subtext, Enum.Font.GothamBold)
    killAuraCountLbl = makeLabel(kaKillCard, "0", 90, 0, 260, 44, 16, C.yellow, Enum.Font.GothamBlack)

    local kaRadCard = makeCard(pageKillAura, 214, 44)
    makeLabel(kaRadCard, "RADIUS", 12, 0, 80, 44, 11, C.subtext, Enum.Font.GothamBold)
    killAuraRadiusLbl = makeLabel(kaRadCard, "50 studs", 90, 0, 260, 44, 16, C.accent2, Enum.Font.GothamBlack)

    -- ── Radius Slider ───────────────────────────────────────────────
    sectionTitle(pageKillAura, "ATUR RADIUS  (1 – 500 studs)", 268)

    local kaSliderBg = Instance.new("Frame")
    kaSliderBg.Size = UDim2.new(1, -20, 0, 20)
    kaSliderBg.Position = UDim2.new(0, 10, 0, 294)
    kaSliderBg.BackgroundColor3 = C.card2
    kaSliderBg.BorderSizePixel = 0
    kaSliderBg.Parent = pageKillAura
    mkCorner(kaSliderBg, 10)
    mkStroke(kaSliderBg, 1, C.border)

    local kaSliderFill = Instance.new("Frame")
    kaSliderFill.Size = UDim2.new((killAuraRadius - 1) / 499, 0, 1, 0)
    kaSliderFill.BackgroundColor3 = C.accent
    kaSliderFill.BorderSizePixel = 0
    kaSliderFill.Parent = kaSliderBg
    mkCorner(kaSliderFill, 10)

    local kaSliderKnob = Instance.new("Frame")
    kaSliderKnob.Size = UDim2.new(0, 16, 0, 16)
    kaSliderKnob.AnchorPoint = Vector2.new(0.5, 0.5)
    kaSliderKnob.Position = UDim2.new((killAuraRadius - 1) / 499, 0, 0.5, 0)
    kaSliderKnob.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    kaSliderKnob.BorderSizePixel = 0
    kaSliderKnob.ZIndex = 5
    kaSliderKnob.Parent = kaSliderBg
    mkCorner(kaSliderKnob, 8)

    -- Label min/max di bawah slider
    local kaLblMin = makeLabel(pageKillAura, "1", 10, 318, 40, 16, 10, C.subtext, Enum.Font.Gotham)
    local kaLblMax = makeLabel(pageKillAura, "500", 0, 318, 40, 16, 10, C.subtext, Enum.Font.Gotham, Enum.TextXAlignment.Right)
    kaLblMax.Size = UDim2.new(1, -10, 0, 16)
    kaLblMax.Position = UDim2.new(0, 0, 0, 318)

    -- Input angka manual
    local kaInputCard = makeCard(pageKillAura, 342, 44)
    makeLabel(kaInputCard, "INPUT RADIUS", 12, 0, 120, 44, 11, C.subtext, Enum.Font.GothamBold)
    local kaInput = Instance.new("TextBox")
    kaInput.Size = UDim2.new(0, 90, 0, 26)
    kaInput.Position = UDim2.new(0, 140, 0.5, -13)
    kaInput.BackgroundColor3 = C.card2
    kaInput.Text = tostring(killAuraRadius)
    kaInput.TextColor3 = C.text
    kaInput.Font = Enum.Font.GothamBold
    kaInput.TextSize = 13
    kaInput.BorderSizePixel = 0
    kaInput.PlaceholderText = "1-500"
    kaInput.PlaceholderColor3 = C.subtext
    kaInput.Parent = kaInputCard
    mkCorner(kaInput, 4)
    mkStroke(kaInput, 1, C.border)
    kaRadiusInput = kaInput

    local kaApplyBtn = Instance.new("TextButton")
    kaApplyBtn.Size = UDim2.new(0, 60, 0, 26)
    kaApplyBtn.Position = UDim2.new(0, 238, 0.5, -13)
    kaApplyBtn.BackgroundColor3 = C.accent
    kaApplyBtn.Text = "APPLY"
    kaApplyBtn.TextColor3 = Color3.fromRGB(255,255,255)
    kaApplyBtn.Font = Enum.Font.GothamBlack
    kaApplyBtn.TextSize = 11
    kaApplyBtn.BorderSizePixel = 0
    kaApplyBtn.Parent = kaInputCard
    mkCorner(kaApplyBtn, 4)

    -- ── Quick Radius Presets ────────────────────────────────────────
    sectionTitle(pageKillAura, "PRESET RADIUS", 396)
    ...
    local function setRadius(r)
        r = math.clamp(math.floor(r), 1, 500)
        killAuraRadius = r
        killAuraRadiusLbl.Text = r .. " studs"
        kaInput.Text = tostring(r)
        local pct = (r - 1) / 499
        kaSliderFill.Size = UDim2.new(pct, 0, 1, 0)
        kaSliderKnob.Position = UDim2.new(pct, 0, 0.5, 0)
    end
    ...
    for i, p in ipairs(presets) do
        ...
        local row = ((i - 1) % 2)
        local pb = Instance.new("TextButton")
        pb.Size = UDim2.new(0, 66, 0, 30)
        pb.Position = UDim2.new(0, 10 + (col - 1) * 74, 0, 420 + row * 38)
        pb.BackgroundColor3 = C.card
        pb.Text = p.label
        pb.TextColor3 = C.accent2
        pb.Font = Enum.Font.GothamBlack
        pb.TextSize = 12
        pb.BorderSizePixel = 0
        pb.Parent = pageKillAura
        mkCorner(pb, 6)
        mkStroke(pb, 1, C.accent)
        local pv = p.val
        pb.MouseButton1Click:Connect(function() setRadius(pv) end)
    end

    -- ── Whitelist Info ──────────────────────────────────────────────
    sectionTitle(pageKillAura, "WHITELIST", 500)
    local kaWlCard = makeCard(pageKillAura, 526, 46)
    local kaWlLbl = Instance.new("TextLabel")
    kaWlLbl.Size = UDim2.new(1, -12, 1, 0)
    kaWlLbl.Position = UDim2.new(0, 10, 0, 0)
    kaWlLbl.BackgroundTransparency = 1
    kaWlLbl.Text = "Player di whitelist otomatis aman dari Kill Aura.\nTambah whitelist di tab General → Whitelist."
    kaWlLbl.TextColor3 = C.subtext
    kaWlLbl.Font = Enum.Font.Gotham
    kaWlLbl.TextSize = 10
    kaWlLbl.TextWrapped = true
    kaWlLbl.TextXAlignment = Enum.TextXAlignment.Left
    kaWlLbl.TextYAlignment = Enum.TextYAlignment.Center
    kaWlLbl.Parent = kaWlCard

    -- ── Reset Kill Count ────────────────────────────────────────────
    local kaResetBtn = Instance.new("TextButton")
    kaResetBtn.Size = UDim2.new(1, -20, 0, 34)
    kaResetBtn.Position = UDim2.new(0, 10, 0, 582)
    kaResetBtn.BackgroundColor3 = C.card
    kaResetBtn.Text = "🔄  Reset Kill Count"
    kaResetBtn.TextColor3 = C.text
    kaResetBtn.Font = Enum.Font.GothamBold
    kaResetBtn.TextSize = 12
    kaResetBtn.BorderSizePixel = 0
    kaResetBtn.Parent = pageKillAura
    mkCorner(kaResetBtn, 6)
    mkStroke(kaResetBtn, 1, C.border)
    kaResetBtn.MouseButton1Click:Connect(function()
        killAuraKillCount = 0
        killAuraCountLbl.Text = "0"
    end)

    -- ── Keybind info ────────────────────────────────────────────────
    local kaKeybindCard = makeCard(pageKillAura, 626, 30)
    local kaKeybindLbl = Instance.new("TextLabel")
    kaKeybindLbl.Size = UDim2.new(1, -12, 1, 0)
    kaKeybindLbl.Position = UDim2.new(0, 10, 0, 0)
    kaKeybindLbl.BackgroundTransparency = 1
    kaKeybindLbl.Text = "⌨  Tekan  [ Z ]  untuk toggle Kill Aura ON/OFF"
    kaKeybindLbl.TextColor3 = C.subtext
    kaKeybindLbl.Font = Enum.Font.GothamBold
    kaKeybindLbl.TextSize = 11
    kaKeybindLbl.TextXAlignment = Enum.TextXAlignment.Left
    kaKeybindLbl.Parent = kaKeybindCard

    -- ── Wall Breaker Tool ───────────────────────────────────────────
    sectionTitle(pageKillAura, "🔨  WALL BREAKER TOOL", 666)

    local wbInfoCard = makeCard(pageKillAura, 692, 44)
    local wbInfoLbl = Instance.new("TextLabel")
    wbInfoLbl.Size = UDim2.new(1, -12, 1, 0)
    wbInfoLbl.Position = UDim2.new(0, 10, 0, 0)
    wbInfoLbl.BackgroundTransparency = 1
    wbInfoLbl.Text = "Klik kiri = hancurkan 1 tembok  |  Tahan klik kanan = AOE 10 stud"
    wbInfoLbl.TextColor3 = C.subtext
    wbInfoLbl.Font = Enum.Font.Gotham
    wbInfoLbl.TextSize = 10
    wbInfoLbl.TextWrapped = true
    wbInfoLbl.TextXAlignment = Enum.TextXAlignment.Left
    wbInfoLbl.TextYAlignment = Enum.TextYAlignment.Center
    wbInfoLbl.Parent = wbInfoCard

    local wbGetBtn = Instance.new("TextButton")
    wbGetBtn.Size = UDim2.new(1, -20, 0, 40)
    wbGetBtn.Position = UDim2.new(0, 10, 0, 744)
    wbGetBtn.BackgroundColor3 = Color3.fromRGB(20, 30, 50)
    wbGetBtn.Text = "🔨  Dapatkan Wall Breaker"
    wbGetBtn.TextColor3 = Color3.fromRGB(100, 180, 255)
    wbGetBtn.Font = Enum.Font.GothamBlack
    wbGetBtn.TextSize = 13
    wbGetBtn.BorderSizePixel = 0
    wbGetBtn.Parent = pageKillAura
    mkCorner(wbGetBtn, 6)
    mkStroke(wbGetBtn, 1, Color3.fromRGB(100, 180, 255))

    wbGetBtn.MouseButton1Click:Connect(function()
        createWallBreakerTool()
        wbGetBtn.Text = "✅  Wall Breaker ada di Inventory!"
        wbGetBtn.TextColor3 = C.green
        wbGetBtn.BackgroundColor3 = Color3.fromRGB(20, 35, 20)
        mkStroke(wbGetBtn, 1, C.green)
        task.wait(3)
        wbGetBtn.Text = "🔨  Dapatkan Wall Breaker"
        wbGetBtn.TextColor3 = Color3.fromRGB(100, 180, 255)
        wbGetBtn.BackgroundColor3 = Color3.fromRGB(20, 30, 50)
    ...
    pageKillAura.CanvasSize = UDim2.new(0, 0, 0, 820)

    -- ── Slider Drag Logic ───────────────────────────────────────────
    local kaSliderDragging = false
    kaSliderBg.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
            kaSliderDragging = true
        end
    end)
    UserInputService.InputEnded:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
            kaSliderDragging = false
        end
    end)
    UserInputService.InputChanged:Connect(function(inp)
        if kaSliderDragging and (inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch) then
            local absPos = kaSliderBg.AbsolutePosition
            local absSize = kaSliderBg.AbsoluteSize
            local mouseX = inp.Position.X
            local pct = math.clamp((mouseX - absPos.X) / absSize.X, 0, 1)
            local newRadius = math.floor(1 + pct * 499)
            setRadius(newRadius)
        end
    end)

    -- ── Apply input box ─────────────────────────────────────────────
    kaApplyBtn.MouseButton1Click:Connect(function()
        local v = tonumber(kaInput.Text)
        if v then
            setRadius(v)
        end
    end)
    kaInput.FocusLost:Connect(function(enterPressed)
        if enterPressed then
            local v = tonumber(kaInput.Text)
            if v then setRadius(v) end
        end
    end)
    ...
    kaToggleBtn.MouseButton1Click:Connect(function()
        ...
        if killAuraEnabled then
            kaToggleBtn.Text = "☠  KILL AURA : ON"
            kaToggleBtn.TextColor3 = C.green
            kaToggleBtn.BackgroundColor3 = Color3.fromRGB(20, 35, 20)
            mkStroke(kaToggleBtn, 1, C.green)
            killAuraStatusLbl.Text = "ACTIVE"
            killAuraStatusLbl.TextColor3 = C.green
        else
            kaToggleBtn.Text = "☠  KILL AURA : OFF"
            kaToggleBtn.TextColor3 = C.red
            kaToggleBtn.BackgroundColor3 = Color3.fromRGB(35, 20, 20)
            mkStroke(kaToggleBtn, 1, C.red)
            killAuraStatusLbl.Text = "OFF"
            killAuraStatusLbl.TextColor3 = C.red
        end
    end)
end

-- ========== KILL AURA CORE LOOP ==========
-- Tanpa void teleport. Murni firetouchinterest + RemoteEvent scan.
-- firetouchinterest: memalsukan sentuhan antara HRP target dan part berbahaya di map
-- Ini berjalan di sisi local tapi trigger script server yang handle damage.

local kaKilledThisCycle = {}

-- Cache kill-parts agar tidak scan ulang tiap frame
local kaKillPartsCache = nil
local kaKillPartsCacheTime = 0

local function getKillParts()
    local now = tick()
    if kaKillPartsCache and (now - kaKillPartsCacheTime) < 10 then
        return kaKillPartsCache
    end
    local parts = {}
    -- Cari killbrick, lava, void trigger, damage brick di seluruh workspace
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("BasePart") then
            local n = obj.Name:lower()
            if n:find("kill") or n:find("lava") or n:find("void") or n:find("death")
            or n:find("damage") or n:find("instakill") or n:find("instant") then
                table.insert(parts, obj)
            end
        end
    end
    kaKillPartsCache = parts
    kaKillPartsCacheTime = now
    return parts
end

-- Cache RemoteEvent damage di ReplicatedStorage
local kaDmgRemotesCache = nil
local kaDmgRemotesCacheTime = 0

local function getDamageRemotes()
    local now = tick()
    if kaDmgRemotesCache and (now - kaDmgRemotesCacheTime) < 15 then
        return kaDmgRemotesCache
    end
    local remotes = {}
    ...
    for _, v in pairs(RS:GetDescendants()) do
        if v:IsA("RemoteEvent") or v:IsA("RemoteFunction") then
            ...
            if n:find("damage") or n:find("hit") or n:find("kill") or n:find("shoot")
            or n:find("fire") or n:find("bullet") or n:find("attack") or n:find("hurt") then
    ...
    for _, v in pairs(Workspace:GetDescendants()) do
        if v:IsA("RemoteEvent") then
            local n = v.Name:lower()
            if n:find("damage") or n:find("hit") or n:find("kill") then
                table.insert(remotes, v)
            end
        end
    end
    kaDmgRemotesCache = remotes
    kaDmgRemotesCacheTime = now
    return remotes
end

local function tryKillNoTeleport(plr, hrp, hum)
    -- ── 1. Auto-activate gun di tangan → arahkan mouse ke target tanpa teleport ──
    pcall(function()
            ...
            if tool:IsA("Tool") then
                ...
                for _, v in pairs(tool:GetDescendants()) do
                    if v:IsA("RemoteEvent") then
                        ...
                        pcall(function() v:FireServer(hrp.Position, hrp) end)
                        pcall(function() v:FireServer(hrp.Position, plr) end)
                        pcall(function() v:FireServer(CFrame.new(hrp.Position), hrp) end)
                    end
                    if v:IsA("RemoteFunction") then
                        pcall(function() v:InvokeServer(hrp.Position, hrp) end)
                    end
                end
            end
        end
    ...
    -- ── 2. Fire RemoteEvent damage di ReplicatedStorage ──────────────────
    pcall(function()
        local remotes = getDamageRemotes()
        for _, remote in pairs(remotes) do
            if remote:IsA("RemoteEvent") then
                pcall(function() remote:FireServer(hrp.Position, hrp, 99999) end)
                pcall(function() remote:FireServer(plr, 99999) end)
                pcall(function() remote:FireServer(plr.Character, hrp, 99999) end)
                pcall(function() remote:FireServer(hrp.Position, plr.Character) end)
            elseif remote:IsA("RemoteFunction") then
                pcall(function() remote:InvokeServer(plr, 99999) end)
            end
        end
    end)

    -- ── 3. firetouchinterest: killbrick/lava menyentuh HRP target ────────
    pcall(function()
        local killParts = getKillParts()
        for _, kp in pairs(killParts) do
            pcall(function() firetouchinterest(hrp, kp, 0) end)
            pcall(function() firetouchinterest(hrp, kp, 1) end)
        end
    end)

    -- ── 4. firetouchinterest: semua part karakter kita vs target ─────────
    pcall(function()
        local myChar = LocalPlayer.Character
        if not myChar then return end
    ...
    while true do
        ...
        for _, plr in pairs(Players:GetPlayers()) do
            ...
            if dist <= killAuraRadius then
                task.spawn(function()
                    tryKillNoTeleport(plr, hrp, hum)
                end)

                if not kaKilledThisCycle[plr] then
                    kaKilledThisCycle[plr] = true
                    killAuraKillCount = killAuraKillCount + 1
                    if killAuraCountLbl then
                        killAuraCountLbl.Text = tostring(killAuraKillCount)
                    end
                end
            else
                kaKilledThisCycle[plr] = nil
...
-- ========== KILL AURA KEYBIND TOGGLE (default: Z) ==========
local kaKeybind = Enum.KeyCode.Z

UserInputService.InputBegan:Connect(function(input, gpe)
    if gpe then return end
    if input.KeyCode == kaKeybind then
        killAuraEnabled = not killAuraEnabled
        if killAuraStatusLbl then
            killAuraStatusLbl.Text   = killAuraEnabled and "ACTIVE" or "OFF"
            killAuraStatusLbl.TextColor3 = killAuraEnabled and C.green or C.red
        end
        -- Cari tombol toggle di pageKillAura dan update tampilannya
        for _, obj in pairs(pageKillAura:GetDescendants()) do
            if obj:IsA("TextButton") and obj.Text:find("KILL AURA") then
                if killAuraEnabled then
                    obj.Text = "☠  KILL AURA : ON"
                    obj.TextColor3 = C.green
                    obj.BackgroundColor3 = Color3.fromRGB(20, 35, 20)
                    mkStroke(obj, 1, C.green)
                else
                    obj.Text = "☠  KILL AURA : OFF"
                    obj.TextColor3 = C.red
                    obj.BackgroundColor3 = Color3.fromRGB(35, 20, 20)
                    mkStroke(obj, 1, C.red)
                    kaKilledThisCycle = {}
                end
                break
            end
...
-- ========== WALL BREAKER TOOL ==========
-- Semua logic dijalankan langsung dari main script (kompatibel Xeno/executor)
-- Tool masuk inventory, logic listen dari tool events di sini

local wbToolRef      = nil
local wbAoeActive    = false
local wbMouse        = LocalPlayer:GetMouse()
local wbToolEquipped = false

local wbProtected = {
    "humanoidrootpart","head","torso","upperarm","lowerarm",
    "hand","upperleg","lowerleg","foot","spawn","baseplate",
    "handle","weapon","gun","rootpart"
}

local function wbIsProtected(part)
    if not part then return true end
    if part:IsA("Terrain") then return true end
    local n = part.Name:lower()
    for _, kw in pairs(wbProtected) do
...
local function wbBreakPart(part)
    if not part or not part:IsA("BasePart") then return end
    if wbIsProtected(part) then return end
    pcall(function()
        -- Flash merah sebentar lalu hancurkan
        ...
        -- Hancurkan semua part dalam model yang sama
        local model = part.Parent
        if model and (model:IsA("Model") or model:IsA("Folder")) then
            for _, p in pairs(model:GetDescendants()) do
                if p:IsA("BasePart") and not wbIsProtected(p) then
                    p.Transparency = 1
                    p.CanCollide   = false
                end
            end
...
local function createWallBreakerTool()
    -- Hapus yang lama
    pcall(function()
        local bp = LocalPlayer:FindFirstChild("Backpack")
        if bp then
            local old = bp:FindFirstChild("Wall Breaker")
            if old then old:Destroy() end
        end
        local ch = LocalPlayer.Character
        if ch then
            local old = ch:FindFirstChild("Wall Breaker")
            if old then old:Destroy() end
        end
    end)

    local tool = Instance.new("Tool")
    tool.Name = "Wall Breaker"
    tool.RequiresHandle = true
    tool.ToolTip = "LMB: hancurkan 1 tembok | RMB tahan: AOE hancurkan radius 10 stud"
    tool.CanBeDropped = false

    -- Handle visual (palu besar merah)
    ...
    mesh.Scale = Vector3.new(3.5, 1, 1)
    mesh.Parent = handle
    ...
    tool.Activated:Connect(function()
        -- Klik kiri: hancurkan 1 target
        local target = wbMouse.Target
        wbBreakPart(target)
    end)
...
-- RMB hold untuk AOE wall break
UserInputService.InputBegan:Connect(function(inp, gpe)
    if gpe then return end
    if not wbToolEquipped then return end
...
-- AOE loop: jalankan dari RunService
RunService.RenderStepped:Connect(function()
    if not wbAoeActive or not wbToolEquipped then return end
    local hitPos = wbMouse.Hit.Position
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("BasePart") and not wbIsProtected(obj) then
            if (obj.Position - hitPos).Magnitude <= 12 then
                pcall(function()
                    obj.Transparency = 1
...
-- Pastikan tool tetap ada setelah respawn
...
local tabDefs = {
    ...
    {icon="☠️", label="KillAura",  page=pageKillAura},
...
do
    ...
    footerLbl.Text = '<font color="rgb(220,38,38)">majesty.gg</font>  ·  <font color="rgb(120,120,120)">Thank you for using MAJESTY ONTOP</font>'
