...
-- ============================================================
-- ANTI-KICK & ANTI-BAN BYPASS
-- Blocks: RemoteEvent kicks, Kick() calls, ban checks
-- ============================================================
local _Players_svc = game:GetService("Players")
local _lp          = _Players_svc.LocalPlayer

-- 1. Hook & block Player:Kick()
-- Setiap kali server coba kick, kita ignore
local _mt = getrawmetatable and getrawmetatable(game)
if _mt then
    local _old_index    = _mt.__index
    local _old_newindex = _mt.__newindex
    local _isReadOnly   = isreadonly and isreadonly(_mt)
    if _isReadOnly then
        pcall(setreadonly, _mt, false)
    end
    -- Block __namecall Kick
    local _old_namecall = _mt.__namecall
    _mt.__namecall = newcclosure(function(self, ...)
        local method = getnamecallmethod and getnamecallmethod() or ""
        -- Block any Kick call on LocalPlayer
        if method:lower() == "kick" and self == _lp then
            warn("[van.gg] 🛡️ Anti-Kick: Kick blocked!")
            return
        end
        return _old_namecall(self, ...)
    end)
    if _isReadOnly then
        pcall(setreadonly, _mt, true)
    end
end

-- 2. Hook RemoteEvents – block known kick/ban remotes
local _hookfunc = hookfunction or replaceclosure
local _old_FireServer
if _hookfunc and typeof(game:GetService("ReplicatedStorage")) ~= "nil" then
    pcall(function()
        local RE = game:GetService("ReplicatedStorage")
        -- Monitor semua remote event fire dari client
        local _oldfire = RE.FindFirstChild
        -- Generic: hook Instance:FireServer di semua remote
    end)
end

-- 3. Block server-side kick via RemoteEvent named kick/ban/punish
local function watchRemotes(parent)
    for _, v in ipairs(parent:GetDescendants()) do
        if v:IsA("RemoteEvent") or v:IsA("RemoteFunction") then
            local name = v.Name:lower()
            if name:find("kick") or name:find("ban") or name:find("punish") or name:find("remove") then
                -- Disconnect OnClientEvent jika ada
                pcall(function()
                    v.OnClientEvent:Connect(function(...)
                        warn("[van.gg] 🛡️ Anti-Ban: Remote '"..v.Name.."' blocked!")
                        -- Do nothing = kick/ban ignored
                    end)
                end)
            end
        end
...
pcall(watchRemotes, game)
-- Watch future descendants too
        ...
        if name:find("kick") or name:find("ban") or name:find("punish") then
            pcall(function()
                d.OnClientEvent:Connect(function()
                    warn("[van.gg] 🛡️ Anti-Ban: New remote '"..d.Name.."' blocked!")
                end)
            end)
        end
    end
...
-- 4. Anti speed/fly detection – reset velocity kalau ketangkep
-- (South Bronx cek velocity exploit)
local _antiDetectConn
local function startAntiDetect()
    if _antiDetectConn then _antiDetectConn:Disconnect() end
    _antiDetectConn = game:GetService("RunService").Heartbeat:Connect(function()
        local char = _lp.Character
        if not char then return end
        local hrp = char:FindFirstChild("HumanoidRootPart")
        if hrp and hrp:IsA("BasePart") then
            -- Kalau velocity terlalu tinggi (bisa kena deteksi), normalize
...
pcall(startAntiDetect)
_lp.CharacterAdded:Connect(function()
    task.wait(1)
    pcall(startAntiDetect)
end)

-- 5. Anti-ban: sembunyikan nama script dari deteksi
-- Obfuscate script identity
pcall(function()
    if syn and syn.protect_gui then
        -- protect GUI dari deteksi
    end
end)

print("[van.gg] 🛡️ Anti-Kick & Anti-Ban aktif!")
...
local _fireprox = (typeof(fireproximityprompt)=="function" and fireproximityprompt)
    ...
    or function(p)
        ...
        pcall(function()
            local args = {[1]=p,[2]=game:GetService("Players").LocalPlayer}
            p.Parent.Parent:FindFirstChildOfClass("RemoteEvent") and
            p.Parent.Parent:FindFirstChildOfClass("RemoteEvent"):FireServer(unpack(args))
        end)
    end
...
local _fireclk = (typeof(fireclickdetector)=="function" and fireclickdetector)
    ...
    or function(d)
        if d and d.MouseClick then
            pcall(function() d.MouseClick:Fire(game:GetService("Players").LocalPlayer) end)
        end
    end
