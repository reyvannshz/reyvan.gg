...
local minKeyCode         = Enum.KeyCode.F1            -- default F1
...
do
    ...
    -- ========== MINIMIZE KEYBIND (HIDE/SHOW GUI) ==========
    sectionTitle(pageEsp, "MINIMIZE KEYBIND", 580)  -- y position after Vehicle Fly section
    local mkRow2 = mkRow(pageEsp, 608, 34)          -- adjust y
    local mkLbl = Instance.new("TextLabel")
    mkLbl.Size = UDim2.new(0.55, 0, 1, 0)
    mkLbl.Position = UDim2.new(0, 10, 0, 0)
    mkLbl.BackgroundTransparency = 1
    mkLbl.Text = "Hide / Show GUI"
    mkLbl.TextColor3 = C.text
    mkLbl.Font = Enum.Font.Gotham
    mkLbl.TextSize = 11
    ...
    minKbBtn.BackgroundTransparency = 1
    minKbBtn.Text = tostring(minKeyCode):gsub("Enum%.KeyCode%.","")  -- show current key
    minKbBtn.TextColor3 = C.text
    minKbBtn.Font = Enum.Font.Gotham
    minKbBtn.TextSize = 10
    minKbBtn.BorderSizePixel = 0
    minKbBtn.Parent = mkRow2
    ...
    minKbBtn.MouseButton1Click:Connect(function()
        ...
        isBindingMin = true
        minKbBtn.Text = "..."
        minKbBtn.TextColor3 = C.subtext
    end)

    pageEsp.CanvasSize = UDim2.new(0, 0, 0, 660)  -- increase canvas size to accommodate new section
end
...
do
    ...
    UserInputService.InputBegan:Connect(function(input,gpe)
        ...
        if isBindingMin then
            ...
            if minKeybindBtnRef then
                minKeybindBtnRef.Text = tostring(minKeyCode or minKeyMBtn):gsub("Enum%..*%.","")
                minKeybindBtnRef.TextColor3 = C.text
            end
        end
    end)
end
