...
local function equipTool(name)
    -- Coba sampai 6x dengan verifikasi dan penanganan SeatPart
    for _attempt = 1, 6 do
        local ch  = LocalPlayer.Character
        if not ch then task.wait(0.4); continue end
        local hum = ch:FindFirstChildOfClass("Humanoid")
        if not hum then task.wait(0.4); continue end

        -- Sudah ter-equip? Langsung return true
        if ch:FindFirstChild(name) then return true end

        -- Kalau masih duduk di kendaraan, paksa UnequipTools dulu
        ...
        if not tool then
            -- Item tidak ada di backpack sama sekali
            return false
        end
        ...
        -- Verifikasi: tool harus pindah ke karakter
        if ch:FindFirstChild(name) then return true end

        -- Belum berhasil, unequip dulu lalu retry
        pcall(function() hum:UnequipTools() end)
        task.wait(0.3)
    end
    return false
...
-- cookInteract: equip bahan, verifikasi equip, lalu interact kompor,
-- verifikasi item benar-benar berkurang. Retry hingga 5x.
local function cookInteract(toolName, radius)
    local r = radius or 8

    if toolName then
        -- Snapshot jumlah item sebelum interact
        local countBefore = countItem(toolName)
        if countBefore == 0 then return false end  -- tidak ada item, tidak perlu lanjut

        for _try = 1, 5 do
            -- 1. Equip tool dengan verifikasi
            local equipped = equipTool(toolName)
            if not equipped then
                task.wait(0.5)
                continue  -- gagal equip, coba lagi
            end
            task.wait(0.3)  -- beri waktu animasi equip selesai

            -- 2. Verifikasi tool benar-benar di tangan (ada di karakter)
            local ch = LocalPlayer.Character
            if not ch or not ch:FindFirstChild(toolName) then
                task.wait(0.3)
                continue  -- tool belum di tangan, coba equip lagi
            end

            -- 3. Interact: fire prompt + tekan E
            firePromptNearby(r)
            task.wait(0.15)
            pcall(function()
                VIM:SendKeyEvent(true,  Enum.KeyCode.E, false, game)
            ...
            task.wait(0.2)
            firePromptNearby(r)  -- backup kedua
            task.wait(0.3)

            -- 4. Verifikasi: item harus berkurang dari total inventory
            local countAfter = countItem(toolName)
            if countAfter < countBefore then
                -- Berhasil! Unequip dan keluar
                pcall(function()
            ...
            -- Belum berkurang, unequip lalu retry
            pcall(function()
                local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
                if hum then hum:UnequipTools() end
            end)
        ...
        -- Setelah semua retry habis, unequip dan return false
        pcall(function()
            local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
            if hum then hum:UnequipTools() end
        end)
        ...
        -- Tidak ada tool (misal kompor kosong), langsung interact saja
        firePromptNearby(r)
        task.wait(0.12)
        pcall(function()
            VIM:SendKeyEvent(true,  Enum.KeyCode.E, false, game)
...
local function doAutoSell(setStatus2)
    -- ── Validasi awal: cek apakah ada MS di inventory ──
    local msTotal = countAllMS()
    if msTotal == 0 then
        setStatus2("Tidak ada MS untuk dijual", Color3.fromRGB(160,160,180))
        return
    end

    setStatus2("Deteksi "..msTotal.." MS – bersiap jual...", Color3.fromRGB(50,210,110))

    -- ── Stabilisasi: unequip semua, tunggu karakter stabil ──
    unequipAll()
    task.wait(0.6)

    -- Tunggu sampai tidak duduk di kendaraan (max 3 detik)
    local seatWait = 0
    repeat
        local _ch = LocalPlayer.Character
        local _hm = _ch and _ch:FindFirstChildOfClass("Humanoid")
        if not _hm or not _hm.SeatPart then break end
        task.wait(0.3); seatWait += 0.3
    until seatWait >= 3

    -- Tunggu satu frame agar HRP terupdate posisinya
    task.wait(0.4)

    -- ── Validasi ulang setelah stabilisasi ──
    msTotal = countAllMS()
    if msTotal == 0 then
        setStatus2("MS tidak ditemukan setelah stabilisasi!", Color3.fromRGB(210,40,40))
        return
    end

    setStatus2("Mulai jual "..msTotal.." MS...", Color3.fromRGB(50,210,110))
    ...
    while countAllMS() > 0 do
        if fail >= maxFail then
            setStatus2("Gagal jual setelah "..maxFail.."x retry. Abort.", Color3.fromRGB(210,40,40))
            break
        end

        -- ── Deteksi item MS yang bisa di-equip ──
        local msName = getEquippableMS()
        if not msName then
            setStatus2("Tidak ada MS terdeteksi di backpack!", Color3.fromRGB(210,40,40))
            break
        end

        -- ── Snapshot inventory SEBELUM equip (equip bisa memindahkan item) ──
        local bS = countItem(CFG.ITEM_MS_SMALL)
        local bM = countItem(CFG.ITEM_MS_MEDIUM)
        local bL = countItem(CFG.ITEM_MS_LARGE)
        local bTotal = bS + bM + bL
        ...
        if not ok then
            fail += 1
            setStatus2("Gagal equip "..msName.." ("..fail.."/"..maxFail..")", Color3.fromRGB(210,40,40))
            -- Jika sudah 3x gagal equip, re-teleport ke NPC lalu coba lagi
            if fail == 3 then
                setStatus2("Re-teleport ke NPC...", Color3.fromRGB(255,160,40))
        ...
        task.wait(0.5)  -- tunggu equip stabil

        -- ── Aksi jual: kirim E + firePrompt, diulang 2x ──
        ...
        for _try = 1, 2 do
            ...
            -- Cek apakah sudah terjual lebih awal
            local earlyDiff = bTotal - (countItem(CFG.ITEM_MS_SMALL) + countItem(CFG.ITEM_MS_MEDIUM) + countItem(CFG.ITEM_MS_LARGE))
            if earlyDiff > 0 then break end
        end

        -- ── Tunggu konfirmasi item berkurang (max SELL_TIMEOUT) ──
        local elapsed = 0
        local terjual = false
        while elapsed < CFG.SELL_TIMEOUT do
            local nowTotal = countItem(CFG.ITEM_MS_SMALL) + countItem(CFG.ITEM_MS_MEDIUM) + countItem(CFG.ITEM_MS_LARGE)
            local diff = bTotal - nowTotal
            if diff > 0 then
                sold      += diff
                totalSold += diff
                terjual    = true
            ...
            -- Coba fire prompt sekali lagi setiap 2 detik jika belum terjual
            if elapsed > 0 and math.floor(elapsed * 10) % 20 == 0 then
                firePromptNearby(CFG.SELL_RADIUS)
            end
            task.wait(0.3)
            elapsed += 0.3
        ...
        -- ── Unequip sebelum iterasi berikutnya ──
        unequipAll()
        task.wait(0.4)
        ...
        if terjual then
            ...
            fail += 1
            setStatus2("Tidak terjual ("..fail.."/"..maxFail..") – retry...", Color3.fromRGB(255,155,35))
            -- Re-teleport ke NPC jika sudah 4x gagal berturut-turut
            if fail == 4 then
                setStatus2("Re-teleport ke NPC untuk retry...", Color3.fromRGB(255,160,40))
                fullyTeleport(NPC_MS_POS)
                task.wait(1.5)
                unequipAll()
                task.wait(0.5)
    ...
    unequipAll()
    if sold > 0 then
        setStatus2("Selesai! Terjual "..sold.." MS (total: "..totalSold..")", Color3.fromRGB(50,210,110))
    else
...
-- Countdown: tiap detik update phase+timer, berhenti jika AutoMS_Running & isRunning keduanya false
...
local function doOneCook()
    ...
    -- ════════════════════════════════════
    -- FASE 1: MASUKKAN WATER
    -- ════════════════════════════════════
    _setStatus("Masukkan Water...", Color3.fromRGB(100,180,255))
    _setPhase("Masukkan Water...")

    if countItem(CFG.ITEM_WATER) == 0 then
        _setStatus("TIDAK ADA WATER!", Color3.fromRGB(255,60,90))
        isBusy = false; return false
    end

    -- Retry masukkan water hingga berhasil (item berkurang) atau 5x
    local waterOK = false
    for _w = 1, 5 do
        local wb = countItem(CFG.ITEM_WATER)
        if wb == 0 then break end
        cookInteract(CFG.ITEM_WATER)
        task.wait(0.4)
        if countItem(CFG.ITEM_WATER) < wb then
            waterOK = true; break
        end
        _setStatus("Retry Water ".._w.."/5...", Color3.fromRGB(100,180,255))
        task.wait(0.5)
    end
    -- Kalau retry habis tapi water masih ada di backpack, log warning tapi lanjut
    -- (mungkin server sudah terima tapi item belum despawn dari client)

    -- Ambil timer dari server, fallback ke default
    local boilSecs
    for _ = 1, 40 do boilSecs = popTimer(); if boilSecs then break end; task.wait(0.1) end
    ...
    -- ════════════════════════════════════
    -- FASE 2: MASUKKAN SUGAR
    -- ════════════════════════════════════
    _setStatus("Tunggu sinyal Sugar...", Color3.fromRGB(255,220,100))
    _setPhase("Tunggu Sugar...")
    waitRPC("add_sugar", 12)
    if not isRunning then isBusy = false; return false end

    if countItem(CFG.ITEM_SUGAR) == 0 then
        _setStatus("TIDAK ADA SUGAR!", Color3.fromRGB(255,60,90))
        isBusy = false; return false
    end
    ...
    for _s = 1, 5 do
        local sb = countItem(CFG.ITEM_SUGAR)
        if sb == 0 then break end
        cookInteract(CFG.ITEM_SUGAR)
        task.wait(0.4)
        if countItem(CFG.ITEM_SUGAR) < sb then break end
        _setStatus("Retry Sugar ".._s.."/5...", Color3.fromRGB(255,220,100))
        task.wait(0.5)
    end

    -- ════════════════════════════════════
    -- FASE 3: MASUKKAN GELATIN
    -- ════════════════════════════════════
    _setStatus("Tunggu sinyal Gelatin...", Color3.fromRGB(255,200,50))
    _setPhase("Tunggu Gelatin...")
    waitRPC("add_gelatin", 12)
    if not isRunning then isBusy = false; return false end

    if countItem(CFG.ITEM_GEL) == 0 then
        _setStatus("TIDAK ADA GELATIN!", Color3.fromRGB(255,60,90))
        isBusy = false; return false
    end
    ...
    for _g = 1, 5 do
        local gb = countItem(CFG.ITEM_GEL)
        if gb == 0 then break end
        cookInteract(CFG.ITEM_GEL)
        task.wait(0.4)
        if countItem(CFG.ITEM_GEL) < gb then break end
        _setStatus("Retry Gelatin ".._g.."/5...", Color3.fromRGB(255,200,50))
        task.wait(0.5)
    end

    -- Ambil timer cook dari server
    local cookSecs
    for _ = 1, 40 do cookSecs = popTimer(); if cookSecs then break end; task.wait(0.1) end
    ...
    if not bag then
        _setStatus("No Empty Bag! Skip.", Color3.fromRGB(255,60,90))
        isBusy = false; return false
    end

    -- Ambil marshmallow dengan retry
    _setStatus("Ambil MS...", Color3.fromRGB(100,220,160))
    _setPhase("Ambil MS...")
    local msBefore = countItem(CFG.ITEM_MS_SMALL) + countItem(CFG.ITEM_MS_MEDIUM) + countItem(CFG.ITEM_MS_LARGE)
    for _m = 1, 4 do
        cookInteract(CFG.ITEM_EMPTY)
        task.wait(0.5)
        local msNow = countItem(CFG.ITEM_MS_SMALL) + countItem(CFG.ITEM_MS_MEDIUM) + countItem(CFG.ITEM_MS_LARGE)
        if msNow > msBefore then break end
        if _m < 4 then
            _setStatus("Retry ambil MS ".._m.."/4...", Color3.fromRGB(100,160,255))
            task.wait(0.5)
        end
    end
...
local function doAutoFully(setFullyStatus)
    ...
    while fullyRunning do
        ...
        if not fullySavedPos then
            setFullyStatus("APART BELUM DIPILIH! Pilih di tab Auto Fully.", Color3.fromRGB(255,60,90))
            task.wait(3); break
        end
        ...
        -- ── Stabilisasi setelah teleport: unequip + tunggu HRP settle ──
        unequipAll()
        task.wait(1.5)

        -- Validasi posisi: pastikan karakter sudah dekat area masak
        do
            local _ch  = LocalPlayer.Character
            local _hrp = _ch and _ch:FindFirstChild("HumanoidRootPart")
            if _hrp then
                local dist = (_hrp.Position - fullySavedPos).Magnitude
                if dist > 12 then
                    setFullyStatus("Posisi meleset ("..math.floor(dist).."st), teleport ulang...", Color3.fromRGB(255,160,40))
                    task.wait(0.5)
                    fullyTeleport(fullySavedPos)
                    task.wait(1.5)
                    unequipAll()
        ...
        -- Validasi bahan sebelum masak
        if not hasAllIngredients() then
            setFullyStatus("Bahan tidak lengkap, kembali beli...", Color3.fromRGB(255,160,40))
        ...
        while fullyRunning and hasAllIngredients() do
            local ok, result = pcall(doOneCook)
            if ok and result then
                cooked += 1
            elseif not ok then
                _setStatus("ERROR: "..tostring(result), Color3.fromRGB(255,60,90))
                task.wait(1)
            end
        ...
        -- ── Stabilisasi setelah teleport ke NPC ──
        -- Unequip semua tool agar tidak ada konflik saat equip MS untuk jual
        unequipAll()
        task.wait(0.8)

        -- Tunggu SeatPart cleared (karakter mungkin masih terhitung duduk setelah teleport)
        local _stWait = 0
        repeat
            local _stCh = LocalPlayer.Character
            local _stHm = _stCh and _stCh:FindFirstChildOfClass("Humanoid")
            if not _stHm or not _stHm.SeatPart then break end
            task.wait(0.3); _stWait += 0.3
        until _stWait >= 4

        -- Satu tick tambahan agar HRP position terupdate di sisi server
        task.wait(0.5)

        -- ── Validasi ulang: pastikan MS benar-benar ada sebelum jual ──
        local msBeforeSell = countAllMS()
        setFullyStatus("Validasi MS: "..msBeforeSell.." item ditemukan...", Color3.fromRGB(80,200,130))
        task.wait(0.3)

        if msBeforeSell == 0 then
            setFullyStatus("Tidak ada MS di inventory – skip jual.", Color3.fromRGB(255,160,40))
            task.wait(1)
        else
            setFullyStatus("Jual "..msBeforeSell.." MS...", Color3.fromRGB(52,210,110))
...
-- Cek apakah ada musuh dalam radius tertentu dari sebuah posisi
        ...
        if plr ~= LocalPlayer and not isWhitelisted(plr) then
            ...
            if hum and hrp and hum.Health > 0 then
                if (hrp.Position - pos).Magnitude <= radius then
                    return true, plr.Name, math.floor((hrp.Position - pos).Magnitude)
                end
            end
        end
    end
...
-- ================================================================
-- DETEKSI BAHAN BELUM DIMASUKKAN & RESUME MASAK
-- Logika: cek inventory bahan → masukkan yang kurang ke kompor
-- sesuai urutan fase, lalu lanjut masak normal
-- ================================================================

-- Fase masak yang bisa diresume:
-- "need_water"   → water belum masuk sama sekali
-- "need_sugar"   → water sudah masuk, tunggu/masukkan sugar
-- "need_gelatin" → sugar sudah masuk, tunggu/masukkan gelatin
-- "need_bag"     → gelatin sudah masuk, tinggal tunggu & ambil bag
-- "fresh"        → kompor kosong, mulai dari awal
...
local function detectCookPhase()
    ...
    -- Cek rpcQueue untuk sinyal dari server tentang fase kompor saat ini
    local phaseFromRPC = nil
    for _, v in ipairs(rpcQueue) do
        if v.type == "wait_boil"   then phaseFromRPC = "boiling"   break
        elseif v.type == "add_sugar"   then phaseFromRPC = "add_sugar"  break
        elseif v.type == "add_gelatin" then phaseFromRPC = "add_gelatin" break
        elseif v.type == "wait_cook"   then phaseFromRPC = "cooking"    break
        elseif v.type == "bag_result"  then phaseFromRPC = "bag_ready"  break
        end
    end

    -- Prioritas: gunakan sinyal RPC kalau ada
    if phaseFromRPC == "add_sugar"   then return "need_sugar",   hasWater, hasSugar, hasGelatin end
    if phaseFromRPC == "add_gelatin" then return "need_gelatin", hasWater, hasSugar, hasGelatin end
    if phaseFromRPC == "bag_ready"   then return "need_bag",     hasWater, hasSugar, hasGelatin end
    if phaseFromRPC == "boiling" or phaseFromRPC == "cooking" then
        return "cooking_progress", hasWater, hasSugar, hasGelatin
    end

    -- Fallback: deduksi dari inventory
    -- Kalau semua bahan masih ada = kompor belum diisi sama sekali
    if hasWater and hasSugar and hasGelatin then
        return "fresh", hasWater, hasSugar, hasGelatin
    end
    -- Water sudah masuk (tidak ada di inv), sugar & gelatin masih ada
    if not hasWater and hasSugar and hasGelatin then
        return "need_sugar", hasWater, hasSugar, hasGelatin
    end
    -- Water & sugar sudah masuk, gelatin masih ada
    if not hasWater and not hasSugar and hasGelatin then
        return "need_gelatin", hasWater, hasSugar, hasGelatin
    end
    -- Semua bahan sudah masuk, tinggal tunggu bag/masak
    if not hasWater and not hasSugar and not hasGelatin then
        if hasEmptyBag then
            return "need_bag", hasWater, hasSugar, hasGelatin
        else
            return "cooking_progress", hasWater, hasSugar, hasGelatin
        end
    end
...
-- Resume masak dari fase yang terdeteksi, lalu lanjut normal
local function resumeCookFromPhase()
    ...
    -- ── FASE: kompor kosong / fresh → masukkan water dulu ──
    ...
    -- ── FASE: semua bahan masuk, tunggu selesai masak ──
    if phase == "need_bag" or phase == "cooking_progress" then
        ...
        cookSecs = cookSecs or CFG.COOK_WAIT
        if not countdown(cookSecs, "Memasak...", Color3.fromRGB(80,140,255)) then isBusy=false; return false end
        ...
        _setPhase("Resume Complete #"..totalMS())
        _setTimer("Done")
    end
...
local function triggerSafeEscape(newHP)
    ...
    -- Stop masak sementara (baik auto ms maupun fully)
    ...
    -- 1. Teleport ke safe spot
    tpToSafe()

    task.spawn(function()
        task.wait(0.5)
        if safeModeStatusLbl then safeModeStatusLbl.Text = "DI SAFE SPOT – CEK MUSUH..."; safeModeStatusLbl.TextColor3 = Color3.fromRGB(255,200,0) end

        -- 2. Scan musuh via ESP di sekitar apartemen kita
        local apartPos = fullySavedPos or Vector3.new(SAFE_X, SAFE_Y, SAFE_Z)
        local scanRadius = FULLY_ENEMY_RADIUS
        local waitTick = tick()
        local maxWait  = 20  -- max tunggu 20 detik

        while tick() - waitTick < maxWait do
            local enemyFound, enemyName, enemyDist = hasEnemyNear(apartPos, scanRadius)
            if not enemyFound then
                break
            end
            if safeModeStatusLbl then
                safeModeStatusLbl.Text = string.format("MUSUH: %s (%.0fm) – TUNGGU...", enemyName, enemyDist)
                safeModeStatusLbl.TextColor3 = Color3.fromRGB(255,80,80)
            end
            task.wait(1)
        end

        -- 3. Aman – balik ke apartemen
        if fullySavedPos then
            if safeModeStatusLbl then safeModeStatusLbl.Text = "AMAN – BALIK APART..."; safeModeStatusLbl.TextColor3 = Color3.fromRGB(100,255,180) end
            fullyTeleport(fullySavedPos)
            task.wait(1)
        end

        -- ── 4. Deteksi fase masak & bahan yang belum dimasukkan ──
        local phase, hasWater, hasSugar, hasGelatin = detectCookPhase()
        local phaseDesc = {
            fresh            = "KOMPOR KOSONG – MASAK DARI AWAL",
            need_water       = "BELUM MASUKKAN WATER",
            need_sugar       = "BELUM MASUKKAN SUGAR",
            need_gelatin     = "BELUM MASUKKAN GELATIN",
            need_bag         = "SEMUA BAHAN MASUK – TUNGGU SELESAI",
            cooking_progress = "SEDANG MASAK – LANJUT TUNGGU",
            unknown          = "CEK KOMPOR...",
        }
        local phaseColor = {
            fresh            = Color3.fromRGB(100,180,255),
            need_water       = Color3.fromRGB(100,180,255),
            need_sugar       = Color3.fromRGB(255,220,100),
            need_gelatin     = Color3.fromRGB(255,200,50),
            need_bag         = Color3.fromRGB(80,220,130),
            cooking_progress = Color3.fromRGB(80,140,255),
            unknown          = Color3.fromRGB(160,160,180),
        }

        if safeModeStatusLbl then
            safeModeStatusLbl.Text       = phaseDesc[phase] or "CEK KOMPOR..."
            safeModeStatusLbl.TextColor3 = phaseColor[phase] or C.subtext
        end
        task.wait(1)

        -- ── 5. Update health & reset flag ──
        local char2 = LocalPlayer.Character
        local hum2  = char2 and char2:FindFirstChildOfClass("Humanoid")
        if hum2 then lastHealth = hum2.Health end
        safeModeActive = false

        -- ── 6. Resume masak dari fase yang terdeteksi ──
        if wasFullyRunning and safeMode then
            isRunning    = true
            fullyRunning = true

            local function dummyStatus(msg, col)
                _setStatus(msg, col)
                if safeModeStatusLbl then
            ...
            task.spawn(function()
                -- Kalau bahan masih ada di inventory → resume dari fase
                if hasWater or hasSugar or hasGelatin or phase == "need_bag" or phase == "cooking_progress" then
                    dummyStatus("Resume masak: "..phase, Color3.fromRGB(82,130,255))
                    unequipAll()
                    -- Masukkan bahan yang belum masuk dulu, lanjut masak satu siklus
                    local ok = pcall(resumeCookFromPhase)
                    -- Lanjut masak siklus-siklus berikutnya jika bahan masih ada
                    while fullyRunning and hasAllIngredients() do
                        pcall(doOneCook)
                        if fullyRunning then task.wait(0.3) end
                    end
                    isRunning = false
                    -- Setelah bahan habis → lanjut ke loop fully berikutnya (jual → beli → masak)
                    if fullyRunning then
                        task.spawn(function() doAutoFully(dummyStatus) end)
                    end
                else
                    -- Bahan sudah habis semua di inventory (sudah masuk kompor semua atau habis dibeli)
                    -- Langsung restart fully loop
                    isRunning    = false
                    fullyRunning = false
                    task.wait(0.3)
                    task.spawn(function() doAutoFully(dummyStatus) end)
            ...
            if safeModeStatusLbl then
                ...
                safeModeStatusLbl.TextColor3 = C.accent
            end
        end
    end)
end
