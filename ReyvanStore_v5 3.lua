...
-- Compatible: Xeno, Delta, Arceus X, KRNL, Synapse X, Fluxus
-- Semua pakai pcall agar tidak crash di executor manapun
-- ============================================================
local _lp = game:GetService("Players").LocalPlayer
local _RS = game:GetService("RunService")

-- ── Layer 1: Hook __namecall (PC executor: Synapse/KRNL/Fluxus) ──
-- Xeno tidak support getrawmetatable jadi dibungkus pcall penuh
pcall(function()
    if not (getrawmetatable and newcclosure and getnamecallmethod) then return end
    local mt = getrawmetatable(game)
    local ro = isreadonly and isreadonly(mt) or false
    if ro then pcall(setreadonly, mt, false) end
    local old = mt.__namecall
    mt.__namecall = newcclosure(function(self, ...)
        local m = getnamecallmethod()
        if m == "Kick" and self == _lp then
            warn("[van.gg] 🛡 Kick blocked (namecall)")
            return
        end
        return old(self, ...)
    end)
...
-- ── Layer 2: Hook Player:Kick langsung (Xeno support hookfunction) ──
pcall(function()
    if not hookfunction then return end
    local oldKick = _lp.Kick
    hookfunction(oldKick, newcclosure(function(self, ...)
        if self == _lp then
            warn("[van.gg] 🛡 Kick blocked (hookfunction)")
            return
        end
        return oldKick(self, ...)
    end))
...
-- ── Layer 3: Block RemoteEvent kick/ban (semua executor) ──
-- Cara aman: connect OnClientEvent lebih dulu, tidak lakukan apa2
local _kickWords = {"kick","ban","punish","remove","exile","suspend"}
...
local function blockRemote(obj)
    ...
    if not isKickRemote(obj.Name) then return end
    pcall(function()
        if obj:IsA("RemoteEvent") then
            obj.OnClientEvent:Connect(function()
                warn("[van.gg] 🛡 Remote '"..obj.Name.."' blocked")
                -- sengaja tidak melakukan apa-apa = kick diabaikan
            end)
...
-- Scan semua remote yang sudah ada
task.spawn(function()
...
-- ── Layer 4: Anti velocity detection (Xeno safe) ──
-- South Bronx deteksi teleport via velocity spike
local _antiConn
local function startAntiDetect()
    if _antiConn then pcall(function() _antiConn:Disconnect() end) end
    _antiConn = _RS.Heartbeat:Connect(function()
        pcall(function()
            local char = _lp.Character
            if not char then return end
            local hrp = char:FindFirstChild("HumanoidRootPart")
            if hrp then
                -- Normalkan velocity supaya server tidak deteksi
...
task.spawn(startAntiDetect)
_lp.CharacterAdded:Connect(function()
    task.wait(1)
    task.spawn(startAntiDetect)
end)

-- ── Layer 5: Protect GUI dari deteksi (executor yang support) ──
pcall(function()
    -- Synapse X
    if syn and syn.protect_gui then
        local gui = game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("VanGG")
        if gui then syn.protect_gui(gui) end
    end
    -- Xeno / Delta protect
    if protect_gui then
        local gui = game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("VanGG")
...
print("[van.gg] 🛡 Anti-Kick & Anti-Ban aktif (Xeno compatible)")
