...
contentArea.Size = UDim2.new(1, 0, 1, -88)   -- 36 topbar + 44 bottomnav + 8 margin
...
do
    ...
    pageAuto.CanvasSize = UDim2.new(0,0,0,580)
end
...
do
    ...
    makeLabel(vFlyInfoCard, "Space = Naik   |   Shift = Turun   |   WASD = Steer", 12, 20, 380, 16, 10, C.subtext, Enum.Font.Gotham)
    ...
    local function getVehicleSeat()
        ...
        -- Juga cek via Humanoid.SeatPart
    ...
    -- Key state untuk naik/turun
    UserInputService.InputBegan:Connect(function(input, gpe)
        if not vFlyEnabled then return end
        if gpe then return end
        if input.KeyCode == Enum.KeyCode.Space then vFlyUp = true end
        if input.KeyCode == Enum.KeyCode.LeftShift or input.KeyCode == Enum.KeyCode.RightShift then vFlyDown = true end
    end)
    UserInputService.InputEnded:Connect(function(input)
        if input.KeyCode == Enum.KeyCode.Space then vFlyUp = false end
        if input.KeyCode == Enum.KeyCode.LeftShift or input.KeyCode == Enum.KeyCode.RightShift then vFlyDown = false end
    end)
    ...
    pageEsp.CanvasSize = UDim2.new(0, 0, 0, VFO + 234)
end
...
do
    ...
    pageTP.CanvasSize = UDim2.new(0, 0, 0, 784)
end
...
local tabDefs = {
    ...
    {icon="🎨", label="Tema",      page=pageTheme},
...
-- ===== PAGE: TEMA GUI =====
do
    -- Fungsi terapkan tema ke seluruh GUI
    local function applyTheme(newAccent, newAccent2, themeName)
        C.accent  = newAccent
        C.accent2 = newAccent2
        fovColor  = newAccent
        espBoxColor = newAccent

        -- Update semua UIStroke dan elemen accent di GUI
        local function updateChildren(obj)
            for _, child in pairs(obj:GetDescendants()) do
                pcall(function()
                    if child:IsA("UIStroke") then
                        if child.Color == C.accent or child.Color == C.accent2 then
                            child.Color = newAccent
                        end
                    end
                    if child:IsA("Frame") and child.BackgroundColor3 == C.accent then
                        child.BackgroundColor3 = newAccent
                    end
                    if child:IsA("TextButton") and child.BackgroundColor3 == C.accent then
                        child.BackgroundColor3 = newAccent
                    end
                    if child:IsA("TextLabel") and child.TextColor3 == C.accent2 then
                        child.TextColor3 = newAccent2
                    end
        ...
        -- Update accent bar di titlebar
        if aimbotFovCircle then
            aimbotFovCircle.Color = newAccent
        end
    end
    ...
    local themes = {
        {
            name   = "🔴 Merah (Default)",
            desc   = "Tema merah valary.gg klasik",
            accent = Color3.fromRGB(220, 38, 38),
            acc2   = Color3.fromRGB(239, 68, 68),
            preview= Color3.fromRGB(220, 38, 38),
        },
        {
            name   = "🔵 Biru Neon",
            desc   = "Biru elektrik cyberpunk",
            accent = Color3.fromRGB(0, 120, 255),
            acc2   = Color3.fromRGB(56, 160, 255),
            preview= Color3.fromRGB(0, 120, 255),
        },
        {
            name   = "🟢 Hijau Matrix",
            desc   = "Hijau terminal hacker",
            accent = Color3.fromRGB(34, 197, 94),
            acc2   = Color3.fromRGB(74, 222, 128),
            preview= Color3.fromRGB(34, 197, 94),
        },
        {
            name   = "🟣 Ungu Galaxy",
            desc   = "Violet neon gelap",
            accent = Color3.fromRGB(139, 92, 246),
            acc2   = Color3.fromRGB(167, 139, 250),
            preview= Color3.fromRGB(139, 92, 246),
        },
        {
            name   = "🟠 Orange Flame",
            desc   = "Api oranye membara",
            accent = Color3.fromRGB(234, 88, 12),
            acc2   = Color3.fromRGB(251, 146, 60),
            preview= Color3.fromRGB(234, 88, 12),
        },
        {
            name   = "🩷 Pink Kawaii",
            desc   = "Rose gold pastel",
            accent = Color3.fromRGB(219, 39, 119),
            acc2   = Color3.fromRGB(236, 72, 153),
            preview= Color3.fromRGB(219, 39, 119),
        },
        {
            name   = "🩵 Cyan Ice",
            desc   = "Biru es dingin",
            ...
            preview= Color3.fromRGB(6, 182, 212),
        },
        {
            name   = "🟡 Kuning Gold",
            desc   = "Emas mewah premium",
            accent = Color3.fromRGB(202, 138, 4),
            acc2   = Color3.fromRGB(234, 179, 8),
            preview= Color3.fromRGB(202, 138, 4),
        },
    }

    sectionTitle(pageTheme, "🎨 PILIH TEMA GUI", 8)

    local infoCard = makeCard(pageTheme, 36, 34)
    makeLabel(infoCard, "ℹ️  Klik tema untuk mengubah warna aksen GUI", 10, 0, 400, 34, 11, C.subtext, Enum.Font.Gotham)

    local activeThemeIdx = 1
    local themeCards = {}

    local function refreshThemeCards()
        for i, tc in ipairs(themeCards) do
            local isActive = (i == activeThemeIdx)
            pcall(function()
                local stroke = tc:FindFirstChildOfClass("UIStroke")
                if stroke then
                    stroke.Color = isActive and themes[i].accent or C.border
                    stroke.Thickness = isActive and 2 or 1
                end
                local activeDot = tc:FindFirstChild("activedot")
                if activeDot then activeDot.Visible = isActive end
            end)
        end
    end

    for i, theme in ipairs(themes) do
        local card = makeCard(pageTheme, 78 + (i-1) * 68, 56)
        themeCards[i] = card

        -- Color preview bar
        local previewBar = Instance.new("Frame")
        previewBar.Size = UDim2.new(0, 8, 1, -12)
        previewBar.Position = UDim2.new(0, 4, 0, 6)
        previewBar.BackgroundColor3 = theme.preview
        previewBar.BorderSizePixel = 0
        previewBar.Parent = card
        mkCorner(previewBar, 4)

        -- Color circle
        local circle = Instance.new("Frame")
        circle.Size = UDim2.new(0, 32, 0, 32)
        circle.Position = UDim2.new(0, 18, 0.5, -16)
        circle.BackgroundColor3 = theme.accent
        circle.BorderSizePixel = 0
        circle.Parent = card
        mkCorner(circle, 16)

        -- Inner circle accent2
        local innerCircle = Instance.new("Frame")
        innerCircle.Size = UDim2.new(0, 18, 0, 18)
        innerCircle.Position = UDim2.new(0.5, -9, 0.5, -9)
        innerCircle.BackgroundColor3 = theme.acc2
        innerCircle.BorderSizePixel = 0
        innerCircle.Parent = circle
        mkCorner(innerCircle, 9)
        ...
        -- Active dot indicator (pojok kanan)
        local activeDot = Instance.new("Frame")
        activeDot.Name = "activedot"
        activeDot.Size = UDim2.new(0, 10, 0, 10)
        activeDot.Position = UDim2.new(1, -18, 0.5, -5)
        activeDot.BackgroundColor3 = theme.accent
        activeDot.BorderSizePixel = 0
        activeDot.Visible = (i == 1)
        activeDot.Parent = card
        mkCorner(activeDot, 5)

        -- "AKTIF" label
        local activeLbl = Instance.new("TextLabel")
        activeLbl.Name = "activelbl"
        activeLbl.Size = UDim2.new(0, 38, 0, 16)
        activeLbl.Position = UDim2.new(1, -50, 0.5, -8)
        activeLbl.BackgroundTransparency = 1
        activeLbl.Text = i == 1 and "AKTIF" or ""
        activeLbl.TextColor3 = theme.accent
        activeLbl.Font = Enum.Font.GothamBlack
        ...
        local capI = i
        local capTheme = theme
        clickBtn.MouseButton1Click:Connect(function()
            -- Update active label pada kartu lama
            if themeCards[activeThemeIdx] then
                local oldLbl = themeCards[activeThemeIdx]:FindFirstChild("activelbl")
                if oldLbl then oldLbl.Text = "" end
            end

            activeThemeIdx = capI
            applyTheme(capTheme.accent, capTheme.acc2, capTheme.name)
            refreshThemeCards()

            -- Update label kartu baru
            local newLbl = card:FindFirstChild("activelbl")
            if newLbl then
                newLbl.Text = "AKTIF"
                newLbl.TextColor3 = capTheme.accent
            end
        end)
    ...
    -- Tip section bawah
    local tipOffset = 78 + #themes * 68 + 8
    sectionTitle(pageTheme, "ℹ️  INFO", tipOffset)

    local tipCard = makeCard(pageTheme, tipOffset + 28, 44)
    local tipLbl = Instance.new("TextLabel")
    tipLbl.Size = UDim2.new(1, -16, 1, 0)
    tipLbl.Position = UDim2.new(0, 10, 0, 0)
    tipLbl.BackgroundTransparency = 1
    tipLbl.RichText = true
    tipLbl.Text = "Tema mengubah warna aksen GUI secara <font color='rgb(220,38,38)'>real-time</font>. Pilih sesuai selera!"
    tipLbl.TextColor3 = C.subtext
    ...
    pageTheme.CanvasSize = UDim2.new(0, 0, 0, tipOffset + 90)
end
...
do
    ...
    local creditData = {
        ...
        {role="Helper",           name="Qiee", icon="🤝", color=Color3.fromRGB(100,220,150)},
    }
    ...
    pageCredits.CanvasSize = UDim2.new(0, 0, 0, 38 + #creditData * 68 + 60)
end
