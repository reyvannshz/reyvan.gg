...
do
    ...
    -- Info card
    local buyInfoCard = makeCard(pageAuto, 1062, 44)
    makeLabel(buyInfoCard, "💧 Water $20  ·  🧊 Sugar Block $100  ·  🍮 Gelatin $70", 10, 0, 440, 44, 10, C.subtext, Enum.Font.Gotham)

    -- NPC coord info
    local buyNpcCard = makeCard(pageAuto, 1114, 30)
    makeLabel(buyNpcCard, "🗺  NPC Shop: Teleport otomatis → klik dialog → beli item", 10, 0, 440, 30, 10, C.subtext, Enum.Font.Gotham)
    ...
    buyToggleBtn.MouseButton1Click:Connect(function()
        ...
        if autoBuy_Running then
            ...
            task.spawn(function()
                ...
                -- ══════════════════════════════════════════════
                --   KONFIGURASI BAHAN (sesuai nama tombol di GUI SHOP)
                --   Nama harus PERSIS sama dengan Text pada TextButton di GUI game
                -- ══════════════════════════════════════════════
                local BAHAN = {
                ...
                -- ── Koordinat NPC Marshmellow (dari savedLocations) ──
                local NPC_CF = CFrame.new(510.1238, 6.5872, 596.9278)
                ...
                -- ── Cek item di Backpack / Character ──
                ...
                -- ── Cari TextButton di seluruh GUI PlayerGui berdasarkan teks ──
                local function findShopButton(btnText)
                    local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
                    if not playerGui then return nil end
                    for _, gui in pairs(playerGui:GetDescendants()) do
                        if gui:IsA("TextButton") or gui:IsA("ImageButton") then
                            -- Cocokkan teks (case-insensitive, partial)
                            local t = gui.Text or ""
                            if t:lower():find(btnText:lower()) then
                                return gui
                            end
                        end
                        -- Cek juga TextLabel di dalam Frame yang mungkin menjadi "baris" shop
                        if gui:IsA("TextLabel") then
                            local t = gui.Text or ""
                            if t:lower():find(btnText:lower()) then
                                -- Coba parent-nya (mungkin Frame clickable)
                                local parent = gui.Parent
                                if parent and (parent:IsA("TextButton") or parent:IsA("ImageButton") or parent:IsA("Frame")) then
                                    return parent
                                end
                            end
                        end
                ...
                -- ── Klik GUI button dengan fireclick / simulasi ──
                local function clickGuiButton(btn)
                    if not btn then return false end
                    -- Coba fireclick (Synapse X / ScriptWare)
                    local ok = pcall(function() fireclick(btn) end)
                    if ok then task.wait(0.2) return true end
                    -- Coba MouseButton1Click fire
                    pcall(function() btn.MouseButton1Click:Fire() end)
                    task.wait(0.2)
                    -- Coba MouseButton1Down + Up
                    pcall(function()
                ...
                -- ── Cari ProximityPrompt di sekitar NPC ──
                ...
                -- ── Buka dialog NPC (klik "Yea.. you the guy?") ──
                local function openShop()
                    -- 1. Coba ProximityPrompt NPC
                    local pp = findNearestPrompt()
                    if pp then
                        pcall(function() fireproximityprompt(pp) end)
                        task.wait(0.5)
                    end

                    -- 2. Tekan E untuk interact
                    pcall(function()
                    ...
                    -- 3. Cari dan klik tombol dialog "Yea.. you the guy?"
                    local dialogBtn = findShopButton("Yea")
                    if not dialogBtn then
                        -- Coba keyword lain
                        dialogBtn = findShopButton("guy")
                    end
                    if dialogBtn then
                        print("[AutoBuy] 💬 Klik dialog NPC...")
                        clickGuiButton(dialogBtn)
                        task.wait(0.8)
                    else
                ...
                local function beliBahan(bahan)
                    ...
                    for attempt = 1, 5 do
                        ...
                        if hasItem(bahan.toolKw) then
                            print(string.format("[AutoBuy] ✅ %s sudah di Backpack!", bahan.label))
                            return true
                        end

                        print(string.format("[AutoBuy] Percobaan %d/5 untuk %s", attempt, bahan.label))

                        -- Step 1: Teleport ke NPC
                        buyStatusLbl.Text = "TELEPORT"
                        buyStatusLbl.TextColor3 = C.yellow
                        tpTo(NPC_CF)
                        task.wait(0.5)

                        -- Step 2: Buka shop
                        buyStatusLbl.Text = "BUKA SHOP"
                        buyStatusLbl.TextColor3 = C.yellow
                        openShop()
                        task.wait(0.6)

                        -- Step 3: Cari tombol item di GUI Shop
                        buyStatusLbl.Text = "KLIK ITEM"
                        buyStatusLbl.TextColor3 = C.yellow
                        local shopBtn = findShopButton(bahan.shopBtnText)

                        if shopBtn then
                            print(string.format("[AutoBuy] 🖱️  Klik tombol '%s' di GUI Shop...", bahan.shopBtnText))
                            clickGuiButton(shopBtn)
                            task.wait(1.0)
                        else
                            print(string.format("[AutoBuy] ⚠️  Tombol '%s' tidak ditemukan di GUI.", bahan.shopBtnText))
                            -- Coba buka shop lagi
                            openShop()
                            task.wait(0.5)
                            shopBtn = findShopButton(bahan.shopBtnText)
                            if shopBtn then
                                clickGuiButton(shopBtn)
                                task.wait(1.0)
                            end
                        ...
                        task.wait(0.5)
                        if hasItem(bahan.toolKw) then
                            print(string.format("[AutoBuy] ✅ BERHASIL! %s masuk Backpack.", bahan.label))
                            return true
                        end
                    ...
                    print(string.format("[AutoBuy] ❌ GAGAL membeli %s setelah 5x.", bahan.label))
                    return false
                end

                -- ══════════════════════════════════════════════
                --   MAIN LOOP: Water → Sugar Block → Gelatin
                -- ══════════════════════════════════════════════
                print("========================================")
                ...
                for i, bahan in ipairs(BAHAN) do
                    ...
                    if ok then
                        ...
                        allOk = false
                        print(string.format("[AutoBuy] ⚠️  Step %d gagal: %s", i, bahan.label))
                    end

                    -- Jeda antar bahan
                    if i < #BAHAN and autoBuy_Running then
                        print("[AutoBuy] ⏳ Jeda 1.5 detik...")
                        task.wait(1.5)
                    end
                end
                ...
                -- Ringkasan Backpack
                print("[AutoBuy] 📦 Isi Backpack sekarang:")
    ...
    pageAuto.CanvasSize = UDim2.new(0,0,0,1160)
end
