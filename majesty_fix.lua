...
do
    ...
    -- ===== PILIH LOKASI (Y=142) =====
    sectionTitle(pageTP, "PILIH LOKASI", 142)

    -- Tombol lokasi saved: mulai Y=166, setiap 44px
    -- 5 lokasi × 44px = 220px → selesai di Y=386
    for i, loc in ipairs(savedLocations) do
        local locBtn = Instance.new("TextButton")
        locBtn.Size = UDim2.new(1, -20, 0, 38)
        locBtn.Position = UDim2.new(0, 10, 0, 166 + (i-1)*44)
    ...
    -- ===== AUTO LOOP TELEPORT (setelah list lokasi selesai) =====
    -- 5 lokasi × 44px = 220px, mulai 166 → selesai 386
    -- Auto loop section mulai Y=396
    local loopSectionY = 166 + (#savedLocations * 44) + 10

    sectionTitle(pageTP, "AUTO LOOP TELEPORT", loopSectionY)

    local loopBtnY = loopSectionY + 26

    local loopToggle = Instance.new("TextButton")
    loopToggle.Size = UDim2.new(1, -20, 0, 38)
    loopToggle.Position = UDim2.new(0, 10, 0, loopBtnY)
    ...
    local intervalCard = makeCard(pageTP, loopBtnY + 46, 30)
    makeLabel(intervalCard, "⏱  Interval teleport: setiap 30 detik", 10, 0, 300, 30, 11, C.subtext, Enum.Font.Gotham)

    -- ===== TELEPORT KE PLAYER =====
    local playerSectionY = loopBtnY + 46 + 38

    sectionTitle(pageTP, "TELEPORT KE PLAYER", playerSectionY)

    local playerListFrame = Instance.new("ScrollingFrame")
    playerListFrame.Size = UDim2.new(1, -20, 0, 100)
    playerListFrame.Position = UDim2.new(0, 10, 0, playerSectionY + 26)
    ...
    local refreshBtnY = playerSectionY + 26 + 108

    local refreshBtn = Instance.new("TextButton")
    refreshBtn.Size = UDim2.new(1, -20, 0, 30)
    refreshBtn.Position = UDim2.new(0, 10, 0, refreshBtnY)
    ...
    -- Canvas size dinamis sesuai konten
    pageTP.CanvasSize = UDim2.new(0, 0, 0, refreshBtnY + 48)
end
