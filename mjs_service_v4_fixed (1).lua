...
local function equipTool(name)
    -- Retry loop: hingga 5x, dengan penanganan SeatPart dan verifikasi
    for attempt = 1, 5 do
        local ch = LocalPlayer.Character
        ...
        -- Jika masih duduk di kendaraan, paksa keluar dulu
        ...
        -- Cek apakah tool sudah ada di karakter (sudah ter-equip sebelumnya)
        if ch:FindFirstChild(name) then return true end

        -- Cari di backpack
        local t = LocalPlayer.Backpack:FindFirstChild(name)
        if not t then return false end  -- item tidak ada sama sekali

        hum:EquipTool(t)
        task.wait(0.35)

        -- Verifikasi: pastikan tool benar-benar pindah ke karakter
        if ch:FindFirstChild(name) then return true end
        task.wait(0.3)
    end
    return false
...
-- Interact kompor: versi robust dengan retry dan verifikasi item masuk kompor
local function cookInteract(toolName, radius)
    local beforeCount = toolName and countItem(toolName) or nil
    ...
    -- Retry interaksi hingga 4x, sampai item berkurang (konfirmasi masuk kompor)
    local maxTry = toolName and 4 or 2
    for attempt = 1, maxTry do
        ...
        -- Verifikasi: cek apakah item sudah berkurang dari inventory
        if toolName and beforeCount then
            local afterCount = countItem(toolName)
            if afterCount < beforeCount then
                break  -- item berhasil masuk kompor
            end
...
local function doOneCook()
    ...
    -- Verifikasi water berkurang, retry hingga 3x
    local waterBefore = countItem(CFG.ITEM_WATER)
    if waterBefore == 0 then
        _setStatus("Tidak ada Water!", Color3.fromRGB(255,60,90))
        isBusy = false; return false
    end

    local waterDone = false
    for _wTry = 1, 3 do
        cookInteract(CFG.ITEM_WATER)
        task.wait(0.3)
        if countItem(CFG.ITEM_WATER) < waterBefore then
            waterDone = true; break
        end
        _setStatus("Retry masukkan Water (".. _wTry .."/3)...", Color3.fromRGB(100,180,255))
        task.wait(0.5)
    end

    if not waterDone then
        -- Bisa jadi sudah masuk tapi server lambat, lanjut saja dengan timer default
        _setStatus("Water mungkin masuk – lanjut timer...", Color3.fromRGB(180,180,100))
    end
    ...
    local sugarBefore = countItem(CFG.ITEM_SUGAR)
    if sugarBefore == 0 then
    ...
    local sugarDone = false
    for _sTry = 1, 3 do
        cookInteract(CFG.ITEM_SUGAR)
        task.wait(0.3)
        if countItem(CFG.ITEM_SUGAR) < sugarBefore then
            sugarDone = true; break
        end
        _setStatus("Retry masukkan Sugar (".. _sTry .."/3)...", Color3.fromRGB(255,220,100))
        task.wait(0.5)
    end
    ...
    local gelBefore = countItem(CFG.ITEM_GEL)
    if gelBefore == 0 then
    ...
    for _gTry = 1, 3 do
        ...
        task.wait(0.3)
        if countItem(CFG.ITEM_GEL) < gelBefore then
            gelDone = true; break
        end
        _setStatus("Retry masukkan Gelatin (".. _gTry .."/3)...", Color3.fromRGB(255,200,50))
        task.wait(0.5)
    end
    ...
    for _mTry = 1, 3 do
        ...
        task.wait(0.5)
        local msAfter = countItem(CFG.ITEM_MS_SMALL) + countItem(CFG.ITEM_MS_MEDIUM) + countItem(CFG.ITEM_MS_LARGE)
        if msAfter > msBefore then break end
        if _mTry < 3 then
            _setStatus("Retry ambil MS (".._mTry.."/3)...", Color3.fromRGB(100,160,255))
            task.wait(0.5)
        end
    end
...
local function doAutoFully(setFullyStatus)
    ...
    while fullyRunning do
        ...
        if fullySavedPos then
            ...
            setFullyStatus("Apart belum dipilih! Pilih Apart di tab Auto Fully.", Color3.fromRGB(255,60,90))
            task.wait(3)
            break
        end
        if not fullyRunning then break end

        -- ── Stabilisasi setelah teleport ke Apart ──
        -- Unequip agar tidak ada tool aktif yang mengganggu interaksi kompor
        unequipAll()
        task.wait(0.5)

        -- Tunggu HRP benar-benar landed (tidak di udara / belum settle)
        task.wait(1.2)

        -- Validasi posisi: pastikan player benar-benar sudah dekat area masak
        local _apCh = LocalPlayer.Character
        local _apHRP = _apCh and _apCh:FindFirstChild("HumanoidRootPart")
        if _apHRP then
            local distToApart = (_apHRP.Position - fullySavedPos).Magnitude
            if distToApart > 15 then
                -- Posisi meleset, teleport ulang
                setFullyStatus("Posisi meleset ("..math.floor(distToApart).."st), teleport ulang...", Color3.fromRGB(255,160,40))
                task.wait(0.5)
                fullyTeleport(fullySavedPos)
                task.wait(1.5)
                unequipAll()
        ...
        -- Validasi bahan tersedia sebelum mulai masak
        if not hasAllIngredients() then
            setFullyStatus("Bahan tidak lengkap! Cek inventory.", Color3.fromRGB(255,60,90))
            task.wait(2)
            -- Skip ke loop berikutnya untuk beli bahan lagi
            if not fullyRunning then break end
            continue
        end
        ...
        while fullyRunning and hasAllIngredients() do
            ...
            if ok and err then
                cooked += 1
            elseif not ok then
                _setStatus("ERROR masak: "..(tostring(err) or "?"), Color3.fromRGB(255,60,90))
                task.wait(1)
            end
