...
do
    ...
    makeLabel(buyInfoCard, "💧 Water $20  ·  🧊 Sugar Block Bag $100  ·  🍮 Gelatin $70", 10, 0, 440, 30, 10, C.subtext, Enum.Font.Gotham)

    -- Hint card
    local buyHintCard = makeCard(pageAuto, 1100, 30)
    makeLabel(buyHintCard, "⚠  Pastikan kamu sudah berdiri di dekat NPC sebelum klik START", 10, 0, 440, 30, 10, C.yellow, Enum.Font.Gotham)
    ...
    buyToggleBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            ...
            -- ══════════════════════════════════════════
            --   KONFIGURASI BAHAN
            --   shopBtnText = teks persis pada TextButton di GUI SHOP game
            -- ══════════════════════════════════════════
            local BAHAN = {
                { label = "Water",         toolKw = "water",   shopBtnText = "Water",           emoji = "💧" },
                { label = "Sugar Block",   toolKw = "sugar",   shopBtnText = "Sugar Block Bag", emoji = "🧊" },
                { label = "Gelatin",       toolKw = "gelatin", shopBtnText = "Gelatin",          emoji = "🍮" },
            }
            ...
            -- ──────────────────────────────────────────
            --  HELPER: Scan seluruh PlayerGui cari TextButton / Frame
            --  yang teksnya mengandung kata kunci
            --  Mengembalikan daftar semua kandidat (sorted by visibility)
            -- ──────────────────────────────────────────
            local function findGuiButtons(keyword)
                ...
                for _, obj in pairs(playerGui:GetDescendants()) do
                    -- Cek TextButton / ImageButton langsung
                    ...
                    -- Cek TextLabel yang mungkin ada di dalam Frame/Button row
                    if obj:IsA("TextLabel") then
                        local txt = (obj.Text or ""):lower()
                        if txt:find(keyword) then
                            -- Cek parent-chain: apakah parent adalah button/clickable
                            local p = obj.Parent
                            for _ = 1, 4 do
            ...
            -- ──────────────────────────────────────────
            --  HELPER: Klik sebuah GUI button dengan semua metode
            -- ──────────────────────────────────────────
            local function clickBtn(btn)
                ...
                -- Metode 1: fireclick (Synapse X, ScriptWare, beberapa executor)
                pcall(function() fireclick(btn) end)
                task.wait(0.1)

                -- Metode 2: MouseButton1Click:Fire()
                pcall(function() btn.MouseButton1Click:Fire() end)
                task.wait(0.1)

                -- Metode 3: Simulate mouse press di posisi tengah tombol
                pcall(function()
                    local abs = btn.AbsolutePosition
                    local siz = btn.AbsoluteSize
                    local cx  = abs.X + siz.X / 2
                    local cy  = abs.Y + siz.Y / 2
            ...
            -- ──────────────────────────────────────────
            --  HELPER: Tunggu GUI muncul dalam timeout detik
            --  Mengembalikan button jika ditemukan, nil jika timeout
            -- ──────────────────────────────────────────
            local function waitForGui(keyword, timeout)
                timeout = timeout or 3
                local t = 0
                while t < timeout do
                    if not autoBuy_Running then return nil end
                    local btns = findGuiButtons(keyword)
                    for _, b in pairs(btns) do
                        if b.Visible then return b end
                    end
                    task.wait(0.15)
                    t = t + 0.15
                end
            ...
            -- ──────────────────────────────────────────
            --  HELPER: Klik dialog NPC "Yea.. you the guy?"
            --  Dipanggil SETELAH player sudah tekan E sendiri
            -- ──────────────────────────────────────────
            local function clickDialogNPC()
                -- Cari semua variasi teks dialog NPC
                local keywords = { "yea", "you the guy", "buy", "shop" }
                for _, kw in pairs(keywords) do
                    local btn = waitForGui(kw, 2)
                    if btn then
                        print("[AutoBuy] 💬 Klik dialog: '" .. (btn.Text or kw) .. "'")
                        clickBtn(btn)
                        task.wait(0.5)
                        return true
                    end
                end
                print("[AutoBuy] ⚠️  Dialog NPC tidak ditemukan (mungkin sudah terbuka)")
                return false
            end

            -- ──────────────────────────────────────────
            --  HELPER: Tunggu GUI SHOP terbuka dan beli item
            -- ──────────────────────────────────────────
            local function buyFromShop(bahan)
                -- Tunggu GUI SHOP muncul (ada kata "SHOP" di GUI)
                print("[AutoBuy] ⏳ Menunggu GUI SHOP terbuka...")
                local shopHeader = waitForGui("shop", 3)

                -- Langsung cari tombol item
                local itemBtn = waitForGui(bahan.shopBtnText, 3)
                if not itemBtn then
                    -- Coba fallback keyword lebih pendek
                    local fallback = bahan.toolKw
                    itemBtn = waitForGui(fallback, 2)
                end

                if itemBtn then
                    print(string.format("[AutoBuy] 🖱️  Klik tombol '%s' di SHOP GUI...", itemBtn.Text or bahan.shopBtnText))
                    clickBtn(itemBtn)
                    task.wait(0.8)
                    return true
                else
                    print(string.format("[AutoBuy] ⚠️  Tombol '%s' tidak ditemukan di GUI SHOP.", bahan.shopBtnText))
                    return false
                end
            end

            -- ══════════════════════════════════════════
            --   FUNGSI UTAMA: Beli satu bahan (max 5 percobaan)
            -- ══════════════════════════════════════════
            local function beliBahan(bahan)
                ...
                for attempt = 1, 5 do
                    ...
                    -- Sudah punya? Skip
                    if hasItem(bahan.toolKw) then
                        print(string.format("[AutoBuy] ✅ %s sudah ada di Backpack!", bahan.label))
                        return true
                    end

                    print(string.format("[AutoBuy] Percobaan %d/5 — %s", attempt, bahan.label))
                    buyStatusLbl.Text = "STEP " .. attempt
                    buyStatusLbl.TextColor3 = C.yellow

                    -- STEP A: Klik dialog NPC (kalau GUI dialog muncul)
                    clickDialogNPC()
                    task.wait(0.3)

                    -- STEP B: Klik tombol item di SHOP GUI
                    local ok = buyFromShop(bahan)
                    ...
                    if attempt < 5 then
                        print("[AutoBuy] Coba lagi dalam 1 detik...")
                        task.wait(1)
                    end
                end

                print(string.format("[AutoBuy] ❌ GAGAL membeli %s setelah 5 percobaan.", bahan.label))
                return false
            end

            -- ══════════════════════════════════════════
            --   MAIN: Water → Sugar Block → Gelatin
            -- ══════════════════════════════════════════
            print("========================================")
            print("[AutoBuy] 🛒 AUTO BUY BAHAN MARSHMALLOW DIMULAI")
            ...
            for i, bahan in ipairs(BAHAN) do
                ...
                print(string.format("\n[AutoBuy] ── STEP %d/3: %s %s ──", i, bahan.emoji, bahan.label))

                local ok = beliBahan(bahan)
                if ok then
                    print(string.format("[AutoBuy] ✅ Step %d selesai: %s berhasil dibeli!", i, bahan.label))
                else
                    allOk = false
                    print(string.format("[AutoBuy] ⚠️  Step %d: %s gagal, lanjut.", i, bahan.label))
                end
            ...
            if allOk then
                ...
                buyStatusLbl.TextColor3 = C.yellow
                print("\n[AutoBuy] ⚠️  Ada bahan yang tidak berhasil dibeli.")
            end
    ...
    pageAuto.CanvasSize = UDim2.new(0,0,0,1145)
end
