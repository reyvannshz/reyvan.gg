...
-- ========== KILL AURA CORE LOOP ==========
-- South Bronx = FE game. Client tidak bisa ubah Health/CFrame player lain secara langsung.
-- Metode yang bekerja di FE:
--   1. firetouchinterest  → picu kill/damage part milik game (lava, void trigger, dll)
--   2. sethiddenproperty  → set NetworkOwnership agar kita punya kontrol physics target
--   3. Teleport KITA ke target + simulasikan hit dengan tool/senjata yang sedang dipegang
--   4. fireproximityprompt → trigger interaksi berbahaya di dekat target
--   5. Loop spam sethiddenproperty + CFrame void setelah dapat network ownership

local kaKilledThisCycle = {}

-- Cari semua kill-parts di workspace (lava, void, killbrick, dll)
local function getKillParts()
    local parts = {}
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("BasePart") then
            local n = obj.Name:lower()
            if n:find("kill") or n:find("void") or n:find("lava") or n:find("death") or n:find("damage") then
                table.insert(parts, obj)
            end
            -- Cek juga script di dalamnya yang handle damage
            for _, child in pairs(obj:GetChildren()) do
                if child:IsA("Script") or child:IsA("LocalScript") then
                    local src = ""
                    pcall(function() src = child.Source:lower() end)
                    if src:find("health") or src:find("damage") or src:find("humanoid") then
                        table.insert(parts, obj)
                        break
                    end
                end
...
local function tryKillPlayer(plr, hrp, hum)
    -- ── Metode A: sethiddenproperty untuk dapat NetworkOwnership ──────
    pcall(function()
        sethiddenproperty(hrp, "NetworkOwnershipRule", 0)
    end)

    -- ── Metode B: Setelah dapat ownership, paksa CFrame ke void ───────
    pcall(function()
        hrp.CFrame = CFrame.new(hrp.Position.X, -9e9, hrp.Position.Z)
    end)

    -- ── Metode C: firetouchinterest ke semua kill-part di map ─────────
    pcall(function()
        local killParts = getKillParts()
        for _, kp in pairs(killParts) do
            firetouchinterest(hrp, kp, 0)  -- 0 = touch begin
            task.wait()
            firetouchinterest(hrp, kp, 1)  -- 1 = touch end
        end
    end)

    -- ── Metode D: firetouchinterest dengan HRP kita sendiri ───────────
    -- Kita punya senjata/tool? Picu touch antara tool kita dan target
    pcall(function()
    ...
    -- ── Metode E: Health manipulation via sethiddenproperty ──────────
    pcall(function()
        sethiddenproperty(hum, "Health", 0)
    end)

    -- ── Metode F: TakeDamage + Health normal (kadang FE game allow ini) ─
    pcall(function() hum.Health = 0 end)
    pcall(function() hum:TakeDamage(math.huge) end)
    pcall(function() hum:ChangeState(Enum.HumanoidStateType.Dead) end)

    -- ── Metode G: Cari RemoteEvent damage di game ────────────────────
    pcall(function()
        -- South Bronx pakai RemoteEvent untuk damage sistem
        -- Cari semua RemoteEvent dan coba fire dengan data damage
        local remotes = {}
        ...
        for _, remote in pairs(remotes) do
            ...
            if n:find("damage") or n:find("kill") or n:find("hit") or n:find("shoot") or n:find("fire") then
                pcall(function()
                    remote:FireServer(plr, 9999)
                end)
                pcall(function()
                    remote:FireServer(plr.Character, 9999)
                end)
                pcall(function()
                    remote:FireServer(hrp.Position, plr, 9999)
                end)
            end
        end
    end)
    ...
    while true do
        ...
        for _, plr in pairs(Players:GetPlayers()) do
            ...
            if dist <= killAuraRadius then
                -- Jalankan kill di thread terpisah agar tidak blocking loop utama
                task.spawn(function()
                    tryKillPlayer(plr, hrp, hum)
                end)
