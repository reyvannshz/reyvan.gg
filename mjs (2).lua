...
do
    ...
    local function getVehicleRoot(seat)
        ...
        -- Naik ke model kendaraan, cari PrimaryPart
        local model = seat:FindFirstAncestorOfClass("Model")
        if model then
            if model.PrimaryPart then
                return model.PrimaryPart
            end
            -- Fallback: cari BasePart terbesar (body kendaraan)
    ...
    local function startVehicleFly()
        ...
        vFlyConn = RunService.RenderStepped:Connect(function(dt)
            ...
            -- Hanya zero velocity dan gerak jika ada input
            if moveVec.Magnitude > 0 then
                ...
                -- Tidak ada input: hover di tempat (nol velocity saja)
                pcall(function()
                    root.AssemblyLinearVelocity  = Vector3.zero
                    root.AssemblyAngularVelocity = Vector3.zero
                end)
