...
local function onCharacterAdded(newChar)
    ...
    task.spawn(function()
        ...
        -- Tunggu 2 frame physics siap
        RunService.Stepped:Wait()
        RunService.Stepped:Wait()

        -- Anchor, set posisi, tahan, lepas
        hrp.Anchored = true
...
-- ========== TELEPORT VIA LoadCharacter ==========
-- Tidak mati, tidak lewat void
-- LoadCharacter() paksa respawn bersih dari server
-- posisi diset di onCharacterAdded sebelum server validate

local function doTeleport(targetCFrame)
    task.spawn(function()
        pendingTP = targetCFrame
        -- Paksa respawn tanpa mati — server spawn karakter baru di tempat
