...
-- ========== FUNGSI TELEPORT V5 - ANTI KICK DELAY ==========
-- Gerak dibagi jadi banyak langkah kecil dengan jeda panjang
-- Mirip seperti player jalan biasa, susah dideteksi anti-cheat
...
local function doTeleport(targetCFrame)
    ...
    local totalDist = (startPos.Position - targetCFrame.Position).Magnitude

    -- Hitung jumlah langkah berdasarkan jarak
    -- Makin jauh = makin banyak langkah supaya speed per langkah tetap wajar
    local steps = math.clamp(math.floor(totalDist / 10), 15, 60)
    local delayPerStep = 0.08  -- 0.08s per langkah = tidak terlalu cepat

    humanoid:ChangeState(Enum.HumanoidStateType.Physics)
    task.wait(0.5) -- jeda awal sebelum mulai bergerak

    for i = 1, steps do
        -- Selalu ambil char terbaru tiap langkah
        ...
        hrp.CFrame = startPos:Lerp(targetCFrame, t)

        task.wait(delayPerStep)
    end

    -- Finalize posisi tepat di tujuan
    char = LocalPlayer.Character
    if char then
        hrp = char:FindFirstChild("HumanoidRootPart")
        humanoid = char:FindFirstChildOfClass("Humanoid")
