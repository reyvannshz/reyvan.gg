...
local pendingTP = nil  -- CFrame tujuan yang akan dipakai saat spawn (fallback)
...
-- Fungsi inti: teleport aman via stepping (anti-kick, anti-mati)
local _tpBusy = false
local function doTeleport(targetCFrame)
    ...
    -- ── Metode 1: jarak dekat ≤ 60 studs — langsung set posisi ──
    if dist <= 60 then
        -- Freeze physics dulu supaya tidak terpelanting
    ...
    -- ── Metode 2: jarak menengah 60–500 studs — stepping 3 tahap ──
    -- Setiap lompatan max ~60 studs, jeda antar lompatan kecil
    if dist <= 500 then
        local steps     = math.ceil(dist / 55)   -- tiap step ~55 studs
        local direction = (endPos - startPos).Unit

        for i = 1, steps do
            local stepPos
            if i == steps then
                stepPos = endPos   -- step terakhir = tepat di tujuan
            else
                stepPos = startPos + direction * (55 * i)
                -- Offset Y sedikit ke atas agar tidak nyangkut di tanah
                stepPos = Vector3.new(stepPos.X, stepPos.Y + 1, stepPos.Z)
            end
            ...
            task.wait(0.06)   -- jeda antar step (tidak terdeteksi sebagai speed hack)
        end

        -- Stabilkan di posisi akhir
        hrp.AssemblyLinearVelocity  = Vector3.zero
        hrp.AssemblyAngularVelocity = Vector3.zero
        hrp.CFrame = targetCFrame
        task.wait(0.1)
    ...
    -- ── Metode 3: jarak jauh > 500 studs — stepping cepat 6 titik waypoint ──
    -- Dibagi jadi 6 waypoint, tiap waypoint dilompati cepat
    -- Ini jauh lebih aman dari kick dibanding 1 lompatan langsung
    do
        local waypoints = 6
        local direction = (endPos - startPos).Unit
        ...
        for i = 1, waypoints do
            local t       = i / waypoints
            local wPos    = startPos + (endPos - startPos) * t
            -- Angkat sedikit agar tidak terperangkap geometri
            wPos = Vector3.new(wPos.X, wPos.Y + 2, wPos.Z)
            if i == waypoints then
                wPos = endPos   -- titik akhir tepat
            end

            hrp.AssemblyLinearVelocity  = Vector3.zero
            hrp.AssemblyAngularVelocity = Vector3.zero
            humanoid:ChangeState(Enum.HumanoidStateType.Physics)
            hrp.CFrame = CFrame.new(wPos, wPos + direction)

            -- Jeda tiap waypoint: semakin jauh semakin perlu jeda
            -- Total waktu ~0.4–0.8 detik untuk jarak > 500 studs
            task.wait(0.07)
        end

        -- Landing: set tepat di tujuan + stabilkan
        hrp.AssemblyLinearVelocity  = Vector3.zero
        hrp.AssemblyAngularVelocity = Vector3.zero
        hrp.CFrame = targetCFrame
        task.wait(0.12)
...
-- Tambahan proteksi: matikan fall damage sementara selama teleport
-- agar tidak mati kalau landing dari atas
local function safeTeleport(targetCFrame, statusLabel)
    task.spawn(function()
        ...
        -- Simpan state fall damage & platform stand sebelum TP
        local origPlatStand = humanoid.PlatformStand

        -- Aktifkan platform stand agar tidak kena damage saat bergerak
        pcall(function()
            humanoid.PlatformStand = true
        end)

        -- Anti fall damage: sementara paksa health selalu penuh selama TP
        local conn
        ...
        -- Tunggu sebentar lalu matikan proteksi
        task.wait(0.5)
        conn:Disconnect()
        pcall(function()
            if humanoid and humanoid.Parent then
                humanoid.PlatformStand = origPlatStand
                humanoid.Health = humanoid.MaxHealth  -- pulihkan health
            end
        end)
