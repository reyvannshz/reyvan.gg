...
local aimbotSmooth    = 8               -- 1-20, makin tinggi makin lambat
local aimbotTarget    = "Head"          -- "Head" atau "HumanoidRootPart"
local aimbotActive    = false           -- true saat keybind ditahan
local aimbotFovCircle = nil
local aimbotKeybind   = Enum.UserInputType.MouseButton2  -- default RMB
local aimbotKeybindIsKey = false        -- false = MouseButton, true = KeyCode
local aimbotKeybindCode = nil           -- KeyCode jika pakai keyboard
local aimbotKeybindLabel = "RMB"       -- teks display
local isBindingKey    = false           -- lagi nunggu input keybind
local keybindBtnRef   = nil             -- ref UI tombol keybind
-- UI refs
...
-- Camera mode: rotate camera toward target
local function aimCamera(targetPart)
    if not targetPart then return end
    local cf = Camera.CFrame
    local dir = (targetPart.Position - cf.Position).Unit
    local goal = CFrame.new(cf.Position, cf.Position + dir)
    local t = math.clamp(1 - (aimbotSmooth / 20), 0.05, 1)
    Camera.CFrame = cf:Lerp(goal, t)
end

-- FreeAim mode: move mouse delta each frame so crosshair follows target
local function aimFreeAim(targetPart)
    if not targetPart then return end
    local sp, vis = Camera:WorldToViewportPoint(targetPart.Position)
    if not vis then return end
    local cx = Camera.ViewportSize.X / 2
    local cy = Camera.ViewportSize.Y / 2
    local dx = sp.X - cx
    local dy = sp.Y - cy
    -- Smoothness: 1 = instant, 20 = very slow
    local speed = math.clamp(1 - (aimbotSmooth - 1) / 20, 0.05, 1)
    local mx = dx * speed
    local my = dy * speed
...
print("=== MAJESTY ONTOP v8.0 - VALARY THEME ===")
print("Fitur:")
print("- 📦 Inventory Tracker")
print("- ⚙️  Auto MS")
print("- 👁️  ESP + Item Dipegang")
print("- 🚀 Teleport")
print("- 🎯 AIMBOT: Camera/FreeAim | Custom Keybind | FOV Circle")
print("========================================")
