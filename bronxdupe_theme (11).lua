...
-- ========== TELEPORT - SMALL STEP METHOD ==========
-- Berdasarkan script South Bronx yang sudah terbukti work:
-- Set CFrame bertahap, tiap step SANGAT kecil (3-5 studs)
-- dengan task.wait() antar setiap step
...
local function doTeleport(targetCFrame)
    task.spawn(function()
        ...
        -- Step size kecil: 4 studs per step seperti script South Bronx asli
        local stepSize = 4
        local steps    = math.ceil(dist / stepSize)

        for i = 1, steps do
            -- Re-check karakter tiap step
            ...
            task.wait()  -- 1 frame wait — persis seperti script South Bronx asli
        end
...
local function safeTeleport(targetCFrame, statusLabel)
    ...
    if statusLabel then
        ...
        local estTime = steps * (1/60) + 0.3  -- steps * 1 frame + buffer
