...
-- ========== RESPAWN OVERLAY ==========
local respawnOverlayActive = false
local respawnTimerRunning = false
local respawnOverlay = nil
local respawnLogo = nil
local respawnTimerLabel = nil
...
local function createRespawnOverlay()
    if respawnOverlay then respawnOverlay:Destroy() end
    ...
    centerFrame.BackgroundColor3 = Color3.fromRGB(15, 20, 25)
    ...
    -- Logo / Text "MAJESTY STORE"
    local logo = Instance.new("TextLabel")
    logo.Size = UDim2.new(1, -40, 0, 60)
    ...
    glow.Size = UDim2.new(1, -36, 0, 64)
    glow.Position = UDim2.new(0, 18, 0, 28)
    glow.BackgroundTransparency = 1
    glow.Text = "MAJESTY STORE"
    glow.TextColor3 = Color3.fromRGB(0, 200, 100)
    ...
    timerText.Size = UDim2.new(1, -40, 0, 80)
    timerText.Position = UDim2.new(0, 20, 0, 120)
    timerText.BackgroundTransparency = 1
    timerText.Text = "RESPAWN IN\n00"
    timerText.TextColor3 = Color3.fromRGB(255, 255, 255)
    timerText.Font = Enum.Font.GothamBold
    timerText.TextSize = 32
    timerText.TextScaled = true
    timerText.TextWrapped = true
    timerText.Parent = centerFrame
    
    -- Sub text
    local subText = Instance.new("TextLabel")
    subText.Size = UDim2.new(1, -40, 0, 40)
    subText.Position = UDim2.new(0, 20, 0, 210)
    subText.BackgroundTransparency = 1
    subText.Text = "majesty.gg"
    subText.TextColor3 = Color3.fromRGB(150, 150, 150)
    subText.Font = Enum.Font.Gotham
    subText.TextSize = 14
    subText.Parent = centerFrame
    
    -- Small image decoration (simple ASCII-style with Frame)
    local imgContainer = Instance.new("Frame")
    imgContainer.Size = UDim2.new(0, 80, 0, 80)
    imgContainer.Position = UDim2.new(0.5, -40, 0, 220)
    imgContainer.BackgroundColor3 = Color3.fromRGB(0, 40, 30)
    imgContainer.BackgroundTransparency = 0.5
    imgContainer.BorderSizePixel = 0
    imgContainer.Parent = centerFrame
    local imgCorner = Instance.new("UICorner")
    imgCorner.CornerRadius = UDim.new(1, 0)
    imgCorner.Parent = imgContainer
    
    local imgIcon = Instance.new("TextLabel")
    imgIcon.Size = UDim2.new(1, 0, 1, 0)
    imgIcon.BackgroundTransparency = 1
    imgIcon.Text = "✨"
    imgIcon.TextColor3 = Color3.fromRGB(0, 255, 136)
    imgIcon.Font = Enum.Font.GothamBold
    imgIcon.TextSize = 40
    imgIcon.Parent = imgContainer
    
    return overlay, timerText
end

respawnOverlay, respawnTimerLabel = createRespawnOverlay()

-- ========== RESPAWN HANDLER ==========
local respawnCheckConnection = nil
local respawnActive = false
local respawnStartTime = 0
local RESPAWN_WAIT_TIME = 20 -- 20 seconds wait before auto TP

local function showRespawnOverlay()
    if not respawnOverlay then return end
    respawnOverlay.Visible = true
    respawnOverlayActive = true
    respawnStartTime = tick()
    
    -- Update timer every second
    if respawnTimerRunning then return end
    respawnTimerRunning = true
    
    task.spawn(function()
        while respawnOverlayActive and respawnOverlay and respawnOverlay.Visible do
            local elapsed = tick() - respawnStartTime
            local remaining = math.max(0, RESPAWN_WAIT_TIME - elapsed)
            local secondsLeft = math.ceil(remaining)
            
            if respawnTimerLabel then
                if secondsLeft <= 0 then
                    respawnTimerLabel.Text = "RESPAWN IN\n00"
                else
...
local function hideRespawnOverlay()
...
local function onCharacterRespawned(character)
    -- Hide overlay when character is back
    hideRespawnOverlay()
    
    -- If there's pending TP destination, teleport after respawn
    if tpPending and tpDestination then
        task.spawn(function()
            ...
            if hrp and hum then
                ...
                if tpStatusValue then
                    ...
                    tpStatusValue.TextColor3 = C.accent
                    task.wait(2)
...
-- Monitor character death and respawn
local function monitorRespawn()
    if respawnCheckConnection then
        respawnCheckConnection:Disconnect()
    end
    
    respawnCheckConnection = LocalPlayer.CharacterAdded:Connect(function(character)
        -- Character respawned, hide overlay
        onCharacterRespawned(character)
    end)
    
    -- Check every frame if character is dead
    task.spawn(function()
        while true do
            task.wait(0.5)
            ...
            if char and hum and hum.Health <= 0 and not respawnOverlayActive then
                -- Player is dead, show overlay
                showRespawnOverlay()
            elseif char and hum and hum.Health > 0 and respawnOverlayActive then
                -- Player is alive, hide overlay
                hideRespawnOverlay()
            end
        end
    end)
...
-- ========== MODIFY TP FUNCTION TO SHOW OVERLAY ==========
local originalTpTo = nil
local tpDestination = nil
local tpPending = false

local function tpToWithOverlay(x, y, z)
    task.spawn(function()
        local char = LocalPlayer.Character
        local hum = char and char:FindFirstChildOfClass("Humanoid")
...
-- Override the tpTo function in the Teleport page
local C_colors = {
...
do
    ...
    -- ========== HIDE/SHOW GUI KEYBIND UI ==========
    mkSep(pageAimbot, y, "HIDE / SHOW GUI KEYBIND"); y = y + 20
    local guiKbRow = mkRow(pageAimbot, y, 34)
    local guiKbLbl = Instance.new("TextLabel"); guiKbLbl.Size=UDim2.new(0.55,0,1,0); guiKbLbl.Position=UDim2.new(0,10,0,0); guiKbLbl.BackgroundTransparency=1; guiKbLbl.Text="Toggle GUI Key"; guiKbLbl.TextColor3=C.text; guiKbLbl.Font=Enum.Font.Gotham; guiKbLbl.TextSize=11; guiKbLbl.TextXAlignment=Enum.TextXAlignment.Left; guiKbLbl.Parent=guiKbRow
    local guiKbBtn = Instance.new("TextButton"); guiKbBtn.Size=UDim2.new(0,80,0,22); guiKbBtn.Position=UDim2.new(1,-88,0.5,-11); guiKbBtn.BackgroundTransparency=1; guiKbBtn.Text="F1"; guiKbBtn.TextColor3=C.text; guiKbBtn.Font=Enum.Font.Gotham; guiKbBtn.TextSize=10; guiKbBtn.BorderSizePixel=0; guiKbBtn.Parent=guiKbRow
    guiKeybindBtnRef = guiKbBtn
    guiKbBtn.MouseButton1Click:Connect(function()
        if isBindingGuiKey then return end
        isBindingGuiKey = true; guiKbBtn.Text = "..."; guiKbBtn.TextColor3 = C.subtext
    end)
    y = y + 40
    local guiKbInfo = makeCard(pageAimbot, y, 24)
    makeLabel(guiKbInfo, "Default: F1  |  Tekan tombol di atas untuk ganti keybind", 10, 0, 440, 24, 9, C.subtext)
    y = y + 32
    ...
    UserInputService.InputBegan:Connect(function(input,gpe)
        ...
        if isBindingGuiKey then
            if input.UserInputType==Enum.UserInputType.MouseButton1 then return end; isBindingGuiKey=false
            local kn3=tostring(input.KeyCode):gsub("Enum%.KeyCode%.",""); local un3=tostring(input.UserInputType):gsub("Enum%.UserInputType%.","")
            if un3=="MouseButton2" then guiToggleKeyType="MouseButton";guiToggleMBtn=Enum.UserInputType.MouseButton2;guiToggleKeyCode=nil
            elseif un3=="MouseButton3" then guiToggleKeyType="MouseButton";guiToggleMBtn=Enum.UserInputType.MouseButton3;guiToggleKeyCode=nil
            elseif un3=="Keyboard" and kn3~="Unknown" then guiToggleKeyType="KeyCode";guiToggleKeyCode=input.KeyCode;guiToggleMBtn=nil
            else isBindingGuiKey=true; return end
            local dispName = guiToggleKeyCode and tostring(guiToggleKeyCode):gsub("Enum%..*%.","") or (guiToggleMBtn==Enum.UserInputType.MouseButton2 and "RMB" or "MMB")
            guiKbBtn.Text=dispName; guiKbBtn.TextColor3=C.text
        end
    end)
end
...
UserInputService.InputBegan:Connect(function(input)
    ...
    -- ========== TOGGLE GUI HIDE/SHOW ==========
    if isBindingGuiKey or isBindingKey or isBindingMin then return end
    local matchKey = guiToggleKeyType=="KeyCode" and guiToggleKeyCode~=nil and input.UserInputType==Enum.UserInputType.Keyboard and input.KeyCode==guiToggleKeyCode
    local matchMBtn = guiToggleKeyType=="MouseButton" and guiToggleMBtn~=nil and input.UserInputType==guiToggleMBtn
    if matchKey or matchMBtn then
...
print("=== MAJESTY STORE v8.3.1 LOADED ===")
print("discord.gg/VPeZbhCz8M")
print("[RESPAWN OVERLAY] Menampilkan logo MAJESTY STORE dengan timer 20 detik saat mati/teleport")
print("[GUI TOGGLE] Default: F1 | Bisa diatur di tab AIMBOT > Hide/Show GUI Keybind")
