...
do
    ...
    UserInputService.InputBegan:Connect(function(input, gpe)
        if isBindingKey then
            -- Abaikan klik kiri (biar nggak conflict)
            ...
            elseif input.UserInputType == Enum.UserInputType.Keyboard then
                ...
                aimbotKeybindLabel = tostring(input.KeyCode):gsub("Enum.KeyCode.", "")
            end
...
-- Track posisi mouse real-time buat FOV circle dan FreeAim
...
-- ========== AIMBOT FOV CIRCLE (mengikuti mouse) ==========
...
RunService.RenderStepped:Connect(function()
    -- FOV circle mengikuti posisi mouse (bukan tengah layar)
...
-- Prediction: hitung velocity dan prediksi posisi target berikutnya
local function getPredictedPosition(part, player)
    ...
    if dt > 0 and dt < 0.5 then
        -- Hitung velocity dari pergerakan posisi
        local rawVel = (currentPos - cache.lastPos) / dt
        -- Smooth velocity biar nggak jitter (exponential moving average)
        cache.lastVel = cache.lastVel:Lerp(rawVel, 0.4)
    end
    ...
    -- Prediksi posisi: tambahkan velocity × time ahead × strength
    local timeAhead = predStrength * 0.5   -- max ~0.5 detik ke depan
    return currentPos + cache.lastVel * timeAhead
end

-- Cari target terbaik dalam FOV relatif ke posisi MOUSE (bukan tengah layar)
...
-- Camera mode: rotate camera to predicted target position
...
-- FreeAim mode: gerakkan mouse smooth ke target (crosshair mengikuti target)
-- Anti-flick: hitung delta dari mouse ke screen pos target, lerp smooth
local function aimFreeAim(targetPart, targetPlayer)
    ...
    -- Delta dari posisi mouse sekarang ke target di layar
    local dx = sp.X - mousePos.X
    local dy = sp.Y - mousePos.Y

    -- Smooth factor: makin tinggi smooth makin lambat
    local speed = math.clamp(1 - (aimbotSmooth - 1) / 20, 0.03, 1)

    -- Accumulate smooth movement (lerp accumulator ke target delta)
    -- Ini yang mencegah flick — kita tidak langsung apply dx/dy penuh
    freeAimAccX = freeAimAccX + (dx - freeAimAccX) * speed
    freeAimAccY = freeAimAccY + (dy - freeAimAccY) * speed

    -- Clamp agar tidak overshoot saat target dekat
    local maxStep = 80
    local mx_move = math.clamp(freeAimAccX, -maxStep, maxStep)
    local my_move = math.clamp(freeAimAccY, -maxStep, maxStep)

    -- Jika sudah sangat dekat, reset accumulator (prevent drift)
    ...
    pcall(function() mousemoverel(mx_move, my_move) end)
end

-- Keybind hold check
local function isAimbotKeyHeld()
    if isBindingKey then return false end
    if aimbotKeybindIsKey then
        return UserInputService:IsKeyDown(aimbotKeybindCode)
    else
...
-- Reset accumulator saat aimbot dilepas biar tidak muncul flick saat activate ulang
...
RunService.RenderStepped:Connect(function()
    ...
    if not aimbotActive then
        -- Reset accumulator saat tombol dilepas
        freeAimAccX = 0
        freeAimAccY = 0
        wasActive = false
        return
    ...
    -- Reset accumulator juga saat pertama kali aktif (prevent first-frame flick)
...
print("=== MAJESTY ONTOP v9.0 - VALARY THEME ===")
print("Fitur:")
