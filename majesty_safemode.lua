...
do
    ...
    sectionTitle(pageAuto,"SAFE MODE",468)

    local safeTogBtn = Instance.new("TextButton")
    safeTogBtn.Size = UDim2.new(1,-20,0,36)
    safeTogBtn.Position = UDim2.new(0,10,0,492)
    ...
    local smCard1 = makeCard(pageAuto,536,44)
    makeLabel(smCard1,"STATUS",12,0,80,44,10,C.subtext)
    safeModeStatusLbl = makeLabel(smCard1,"OFF",90,0,300,44,13,C.red)

    local smInfoCard = makeCard(pageAuto,588,22)
    makeLabel(smInfoCard,"Detect hit saat Auto MS → VTP ke Jual/Beli Marshmellow",10,0,430,22,9,C.subtext)
    ...
    sectionTitle(pageAuto,"AUTO SELL",618)

    local sellTogBtn = Instance.new("TextButton")
    sellTogBtn.Size = UDim2.new(1,-20,0,36)
    sellTogBtn.Position = UDim2.new(0,10,0,644)
    ...
    local ssc = makeCard(pageAuto,688,44); makeLabel(ssc,"SELL",12,0,80,44,10,C.subtext)
    sellStatusLbl_ref = makeLabel(ssc,"OFF",90,0,200,44,13,C.red)
    local sic = makeCard(pageAuto,740,44); makeLabel(sic,"ITEM",12,0,80,44,10,C.subtext)
    sellItemLbl_ref = makeLabel(sic,"-",90,0,280,44,13,C.text)
    local sfc = makeCard(pageAuto,792,44); makeLabel(sfc,"FASE",12,0,80,44,10,C.subtext)
    local asPhaseLbl = makeLabel(sfc,"Idle",90,0,280,44,12,C.accent2)
    local stc = makeCard(pageAuto,844,40); makeLabel(stc,"TERJUAL",12,0,80,40,10,C.subtext)
    ...
    -- ========== BUY (from arul) ==========
    sectionTitle(pageAuto,"BUY BAHAN",910)
    ...
    makeSliderRow(992,"Sugar Block Bag",
    ...
    local buyTogBtn=Instance.new("TextButton"); buyTogBtn.Size=UDim2.new(1,-20,0,36); buyTogBtn.Position=UDim2.new(0,10,0,1108)
    ...
    local bsc=makeCard(pageAuto,1152,44); makeLabel(bsc,"STATUS",12,0,80,44,10,C.subtext)
    local bsl=makeLabel(bsc,"OFF",90,0,180,44,13,C.red)
    local bpc=makeCard(pageAuto,1204,44); makeLabel(bpc,"PHASE",12,0,80,44,10,C.subtext)
    local bpl=makeLabel(bpc,"Idle",90,0,280,44,13,C.accent2)
    local bicc=makeCard(pageAuto,1256,44); makeLabel(bicc,"ITEM",12,0,80,44,10,C.subtext)
...
-- Fungsi TP ke Jual/Beli Marshmellow (standalone, tidak bergantung scope VTP)
local SAFE_X = 510.9961853027344
local SAFE_Y = 3.5872106552124023
local SAFE_Z = 598.3929443359375
...
-- Health monitor: detect hit saat Auto MS aktif
task.spawn(function()
    while true do
        task.wait(0.1)  -- polling cepat untuk deteksi hit

        -- Hanya aktif jika Safe Mode ON dan Auto MS sedang berjalan
        if not safeMode or not AutoMS_Running then
            -- Reset lastHealth saat MS aktif lagi
            local char = LocalPlayer.Character
            local hum  = char and char:FindFirstChildOfClass("Humanoid")
            if hum and hum.Health > 0 then lastHealth = hum.Health end
            continue
        end

        if safeModeActive then continue end  -- sudah dalam proses escape

        local char = LocalPlayer.Character
        local hum  = char and char:FindFirstChildOfClass("Humanoid")
        if not char or not hum or hum.Health <= 0 then
            lastHealth = 100; continue
        end

        local currentHealth = hum.Health

        -- Detect: HP berkurang (kena hit)
        if currentHealth < lastHealth then
            ...
            print("[SAFE MODE] Kena hit " .. dmg .. " HP! TP ke Jual/Beli Marshmellow")

            -- Pause autoMS loop
            AutoMS_Running = false
            if statusValue then
                statusValue.Text = "SAFE!"; statusValue.TextColor3 = Color3.fromRGB(255, 60, 60)
            end
            if phaseValue then phaseValue.Text = "Kabur..." end

            task.wait(0.1)

            -- Eksekusi TP via VTP ke Jual/Beli Marshmellow
            tpToSafe()
            task.wait(0.8)

            if safeModeStatusLbl then
                safeModeStatusLbl.Text = "SELAMAT - di Jual/Beli"
            ...
            print("[SAFE MODE] Berhasil TP ke Jual/Beli Marshmellow")

            -- Tunggu sebentar sebelum bisa detect lagi
            task.wait(3)
            lastHealth = hum.Health  -- reset setelah aman
