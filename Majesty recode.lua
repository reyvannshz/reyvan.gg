...
do
    ...
    -- ========== BUY (from arul) ==========
    sectionTitle(pageAuto,"BUY BAHAN",700)
    ...
    makeSliderRow(782,"Sugar Block Bag",
    ...
    local buyTogBtn=Instance.new("TextButton"); buyTogBtn.Size=UDim2.new(1,-20,0,36); buyTogBtn.Position=UDim2.new(0,10,0,898)
    ...
    local bsc=makeCard(pageAuto,942,44); makeLabel(bsc,"STATUS",12,0,80,44,10,C.subtext)
    local bsl=makeLabel(bsc,"OFF",90,0,180,44,13,C.red)
    local bpc=makeCard(pageAuto,994,44); makeLabel(bpc,"PHASE",12,0,80,44,10,C.subtext)
    local bpl=makeLabel(bpc,"Idle",90,0,280,44,13,C.accent2)
    local bicc=makeCard(pageAuto,1046,44); makeLabel(bicc,"ITEM",12,0,80,44,10,C.subtext)
    ...
    pageAuto.CanvasSize=UDim2.new(0,0,0,1110)
end
...
do
    ...
    local function makeStatCell(parent,lTxt,vTxt,vColor)
        local cell=Instance.new("Frame"); cell.Size=UDim2.new(0.33,-4,1,0); cell.BackgroundColor3=C.card; cell.BorderSizePixel=0; cell.Parent=parent; mkCorner(cell,5); mkStroke(cell,1,C.border)
        local lb=Instance.new("TextLabel"); lb.Size=UDim2.new(1,-8,0,14); lb.Position=UDim2.new(0,8,0,6); lb.BackgroundTransparency=1; lb.Text=lTxt; lb.TextColor3=C.subtext; lb.Font=Enum.Font.Gotham; lb.TextSize=9; lb.TextXAlignment=Enum.TextXAlignment.Left; lb.Parent=cell
        local vl=Instance.new("TextLabel"); vl.Size=UDim2.new(1,-8,0,20); vl.Position=UDim2.new(0,8,0,20); vl.BackgroundTransparency=1; vl.Text=vTxt; vl.TextColor3=vColor or C.text; vl.Font=Enum.Font.Gotham; vl.TextSize=12; vl.TextXAlignment=Enum.TextXAlignment.Left; vl.Parent=cell
        return vl
    end
    tpStatusValue = makeStatCell(stGrid,"STATUS", "STANDBY", C.yellow)
    tpLoopValue   = makeStatCell(stGrid,"MODE",   "ONCE",    C.accent)
    tpMethodValue = makeStatCell(stGrid,"METODE", "JALAN",   C.accent2)

    -- Mode selector
    sectionTitle(pageTP,"PILIH METODE TELEPORT",60)
    local modeSel=Instance.new("Frame"); modeSel.Size=UDim2.new(1,-20,0,34); modeSel.Position=UDim2.new(0,10,0,86)
    modeSel.BackgroundColor3=C.card2; modeSel.BorderSizePixel=0; modeSel.Parent=pageTP; mkCorner(modeSel,5); mkStroke(modeSel,1,C.border)
    local wMBtn=Instance.new("TextButton"); wMBtn.Size=UDim2.new(0.5,0,1,0); wMBtn.BackgroundColor3=Color3.fromRGB(0,26,46); wMBtn.Text="Jalan Kaki (Moonwalk)"; wMBtn.TextColor3=C.accent2; wMBtn.Font=Enum.Font.Gotham; wMBtn.TextSize=11; wMBtn.BorderSizePixel=0; wMBtn.Parent=modeSel; mkCorner(wMBtn,5)
    local vMBtn=Instance.new("TextButton"); vMBtn.Size=UDim2.new(0.5,0,1,0); vMBtn.Position=UDim2.new(0.5,0,0,0); vMBtn.BackgroundColor3=C.card2; vMBtn.Text="Kendaraan (Anti Void)"; vMBtn.TextColor3=C.subtext; vMBtn.Font=Enum.Font.Gotham; vMBtn.TextSize=11; vMBtn.BorderSizePixel=0; vMBtn.Parent=modeSel; mkCorner(vMBtn,5)
    local function setTpMode(mode)
        tpMethod=mode
        if mode=="walk" then wMBtn.BackgroundColor3=Color3.fromRGB(0,26,46);wMBtn.TextColor3=C.accent2;vMBtn.BackgroundColor3=C.card2;vMBtn.TextColor3=C.subtext;tpMethodValue.Text="JALAN";tpMethodValue.TextColor3=C.accent2
        else vMBtn.BackgroundColor3=Color3.fromRGB(26,0,40);vMBtn.TextColor3=C.purple;wMBtn.BackgroundColor3=C.card2;wMBtn.TextColor3=C.subtext;tpMethodValue.Text="MOBIL";tpMethodValue.TextColor3=C.purple end
    end
    wMBtn.MouseButton1Click:Connect(function() setTpMode("walk") end)
    vMBtn.MouseButton1Click:Connect(function() setTpMode("vehicle") end)

    -- AUTO RANGE INFO - Jalan kaki
    local wInfoCard=makeCard(pageTP,128,44)
    makeLabel(wInfoCard,"Jalan Kaki  -  Auto Range",10,6,280,14,11,C.accent2)
    makeLabel(wInfoCard,"Step & delay otomatis dihitung dari jarak  |  Moonwalk + anchor anti-ragdoll",10,24,430,14,9,C.subtext)
    -- Live display jalan kaki
    local wLiveCard=makeCard(pageTP,180,32)
    makeLabel(wLiveCard,"STEP",10,0,40,32,9,C.subtext)
    walkStepLiveLabel  = makeLabel(wLiveCard,"--",50,0,70,32,11,C.accent2)
    makeLabel(wLiveCard,"DELAY",130,0,44,32,9,C.subtext)
    walkDelayLiveLabel = makeLabel(wLiveCard,"--",178,0,60,32,11,C.accent2)
    makeLabel(wLiveCard,"SPEED",248,0,44,32,9,C.subtext)
    walkSpeedLiveLabel = makeLabel(wLiveCard,"--",296,0,90,32,11,C.accent)

    -- AUTO RANGE INFO - Kendaraan
    local vInfoCard=makeCard(pageTP,220,44)
    makeLabel(vInfoCard,"Kendaraan  -  Auto Range",10,6,280,14,11,C.purple)
    makeLabel(vInfoCard,"Step & delay otomatis dihitung dari jarak  |  Anchor anti-void per step",10,24,430,14,9,C.subtext)
    -- Live display kendaraan
    local vLiveCard=makeCard(pageTP,272,32)
    makeLabel(vLiveCard,"STEP",10,0,40,32,9,C.subtext)
    vehStepLiveLabel  = makeLabel(vLiveCard,"--",50,0,70,32,11,C.purple)
    makeLabel(vLiveCard,"DELAY",130,0,44,32,9,C.subtext)
    vehDelayLiveLabel = makeLabel(vLiveCard,"--",178,0,60,32,11,C.purple)
    makeLabel(vLiveCard,"SPEED",248,0,44,32,9,C.subtext)
    vehSpeedLiveLabel = makeLabel(vLiveCard,"--",296,0,90,32,11,C.accent)

    -- Locations (Kill → Respawn → TP like arul)
    -- Setup respawn TP system
    local tpDestination_v2 = nil
    local tpPending_v2     = false
    local function onCharacterAdded_v2(char2)
        if not tpPending_v2 or not tpDestination_v2 then return end
        tpPending_v2 = false
        task.spawn(function()
            local hrp2 = char2:WaitForChild("HumanoidRootPart", 10)
            local hum2 = char2:WaitForChild("Humanoid", 10)
            if not hrp2 or not hum2 then return end
            task.wait(1)
            hrp2.CFrame = CFrame.new(tpDestination_v2.x, tpDestination_v2.y+3, tpDestination_v2.z)
            tpDestination_v2 = nil
        end)
    end
    if LocalPlayer.Character then task.spawn(function() onCharacterAdded_v2(LocalPlayer.Character) end) end
    LocalPlayer.CharacterAdded:Connect(onCharacterAdded_v2)

    local function tpToRespawn(x, y, z)
        task.spawn(function()
            local char2=LocalPlayer.Character
            local hum2=char2 and char2:FindFirstChildOfClass("Humanoid")
            tpDestination_v2={x=x, y=y, z=z}; tpPending_v2=true
            if char2 and hum2 and hum2.Health>0 then hum2.Health=0 end
        end)
    end

    -- Teleport locations from arul.txt
    local tpLocations = {
    ...
    sectionTitle(pageTP,"PILIH LOKASI",312)
    local infoCard=makeCard(pageTP,336,22)
    makeLabel(infoCard,"Kill → Respawn → TP ke tujuan",10,0,380,22,9,C.subtext)

    for i, loc in ipairs(tpLocations) do
        local locBtn=Instance.new("TextButton")
        locBtn.Size=UDim2.new(1,-20,0,36); locBtn.Position=UDim2.new(0,10,0,366+(i-1)*44)
        locBtn.BackgroundColor3=C.card; locBtn.Text=loc.name; locBtn.TextColor3=C.text
        locBtn.Font=Enum.Font.Gotham; locBtn.TextSize=12; locBtn.TextXAlignment=Enum.TextXAlignment.Left
        ...
        local cLbl=Instance.new("TextLabel"); cLbl.Size=UDim2.new(0,160,1,0); cLbl.Position=UDim2.new(1,-165,0,0)
        cLbl.BackgroundTransparency=1; cLbl.Text=string.format("%.0f / %.1f / %.0f",loc.x,loc.y,loc.z)
        cLbl.TextColor3=C.subtext; cLbl.Font=Enum.Font.Gotham; cLbl.TextSize=9; cLbl.TextXAlignment=Enum.TextXAlignment.Right; cLbl.Parent=locBtn
    ...
    -- Auto loop
    local loopY=366+#tpLocations*44+8
    sectionTitle(pageTP,"AUTO LOOP TELEPORT",loopY)
    local loopTog=Instance.new("TextButton"); loopTog.Size=UDim2.new(1,-20,0,34); loopTog.Position=UDim2.new(0,10,0,loopY+26)
    loopTog.BackgroundColor3=C.card; loopTog.Text="Auto Loop : OFF"; loopTog.TextColor3=C.red
    loopTog.Font=Enum.Font.Gotham; loopTog.TextSize=13; loopTog.BorderSizePixel=0; loopTog.Parent=pageTP; mkCorner(loopTog,5); mkStroke(loopTog,1,C.border)
    local intCard=makeCard(pageTP,loopY+68,28)
    makeLabel(intCard,"Interval: 30 detik  |  Target: NPC Marshmellow  |  Pakai metode terpilih",10,0,430,28,10,C.subtext)
        ...
        if autoTP_Running then
            ...
            autoTP_Thread=task.spawn(function()
                while autoTP_Running do
                    ...
                    tpToRespawn(tpLocations[2].x,tpLocations[2].y,tpLocations[2].z)
                    tpStatusValue.Text="ARRIVED";tpStatusValue.TextColor3=C.accent
    ...
    -- TP ke player
    local plrY=loopY+104
    sectionTitle(pageTP,"TELEPORT KE PLAYER",plrY)
    local plrList=Instance.new("ScrollingFrame"); plrList.Size=UDim2.new(1,-20,0,90); plrList.Position=UDim2.new(0,10,0,plrY+26)
    ...
    local function refreshPlrList()
        ...
        for _,plr in pairs(Players:GetPlayers()) do
            if plr~=LocalPlayer then count=count+1
                local pb=Instance.new("TextButton"); pb.Size=UDim2.new(1,-8,0,26); pb.BackgroundColor3=C.card2; pb.Text=plr.Name; pb.TextColor3=C.text; pb.Font=Enum.Font.Gotham; pb.TextSize=11; pb.TextXAlignment=Enum.TextXAlignment.Left; pb.BorderSizePixel=0; pb.Parent=plrList; mkCorner(pb,4)
                ...
                pb.MouseButton1Click:Connect(function()
                    ...
                    task.spawn(function() doTeleport(tgt.CFrame+Vector3.new(2,0,0)); tpStatusValue.Text="ARRIVED";tpStatusValue.TextColor3=C.accent; task.wait(2);tpStatusValue.Text="STANDBY";tpStatusValue.TextColor3=C.yellow end)
                end)
            end
        end
        plrList.CanvasSize=UDim2.new(0,0,0,count*30)
    end
    local rfPlrBtn=Instance.new("TextButton"); rfPlrBtn.Size=UDim2.new(1,-20,0,32); rfPlrBtn.Position=UDim2.new(0,10,0,plrY+124)
    rfPlrBtn.BackgroundColor3=C.card; rfPlrBtn.Text="Refresh Daftar Player"; rfPlrBtn.TextColor3=C.text; rfPlrBtn.Font=Enum.Font.Gotham; rfPlrBtn.TextSize=11; rfPlrBtn.BorderSizePixel=0; rfPlrBtn.Parent=pageTP; mkCorner(rfPlrBtn,5); mkStroke(rfPlrBtn,1,C.border)
    ...
    pageTP.CanvasSize=UDim2.new(0,0,0,plrY+180)
end
