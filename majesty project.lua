...
do
    ...
    sectionTitle(pageAuto,"AUTO SELL",522)

    local asSellTogBtn = Instance.new("TextButton")
    asSellTogBtn.Size = UDim2.new(1,-20,0,36)
    asSellTogBtn.Position = UDim2.new(0,10,0,548)
    asSellTogBtn.BackgroundColor3 = C.card
    asSellTogBtn.Text = "AUTO SELL : OFF"
    asSellTogBtn.TextColor3 = C.red
    asSellTogBtn.Font = Enum.Font.GothamBold
    asSellTogBtn.TextSize = 13
    asSellTogBtn.BorderSizePixel = 0
    asSellTogBtn.Parent = pageAuto
    mkCorner(asSellTogBtn,5); mkStroke(asSellTogBtn,1,C.border)

    local asC1 = makeCard(pageAuto,592,40)
    makeLabel(asC1,"STATUS",10,0,70,40,9,C.subtext)
    local asStatusLbl = makeLabel(asC1,"OFF",82,0,200,40,12,C.red)

    local asC2 = makeCard(pageAuto,640,40)
    makeLabel(asC2,"ITEM",10,0,70,40,9,C.subtext)
    local asItemLbl = makeLabel(asC2,"-",82,0,280,40,12,C.text)

    local asC3 = makeCard(pageAuto,688,40)
    makeLabel(asC3,"TERJUAL",10,0,70,40,9,C.subtext)
    local asSoldLbl = makeLabel(asC3,"0",82,0,200,40,12,C.yellow)

    local asC4 = makeCard(pageAuto,736,40)
    makeLabel(asC4,"FASE",10,0,70,40,9,C.subtext)
    local asPhaseLbl = makeLabel(asC4,"Idle",82,0,280,40,12,C.accent2)

    -- Min. jumlah bag sebelum trigger sell
    local asThrCard = makeCard(pageAuto,784,44)
    makeLabel(asThrCard,"Min. Bag untuk Sell",10,4,200,20,10,C.subtext)
    local asThrVal = 1
    local asThrLbl = Instance.new("TextLabel")
    asThrLbl.Size = UDim2.new(0,28,0,24); asThrLbl.Position = UDim2.new(1,-96,0.5,-12)
    asThrLbl.BackgroundTransparency = 1; asThrLbl.Text = "1"
    asThrLbl.TextColor3 = C.yellow; asThrLbl.Font = Enum.Font.GothamBold
    asThrLbl.TextSize = 13; asThrLbl.TextXAlignment = Enum.TextXAlignment.Center
    asThrLbl.Parent = asThrCard
    local asMinus = Instance.new("TextButton")
    asMinus.Size = UDim2.new(0,28,0,24); asMinus.Position = UDim2.new(1,-130,0.5,-12)
    asMinus.Text = "-"; asMinus.TextSize = 16; asMinus.Font = Enum.Font.GothamBold
    asMinus.BackgroundColor3 = Color3.fromRGB(50,15,15); asMinus.TextColor3 = C.red
    asMinus.BorderSizePixel = 0; asMinus.Parent = asThrCard; mkCorner(asMinus,5)
    local asPlus = Instance.new("TextButton")
    asPlus.Size = UDim2.new(0,28,0,24); asPlus.Position = UDim2.new(1,-62,0.5,-12)
    asPlus.Text = "+"; asPlus.TextSize = 16; asPlus.Font = Enum.Font.GothamBold
    asPlus.BackgroundColor3 = Color3.fromRGB(0,40,20); asPlus.TextColor3 = C.accent
    asPlus.BorderSizePixel = 0; asPlus.Parent = asThrCard; mkCorner(asPlus,5)
    asMinus.MouseButton1Click:Connect(function()
        asThrVal = math.max(1, asThrVal - 1); asThrLbl.Text = tostring(asThrVal)
    end)
    asPlus.MouseButton1Click:Connect(function()
        asThrVal = math.min(20, asThrVal + 1); asThrLbl.Text = tostring(asThrVal)
    end)

    local asInfoCard = makeCard(pageAuto,836,22)
    makeLabel(asInfoCard,"Deteksi ProximityPrompt terdekat dalam radius 15 studs",10,0,430,22,9,C.subtext)

    local autoSell_UI  = false
    local asSoldCount  = 0
    local asSelling    = false  -- guard agar tidak double trigger
    local sellOrder_UI = {"Small Marshmallow Bag","Medium Marshmallow Bag","Large Marshmallow Bag"}

    asSellTogBtn.MouseButton1Click:Connect(function()
        autoSell_UI = not autoSell_UI
        if autoSell_UI then
            asSellTogBtn.Text = "AUTO SELL : ON"; asSellTogBtn.TextColor3 = C.accent
            mkStroke(asSellTogBtn, 1, C.accent)
            asStatusLbl.Text = "ON"; asStatusLbl.TextColor3 = C.accent
            asPhaseLbl.Text = "Menunggu item..."; asPhaseLbl.TextColor3 = C.yellow
            print("[AUTO SELL] Aktif - mendeteksi Marshmallow Bag di inventory")
        else
            autoSell_UI = false; asSelling = false
            asSellTogBtn.Text = "AUTO SELL : OFF"; asSellTogBtn.TextColor3 = C.red
            mkStroke(asSellTogBtn, 1, C.border)
            asStatusLbl.Text = "OFF"; asStatusLbl.TextColor3 = C.red
            asItemLbl.Text = "-"; asPhaseLbl.Text = "Idle"; asPhaseLbl.TextColor3 = C.subtext
...
-- Hitung jumlah item spesifik di inventory (Backpack + equipped)
...
-- Hitung total semua MS Bag di inventory
local function countAllMSBags()
    local total = 0
    for _, name in ipairs(sellOrder_UI) do total += countMSItem(name) end
    return total
end

-- Cari ProximityPrompt sell terdekat dalam radius
...
-- Equip item dari backpack berdasarkan nama persis
...
task.spawn(function()
    while true do
        ...
        -- Guard: skip jika OFF, sedang sell, atau karakter tidak valid
        ...
        -- Cek jumlah bag vs threshold
        local totalBags = countAllMSBags()
        if totalBags < asThrVal then
            asItemLbl.Text   = "Bag: " .. totalBags .. " (min " .. asThrVal .. ")"
            asPhaseLbl.Text  = "Menunggu item..."
            asPhaseLbl.TextColor3 = C.yellow
            continue
        end

        -- Tentukan item yang akan dijual (urutan: Small → Medium → Large)
        ...
        -- Cek ada ProximityPrompt sell dalam radius 15 studs
        local prompt, dist = findSellPrompt(hrp.Position, 15)
        if not prompt then
            asItemLbl.Text   = targetItem
            asPhaseLbl.Text  = "Cari NPC jual (dekati NPC)"
            asPhaseLbl.TextColor3 = C.subtext
            continue
        end

        -- Mulai proses sell
        asSelling = true
        print("[AUTO SELL] Menjual: " .. targetItem .. " (dist: " .. string.format("%.1f", dist) .. " studs)")

        asItemLbl.Text  = targetItem
        asPhaseLbl.Text = "Equip item..."; asPhaseLbl.TextColor3 = C.accent

        -- Equip item
        local equipped = equipItem(targetItem)
        if not equipped then
            -- Fallback: holdItem (keyword match)
            holdItem(targetItem)
            task.wait(0.3)
        end

        asPhaseLbl.Text = "Menjual..."

        -- Trigger sell via ProximityPrompt
        local ok = pcall(fireproximityprompt, prompt)
        if not ok then
            -- Fallback: VirtualInputManager press E
            pcall(function()
        ...
        -- Update count jika item berkurang
        local remaining = countMSItem(targetItem)
        if remaining == 0 then
            asSoldCount += 1
            asSoldLbl.Text = tostring(asSoldCount)
            print("[AUTO SELL] Berhasil jual " .. targetItem .. " | Total: " .. asSoldCount)
        end

        asPhaseLbl.Text  = "ON - Siap"
        asPhaseLbl.TextColor3 = C.accent
        asItemLbl.Text   = "Bag: " .. countAllMSBags()
        asSelling = false
    end
end)
