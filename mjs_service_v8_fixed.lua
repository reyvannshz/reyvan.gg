...
-- Jual satu item: equip → fireproximityprompt + hold E sampai terkonfirmasi terjual.
-- Menggunakan fireproximityprompt (bypass HoldDuration) + VIM hold sebagai fallback.
-- Setelah inventory berkurang (item hilang) → langsung stop, unequip, lanjut item berikutnya.
local function trySellOne(msName, setStatus2)
    ...
    -- Tunggu server kenali item di tangan (minimal)
    task.wait(0.4)

    setStatus2("Jual: "..msName.." — interact NPC...", Color3.fromRGB(50,210,110))

    -- ── Loop interact setiap 0.6 detik sampai item terjual ──
    -- Gunakan fireproximityprompt untuk bypass HoldDuration prompt NPC.
    -- VIM hold E sebagai fallback tambahan.
    local elapsed   = 0
    local sold      = false
    local pressInt  = 0.6   -- interval interact (detik)
    local pressAcc  = 0

    while elapsed < CFG.SELL_TIMEOUT do
        -- Cek apakah inventory sudah berkurang (item sold)
        ...
        -- Fire proximity prompt + hold E setiap pressInt detik
        if pressAcc >= pressInt or elapsed == 0 then
            -- 1) fireproximityprompt langsung (bypass HoldDuration)
            firePromptNearby(CFG.SELL_RADIUS + 5)
            task.wait(0.05)
            -- 2) VIM hold E sebagai fallback (tahan lebih lama dari biasa)
            pcall(function()
                VIM:SendKeyEvent(true,  Enum.KeyCode.E, false, game)
                task.wait(0.5)
                VIM:SendKeyEvent(false, Enum.KeyCode.E, false, game)
            ...
            task.wait(0.05)
            -- 3) fireproximityprompt sekali lagi setelah hold
            firePromptNearby(CFG.SELL_RADIUS + 5)
            pressAcc = 0
        end
