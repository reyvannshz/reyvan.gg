...
-- Fire SEMUA ProximityPrompt dalam radius tanpa filter nama
-- Ini paling reliable karena tidak bergantung nama objek di game
local function fireAllPromptsAt(pos, radius)
    radius = radius or 15
    local fired = false
    for _, obj in ipairs(Workspace:GetDescendants()) do
...
-- Teleport ke posisi lalu fire semua prompt di sekitar
local function goAndInteract(pos, label, radius)
    if not isRunning then return end
    if not safeChar() then return end
    currentStep = label
    ...
    pcall(function()
        rootPart.CFrame = CFrame.new(
            pos.X, pos.Y + 3, pos.Z + 2
        )
    end)
    task.wait(0.4) -- tunggu game update posisi
    fireAllPromptsAt(pos, radius or 12)
    task.wait(0.4)
end

-- ============================================================
-- FARMING FUNCTIONS - Fixed Logic South Bronx
-- ============================================================

-- Beli 1 item dari NPC (interact sekali)
local function beliItem(coord, label)
    if not coord then
        currentStep = "❌ "..label.." - belum scan!"
        return
    end
    goAndInteract(coord, "Beli "..label)
end

-- Jual Marshmallow ke NPC sell
local function jualMarshmallow()
    if not isRunning then return end
    if not COORDS.SELL then
        currentStep = "❌ SELL coord tidak ada! Scan dulu."
        return
    end
    -- Teleport ke NPC jual
    currentStep = "Jual Marshmallow"
    pcall(function()
        rootPart.CFrame = CFrame.new(
            COORDS.SELL.X, COORDS.SELL.Y + 3, COORDS.SELL.Z + 2
        )
    end)
    task.wait(0.5)
    -- Fire semua prompt di area jual
    fireAllPromptsAt(COORDS.SELL, 12)
    task.wait(0.5)
    -- Fire lagi untuk konfirmasi
    fireAllPromptsAt(COORDS.SELL, 12)
    task.wait(0.3)
...
-- Satu siklus penuh masak marshmallow
local function cookingCycle()
    if not safeChar() then task.wait(2) return end

    -- Cek koordinat penting sudah di-scan
    if not COORDS.POT then
        currentStep = "❌ Belum scan! Klik SCAN OTOMATIS dulu."
        task.wait(3)
        return
    end

    -- ── STEP 1: Beli semua bahan dari NPC ──────────────────
    beliItem(COORDS.WATER,   "Water")
    if not isRunning then return end
    task.wait(0.3)

    beliItem(COORDS.SUGAR,   "Sugar Block")
    if not isRunning then return end
    task.wait(0.3)

    beliItem(COORDS.GELATIN, "Gelatin")
    if not isRunning then return end
    task.wait(0.3)

    -- ── STEP 2: Bawa Water ke Cooking Pot ─────────────────
    -- Ambil Water dari inventori/ground
    if COORDS.WATER then
        goAndInteract(COORDS.WATER, "Ambil Water")
        task.wait(0.3)
    end

    -- Taruh di pot + mulai masak
    goAndInteract(COORDS.POT, "Masak Water di Pot")
    task.wait(0.5)

    -- Tunggu masak water selesai
    currentStep = "⏳ Masak Water... ("..CONFIG.WATER_COOK.."s)"
    waitSec(CONFIG.WATER_COOK)
    if not isRunning then return end

    -- ── STEP 3: Masukkan Sugar Block ke Pot ───────────────
    if COORDS.SUGAR then
        goAndInteract(COORDS.SUGAR, "Ambil Sugar Block")
        task.wait(0.3)
    end
    goAndInteract(COORDS.POT, "Masukkan Sugar ke Pot")
    task.wait(0.3)

    -- ── STEP 4: Masukkan Gelatin ke Pot ───────────────────
    if COORDS.GELATIN then
        goAndInteract(COORDS.GELATIN, "Ambil Gelatin")
        task.wait(0.3)
    end
    goAndInteract(COORDS.POT, "Masukkan Gelatin ke Pot")
    task.wait(0.5)

    -- Tunggu masak campuran selesai
    currentStep = "⏳ Masak Campuran... ("..CONFIG.MIX_COOK.."s)"
    waitSec(CONFIG.MIX_COOK)
    if not isRunning then return end

    -- ── STEP 5: Ambil Empty Bag lalu kemas ────────────────
    if COORDS.BAG then
        goAndInteract(COORDS.BAG, "Ambil Empty Bag")
        task.wait(0.3)
    end
    goAndInteract(COORDS.POT, "Kemas Marshmallow")
    task.wait(0.5)

    -- ── STEP 6: Ambil hasil Marshmallow ───────────────────
    if COORDS.MALLOW then
        goAndInteract(COORDS.MALLOW, "Ambil Marshmallow")
        task.wait(0.3)
    else
        -- Kalau tidak ada MALLOW coord, coba ambil dari pot
        goAndInteract(COORDS.POT, "Ambil Hasil dari Pot")
        task.wait(0.3)
    end
    ...
    cycleCount += 1
    currentStep = "✅ Siklus #"..cycleCount.." selesai — "..mallowDone.." marshmallow"
    task.wait(1)
end
...
local function createGUI()
    ...
    -- Masing-masing fitur punya thread sendiri supaya tidak conflict
    local farmThread = nil
    local beliThread = nil
    local jualThread = nil
            ...
            farmThread = task.spawn(function()
                while farmActive do
                    if mallowDone >= targetAmount then
                        ...
                        if not ok then
                            warn("[van.gg] cycle error: "..tostring(err))
                            currentStep = "⚠️ Error, retry 3s..."
                            task.wait(3)
                        end
                    end
                end
    ...
    togRow(LP,"Auto Beli Bahan Saja",false,function(s)
        beliActive = s
        if s then
            if beliThread then task.cancel(beliThread) end
            beliThread = task.spawn(function()
                while beliActive do
                    currentStep = "Beli Bahan..."
                    pcall(function()
                        if COORDS.WATER   then goAndInteract(COORDS.WATER,   "Beli Water")       task.wait(0.5) end
                        if COORDS.SUGAR   then goAndInteract(COORDS.SUGAR,   "Beli Sugar Block") task.wait(0.5) end
                        if COORDS.GELATIN then goAndInteract(COORDS.GELATIN, "Beli Gelatin")     task.wait(0.5) end
                    end)
                    task.wait(1)
                end
                currentStep = "Idle"
    ...
    togRow(LP,"Auto Jual Marshmallow Saja",false,function(s)
        jualActive = s
        if s then
            if jualThread then task.cancel(jualThread) end
            jualThread = task.spawn(function()
                while jualActive do
