    ...
    buyToggleBtn.MouseButton1Click:Connect(function()
        ...
        if autoBuy_Running then
            ...
            task.spawn(function()
                -- ── KONFIGURASI BAHAN ──
                local BAHAN = {
                    { label = "Water",       toolKw = "water",   shopKw = "water",   emoji = "💧" },
                    { label = "Sugar Block", toolKw = "sugar",   shopKw = "sugar",   emoji = "🧊" },
                    { label = "Gelatin",     toolKw = "gelatin", shopKw = "gelatin", emoji = "🍮" },
                }

                -- ── HELPER: Kirim keystroke E ──
                ...
                -- ── HELPER: Cari Part toko berdasarkan keyword ──
                local function findShopPartBuy(kw)
                    kw = kw:lower()
                    local chr = LocalPlayer.Character
                    ...
                    for _, obj in pairs(Workspace:GetDescendants()) do
                        if obj.Name:lower():find(kw) then
                            ...
                            if pos then
                                ...
                                if d < bestD and d < 500 then bestD = d; best = obj end
                            end
                        end
                    end
                    return best, bestD
                ...
                -- ── HELPER: Cari ProximityPrompt di dalam obj ──
                local function findPP(obj)
                    if not obj then return nil end
                    local function si(t)
                        if not t then return nil end
                        local pp = t:FindFirstChildOfClass("ProximityPrompt")
                    ...
                    return si(obj) or si(obj.Parent)
                end

                -- ── HELPER: Teleport dekat part lalu interaksi ──
                local function goAndBuy(part)
                    ...
                    -- Teleport 3 stud di depan
                    tpTo(CFrame.new(pos + Vector3.new(0, 0, 3), pos))
                    task.wait(0.4)
                    -- Hadapkan ke toko
                    ...
                    task.wait(0.2)
                    -- Aktifkan ProximityPrompt jika ada
                    local pp = findPP(part)
                    if pp then
                        pcall(function() fireproximityprompt(pp) end)
                        task.wait(0.3)
                    end
                    -- Fallback keystroke E
                    pressE_buy()
                    task.wait(0.5)
                end

                -- ── FUNGSI BELI SATU BAHAN (max 5 percobaan) ──
                local function beliBahan(bahan)
                    ...
                    for attempt = 1, 5 do
                        ...
                        print(string.format("[AutoBuy] Percobaan %d/5 - cari '%s'...", attempt, bahan.shopKw))
                        local shopPart, dist = findShopPartBuy(bahan.shopKw)
                        if shopPart then
                            print(string.format("[AutoBuy] 📍 Ditemukan: '%s' (%.1f stud)", shopPart.Name, dist or 0))
                            goAndBuy(shopPart)
                        else
                            print(string.format("[AutoBuy] ⚠️  Part '%s' tidak ditemukan.", bahan.shopKw))
                        end
                        task.wait(1.5)
                        if hasItemBuy(bahan.toolKw) then
                    ...
                    print(string.format("[AutoBuy] ❌ GAGAL membeli %s.", bahan.label))
                    return false
                end
                ...
                for i, bahan in ipairs(BAHAN) do
                    ...
                    if ok then
                        ...
                        allOk = false
                        print(string.format("[AutoBuy] ⚠️  Step %d gagal, lanjut...", i))
                    end
