...
do
    ...
    makeLabel(afInfoCard,"Pastikan naik motor untuk teleport!",10,22,400,20,9,C.accent)

    -- Save Apart Pos
    local afCoordCard = makeCard(pageAutoFully,90,34)
    fullyCoordLbl_af  = makeLabel(afCoordCard,"Belum disimpan",10,0,270,34,10,C.subtext)
    savePosBtn_af = Instance.new("TextButton")
    savePosBtn_af.Size=UDim2.new(0,80,0,26); savePosBtn_af.Position=UDim2.new(1,-90,0.5,-13)
    savePosBtn_af.BackgroundColor3=C.purple; savePosBtn_af.Text="📍 Save"
    savePosBtn_af.TextColor3=Color3.fromRGB(255,255,255); savePosBtn_af.Font=Enum.Font.GothamBold
    savePosBtn_af.TextSize=10; savePosBtn_af.BorderSizePixel=0; savePosBtn_af.Parent=afCoordCard; mkCorner(savePosBtn_af,5)
    savePosBtn_af.MouseButton1Click:Connect(function()
        local ch   = LocalPlayer.Character
        local root = ch and ch:FindFirstChild("HumanoidRootPart")
        if root then
            fullySavedPos = root.Position
            local p = root.Position
            fullyCoordLbl_af.Text       = string.format("%.1f, %.1f, %.1f", p.X, p.Y, p.Z)
    ...
    afStartBtn.MouseButton1Click:Connect(function()
        ...
        if not fullySavedPos then
            setFullyStatus_af("Simpan koordinat Apart dulu!", C.red); return
        end
        setFullyStatus_af("Auto Fully berjalan...", C.accent)
        task.spawn(function()
    ...
    pageAutoFully.CanvasSize=UDim2.new(0,0,0,270)
end
