...
do
    ...
    kaKeybindLbl.Text = "⌨  Tekan  [ F ]  untuk toggle Kill Aura ON/OFF"
    ...
    pageKillAura.CanvasSize = UDim2.new(0, 0, 0, 800)
...
local function tryKillNoTeleport(plr, hrp, hum)
    -- ── 1. firetouchinterest: picu semua killbrick/lava menyentuh target ──
    pcall(function()
    ...
    -- ── 2. firetouchinterest: picu sentuhan antara semua part karakter KITA dan target ──
    pcall(function()
    ...
    -- ── 3. Fire semua RemoteEvent damage yang ditemukan ──────────────────
    pcall(function()
        ...
        for _, remote in pairs(remotes) do
            if remote:IsA("RemoteEvent") then
                ...
                pcall(function() remote:FireServer({target=plr, damage=99999}) end)
    ...
    -- ── 4. Cari RemoteEvent di dalam tool yang sedang dipegang ───────────
    pcall(function()
        local myChar = LocalPlayer.Character
        if not myChar then return end
            ...
            if tool:IsA("Tool") then
                for _, v in pairs(tool:GetDescendants()) do
                    if v:IsA("RemoteEvent") then
                        ...
                        pcall(function() v:FireServer(CFrame.new(hrp.Position)) end)
                    end
                end
                -- Activate tool sambil Mouse.Target = HRP target
                pcall(function()
...
-- ========== KILL AURA KEYBIND TOGGLE (default: F) ==========
local kaKeybind = Enum.KeyCode.F
