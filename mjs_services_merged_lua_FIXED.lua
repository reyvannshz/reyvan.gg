...
-- Tombol − minimize dan × close (kanan titlebar, gaya mjs)
...
do
    ...
    -- Variable state (dibuat di scope do-block agar bisa diakses tombol)
    -- autoBuy_Running dipakai oleh logika di bawah
    local autoBuy_Running   = false
    local autoBuyStatusLbl  = nil
    local autoBuyPhaseLbl   = nil
    local autoBuyToggleBtn  = nil

    -- Toggle button
    local buyToggleBtn = Instance.new("TextButton")
    buyToggleBtn.Name = "BuyToggleBtn"
    buyToggleBtn.Size = UDim2.new(1, -20, 0, 38)
    buyToggleBtn.Position = UDim2.new(0, 10, 0, 912)
    buyToggleBtn.BackgroundColor3 = Color3.fromRGB(20, 20, 35)
    ...
    -- Status beli
    local buyStatCard = makeCard(pageAuto, 958, 44)
    makeLabel(buyStatCard, "STATUS", 12, 0, 80, 44, 11, C.subtext, Enum.Font.GothamBold)
    local buyStatusLbl = makeLabel(buyStatCard, "OFF", 90, 0, 200, 44, 14, Color3.fromRGB(150,150,150), Enum.Font.GothamBlack)

    -- Phase beli
    local buyPhaseCard = makeCard(pageAuto, 1010, 44)
    ...
    -- Info card
    local buyInfoCard = makeCard(pageAuto, 1062, 30)
    makeLabel(buyInfoCard, "💧 Water  →  🧊 Sugar Block  →  🍮 Gelatin", 10, 0, 400, 30, 10, C.subtext, Enum.Font.Gotham)

    autoBuyStatusLbl = buyStatusLbl
    autoBuyPhaseLbl  = buyPhaseLbl
    autoBuyToggleBtn = buyToggleBtn
    ...
    buyToggleBtn.MouseButton1Click:Connect(function()
        ...
        if autoBuy_Running then
            ...
            task.spawn(function()
                -- ── KONFIGURASI BAHAN ──
                -- toolKw: keyword untuk cek di Backpack (nama tool)
                -- shopKws: multiple keyword untuk cari di Workspace (lebih fleksibel)
                local BAHAN = {
                    { label = "Water",       toolKw = "water",   shopKws = {"water","aqua","h2o"},              emoji = "💧" },
                    { label = "Sugar Block", toolKw = "sugar",   shopKws = {"sugar","gula","block","sucrose"},  emoji = "🧊" },
                    { label = "Gelatin",     toolKw = "gelatin", shopKws = {"gelatin","gel","jelly","agar"},    emoji = "🍮" },
                }

                -- ── HELPER: Kirim keystroke E (press + hold + release) ──
                local function pressE_buy()
                    pcall(function()
                        local vim = game:GetService("VirtualInputManager")
                        vim:SendKeyEvent(true,  Enum.KeyCode.E, false, game)
                    end)
                    pcall(function() keypress(0x45) end)
                    task.wait(0.3)  -- tahan lebih lama agar prompt sempat terbaca
                    pcall(function()
                        local vim = game:GetService("VirtualInputManager")
                ...
                -- ── HELPER: Cek item di Backpack / Character ──
                local function hasItemBuy(kw)
                    kw = kw:lower()
                ...
                -- ── HELPER: Cari semua ProximityPrompt di suatu object ──
                local function getAllPP(obj)
                    ...
                    -- Cek parent juga
                    if obj.Parent then
                        for _, c in pairs(obj.Parent:GetDescendants()) do
                            if c:IsA("ProximityPrompt") then
                                table.insert(result, c)
                            end
                        end
                ...
                -- ── HELPER: Cari Part/Model toko berdasarkan list keyword ──
                -- Lebih fleksibel: cari nama part, label, BillboardGui, atau ProximityPrompt ActionText
                local function findShopPartBuy(shopKws)
                    local chr = LocalPlayer.Character
                    local hrp = chr and chr:FindFirstChild("HumanoidRootPart")
                    if not hrp then return nil, nil end

                    local best, bestD, bestPP = nil, math.huge, nil

                    local function tryObj(obj)
                        -- Ambil posisi
                        local pos
                        ...
                        elseif obj:IsA("Model") then
                            if obj.PrimaryPart then
                                ...
                                local pp = obj:FindFirstChildOfClass("BasePart")
                        ...
                        if d >= bestD or d >= 600 then return end
                    ...
                    for _, obj in pairs(Workspace:GetDescendants()) do
                        ...
                        -- Cek ProximityPrompt ActionText / ObjectText
                        if obj:IsA("ProximityPrompt") then
                            ...
                            for _, kw in pairs(shopKws) do
                                if at:find(kw) or ot:find(kw) then
                                    ...
                                    bestPP = obj  -- simpan langsung
                                    break
                                end
                            end
                        end

                        -- Cek BillboardGui / TextLabel isinya
                        if obj:IsA("TextLabel") or obj:IsA("TextButton") then
                            local txt = (obj.Text or ""):lower()
                            for _, kw in pairs(shopKws) do
                                if txt:find(kw) then
                                    -- Risik ke atas sampai dapat BasePart / Model
                                    local p = obj.Parent
                    ...
                    -- Kalau bestPP sudah ketemu langsung kembalikan
                    if bestPP then return best or bestPP.Parent, bestD, bestPP end
                    return best, bestD, nil
                end

                -- ── HELPER: Interaksi ke part (TP dekat + fire PP + keystroke E) ──
                local function goAndBuy(part, directPP)
                    if not part then return end
                    local pos
                    if part:IsA("BasePart") then
                        pos = part.Position
                    elseif part:IsA("Model") then
                        if part.PrimaryPart then pos = part.PrimaryPart.Position
                        else
                            local bp2 = part:FindFirstChildOfClass("BasePart")
                            if bp2 then pos = bp2.Position end
                        end
                    end
                    if not pos then return end

                    -- Teleport ke posisi lebih dekat (2 stud di depan)
                    local offset = Vector3.new(0, 0, 2)
                    tpTo(CFrame.new(pos + offset, pos))

                    -- Hadapkan karakter ke toko
                    ...
                    -- Kumpulkan semua ProximityPrompt yang ada di sekitar
                    local prompts = {}
                    if directPP then
                        table.insert(prompts, directPP)
                    end
                    for _, pp in pairs(getAllPP(part)) do
                        local already = false
                        for _, ep in pairs(prompts) do if ep == pp then already = true break end end
                        if not already then table.insert(prompts, pp) end
                    end

                    -- Fire semua ProximityPrompt yang ditemukan
                    if #prompts > 0 then
                        for _, pp in pairs(prompts) do
                            print(string.format("[AutoBuy] 🔔 Fire ProximityPrompt: '%s' / '%s'", pp.ActionText or "?", pp.ObjectText or "?"))
                            pcall(function() fireproximityprompt(pp) end)
                            task.wait(0.5)  -- jeda antar fire agar server sempat proses
                        end
                    end

                    -- Keystroke E sebagai fallback / tambahan
                    pressE_buy()
                    task.wait(0.8)  -- jeda setelah interaksi sebelum cek hasil
                end

                -- ── FUNGSI BELI SATU BAHAN (max 8 percobaan dengan jeda makin panjang) ──
                local function beliBahan(bahan)
                    buyPhaseLbl.Text = bahan.emoji .. "  " .. bahan.label
                    print(string.format("[AutoBuy] %s Membeli %s...", bahan.emoji, bahan.label))

                    -- Cek apakah sudah punya sebelum mulai
                    if hasItemBuy(bahan.toolKw) then
                        print(string.format("[AutoBuy] ✅ %s sudah ada di Backpack, skip!", bahan.label))
                        return true
                    end

                    for attempt = 1, 8 do
                        if not autoBuy_Running then return false end

                        print(string.format("[AutoBuy] Percobaan %d/8 - cari '%s'...", attempt, bahan.label))
                        buyStatusLbl.Text = "CARI " .. bahan.emoji .. " (" .. attempt .. "/8)"
                        buyStatusLbl.TextColor3 = C.yellow

                        local shopPart, dist, directPP = findShopPartBuy(bahan.shopKws)
                        if shopPart then
                            print(string.format("[AutoBuy] 📍 Ditemukan: '%s' (%.1f stud)", shopPart.Name, dist or 0))
                            goAndBuy(shopPart, directPP)
                        else
                            -- Tidak ditemukan — coba pendekatan brute-force ProximityPrompt terdekat
                            print(string.format("[AutoBuy] ⚠️  Part '%s' tidak ditemukan, cari PP terdekat...", bahan.label))
                            local chr2 = LocalPlayer.Character
                            local hrp2 = chr2 and chr2:FindFirstChild("HumanoidRootPart")
                            if hrp2 then
                                local bestPP2, bestD2 = nil, math.huge
                                for _, obj in pairs(Workspace:GetDescendants()) do
                                    if obj:IsA("ProximityPrompt") then
                                        local pp2Part = obj.Parent
                                        if pp2Part and pp2Part:IsA("BasePart") then
                                            local d2 = (pp2Part.Position - hrp2.Position).Magnitude
                                            if d2 < bestD2 and d2 < 30 then
                                                bestD2 = d2; bestPP2 = obj
                                            end
                                        end
                                    end
                                end
                                if bestPP2 then
                                    print(string.format("[AutoBuy] 🔔 Fallback fire PP terdekat (%.1f stud)", bestD2))
                                    pcall(function() fireproximityprompt(bestPP2) end)
                                    task.wait(0.5)
                                    pressE_buy()
                                end
                            end
                        ...
                        -- Jeda adaptif: makin banyak percobaan makin lama
                        local jeda = 1.5 + (attempt - 1) * 0.5  -- 1.5s, 2s, 2.5s, dst
                        print(string.format("[AutoBuy] ⏳ Tunggu %.1f detik...", jeda))
                        task.wait(jeda)

                        -- Cek apakah sudah berhasil
                        if hasItemBuy(bahan.toolKw) then
                            print(string.format("[AutoBuy] ✅ BERHASIL! %s masuk ke Backpack.", bahan.label))
                            return true
                        end
                    end

                    print(string.format("[AutoBuy] ❌ GAGAL membeli %s setelah 8 percobaan.", bahan.label))
                    return false
                end

                -- ── LOOP UTAMA AUTO BUY ──
                print("========================================")
                print("[AutoBuy] 🛒 MULAI AUTO BUY BAHAN MARSHMALLOW")
                print("[AutoBuy] Urutan: Water → Sugar Block → Gelatin")
                print("========================================")

                local allOk = true
                for i, bahan in ipairs(BAHAN) do
                    if not autoBuy_Running then break end
                    print(string.format("\n[AutoBuy] ── STEP %d: %s %s ──", i, bahan.emoji, bahan.label))
                    local ok = beliBahan(bahan)
                    if ok then
                        print(string.format("[AutoBuy] ✅ Step %d selesai: %s berhasil!", i, bahan.label))
                    else
                        allOk = false
                        print(string.format("[AutoBuy] ⚠️  Step %d gagal, lanjut ke berikutnya...", i))
                    end

                    -- Jeda antar bahan (2 detik) agar server sempat update inventory
                    if i < #BAHAN and autoBuy_Running then
                        print("[AutoBuy] ⏳ Jeda 2 detik sebelum bahan berikutnya...")
                        buyPhaseLbl.Text = "⏳ Jeda..."
                        task.wait(2)
                    end
                end
                ...
                if allOk then
                    ...
                    buyStatusLbl.TextColor3 = C.green
                    print("\n[AutoBuy] 🎉 SEMUA BAHAN BERHASIL DIBELI!")
                else
                    buyStatusLbl.Text = "SEBAGIAN ⚠"
                    buyStatusLbl.TextColor3 = C.yellow
                    print("\n[AutoBuy] ⚠️  Beberapa bahan tidak berhasil.")
                end

                -- Ringkasan Backpack
                print("[AutoBuy] Isi Backpack:")
                local bp = LocalPlayer:FindFirstChild("Backpack")
                if bp then
                    local n = 0
                    for _, t in pairs(bp:GetChildren()) do
                        if t:IsA("Tool") then print("  • " .. t.Name); n = n + 1 end
                    end
                    if n == 0 then print("  (kosong)") end
                end
                print("========================================")
            end)
        else
            ...
            buyPhaseLbl.Text = "—"
            print("[AutoBuy] 🛑 Dihentikan oleh user.")
        end
    end)
...
do
    ...
    -- 2 swatch rows: box color dan nama player
...
local function createESP(player)
    ...
    espCache[player] = {boxOutline, nameLabel, hpBarBg, hpBarFill, distLabel}
end

-- ========== ESP MAIN LOOP (satu loop terpusat, lebih hemat) ==========
RunService.Heartbeat:Connect(function()
    ...
    for player, drawings in pairs(espCache) do
        ...
        if myHRP2 and espMaxDist > 0 then
            ...
            if d3 > espMaxDist then hideAll() continue end
        end
        ...
        -- Distance
        if myHRP2 then
            local dist3D  = (root.Position - myHRP2.Position).Magnitude
...
print("=== MAJESTY ONTOP v17.0 - ===")
print("Fitur:")
print("-  Inventory Tracker")
print("-   Auto MS")
print("-   ESP Optimized | Name | HP Bar | Dist limit | Box color")
print("-  Teleport")
