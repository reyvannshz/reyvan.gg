...
-- ========== FUNGSI TELEPORT SAFE V2 (ANTI-KICK) ==========
local TP_STEPS = 20     -- lebih banyak langkah = lebih smooth
local TP_DELAY = 0.35   -- jeda lebih lama antar langkah (~7 detik total)
...
local function doTeleport(targetCFrame)
    ...
    -- Paksa state Physics agar server tidak koreksi posisi
    humanoid:ChangeState(Enum.HumanoidStateType.Physics)
    task.wait(0.2)

    for i = 1, TP_STEPS do
        -- Cek ulang char masih valid tiap langkah
        ...
        -- Ease-in-out supaya gerak tidak terasa mendadak
        local ease = alpha * alpha * (3 - 2 * alpha)
        local lerpedCF = startCFrame:Lerp(targetCFrame, ease)
