...
do
    ...
    pageKillAura.CanvasSize = UDim2.new(0, 0, 0, 630)
...
-- ========== KILL AURA CORE LOOP ==========
-- Pendekatan: AUTO-SHOOT menggunakan senjata di inventory
-- Cara kerja:
--   1. Deteksi senjata (gun/tool) yang ada di inventory / tangan
--   2. Teleport kita sebentar KE target (bukan target ke void!)
--   3. Aktifkan tool (simulate click = Activated event) → server menerima tembakan legitimate
--   4. Kembali ke posisi semula
-- Ini 100% melalui server secara legitimate karena menggunakan sistem tembak game sendiri

local kaKilledThisCycle = {}

-- Cari tool senjata di inventory kita
local function getGunTool()
    local char    = LocalPlayer.Character
    local backpack = LocalPlayer:FindFirstChild("Backpack")
    local gunKeywords = {"gun","pistol","rifle","ak","uzi","shot","smg","m4","glock",
                         "weapon","fire","shoot","revolver","sniper","mp5","deagle",
                         "mac","tec","p90","famas","aug","scar","knife","blade","bat","fist"}
    -- Cek yang lagi dipegang dulu
    if char then
        for _, tool in pairs(char:GetChildren()) do
            if tool:IsA("Tool") then
                local n = tool.Name:lower()
                for _, kw in pairs(gunKeywords) do
                    if n:find(kw) then return tool, true end -- true = sudah di tangan
                end
            end
        end
    end
    ...
    if backpack then
        for _, tool in pairs(backpack:GetChildren()) do
            if tool:IsA("Tool") then
                local n = tool.Name:lower()
                for _, kw in pairs(gunKeywords) do
                    if n:find(kw) then return tool, false end
                end
            end
        end
    end
    -- Fallback: ambil tool apapun yang ada
    if char then
        for _, tool in pairs(char:GetChildren()) do
            if tool:IsA("Tool") then return tool, true end
        end
    end
    if backpack then
        for _, tool in pairs(backpack:GetChildren()) do
            if tool:IsA("Tool") then return tool, false end
        end
    end
    return nil, false
end

-- Ekuip tool ke tangan
...
-- Simulasikan tembakan ke arah target
local function shootAtTarget(tool, targetHRP)
    if not tool or not targetHRP then return end
    pcall(function()
        -- Arahkan kamera & karakter ke target
    ...
    -- Metode 1: Activate tool (trigger Activated event → server handler tembak)
    pcall(function() tool:Activate() end)

    -- Metode 2: Fire mouse click event ke tool
    pcall(function()
    ...
    -- Metode 3: Cari RemoteEvent tembak di dalam tool itu sendiri
    pcall(function()
        for _, v in pairs(tool:GetDescendants()) do
            if v:IsA("RemoteEvent") then
                -- Coba berbagai format argument yang umum dipakai gun system
                pcall(function() v:FireServer(targetHRP.Position) end)
                pcall(function() v:FireServer(targetHRP.Position, targetHRP) end)
                pcall(function() v:FireServer(CFrame.new(targetHRP.Position)) end)
            end
            if v:IsA("RemoteFunction") then
                pcall(function() v:InvokeServer(targetHRP.Position) end)
            end
        end
    end)

    -- Metode 4: Scan ReplicatedStorage untuk remote shoot/hit
    pcall(function()
        ...
        local function scanRemotes(parent)
            for _, v in pairs(parent:GetDescendants()) do
                if v:IsA("RemoteEvent") then
                    ...
                    if n:find("shoot") or n:find("hit") or n:find("fire") or n:find("bullet") or n:find("damage") then
                        pcall(function() v:FireServer(targetHRP.Position, targetHRP) end)
                        pcall(function() v:FireServer({Position = targetHRP.Position, Instance = targetHRP}) end)
                    end
                end
            end
        end
...
-- Simpan posisi kita sebelum teleport
local kaSavedCF = nil
    ...
    while true do
        ...
        for _, plr in pairs(Players:GetPlayers()) do
            ...
            if dist <= killAuraRadius then
                ...
                task.spawn(function()
                    -- Ambil senjata fresh tiap target
                    ...
                    local currentGun, alreadyInHand = getGunTool()

                    -- Ekuip kalau belum di tangan
                    if currentGun and not alreadyInHand then
                        equipTool(currentGun)
                    end

                    if currentGun then
                        -- Simpan posisi kita
                        local savedCF = myHRP2.CFrame

                        -- Teleport kita DEKAT target (bukan ke void, hanya 2 stud di sampingnya)
                        pcall(function()
                            myHRP2.CFrame = hrp.CFrame + Vector3.new(2, 0, 0)
                        end)
                        task.wait(0.03)

                        -- Tembak
                        shootAtTarget(currentGun, hrp)
                        task.wait(0.03)

                        -- Balik ke posisi semula
                        pcall(function()
                            myHRP2.CFrame = savedCF
                        end)
                    else
                        -- Tidak ada senjata — coba remote scan langsung
                        pcall(function()
                            local RS = game:GetService("ReplicatedStorage")
                            for _, v in pairs(RS:GetDescendants()) do
                                if v:IsA("RemoteEvent") then
                                    ...
                                    if n:find("damage") or n:find("hit") or n:find("kill") then
                                        pcall(function() v:FireServer(hrp.Position, hrp, 9999) end)
                                        pcall(function() v:FireServer(plr, 9999) end)
                                    end
                                end
                            end
                        end)
