...
local aimbotMode      = "Camera"   -- "Camera" atau "FreeAim"
local aimbotFOV       = 120        -- radius FOV pixel
local aimbotSmooth    = 0.18       -- 0.0 = instant, 1.0 = lambat
local aimbotTarget    = "Head"     -- "Head" atau "HumanoidRootPart"
local aimbotActive    = false      -- true saat RMB ditahan
local aimbotFovCircle = nil        -- Drawing FOV circle
-- UI refs
local aimbotStatusLbl = nil
local aimbotModeLbl   = nil
local fovSliderVal    = nil
local smoothSliderVal = nil
...
minBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        mainFrame:TweenSize(UDim2.new(0,580,0,36),"Out","Quad",0.18,true)
        task.spawn(function() task.wait(0.05)
            for _,v in pairs(mainFrame:GetChildren()) do if v~=titleBar then v.Visible=false end end
        end)
    else
        for _,v in pairs(mainFrame:GetChildren()) do v.Visible=true end
        mainFrame:TweenSize(UDim2.new(0,580,0,460),"Out","Quad",0.18,true)
    end
end)
...
do
    -- ===== HELPER UI FUNCTIONS =====
    local function makeToggleRow(parent, yPos, labelText, defaultOn, callback)
        ...
        togBg.BackgroundColor3 = defaultOn and C.accent or C.border
        togBg.BorderSizePixel = 0
        togBg.Parent = row
        mkCorner(togBg, 10)

        local togKnob = Instance.new("Frame")
        togKnob.Size = UDim2.new(0, 14, 0, 14)
        togKnob.Position = UDim2.new(defaultOn and 1 or 0, defaultOn and -17 or 3, 0.5, -7)
        ...
        togBtn.MouseButton1Click:Connect(function()
            state = not state
            togBg.BackgroundColor3 = state and C.accent or C.border
            togKnob.Position = state and UDim2.new(1, -17, 0.5, -7) or UDim2.new(0, 3, 0.5, -7)
            if callback then callback(state) end
        end)
        return row, function() return state end
    end

    local function makeSliderRow(parent, yPos, labelText, minVal, maxVal, defaultVal, suffix, callback)
        local row = Instance.new("Frame")
        ...
        trackFill.Size = UDim2.new(ratio0, 0, 1, 0)
        trackFill.BackgroundColor3 = C.accent
        trackFill.BorderSizePixel = 0
        trackFill.Parent = trackBg
        mkCorner(trackFill, 2)

        local knob = Instance.new("TextButton")
        knob.Size = UDim2.new(0, 12, 0, 12)
        knob.Position = UDim2.new(ratio0, -6, 0.5, -6)
    ...
    local function makeSectionHeader(parent, yPos, txt)
        local lbl = Instance.new("TextLabel")
    ...
    -- ===== PAGE LAYOUT =====
    makeSectionHeader(pageAimbot, 6, "AIMBOT SETTINGS")

    -- Status row
    local statusRow = Instance.new("Frame")
    statusRow.Size = UDim2.new(1, -20, 0, 38)
    statusRow.Position = UDim2.new(0, 10, 0, 30)
    statusRow.BackgroundColor3 = C.card
    statusRow.BorderSizePixel = 0
    statusRow.Parent = pageAimbot
    mkCorner(statusRow, 6)
    mkStroke(statusRow, 1, C.border)

    local statusLblTitle = Instance.new("TextLabel")
    statusLblTitle.Size = UDim2.new(0.5, 0, 1, 0)
    statusLblTitle.Position = UDim2.new(0, 12, 0, 0)
    statusLblTitle.BackgroundTransparency = 1
    statusLblTitle.Text = "STATUS"
    statusLblTitle.TextColor3 = C.subtext
    statusLblTitle.Font = Enum.Font.GothamBold
    statusLblTitle.TextSize = 11
    statusLblTitle.TextXAlignment = Enum.TextXAlignment.Left
    statusLblTitle.Parent = statusRow

    aimbotStatusLbl = Instance.new("TextLabel")
    aimbotStatusLbl.Size = UDim2.new(0.5, -12, 1, 0)
    aimbotStatusLbl.Position = UDim2.new(0.5, 0, 0, 0)
    ...
    -- Enable toggle
    makeToggleRow(pageAimbot, 76, "Enable Aimbot", false, function(state)
        aimbotEnabled = state
        aimbotStatusLbl.Text = state and "ON  [RMB]" or "OFF"
        aimbotStatusLbl.TextColor3 = state and C.green or C.red
        if aimbotFovCircle then aimbotFovCircle.Visible = state end
    end)

    -- ===== MODE SELECTOR =====
    makeSectionHeader(pageAimbot, 124, "AIMBOT MODE")

    local modeRow = Instance.new("Frame")
    modeRow.Size = UDim2.new(1, -20, 0, 44)
    modeRow.Position = UDim2.new(0, 10, 0, 148)
    modeRow.BackgroundColor3 = C.card
    modeRow.BorderSizePixel = 0
    modeRow.Parent = pageAimbot
    mkCorner(modeRow, 6)
    mkStroke(modeRow, 1, C.border)

    local modeLblTitle = Instance.new("TextLabel")
    modeLblTitle.Size = UDim2.new(0.4, 0, 1, 0)
    modeLblTitle.Position = UDim2.new(0, 12, 0, 0)
    modeLblTitle.BackgroundTransparency = 1
    modeLblTitle.Text = "Mode"
    modeLblTitle.TextColor3 = C.subtext
    modeLblTitle.Font = Enum.Font.GothamBold
    modeLblTitle.TextSize = 11
    modeLblTitle.TextXAlignment = Enum.TextXAlignment.Left
    modeLblTitle.Parent = modeRow

    -- Camera button
    local camBtn = Instance.new("TextButton")
    camBtn.Size = UDim2.new(0, 90, 0, 28)
    camBtn.Position = UDim2.new(1, -196, 0.5, -14)
    camBtn.BackgroundColor3 = C.accent   -- aktif by default
    camBtn.Text = "📷 Camera"
    camBtn.TextColor3 = Color3.fromRGB(255,255,255)
    camBtn.Font = Enum.Font.GothamBold
    camBtn.TextSize = 11
    camBtn.BorderSizePixel = 0
    camBtn.Parent = modeRow
    mkCorner(camBtn, 6)

    -- FreeAim button
    local freeBtn = Instance.new("TextButton")
    freeBtn.Size = UDim2.new(0, 90, 0, 28)
    freeBtn.Position = UDim2.new(1, -100, 0.5, -14)
    freeBtn.BackgroundColor3 = C.card2
    freeBtn.Text = "🖱️ FreeAim"
    freeBtn.TextColor3 = C.subtext
    freeBtn.Font = Enum.Font.GothamBold
    freeBtn.TextSize = 11
    freeBtn.BorderSizePixel = 0
    freeBtn.Parent = modeRow
    mkCorner(freeBtn, 6)
    mkStroke(freeBtn, 1, C.border)

    aimbotModeLbl = Instance.new("TextLabel")  -- hidden ref
    aimbotModeLbl.Visible = false
    aimbotModeLbl.Parent = modeRow
    ...
    local function setAimbotMode(mode)
        ...
        if mode == "Camera" then
            ...
            freeBtn.BackgroundColor3 = C.accent
            freeBtn.TextColor3 = Color3.fromRGB(255,255,255)
            camBtn.BackgroundColor3 = C.card2
            camBtn.TextColor3 = C.subtext
        end
    end
    camBtn.MouseButton1Click:Connect(function() setAimbotMode("Camera") end)
    freeBtn.MouseButton1Click:Connect(function() setAimbotMode("FreeAim") end)

    -- ===== TARGET PART =====
    makeSectionHeader(pageAimbot, 202, "TARGET PART")

    local targetRow = Instance.new("Frame")
    targetRow.Size = UDim2.new(1, -20, 0, 44)
    targetRow.Position = UDim2.new(0, 10, 0, 226)
    targetRow.BackgroundColor3 = C.card
    targetRow.BorderSizePixel = 0
    targetRow.Parent = pageAimbot
    mkCorner(targetRow, 6)
    mkStroke(targetRow, 1, C.border)

    local tgtLblTitle = Instance.new("TextLabel")
    tgtLblTitle.Size = UDim2.new(0.4, 0, 1, 0)
    tgtLblTitle.Position = UDim2.new(0, 12, 0, 0)
    tgtLblTitle.BackgroundTransparency = 1
    tgtLblTitle.Text = "Target"
    tgtLblTitle.TextColor3 = C.subtext
    tgtLblTitle.Font = Enum.Font.GothamBold
    tgtLblTitle.TextSize = 11
    tgtLblTitle.TextXAlignment = Enum.TextXAlignment.Left
    tgtLblTitle.Parent = targetRow

    local headBtn = Instance.new("TextButton")
    headBtn.Size = UDim2.new(0, 80, 0, 28)
    headBtn.Position = UDim2.new(1, -186, 0.5, -14)
    ...
    headBtn.Parent = targetRow
    mkCorner(headBtn, 6)

    local bodyBtn = Instance.new("TextButton")
    bodyBtn.Size = UDim2.new(0, 80, 0, 28)
    bodyBtn.Position = UDim2.new(1, -98, 0.5, -14)
    bodyBtn.BackgroundColor3 = C.card2
    bodyBtn.Text = "🧍 Body"
    bodyBtn.TextColor3 = C.subtext
    bodyBtn.Font = Enum.Font.GothamBold
    bodyBtn.TextSize = 11
    bodyBtn.BorderSizePixel = 0
    bodyBtn.Parent = targetRow
    mkCorner(bodyBtn, 6)
    mkStroke(bodyBtn, 1, C.border)

    local function setAimbotTarget(part)
        aimbotTarget = part
        if part == "Head" then
            headBtn.BackgroundColor3 = C.accent
            headBtn.TextColor3 = Color3.fromRGB(255,255,255)
            ...
            headBtn.BackgroundColor3 = C.card2
            headBtn.TextColor3 = C.subtext
        end
    end
    headBtn.MouseButton1Click:Connect(function() setAimbotTarget("Head") end)
    bodyBtn.MouseButton1Click:Connect(function() setAimbotTarget("HumanoidRootPart") end)

    -- ===== SLIDERS =====
    makeSectionHeader(pageAimbot, 282, "SENSITIVITY & FOV")

    makeSliderRow(pageAimbot, 306, "FOV Radius", 20, 400, aimbotFOV, "px", function(v)
        aimbotFOV = v
        if aimbotFovCircle then aimbotFovCircle.Radius = v end
    end)

    makeSliderRow(pageAimbot, 366, "Smoothness", 0, 20, math.floor(aimbotSmooth * 100), "%", function(v)
        aimbotSmooth = v / 100
    end)
    ...
    infoRow.Position = UDim2.new(0, 10, 0, 428)
    infoRow.BackgroundColor3 = C.card2
    infoRow.BorderSizePixel = 0
    infoRow.Parent = pageAimbot
    mkCorner(infoRow, 6)
    mkStroke(infoRow, 1, C.border)
    ...
    infoLbl.Text = '<font color="rgb(220,38,38)">RMB</font> = hold to aim  •  <font color="rgb(120,120,120)">Camera</font> = gerakin kamera  •  <font color="rgb(120,120,120)">FreeAim</font> = gerakin mouse'
    ...
    pageAimbot.CanvasSize = UDim2.new(0, 0, 0, 476)
end
...
-- Update posisi FOV circle tiap frame biar tetap di tengah layar
RunService.RenderStepped:Connect(function()
...
-- Fungsi cari target terdekat ke tengah layar dalam FOV
local function getBestTarget()
    ...
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            ...
            if char then
                local humanoid = char:FindFirstChildOfClass("Humanoid")
                local part = char:FindFirstChild(aimbotTarget)
...
-- Mode Camera: geser kamera ke arah target
local function aimCamera(targetPart)
    ...
    if aimbotSmooth <= 0.01 then
        Camera.CFrame = targetCF
    else
        Camera.CFrame = currentCF:Lerp(targetCF, 1 - aimbotSmooth)
    end
end

-- Mode FreeAim: geser posisi mouse ke arah target di layar
local function aimFreeAim(targetPart)
    if not targetPart then return end
    ...
    if aimbotSmooth <= 0.01 then
        -- instant
        pcall(function()
            mousemoverel(tx - cx, ty - cy)
        end)
    else
        -- smooth: gerak bertahap
        local dx = (tx - cx) * (1 - aimbotSmooth)
        local dy = (ty - cy) * (1 - aimbotSmooth)
        pcall(function()
            mousemoverel(dx, dy)
        end)
    end
...
-- Aimbot loop (RenderStepped biar smooth)
RunService.RenderStepped:Connect(function()
    ...
    if aimbotMode == "Camera" then
        aimCamera(target)
    elseif aimbotMode == "FreeAim" then
        aimFreeAim(target)
    end
end)
print("=== MAJESTY ONTOP v7.0 - VALARY THEME ===")
print("Fitur:")
...
print("- 🎯 AIMBOT: Camera / FreeAim | RMB Hold | FOV Circle")
print("========================================")
