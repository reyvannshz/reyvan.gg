...
-- ========== TELEPORT - METODE ASLI SOUTH BRONX ==========
-- Dari script GitHub South Bronx yang terbukti work (github.com/012343/O):
-- Set CFrame langsung TANPA loop, TANPA anchor, TANPA task.wait
-- Kunci: pakai "player.CharacterAdded:Wait()" sebelum set tiap step
-- Ini cara PERSIS yang dipakai script asli South Bronx
...
local function doTeleport(targetCFrame)
    task.spawn(function()
        ...
        -- Step 3 studs persis seperti script South Bronx asli
        local STEP = 3
        local steps = math.ceil(dist / STEP)

        for i = 1, steps do
            -- Re-fetch karakter tiap step (persis cara script asli)
            ...
            -- Jeda 0.1 detik antar step (cukup untuk server terima)
            task.wait(0.1)
        end

        -- Set tepat di posisi akhir
        character = player.Character or player.CharacterAdded:Wait()
        hrp       = character:WaitForChild("HumanoidRootPart")
