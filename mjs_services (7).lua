...
local pageWallBreaker = makePage()
...
do
    ...
    sellToggleBtn.Position = UDim2.new(0, 10, 0, 658)
    ...
    makeLabel(sellInfoCard, "📍  Auto teleport ke NPC jual saat sell aktif", 10, 0, 400, 30, 10, C.subtext, Enum.Font.Gotham)

    -- Expose refs ke luar do block agar logic bisa akses
    _G.sellToggleBtn  = sellToggleBtn
    _G.sellStatusLbl  = sellStatusLbl
    _G.sellCountLbl   = sellCountLbl
    ...
    pageAuto.CanvasSize = UDim2.new(0,0,0,920)
end
...
-- ========== WALL BREAKER ==========
-- Client-side only: dinding menghilang di layar kamu, tidak di server
-- Tidak ada CanCollide = false, tidak ada Destroy → tidak crash
-- ON = tool ada di inventory, OFF = tool dihapus dari inventory

local wbEnabled      = false
local wbMouse        = LocalPlayer:GetMouse()
local wbToolRef      = nil
local wbToolEquipped = false
local wbHiddenParts  = {}   -- { [part] = origTransparency }
local wbAoeActive    = false

-- Part yang tidak boleh disembunyikan
local wbSafeWords = {
    "humanoidrootpart","head","torso","upperarm","lowerarm","hand",
    "upperleg","lowerleg","foot","baseplate","spawnlocation","spawn",
    "handle","rootpart"
}
local function wbIsSafe(part)
    if not part or part:IsA("Terrain") then return true end
    local n = part.Name:lower()
    for _, w in pairs(wbSafeWords) do
...
-- Sembunyikan part (client-only transparency)
local function wbHide(part)
    if wbIsSafe(part) then return end
    if wbHiddenParts[part] then return end  -- sudah disembunyikan
    wbHiddenParts[part] = part.LocalTransparencyModifier  -- simpan nilai asli
    part.LocalTransparencyModifier = 1  -- hanya ubah di client, tidak di server
end

-- Tampilkan kembali semua part yang disembunyikan
local function wbRestoreAll()
    for part, origVal in pairs(wbHiddenParts) do
        pcall(function()
...
-- Buat tool Wall Breaker dan masukkan ke inventory
local function wbCreateTool()
    local bp = LocalPlayer:FindFirstChild("Backpack")
    local ch = LocalPlayer.Character
    -- Cek sudah ada
    if bp and bp:FindFirstChild("Wall Breaker") then return end
    if ch and ch:FindFirstChild("Wall Breaker") then return end

    local tool = Instance.new("Tool")
    tool.Name            = "Wall Breaker"
    tool.RequiresHandle  = true
    tool.ToolTip         = "LMB: sembunyikan dinding | Tahan RMB: AOE radius 15 stud"
    ...
    -- Klik kiri: sembunyikan 1 dinding + seluruh model di atas target
    tool.Activated:Connect(function()
        local target = wbMouse.Target
        if not target or wbIsSafe(target) then return end
        -- Sembunyikan target
        wbHide(target)
        -- Sembunyikan juga semua part dalam model yang sama
        local parent = target.Parent
        if parent and (parent:IsA("Model") or parent:IsA("Folder")) then
            for _, p in pairs(parent:GetDescendants()) do
                if p:IsA("BasePart") then wbHide(p) end
            end
        end
    end)
...
-- Hapus tool dari inventory
local function wbRemoveTool()
    wbRestoreAll()
    local bp = LocalPlayer:FindFirstChild("Backpack")
    local ch = LocalPlayer.Character
    if bp then
        local t = bp:FindFirstChild("Wall Breaker")
        if t then t:Destroy() end
    end
    if ch then
        local t = ch:FindFirstChild("Wall Breaker")
        if t then t:Destroy() end
    end
    wbToolRef      = nil
    wbToolEquipped = false
...
-- AOE loop: sembunyikan semua part dalam radius 15 stud dari kursor
RunService.RenderStepped:Connect(function()
    if not wbAoeActive or not wbToolEquipped then return end
    local ok, hitPos = pcall(function() return wbMouse.Hit.Position end)
    if not ok then return end
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("BasePart") and not wbIsSafe(obj) then
            if (obj.Position - hitPos).Magnitude <= 15 then
                wbHide(obj)
            end
        end
    end
...
-- Respawn: pasang tool kembali kalau WB masih ON
...
-- ===== PAGE: WALL BREAKER =====
do
    sectionTitle(pageWallBreaker, "🔨  WALL BREAKER", 8)

    -- Info card
    local wbInfoCard = makeCard(pageWallBreaker, 34, 54)
    local wbInfoLbl = Instance.new("TextLabel")
    wbInfoLbl.Size = UDim2.new(1, -16, 1, 0)
    wbInfoLbl.Position = UDim2.new(0, 10, 0, 0)
    wbInfoLbl.BackgroundTransparency = 1
    wbInfoLbl.Text = "Dinding hanya menghilang di layar kamu (client-side).\nDi layar player lain dinding tetap ada."
    ...
    -- Toggle button
    local wbToggleBtn = Instance.new("TextButton")
    wbToggleBtn.Size = UDim2.new(1, -20, 0, 42)
    wbToggleBtn.Position = UDim2.new(0, 10, 0, 98)
    wbToggleBtn.BackgroundColor3 = Color3.fromRGB(35, 20, 20)
    wbToggleBtn.Text = "🔨  WALL BREAKER : OFF"
    wbToggleBtn.TextColor3 = C.red
    wbToggleBtn.Font = Enum.Font.GothamBlack
    wbToggleBtn.TextSize = 14
    wbToggleBtn.BorderSizePixel = 0
    wbToggleBtn.Parent = pageWallBreaker
    mkCorner(wbToggleBtn, 6)
    mkStroke(wbToggleBtn, 1, C.red)

    -- Status card
    sectionTitle(pageWallBreaker, "STATUS", 150)
    local wbStatCard = makeCard(pageWallBreaker, 176, 44)
    makeLabel(wbStatCard, "STATUS", 12, 0, 80, 44, 11, C.subtext, Enum.Font.GothamBold)
    local wbStatusLbl = makeLabel(wbStatCard, "OFF", 90, 0, 260, 44, 16, C.red, Enum.Font.GothamBlack)

    local wbHiddenCard = makeCard(pageWallBreaker, 228, 44)
    makeLabel(wbHiddenCard, "HIDDEN", 12, 0, 80, 44, 11, C.subtext, Enum.Font.GothamBold)
    local wbHiddenCountLbl = makeLabel(wbHiddenCard, "0", 90, 0, 260, 44, 16, C.yellow, Enum.Font.GothamBlack)

    -- Update hidden count tiap detik
    task.spawn(function()
        while true do
            task.wait(1)
            local count = 0
            for _ in pairs(wbHiddenParts) do count = count + 1 end
            if wbHiddenCountLbl then
                wbHiddenCountLbl.Text = tostring(count) .. " parts"
            end
        end
    end)

    -- Restore button
    local wbRestoreBtn = Instance.new("TextButton")
    wbRestoreBtn.Size = UDim2.new(1, -20, 0, 36)
    wbRestoreBtn.Position = UDim2.new(0, 10, 0, 282)
    wbRestoreBtn.BackgroundColor3 = C.card
    wbRestoreBtn.Text = "👁  Tampilkan Semua Dinding Kembali"
    wbRestoreBtn.TextColor3 = C.text
    wbRestoreBtn.Font = Enum.Font.GothamBold
    wbRestoreBtn.TextSize = 12
    wbRestoreBtn.BorderSizePixel = 0
    wbRestoreBtn.Parent = pageWallBreaker
    mkCorner(wbRestoreBtn, 6)
    mkStroke(wbRestoreBtn, 1, C.border)
    wbRestoreBtn.MouseButton1Click:Connect(function()
        wbRestoreAll()
        wbHiddenCountLbl.Text = "0 parts"
    end)

    -- Cara pakai
    sectionTitle(pageWallBreaker, "CARA PAKAI", 330)
    local howCard = makeCard(pageWallBreaker, 356, 90)
    local howLbl = Instance.new("TextLabel")
    howLbl.Size = UDim2.new(1, -16, 1, 0)
    howLbl.Position = UDim2.new(0, 10, 0, 0)
    howLbl.BackgroundTransparency = 1
    howLbl.RichText = true
    howLbl.Text = '<font color="rgb(220,38,38)">1.</font> Klik ON → tool masuk inventory\n<font color="rgb(220,38,38)">2.</font> Equip tool dari hotbar\n<font color="rgb(220,38,38)">3.</font> Klik kiri = sembunyikan 1 dinding\n<font color="rgb(220,38,38)">4.</font> Tahan klik kanan = AOE radius 15 stud'
    howLbl.TextColor3 = C.text
    howLbl.Font = Enum.Font.Gotham
    howLbl.TextSize = 11
    howLbl.TextWrapped = true
    howLbl.TextXAlignment = Enum.TextXAlignment.Left
    howLbl.TextYAlignment = Enum.TextYAlignment.Top
    howLbl.Parent = howCard

    pageWallBreaker.CanvasSize = UDim2.new(0, 0, 0, 460)

    -- Toggle logic
    wbToggleBtn.MouseButton1Click:Connect(function()
        wbEnabled = not wbEnabled
        if wbEnabled then
            wbToggleBtn.Text = "🔨  WALL BREAKER : ON"
            wbToggleBtn.TextColor3 = Color3.fromRGB(100, 180, 255)
            wbToggleBtn.BackgroundColor3 = Color3.fromRGB(15, 25, 50)
            mkStroke(wbToggleBtn, 1, Color3.fromRGB(100, 180, 255))
            wbStatusLbl.Text = "ACTIVE"
            wbStatusLbl.TextColor3 = Color3.fromRGB(100, 180, 255)
            wbCreateTool()
        else
            wbToggleBtn.Text = "🔨  WALL BREAKER : OFF"
            wbToggleBtn.TextColor3 = C.red
            wbToggleBtn.BackgroundColor3 = Color3.fromRGB(35, 20, 20)
            mkStroke(wbToggleBtn, 1, C.red)
            wbStatusLbl.Text = "OFF"
            wbStatusLbl.TextColor3 = C.red
            wbRemoveTool()
        end
    end)
end
...
local tabDefs = {
    ...
    {icon="🔨", label="WallBreak",  page=pageWallBreaker},
...
local msSellerWords = {"sell","seller","npc","vendor","shop","store","market",
                       "marshmellow","cashier","clerk"}
...
local function findSellerNPC()
    -- Cari NPC/model penjual terdekat
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do
        -- Cari Part/Model yang namanya mengandung kata penjual
        local n = obj.Name:lower()
        local isSeller = false
        for _, kw in pairs(msSellerWords) do
            if n:find(kw) then isSeller = true break end
        end
        if isSeller and obj:IsA("BasePart") then
...
task.spawn(function()
    while true do
        task.wait(0.5)
        if not autoSell_Running then continue end

        local items = getMarshmellowItems()
        if #items == 0 then
            -- Tidak ada marshmellow, update status
            if _G.sellStatusLbl then
                _G.sellStatusLbl.Text = "MENUNGGU MS"
                _G.sellStatusLbl.TextColor3 = Color3.fromRGB(234, 179, 8)
            end
            continue
        end

        if _G.sellStatusLbl then
            _G.sellStatusLbl.Text = "MENJUAL..."
            _G.sellStatusLbl.TextColor3 = Color3.fromRGB(34, 197, 94)
        end

        -- Cari NPC seller
        local sellerPart, dist = findSellerNPC()

        for _, marshmellow in ipairs(items) do
            if not autoSell_Running then break end

            -- Hold marshmellow ke tangan
            pcall(function()
            ...
            if sellerPart then
                -- Hadapkan ke NPC
                facePosition(sellerPart.Position)
                task.wait(0.1)
            end

            -- Tekan E beberapa kali untuk jual
            for _ = 1, 3 do
                if not autoSell_Running then break end
                pressE()
                task.wait(0.1)
            ...
            if _G.sellCountLbl then
                _G.sellCountLbl.Text = tostring(autoSell_Count)
            end

            task.wait(0.3)
        end

        if _G.sellStatusLbl and autoSell_Running then
            _G.sellStatusLbl.Text = "RUNNING"
            _G.sellStatusLbl.TextColor3 = Color3.fromRGB(34, 197, 94)
        end
    end
end)
