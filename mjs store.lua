...
local patDoAutoBuyUI -- forward declaration (diisi di dalam do-block bawah)
...
do
    ...
    -- ========== BUY BAHAN (PatstoreMS Engine - StorePurchase RemoteEvent) ==========
    ...
    -- PatstoreMS autobuy engine (StorePurchase:FireServer)
    local PAT_BUY_ITEMS = {
    ...
    patDoAutoBuyUI = function(setStatus2, overrideQtys)
        -- Coba cari remote lagi jika belum ketemu saat load
        if not patStorePurchase then
            setStatus2("Mencari Remote StorePurchase...", C.yellow)
            pcall(function()
                local rs = game:GetService("ReplicatedStorage")
                local re = rs:WaitForChild("RemoteEvents", 8)
                if re then patStorePurchase = re:WaitForChild("StorePurchase", 8) end
            end)
        end
        if not patStorePurchase then
            setStatus2("Remote StorePurchase tidak ada!", C.red)
            task.wait(1.5)
            return
        end
        local totalBought = 0
        local qtys = overrideQtys or {buyFullGelatin, buyFullSugar, buyFullWater}
        for idx, item in ipairs(PAT_BUY_ITEMS) do
            local qty = qtys[idx] or 1
            if qty <= 0 then continue end
            setStatus2("Beli "..item.display.." ×"..qty.."...", C.accent2)
            bil.Text = item.display
            local before = patCountItem(item.name)
            local successBuy = 0
            for i = 1, qty do
                local ok = pcall(function()
                    patStorePurchase:FireServer(item.name, 1)
                end)
                if ok then successBuy += 1 end
                task.wait(0.4)
            end
            local timeout2 = 0
            local gained = 0
            repeat
                task.wait(0.2)
                timeout2 += 0.2
                gained = patCountItem(item.name) - before
            until gained >= qty or timeout2 > 6
            if gained < qty then
                local missing = qty - gained
                setStatus2("Retry "..missing.." "..item.display, C.yellow)
                for i = 1, missing do
                    pcall(function() patStorePurchase:FireServer(item.name, 1) end)
                    task.wait(0.5)
                end
                timeout2 = 0
                repeat
                    ...
                    timeout2 += 0.2
                    gained = patCountItem(item.name) - before
                until gained >= qty or timeout2 > 5
            end
            totalBought += gained
            patTotalBuy += gained
            if gained < qty then
                setStatus2(item.display.." kurang ("..gained.."/"..qty..")", C.red)
            else
                setStatus2(item.display.." ×"..gained.." selesai!", C.accent)
            end
            task.wait(0.2)
        end
        setStatus2("Beli selesai! "..totalBought.." item.", C.accent)
        task.wait(1)
    end
    ...
    buyTogBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            bpl.Text="Membeli bahan..."; bpl.TextColor3=C.accent2
            patDoAutoBuyUI(function(msg, col)
                bpl.Text = msg; bpl.TextColor3 = col or C.accent2
            end, {buyFullGelatin, buyFullSugar, buyFullWater})
...
do
    ...
    pageVehicleTP.CanvasSize=UDim2.new(0,0,0,kompBase+26+#kompors*44+260)

    -- ========== AUTO FULLY UI (PatstoreMS) ==========
    local fullyBase = kompBase + 26 + #kompors*44 + 8
    sectionTitle(pageVehicleTP,"AUTO FULLY MS",fullyBase)
    local fullyInfoCard = makeCard(pageVehicleTP, fullyBase+26, 44)
    makeLabel(fullyInfoCard,"Loop: Beli bahan → Masak di Apart → Jual → Ulangi",10,2,400,20,9,C.subtext)
    makeLabel(fullyInfoCard,"Pastikan naik motor untuk teleport!",10,22,400,20,9,C.accent)

    -- Save Apart position
    local fullyCoordCard = makeCard(pageVehicleTP, fullyBase+78, 34)
    local fullyCoordLbl  = makeLabel(fullyCoordCard,"Belum disimpan",10,0,300,34,10,C.subtext)
    local savePosBtn     = Instance.new("TextButton")
    savePosBtn.Size            = UDim2.new(0,80,0,26)
    savePosBtn.Position        = UDim2.new(1,-90,0.5,-13)
    ...
    savePosBtn.Parent          = fullyCoordCard
    mkCorner(savePosBtn,5)
    savePosBtn.MouseButton1Click:Connect(function()
        ...
        if root then
            ...
            fullyCoordLbl.Text      = string.format("%.1f, %.1f, %.1f", p.X, p.Y, p.Z)
            fullyCoordLbl.TextColor3= C.accent
        end
    end)

    -- Target stepper
    local fullyTargetCard = makeCard(pageVehicleTP, fullyBase+120, 44)
    makeLabel(fullyTargetCard,"Target MS per loop",10,0,200,44,11,C.text)
    local fullyTargetLbl = makeLabel(fullyTargetCard,tostring(fullyTarget),0,0,0,44,15,C.yellow,Enum.Font.GothamBold,Enum.TextXAlignment.Center)
    fullyTargetLbl.Size     = UDim2.new(0,44,1,0)
    fullyTargetLbl.Position = UDim2.new(0.5,-22,0,0)
    local ftMinW = makeCard(pageVehicleTP,0,0); ftMinW.Parent=fullyTargetCard
    ftMinW.Size=UDim2.new(0,30,0,28); ftMinW.Position=UDim2.new(0.5,-22-36,0.5,-14); ftMinW.BackgroundColor3=C.card2
    local ftMinB=Instance.new("TextButton"); ftMinB.Size=UDim2.new(1,0,1,0); ftMinB.Text="-"; ftMinB.TextSize=16; ftMinB.Font=Enum.Font.GothamBold; ftMinB.BackgroundTransparency=1; ftMinB.TextColor3=C.red; ftMinB.BorderSizePixel=0; ftMinB.Parent=ftMinW
    mkCorner(ftMinW,5)
    local ftPlusW = makeCard(pageVehicleTP,0,0); ftPlusW.Parent=fullyTargetCard
    ftPlusW.Size=UDim2.new(0,30,0,28); ftPlusW.Position=UDim2.new(0.5,22+6,0.5,-14); ftPlusW.BackgroundColor3=C.card2
    local ftPlusB=Instance.new("TextButton"); ftPlusB.Size=UDim2.new(1,0,1,0); ftPlusB.Text="+"; ftPlusB.TextSize=16; ftPlusB.Font=Enum.Font.GothamBold; ftPlusB.BackgroundTransparency=1; ftPlusB.TextColor3=C.accent; ftPlusB.BorderSizePixel=0; ftPlusB.Parent=ftPlusW
    mkCorner(ftPlusW,5)
    ftMinB.MouseButton1Click:Connect(function()
        fullyTarget = math.max(1, fullyTarget - 1)
        fullyTargetLbl.Text = tostring(fullyTarget)
    end)
    ftPlusB.MouseButton1Click:Connect(function()
        fullyTarget = math.min(99, fullyTarget + 1)
        fullyTargetLbl.Text = tostring(fullyTarget)
    end)

    -- Status label
    local fullyStatusCard = makeCard(pageVehicleTP, fullyBase+172, 30)
    local fullyStatusLbl  = makeLabel(fullyStatusCard,"Belum dimulai",10,0,400,30,10,C.subtext)

    -- Start/Stop buttons
    local fullyStartBtn = Instance.new("TextButton")
    fullyStartBtn.Size            = UDim2.new(0.48,-12,0,34)
    fullyStartBtn.Position        = UDim2.new(0,10,0,fullyBase+210)
    fullyStartBtn.BackgroundColor3= C.card
    fullyStartBtn.Text            = "START FULLY"
    fullyStartBtn.TextColor3      = C.accent
    fullyStartBtn.Font            = Enum.Font.GothamBold
    fullyStartBtn.TextSize        = 11
    fullyStartBtn.BorderSizePixel = 0
    fullyStartBtn.Parent          = pageVehicleTP
    mkCorner(fullyStartBtn,5); mkStroke(fullyStartBtn,1,C.accent)

    local fullyStopBtn = Instance.new("TextButton")
    fullyStopBtn.Size            = UDim2.new(0.48,-12,0,34)
    fullyStopBtn.Position        = UDim2.new(0.5,2,0,fullyBase+210)
    fullyStopBtn.BackgroundColor3= C.card
    fullyStopBtn.Text            = "STOP FULLY"
    fullyStopBtn.TextColor3      = C.red
    fullyStopBtn.Font            = Enum.Font.GothamBold
    fullyStopBtn.TextSize        = 11
    fullyStopBtn.BorderSizePixel = 0
    fullyStopBtn.Parent          = pageVehicleTP
    mkCorner(fullyStopBtn,5); mkStroke(fullyStopBtn,1,C.red)

    local function setFullyStatus(msg, col)
        fullyStatusLbl.Text       = msg
        fullyStatusLbl.TextColor3 = col or C.subtext
    end
    ...
    fullyStartBtn.MouseButton1Click:Connect(function()
        ...
        setFullyStatus("Auto Fully berjalan...", C.accent)
        task.spawn(function()
            patDoAutoFully(setFullyStatus)
            setFullyStatus("Dihentikan", C.subtext)
        end)
    end)
    ...
    fullyStopBtn.MouseButton1Click:Connect(function()
        ...
        setFullyStatus("Dihentikan", C.yellow)
    end)
end
...
-- ========== AUTO MS (PatstoreMS Engine) ==========

-- Konfigurasi item PatstoreMS
local PAT_CFG = {
...
local patStorePurchase = patRemotes and patRemotes:WaitForChild("StorePurchase", 10)
local patRpcRE         = patRemotes and patRemotes:WaitForChild("RPC", 10)
local patIsBusy        = false
local patStats         = {small=0, medium=0, large=0}
local patTotalSold     = 0
local patTotalBuy      = 0
local patRpcQueue      = {}

local function patTotalMS() return patStats.small + patStats.medium + patStats.large end

local function patCountItem(name)
    local n = 0
...
local function patCountAllMS()
    return patCountItem(PAT_CFG.ITEM_MS_SMALL)
        + patCountItem(PAT_CFG.ITEM_MS_MEDIUM)
        + patCountItem(PAT_CFG.ITEM_MS_LARGE)
end

local function patGetEquippableMS()
    if patCountItem(PAT_CFG.ITEM_MS_SMALL)  > 0 then return PAT_CFG.ITEM_MS_SMALL  end
    if patCountItem(PAT_CFG.ITEM_MS_MEDIUM) > 0 then return PAT_CFG.ITEM_MS_MEDIUM end
    if patCountItem(PAT_CFG.ITEM_MS_LARGE)  > 0 then return PAT_CFG.ITEM_MS_LARGE  end
    return nil
end

local function patHasAllIngredients()
    return patCountItem(PAT_CFG.ITEM_WATER) >= 1
        and patCountItem(PAT_CFG.ITEM_SUGAR) >= 1
        and patCountItem(PAT_CFG.ITEM_GEL) >= 1
end

local function patEquipTool(name)
    local ch  = LocalPlayer.Character
...
local function patFirePromptNearby(radius)
    local ch   = LocalPlayer.Character
    local root = ch and ch:FindFirstChild("HumanoidRootPart")
...
if patRpcRE then
    patRpcRE.OnClientEvent:Connect(function(_, tblArg)
        if type(tblArg) ~= "table" then return end
        local v1  = tblArg[1]
        local v2  = tblArg[2]
        local msg = tostring(v1 or ""):lower()
        if v2 == "TextLabel" and tonumber(v1) then
            table.insert(patRpcQueue, {type="timer", secs=tonumber(v1)})
            return
        end
        if msg:find("boil") or msg:find("water") then
            table.insert(patRpcQueue, {type="wait_boil"})
        elseif msg:find("sugar") then
            table.insert(patRpcQueue, {type="add_sugar"})
        elseif msg:find("gelatin") then
            table.insert(patRpcQueue, {type="add_gelatin"})
        elseif msg:find("cook") then
            table.insert(patRpcQueue, {type="wait_cook"})
        elseif msg:find("bag") then
            table.insert(patRpcQueue, {type="bag_result"})
        end
    end)
end

local function patWaitRPC(instrType, timeout)
    local start = tick()
    while tick() - start < timeout do
        for i = 1, #patRpcQueue do
            local inst = patRpcQueue[i]
            if inst and inst.type == instrType then
                table.remove(patRpcQueue, i)
                return inst
            end
        end
        task.wait(0.1)
...
local function patPopTimer()
    for i = 1, #patRpcQueue do
        local v = patRpcQueue[i]
...
local function patCookInteract(toolName, radius)
    if toolName then
        patEquipTool(toolName)
        task.wait(0.2)
    end
    patFirePromptNearby(radius or 8)
    task.wait(0.1)
    pcall(function()
        VIM2:SendKeyEvent(true,  Enum.KeyCode.E, false, game)
        task.wait(0.1)
...
local function patSetStatus(msg, col)
    if statusValue  then statusValue.Text  = msg end
    if phaseValue   then phaseValue.Text   = msg end
    if timerValue   then timerValue.TextColor3 = col or C.accent2 end
end

local function patDoOneCook()
    patIsBusy = true
    table.clear(patRpcQueue)

    local snapS = patCountItem(PAT_CFG.ITEM_MS_SMALL)
    local snapM = patCountItem(PAT_CFG.ITEM_MS_MEDIUM)
    local snapL = patCountItem(PAT_CFG.ITEM_MS_LARGE)

    statusValue.Text  = "RUNNING"; statusValue.TextColor3 = C.accent
    phaseValue.Text   = "Water";   timerValue.Text = "..."
    patCookInteract(PAT_CFG.ITEM_WATER)

    local boilSecs
    for _ = 1, 30 do
        boilSecs = patPopTimer()
        if boilSecs then break end
        task.wait(0.1)
    end
    boilSecs = boilSecs or PAT_CFG.WATER_WAIT

    for i = boilSecs, 1, -1 do
        if not AutoMS_Running then patIsBusy = false return false end
        phaseValue.Text  = "Mendidih..."
        timerValue.Text  = i.."s"
        task.wait(1)
    end

    phaseValue.Text = "Tunggu Sugar..."
    patWaitRPC("add_sugar", 10)
    phaseValue.Text = "Masukkan Sugar..."
    patCookInteract(PAT_CFG.ITEM_SUGAR)

    phaseValue.Text = "Tunggu Gelatin..."
    patWaitRPC("add_gelatin", 10)
    phaseValue.Text = "Masukkan Gelatin..."
    patCookInteract(PAT_CFG.ITEM_GEL)

    local cookSecs
    for _ = 1, 30 do
        cookSecs = patPopTimer()
        if cookSecs then break end
        task.wait(0.1)
    end
    cookSecs = cookSecs or PAT_CFG.COOK_WAIT

    for i = cookSecs, 1, -1 do
        if not AutoMS_Running then patIsBusy = false return false end
        phaseValue.Text = "Memasak..."
        timerValue.Text = i.."s"
        task.wait(1)
    end

    phaseValue.Text = "Tunggu Bag..."
    patWaitRPC("bag_result", 12)

    local bag
    local t2 = 0
    repeat
        bag = LocalPlayer.Backpack:FindFirstChild(PAT_CFG.ITEM_EMPTY)
        task.wait(0.3)
        t2 += 0.3
    until bag or t2 > 10

    if not bag then
        statusValue.Text = "ERROR: No Empty Bag"
        statusValue.TextColor3 = C.red
        patIsBusy = false
        return false
    end

    phaseValue.Text = "Ambil Marshmallow..."
    patCookInteract(PAT_CFG.ITEM_EMPTY)
    ...
    repeat
        task.wait(0.3)
        waitMS += 0.3
        newS = patCountItem(PAT_CFG.ITEM_MS_SMALL)  - snapS
        newM = patCountItem(PAT_CFG.ITEM_MS_MEDIUM) - snapM
        newL = patCountItem(PAT_CFG.ITEM_MS_LARGE)  - snapL
    until (newS > 0 or newM > 0 or newL > 0) or waitMS > 8

    if newS > 0 then
        patStats.small += newS
    elseif newM > 0 then
        patStats.medium += newM
    elseif newL > 0 then
        patStats.large += newL
    else
        patStats.small += 1
    ...
    phaseValue.Text = "Complete #"..patTotalMS()
...
local function autoMSLoop()
    while AutoMS_Running do
        if not patHasAllIngredients() then
            statusValue.Text = "BAHAN HABIS!"
            statusValue.TextColor3 = C.red
            AutoMS_Running = false
            break
        end
        local ok = pcall(patDoOneCook)
        if not ok then
...
-- ========== AUTO SELL (PatstoreMS Engine) ==========
local function patDoAutoSell(setStatus2)
    local msTotal = patCountAllMS()
    if msTotal == 0 then
        setStatus2("Tidak ada MS", C.subtext)
        return
    end
    setStatus2("Memulai jual "..msTotal.." MS...", C.accent)
    task.wait(0.3)
    ...
    while patCountAllMS() > 0 do
        local msName = patGetEquippableMS()
        if not msName then break end

        local ok = patEquipTool(msName)
        if not ok then
            failStreak += 1
            setStatus2("Gagal equip ("..failStreak.."/"..maxFail..")", C.red)
            task.wait(1)
            if failStreak >= maxFail then break end
            continue
        end

        local bS = patCountItem(PAT_CFG.ITEM_MS_SMALL)
        local bM = patCountItem(PAT_CFG.ITEM_MS_MEDIUM)
        local bL = patCountItem(PAT_CFG.ITEM_MS_LARGE)
        task.wait(0.2)
        patPressE()
        task.wait(0.3)
        patFirePromptNearby(PAT_CFG.SELL_RADIUS)
        task.wait(0.3)
        patPressE()

        local elapsed = 0
        local terjual = false
        while elapsed < PAT_CFG.SELL_TIMEOUT do
            local diff = (bS - patCountItem(PAT_CFG.ITEM_MS_SMALL))
                       + (bM - patCountItem(PAT_CFG.ITEM_MS_MEDIUM))
                       + (bL - patCountItem(PAT_CFG.ITEM_MS_LARGE))
            if diff > 0 then
                sold        += diff
                patTotalSold += diff
                terjual     = true
        ...
        if terjual then
            setStatus2("Terjual "..sold.." | Sisa: "..patCountAllMS(), C.accent)
            task.wait(0.2)
        else
            failStreak += 1
            setStatus2("Tidak terjual ("..failStreak.."/"..maxFail..")", C.yellow)
            task.wait(1.2)
            if failStreak >= maxFail then
                setStatus2("Gagal. Dekati NPC!", C.red)
                break
            end
        end
    end
    ...
    patUnequipAll()
    if sold > 0 then
        setStatus2("Terjual "..sold.." MS (total: "..patTotalSold..")", C.accent)
    else
        setStatus2("Tidak ada MS terjual. Dekati NPC!", C.yellow)
    end
    task.wait(1)
end
...
task.spawn(function()
    while true do
        ...
        patDoAutoSell(function(msg, col)
...
-- ========== AUTO FULLY MS (PatstoreMS Engine) ==========

local function patMoveVehicle(vehicle, targetPos)
...
local function patFullyTeleport(targetPos)
    local ch  = LocalPlayer.Character
    ...
    if seatPart then
        ...
        if vehicle then
            patMoveVehicle(vehicle, targetPos)
            task.wait(0.5)
        end
    else
        task.wait(0.5)
...
local function patDoAutoFully(setFullyStatus)
    fullyRunning = true
    ...
    while fullyRunning do
        ...
        setFullyStatus("Teleport ke NPC Marshmallow...", C.accent2)
        patFullyTeleport(NPC_MS_POS)
        if not fullyRunning then break end

        setFullyStatus("Beli bahan untuk "..target.." MS...", C.accent2)
        if patDoAutoBuyUI then
            patDoAutoBuyUI(setFullyStatus, {target, target, target})
        else
            -- fallback langsung fire remote
            if patStorePurchase then
                local buyItems = {"Gelatin","Sugar Block Bag","Water"}
                for _, itemName in ipairs(buyItems) do
                    setFullyStatus("Beli "..itemName.." ×"..target, C.accent2)
                    for i = 1, target do
                        pcall(function() patStorePurchase:FireServer(itemName, 1) end)
                        task.wait(0.4)
                    end
                end
            end
        ...
        if fullySavedPos then
            setFullyStatus("Teleport ke Apart...", C.purple)
            patFullyTeleport(fullySavedPos)
        end
        if not fullyRunning then break end

        setFullyStatus("Mulai masak "..target.." MS...", C.accent)
        AutoMS_Running = true
        local cooked = 0
        while fullyRunning and patHasAllIngredients() do
            local ok = pcall(patDoOneCook)
        ...
        setFullyStatus(cooked.." MS selesai! Siap jual...", C.accent)
        task.wait(0.2)

        setFullyStatus("Teleport ke NPC untuk jual...", C.accent)
        patFullyTeleport(NPC_MS_POS)
        if not fullyRunning then break end

        setFullyStatus("Jual semua MS...", C.accent)
        patDoAutoSell(setFullyStatus)
        if not fullyRunning then break end
        task.wait(0.2)

        setFullyStatus("Loop berikutnya...", C.accent2)
        task.wait(0.2)
    end
