-- ========== MAJESTY STORE v8.3.0 + PatstoreMS Engine ==========
...
-- ================================================================
-- PATSTORE ENGINE — STATE (dideklarasi di sini agar UI bisa akses)
-- ================================================================
local CFG = {
...
-- ================================================================
-- PATSTORE ENGINE — UTILITY FUNCTIONS
-- ================================================================
...
-- ================================================================
-- PATSTORE ENGINE — AUTO SELL
-- ================================================================
local function doAutoSell(setStatus2)
    local msTotal = countAllMS()
    if msTotal == 0 then
        setStatus2("Tidak ada MS", Color3.fromRGB(160,160,180)); return
    end
    ...
    while countAllMS() > 0 do
        ...
        task.wait(0.2); pressE(); task.wait(0.3)
        firePromptNearby(CFG.SELL_RADIUS); task.wait(0.3); pressE()
        ...
        while elapsed < CFG.SELL_TIMEOUT do
            ...
            if diff > 0 then
                sold += diff; totalSold += diff; terjual = true; failStreak = 0; break
            end
        ...
        if terjual then
            ...
            if failStreak >= maxFail then
                setStatus2("Gagal. Dekati NPC!", Color3.fromRGB(210,40,40)); break
            end
        end
    end
    unequipAll()
    if sold > 0 then
        ...
        setStatus2("Tidak ada MS terjual. Dekati NPC!", Color3.fromRGB(255,155,35))
    end
    task.wait(1)
end

-- ================================================================
-- PATSTORE ENGINE — AUTO BUY (StorePurchase:FireServer)
-- ================================================================
...
-- ================================================================
-- PATSTORE ENGINE — AUTO COOK (doOneCook)
-- ================================================================
...
local function doOneCook()
    ...
    repeat bag = LocalPlayer.Backpack:FindFirstChild(CFG.ITEM_EMPTY); task.wait(0.3); t2 += 0.3 until bag or t2 > 10
...
-- ================================================================
-- PATSTORE ENGINE — VEHICLE TELEPORT
-- ================================================================
...
local function fullyTeleport(targetPos)
    ...
    if seatPart then
        ...
        if vehicle then moveVehicle(vehicle, targetPos); task.wait(0.5) end
    else task.wait(0.5) end
end

-- ================================================================
-- PATSTORE ENGINE — AUTO FULLY
-- ================================================================

-- State untuk safe escape saat fullyRunning
local fullySafeEscaping  = false  -- true saat lagi kabur ke safe spot
local fullyResumeSignal  = false  -- diset true ketika aman & boleh lanjut masak
local fullyPausedIngr    = false  -- true = lagi nunggu aman, bahan sudah ada tinggal masak

-- Radius deteksi musuh di sekitar apart (studs)
local FULLY_ENEMY_RADIUS = 40

-- Cek apakah ada player musuh (non-whitelist) dalam radius dari posisi apart
local function isEnemyNearApart(apartPos, radius)
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and not isWhitelisted(plr) then
            ...
            if hrp and hum and hum.Health > 0 then
                if (hrp.Position - apartPos).Magnitude <= radius then
...
-- Tunggu sampai tidak ada musuh di sekitar apart, update status setiap detik
local function waitUntilApartSafe(apartPos, setFullyStatus)
    local waited = 0
    while fullyRunning do
        local enemy, name = isEnemyNearApart(apartPos, FULLY_ENEMY_RADIUS)
        if not enemy then
            -- Tambah jeda 2 detik konfirmasi supaya tidak langsung balik
            task.wait(2)
            enemy, name = isEnemyNearApart(apartPos, FULLY_ENEMY_RADIUS)
            if not enemy then return true end
        end
        waited += 1
        setFullyStatus(
            string.format("⚠ Musuh dekat Apart (%s)! Nunggu aman... %ds", name or "?", waited),
...
local function doAutoFully(setFullyStatus)
    ...
    fullyResumeSignal  = false
    fullyPausedIngr    = false

    -- anchor kendaraan agar tidak melayang saat teleport
    ...
    while fullyRunning do
        ...
        local apartPos  = fullySavedPos  -- sudah selalu ada (auto-set dari UI)
        ...
        fullyTeleport(apartPos)
        if not fullyRunning then break end

        -- ── CEK MUSUH SEBELUM MASAK ───────────────────────────────
        local enemy, ename = isEnemyNearApart(apartPos, FULLY_ENEMY_RADIUS)
        if enemy then
            setFullyStatus("Musuh dekat Apart! TP Safe dulu...", Color3.fromRGB(255,80,80))
            tpToSafe()
            task.wait(0.5)
            local safe = waitUntilApartSafe(apartPos, setFullyStatus)
            if not safe or not fullyRunning then break end
            -- Kembali ke apart
            setFullyStatus("Aman! Kembali ke Apart...", Color3.fromRGB(148,80,255))
            fullyTeleport(apartPos)
            if not fullyRunning then break end
        end
        ...
        AutoMS_Running = true; isRunning = true
        local cooked = 0

        while fullyRunning and hasAllIngredients() do
            -- Cek safe mode escape: jika fullySafeEscaping = true,
            -- itu berarti triggerSafeEscape sudah handle TP ke safe,
            -- kita hentikan masak, tunggu aman, lalu kembali
            if fullySafeEscaping then
                AutoMS_Running = false; isRunning = false
                setFullyStatus("Kena tembak! Nunggu aman di Safe...", Color3.fromRGB(255,60,60))
                -- Tunggu sampai safeModeActive selesai (TP selesai) dulu
                local waitEscape = 0
                while safeModeActive and waitEscape < 5 do
                    task.wait(0.3); waitEscape += 0.3
                end
                -- Sekarang tunggu musuh menjauh dari apart
                local safe2 = waitUntilApartSafe(apartPos, setFullyStatus)
                if not safe2 or not fullyRunning then
                    fullySafeEscaping = false; break
                end
                fullySafeEscaping = false
                -- Kembali ke apart & lanjut masak
                setFullyStatus("Aman! Kembali masak...", Color3.fromRGB(148,80,255))
                fullyTeleport(apartPos)
                if not fullyRunning then break end
                task.wait(0.5)
                AutoMS_Running = true; isRunning = true
                setFullyStatus("Lanjut masak "..target.." MS...", Color3.fromRGB(82,130,255))
            end
...
versionLabel.BackgroundTransparency = 1; versionLabel.Text = "v8.3.0 | PatMS Engine"
...
do
    ...
    makeLabel(ic,"PatstoreMS Engine  |  PageUp = toggle",10,0,400,28,10,C.subtext)
...
do
    ...
    local tpLocs = {
        ...
        {name="Apart 1 (Kompor)",      x=902.308837890625,   y=10.105066299438477, z=39.217220306396484},
        {name="Apart 2 (Kompor)",      x=930.7492065429688,  y=10.105064392089844, z=39.23077392578125},
        {name="Apart 3 (Kompor)",      x=989.796875,         y=10.105064392089844, z=248.2229461669922},
        {name="Apart 4 (Kompor)",      x=989.8067626953125,  y=10.105064392089844, z=219.7691192626953},
        {name="Apart 5 (Kompor)",      x=1146.9276123046875, y=10.105062484741211, z=451.07061767578125},
        {name="Apart 6 (Kompor)",      x=1146.8072509765625, y=10.105064392089844, z=422.4674987792969},
    }
...
do
    ...
    local kompors = {
        {name="Kompor Apart 1", x=902.308837890625,   y=10.105066299438477, z=39.217220306396484},
        {name="Kompor Apart 2", x=930.7492065429688,  y=10.105064392089844, z=39.23077392578125},
        {name="Kompor Apart 3", x=989.796875,         y=10.105064392089844, z=248.2229461669922},
        {name="Kompor Apart 4", x=989.8067626953125,  y=10.105064392089844, z=219.7691192626953},
        {name="Kompor Apart 5", x=1146.9276123046875, y=10.105062484741211, z=451.07061767578125},
        {name="Kompor Apart 6", x=1146.8072509765625, y=10.105064392089844, z=422.4674987792969},
    }
...
do
    ...
    local afApartList = {
        {name="Apart 1", x=902.308837890625,   y=10.105066299438477, z=39.217220306396484},
        {name="Apart 2", x=930.7492065429688,  y=10.105064392089844, z=39.23077392578125},
        {name="Apart 3", x=989.796875,         y=10.105064392089844, z=248.2229461669922},
        {name="Apart 4", x=989.8067626953125,  y=10.105064392089844, z=219.7691192626953},
        {name="Apart 5", x=1146.9276123046875, y=10.105062484741211, z=451.07061767578125},
        {name="Apart 6", x=1146.8072509765625, y=10.105064392089844, z=422.4674987792969},
    }
    ...
    afStopBtn.MouseButton1Click:Connect(function()
        fullyRunning = false; AutoMS_Running = false; isRunning = false; fullySafeEscaping = false
    ...
    task.spawn(function()
        while screenGui.Parent do
            if fullyRunning then
                if fullySafeEscaping then
                    afSafeLbl.Text = "🔴 KABUR KE SAFE!"; afSafeLbl.TextColor3 = Color3.fromRGB(255,60,60)
...
-- BOTTOM NAV
-- ================================================================
local tabDefs={{label="AUTO MS",page=pageAuto},{label="GENERAL",page=pageEsp},{label="TP",page=pageTP},{label="VTP",page=pageVehicleTP},{label="AutoFully",page=pageAutoFully},{label="AIMBOT",page=pageAimbot},{label="CREDITS",page=pageCredits}}
local tabBtns={}
...
-- SAFE MODE RUNTIME
-- ================================================================
local SAFE_X,SAFE_Y,SAFE_Z = 120.85433197021484, 4.297231197357178, -587.6337280273438
...
local function triggerSafeEscape(newHP)
    ...
    if fullyRunning then
        -- Sinyal ke doAutoFully agar pause masak
        fullySafeEscaping = true
        AutoMS_Running    = false
    ...
    task.spawn(function()
        ...
        if not fullyRunning then
            ...
            if safeModeStatusLbl then safeModeStatusLbl.Text = "NUNGGU AMAN..."; safeModeStatusLbl.TextColor3 = Color3.fromRGB(255,200,50) end
        end
    end)
end
...
print("=== MAJESTY STORE v8.3.0 + PatstoreMS Engine LOADED ===")
