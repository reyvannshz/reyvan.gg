...
local function doTeleport(targetCFrame)
    task.spawn(function()
        ...
        -- Hitung jumlah waypoint berdasarkan jarak
        -- Tiap waypoint max 60 studs, minimal 1 waypoint
        local numWP = math.max(1, math.ceil(totalDist / 60))

        for wp = 1, numWP do
            -- Re-fetch char tiap waypoint karena bisa respawn
            char = LocalPlayer.Character
            if not char then return end
            hrp  = char:FindFirstChild("HumanoidRootPart")
            if not hrp then return end

            -- Posisi target waypoint ini
            local t       = wp / numWP
            ...
            -- ── PAKSA posisi via RenderStepped selama 0.4 detik ──
            -- Server reset terjadi tiap ~0.1s, kita override tiap frame (~0.016s)
            local lockDone   = false
            local lockTarget = targetWP
            local conn = RunService.RenderStepped:Connect(function()
                if lockDone then return end
                local c2 = LocalPlayer.Character
                local h2 = c2 and c2:FindFirstChild("HumanoidRootPart")
                if not h2 then return end
                h2.CFrame = lockTarget
                pcall(function() h2.AssemblyLinearVelocity  = Vector3.zero end)
                pcall(function() h2.AssemblyAngularVelocity = Vector3.zero end)
            end)

            -- Tahan lock selama 0.4 detik per waypoint
            task.wait(0.4)
            lockDone = true
            conn:Disconnect()

            -- Jeda kecil antar waypoint biar server "menerima" posisi baru
            if not isLast then
                task.wait(0.15)
            end
        end

        -- Final landing: lock 1 detik di posisi tujuan akhir
        char = LocalPlayer.Character
        hrp  = char and char:FindFirstChild("HumanoidRootPart")
        if not hrp then return end

        local finalDone = false
        local finalConn = RunService.RenderStepped:Connect(function()
            if finalDone then return end
            local c2 = LocalPlayer.Character
            local h2 = c2 and c2:FindFirstChild("HumanoidRootPart")
            if not h2 then return end
...
local function safeTeleport(targetCFrame, statusLabel)
    ...
    -- Hitung estimasi waktu TP selesai
    local dist = 0
    pcall(function()
    ...
    local estTime  = numWP * 0.55 + 1.2   -- 0.55s per WP + 1.2s final lock
