...
-- ================================================================
-- AUTO COOK ENGINE (doOneCook) — sistem PatstoreMS
-- ================================================================
...
local function countdown(secs, phaseTxt, color)
    for i = secs, 1, -1 do
        if not AutoMS_Running and not isRunning then return false end
        _setStatusRunning()
        _setPhase(phaseTxt)
        _setTimer(i.."s")
        if statusValue then statusValue.TextColor3 = color or Color3.fromRGB(0,255,136) end
        task.wait(1)
    end
    return true
end
...
local function doAutoFully(setFullyStatus)
    ...
    while fullyRunning do
        ...
        -- ── MASAK ─────────────────────────────────────────────────
        -- Unequip, bersihkan queue, beri jeda settle
        unequipAll()
        ...
        -- Nyalakan Auto MS via autoMSLoop (sistem PatstoreMS)
        setFullyStatus("Mulai masak "..target.." MS...", Color3.fromRGB(82,130,255))
        AutoMS_Running = true
        isRunning      = true
        local cookDone = false
        task.spawn(function()
            autoMSLoop()
            cookDone = true
        end)

        -- Tunggu masak selesai, sambil pantau safe mode & fullyRunning
        while not cookDone do
            if not fullyRunning then
                -- Stop dipencet: matikan auto ms
                AutoMS_Running = false
                isRunning      = false
                -- Tunggu autoMSLoop selesai
                local wt = 0
                while not cookDone and wt < 10 do task.wait(0.3); wt += 0.3 end
                break
            end

            if fullySafeEscaping then
                -- Kena tembak: hentikan masak sementara
                AutoMS_Running = false; isRunning = false
                setFullyStatus("Kena tembak! Kabur ke Safe...", Color3.fromRGB(255,60,60))
                -- Tunggu TP safe selesai
                local we = 0
                while safeModeActive and we < 5 do task.wait(0.3); we += 0.3 end
                -- Tunggu musuh menjauh (max 6 menit)
                local safeWaited = 0
                ...
                while fullyRunning and safeWaited < MAX_SAFE_WAIT do
                    ...
                    safeWaited += 1
                    setFullyStatus(
                        string.format("Musuh ada (%s)! Nunggu... %ds / %ds", ename2 or "?", safeWaited, MAX_SAFE_WAIT),
                        Color3.fromRGB(255, 80, 80)
                    )
                    task.wait(1)
                end
        ...
        AutoMS_Running = false; isRunning = false
        fullySafeEscaping = false
        local cooked = totalMS()  -- pakai totalMS() sebagai indikator
        if not fullyRunning then break end
