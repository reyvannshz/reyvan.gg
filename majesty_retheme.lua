...
-- ===== BOTTOM NAV BAR (valary.gg pill icon style) =====
local tabDefs = {
...
-- Inner container untuk pill-pills (rata tengah)
local navInner = Instance.new("Frame")
navInner.Size = UDim2.new(1, 0, 1, 0)
navInner.BackgroundTransparency = 1
navInner.Parent = bottomNav

local function setTab(idx)
    for i, btn in ipairs(tabBtns) do
        local isActive = (i == idx)
        local pill   = btn.pill
        local iconL  = btn.iconL
        local labelL = btn.labelL
        if pill then
        ...
        if iconL then
            iconL.TextColor3 = isActive and Color3.fromRGB(255, 255, 255) or C.subtext
        end
        if labelL then
            labelL.Visible = isActive
        end
    end
...
-- Hitung lebar setiap slot nav
local slotW = 1 / #tabDefs
for i, td in ipairs(tabDefs) do
    -- Setiap slot adalah frame transparan
    local slot = Instance.new("Frame")
    slot.Size = UDim2.new(slotW, 0, 1, 0)
    slot.Position = UDim2.new(slotW * (i-1), 0, 0, 0)
    slot.BackgroundTransparency = 1
    slot.Parent = navInner

    -- Pill background (merah saat aktif, transparan saat tidak)
    local pill = Instance.new("Frame")
    pill.Name = "pill"
    pill.Size = UDim2.new(0.85, 0, 0, 26)
    pill.Position = UDim2.new(0.075, 0, 0.5, -13)
    pill.BackgroundColor3 = C.navpill
    pill.BackgroundTransparency = 1  -- mulai transparan
    ...
    labelL.Name = "labelL"
    labelL.Size = UDim2.new(1, 0, 0, 10)
    labelL.Position = UDim2.new(0, 0, 1, 1)
