...
-- Countdown: tiap detik update phase+timer, berhenti jika AutoMS_Running DAN isRunning keduanya false
local function countdown(secs, phaseTxt, color)
    for i = secs, 1, -1 do
        if not AutoMS_Running and not isRunning and not fullyRunning then return false end
        _setStatusRunning()
        _setPhase(phaseTxt)
        _setTimer(i.."s")
...
local function doOneCook()
    ...
    if not AutoMS_Running and not isRunning and not fullyRunning then isBusy = false; return false end
...
local function doAutoFully(setFullyStatus)
    ...
    while fullyRunning do
        ...
        -- Loop masak langsung (seperti PatstoreMS) tanpa perantara autoMSLoop
        ...
        while fullyRunning and hasAllIngredients() do
            ...
            -- Set flag sebelum masak agar countdown tidak bail
            AutoMS_Running = true
            isRunning = true
            local ok = doOneCook()
            AutoMS_Running = false
