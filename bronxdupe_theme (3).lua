...
-- ========== TELEPORT AMAN ==========
-- Prinsip: JANGAN pakai PlatformStand, ChangeState, atau _tpBusy flag
-- Cukup: set CFrame + zero velocity, dalam task.spawn agar tidak blocking
...
local function doTeleport(targetCFrame)
    task.spawn(function()
        ...
        -- Set velocity zero dulu
        pcall(function() hrp.AssemblyLinearVelocity  = Vector3.zero end)
        pcall(function() hrp.AssemblyAngularVelocity = Vector3.zero end)

        if dist <= 150 then
            -- Dekat: langsung set, 2x biar confirmed
            ...
            -- Jauh: bagi jadi steps, masing2 ~100 studs
            local steps = math.ceil(dist / 100)
            for i = 1, steps do
                -- Cek ulang char masih ada
                ...
                -- Angkat 5 studs di setiap step tengah, 0 di step terakhir
                local yOffset = (i < steps) and 5 or 0
                hrp.CFrame = CFrame.new(wPos + Vector3.new(0, yOffset, 0))
                pcall(function() hrp.AssemblyLinearVelocity  = Vector3.zero end)
                pcall(function() hrp.AssemblyAngularVelocity = Vector3.zero end)
                task.wait(0.05)
            end
            -- Final: set 2x ke posisi tepat
            hrp.CFrame = targetCFrame
            task.wait(0.05)
            pcall(function() hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") end)
            if hrp then
                hrp.CFrame = targetCFrame
                pcall(function() hrp.AssemblyLinearVelocity = Vector3.zero end)
            end
...
local function safeTeleport(targetCFrame, statusLabel)
    ...
    -- Anti-mati: guard health 1 detik pakai loop ringan
    task.spawn(function()
        local char     = LocalPlayer.Character
        local humanoid = char and char:FindFirstChildOfClass("Humanoid")
        if humanoid then
            local maxHp = humanoid.MaxHealth
            for _ = 1, 20 do   -- cek tiap 0.05s selama ~1 detik
                task.wait(0.05)
                pcall(function()
                    if humanoid and humanoid.Parent and humanoid.Health < maxHp * 0.5 then
    ...
    -- Update status setelah perkiraan selesai
    if statusLabel then
        local dist = 0
        pcall(function()
        ...
        local waitTime = math.clamp(dist / 100 * 0.05 + 0.3, 0.3, 1.5)
