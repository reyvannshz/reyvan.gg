...
-- Gunakan GetMouseLocation() — tidak ada feedback loop dari mousemoverel
        ...
        if plr ~= LocalPlayer then
            ...
            if char then
                ...
                if part and hum and hum.Health > 0 then
                    -- WorldToViewportPoint: koordinat viewport (0,0 = pojok kiri atas)
                    local sp, vis = Camera:WorldToViewportPoint(part.Position)
                    if vis then
                        -- GetMouseLocation juga pakai viewport space → match!
...
-- FreeAim mode
-- GetMouseLocation() = viewport coords, WorldToViewportPoint = viewport coords
-- Keduanya sudah sama sistem koordinat → tidak ada Y offset bug
-- mousemoverel memindahkan mouse → GetMouseLocation otomatis update di frame berikutnya
-- Tidak ada feedback loop karena kita tidak pakai InputChanged
local function aimFreeAim(targetPart, targetPlayer)
    ...
    -- Sudah sangat dekat → stop, reset acc
...
RunService.RenderStepped:Connect(function()
    -- Update FOV circle posisi ikut mouse setiap frame
    local mpos = UserInputService:GetMouseLocation()
    aimbotFovCircle.Position = mpos
    aimbotFovCircle.Radius   = aimbotFOV
...
print("=== MAJESTY ONTOP v11.0 - VALARY THEME ===")
print("Fitur:")
