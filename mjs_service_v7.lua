...
-- Jual satu item: equip → satu kali tekan E ke penjual → tunggu konfirmasi
-- Sesuai mekanik game: cukup tekan E SEKALI per item saat sudah dipegang
local function trySellOne(msName, setStatus2)
    ...
    -- Tunggu server kenali item di tangan sebelum interact
    task.wait(0.5)

    setStatus2("Jual: "..msName.." (E)...", Color3.fromRGB(50,210,110))

    -- ── SATU KALI tekan E ke penjual ──
    pcall(function()
        VIM:SendKeyEvent(true,  Enum.KeyCode.E, false, game)
        task.wait(0.18)
        VIM:SendKeyEvent(false, Enum.KeyCode.E, false, game)
    ...
    -- Tunggu konfirmasi inventory berkurang (max SELL_TIMEOUT detik)
    local elapsed = 0
    local sold    = false
...
versionLabel.BackgroundTransparency = 1; versionLabel.Text = "v8.6.0"
...
local function triggerSafeEscape(newHP)
    ...
    -- ── Teleport instan, TANPA menghentikan loop ──
    -- Loop memasak/fully tetap aktif (isRunning/fullyRunning tidak diubah).
    -- countdown() akan pause sendiri karena safeModeActive = true.
    tpToSafe()

    -- ── Recovery goroutine ──
    task.spawn(function()
        task.wait(0.8)  -- stabilisasi setelah teleport
...
print("=== MAJESTY STORE v8.6.0 LOADED ===")
