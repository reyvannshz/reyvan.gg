...
-- ===== WARNA TEMA VALARY.GG (pixel-accurate) =====
local C = {
    bg       = Color3.fromRGB(10, 10, 10),      -- hitam hampir murni
    topbar   = Color3.fromRGB(13, 13, 13),      -- titlebar sedikit lebih terang
    panel    = Color3.fromRGB(11, 11, 11),      -- area konten
    card     = Color3.fromRGB(18, 18, 18),      -- card/row item
    card2    = Color3.fromRGB(15, 15, 15),      -- card lebih gelap
    ...
    text     = Color3.fromRGB(210, 210, 210),   -- putih agak redup
    subtext  = Color3.fromRGB(90, 90, 90),      -- abu-abu gelap seperti valary
    border   = Color3.fromRGB(28, 28, 28),      -- border sangat tipis gelap
    search   = Color3.fromRGB(16, 16, 16),      -- search bar bg
    navbg    = Color3.fromRGB(10, 10, 10),      -- bottom nav sama gelap
    navpill  = Color3.fromRGB(220, 38, 38),     -- pill aktif nav
    sectionI = Color3.fromRGB(180, 40, 40),     -- ikon section header
}
...
-- ===== TITLE BAR (valary.gg pixel-accurate) =====
...
-- Garis pemisah bawah titlebar (sangat tipis)
...
-- Title: "majesty.gg | MAJESTY ONTOP  !"  
-- "majesty.gg" merah bold, " | " abu, "MAJESTY ONTOP" putih, " !" merah
local titleLabel = Instance.new("TextLabel")
titleLabel.Size = UDim2.new(1, -110, 1, 0)
titleLabel.Position = UDim2.new(0, 10, 0, 0)
titleLabel.BackgroundTransparency = 1
titleLabel.RichText = true
titleLabel.Text = '<b><font color="rgb(220,38,38)">majesty.gg</font></b>'
    .. '<font color="rgb(55,55,55)">  |  </font>'
    .. '<font color="rgb(160,160,160)">MAJESTY ONTOP</font>'
    .. '  <font color="rgb(220,38,38)">!</font>'
...
versionLabel.Size = UDim2.new(0, 44, 1, 0)
versionLabel.Position = UDim2.new(1, -112, 0, 0)
...
minBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        ...
        TweenService:Create(mainFrame, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0,520,0,32)}):Play()
    else
        -- Restore: tween dulu sampai selesai, baru tampilkan konten
        TweenService:Create(mainFrame, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0,520,0,420)}):Play()
        task.spawn(function()
            task.wait(0.18)
            for _, v in pairs(mainFrame:GetChildren()) do
...
-- ===== SEARCH BAR (valary.gg style — compact, kiri atas) =====
local searchBar = Instance.new("Frame")
searchBar.Size = UDim2.new(0, 160, 0, 24)
searchBar.Position = UDim2.new(0, 8, 0, 38)
searchBar.BackgroundColor3 = C.search
searchBar.BorderSizePixel = 0
searchBar.Parent = mainFrame
mkCorner(searchBar, 4)
...
searchInput.PlaceholderColor3 = Color3.fromRGB(60, 60, 60)
searchInput.Text = ""
...
contentArea.Size = UDim2.new(1, 0, 1, -80)   -- 32 topbar + 8 gap + 24 search + 16 gap + 48 bottomnav ≈ 80
...
-- ===== HELPER: Section Title (valary.gg style) =====
local function sectionTitle(parent, text, yPos)
    -- Ikon lingkaran merah kecil di kiri (seperti valary.gg)
    local dot = Instance.new("Frame")
    dot.Size = UDim2.new(0, 6, 0, 6)
    dot.Position = UDim2.new(0, 10, 0, yPos + 8)
    ...
    lbl.Position = UDim2.new(0, 22, 0, yPos)
...
local function makeCard(parent, yPos, h)
    ...
    f.Size = UDim2.new(1, -16, 0, h or 40)
...
-- ===== DUAL NAV SYSTEM (valary.gg accurate) =====
-- NAV 1 (dalam window): tab ikon kecil abu-abu, aktif muncul label merah
-- NAV 2 (luar window floating): pill hitam rounded, aktif = ikon + nama tab
...
local activeTab = nil
local tabBtnsInner = {}   -- referensi inner nav (dalam window)
local tabBtnsOuter = {}   -- referensi outer nav (floating pill)

-- ─────────────────────────────────────────────────────────────
-- NAV DALAM WINDOW (baris ikon horizontal, tanpa background)
-- Persis screenshot: ikon kecil abu, yang aktif ada label teks merah di samping
-- ─────────────────────────────────────────────────────────────
local innerNavH = 36
local innerNav = Instance.new("Frame")
innerNav.Size = UDim2.new(1, 0, 0, innerNavH)
innerNav.Position = UDim2.new(0, 0, 1, -(innerNavH))
innerNav.BackgroundColor3 = Color3.fromRGB(8, 8, 8)
innerNav.BorderSizePixel = 0
innerNav.Parent = mainFrame

-- Garis tipis di atas nav dalam
local innerNavLine = Instance.new("Frame")
innerNavLine.Size = UDim2.new(1, 0, 0, 1)
innerNavLine.Position = UDim2.new(0, 0, 0, 0)
innerNavLine.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
innerNavLine.BorderSizePixel = 0
innerNavLine.Parent = innerNav

-- Update content area agar tidak melebihi nav dalam
contentArea.Size = UDim2.new(1, 0, 1, -(70 + innerNavH))

-- ─────────────────────────────────────────────────────────────
-- NAV LUAR (floating pill di bawah window) — PERSIS valary.gg
-- Pill hitam besar rounded, di luar mainFrame tapi dalam screenGui
-- ─────────────────────────────────────────────────────────────
local outerNavPill = Instance.new("Frame")
outerNavPill.Size = UDim2.new(0, 300, 0, 38)
outerNavPill.Position = UDim2.new(0.5, -150, 0, 0)  -- akan diposisikan via update
outerNavPill.BackgroundColor3 = Color3.fromRGB(14, 14, 14)
outerNavPill.BorderSizePixel = 0
outerNavPill.Parent = screenGui
mkCorner(outerNavPill, 19)
mkStroke(outerNavPill, 1, Color3.fromRGB(30, 30, 30))

-- Posisikan pill bawah mengikuti posisi mainFrame
local function updateOuterNavPos()
    local mfPos = mainFrame.AbsolutePosition
    local mfSiz = mainFrame.AbsoluteSize
    -- Pill di bawah mainFrame, tengah
    local pillW = outerNavPill.AbsoluteSize.X
    outerNavPill.Position = UDim2.new(0,
        mfPos.X + mfSiz.X/2 - 150,
        0,
        mfPos.Y + mfSiz.Y + 8
    )
end
...
-- ─────────────────────────────────────────────────────────────
-- setTab: update kedua nav sekaligus
-- ─────────────────────────────────────────────────────────────
local function setTab(idx)
    -- Update inner nav
    for i, refs in ipairs(tabBtnsInner) do
        local isActive = (i == idx)
        refs.iconL.TextColor3  = isActive and C.accent2 or Color3.fromRGB(55, 55, 55)
        refs.labelL.Visible    = isActive
    end
    -- Update outer nav
    for i, refs in ipairs(tabBtnsOuter) do
        local isActive = (i == idx)
        if isActive then
            TweenService:Create(refs.pill, TweenInfo.new(0.18, Enum.EasingStyle.Quad), {
                BackgroundColor3 = C.accent,
            }):Play()
            refs.iconL.TextColor3  = Color3.fromRGB(255, 255, 255)
            refs.labelL.Visible    = true
        else
            TweenService:Create(refs.pill, TweenInfo.new(0.18, Enum.EasingStyle.Quad), {
                BackgroundColor3 = Color3.fromRGB(14, 14, 14),
            }):Play()
            refs.iconL.TextColor3  = Color3.fromRGB(80, 80, 80)
            refs.labelL.Visible    = false
        end
    end
    -- Show correct page
...
-- ─────────────────────────────────────────────────────────────
-- Build INNER NAV (ikon kecil dalam window)
-- ─────────────────────────────────────────────────────────────
local innerSlotW = 1 / #tabDefs
for i, td in ipairs(tabDefs) do
    local slot = Instance.new("Frame")
    slot.Size = UDim2.new(innerSlotW, 0, 1, 0)
    slot.Position = UDim2.new(innerSlotW * (i-1), 0, 0, 0)
    slot.BackgroundTransparency = 1
    slot.Parent = innerNav

    -- Ikon
    local iconL = Instance.new("TextLabel")
    iconL.Size = UDim2.new(0, 20, 1, 0)
    iconL.Position = UDim2.new(0.5, -10, 0, 0)
    iconL.BackgroundTransparency = 1
    iconL.Text = td.icon
    iconL.TextColor3 = Color3.fromRGB(55, 55, 55)
    iconL.Font = Enum.Font.Gotham
    iconL.TextSize = 13
    iconL.Parent = slot

    -- Label merah (hanya muncul saat aktif, di kanan ikon)
    local labelL = Instance.new("TextLabel")
    labelL.Size = UDim2.new(0, 60, 1, 0)
    labelL.Position = UDim2.new(0.5, 6, 0, 0)
    labelL.BackgroundTransparency = 1
    labelL.Text = td.label
    labelL.TextColor3 = C.accent
    ...
    tabBtnsInner[i] = { iconL = iconL, labelL = labelL }
    local ci = i
    btn.MouseButton1Click:Connect(function() setTab(ci) end)
end

-- ─────────────────────────────────────────────────────────────
-- Build OUTER NAV (floating pill di bawah window)
-- Layout: slot ikon, yang aktif menjadi pill merah + nama
-- ─────────────────────────────────────────────────────────────
local outerSlotW = 1 / #tabDefs
for i, td in ipairs(tabDefs) do
    local slot = Instance.new("Frame")
    slot.Size = UDim2.new(outerSlotW, 0, 1, 0)
    slot.Position = UDim2.new(outerSlotW * (i-1), 0, 0, 0)
    slot.BackgroundTransparency = 1
    slot.Parent = outerNavPill

    -- Sub-pill per slot (aktif = merah, nonaktif = transparan)
    local subPill = Instance.new("Frame")
    subPill.Size = UDim2.new(0.9, 0, 0, 28)
    subPill.Position = UDim2.new(0.05, 0, 0.5, -14)
    subPill.BackgroundColor3 = Color3.fromRGB(14, 14, 14)  -- mulai sama bg
    ...
    iconL.BackgroundTransparency = 1
    iconL.Text = td.icon
    iconL.TextColor3 = Color3.fromRGB(80, 80, 80)
    iconL.Font = Enum.Font.Gotham
    iconL.TextSize = 14
    iconL.Parent = subPill

    -- Label (saat aktif muncul di sebelah kanan ikon)
    local labelL = Instance.new("TextLabel")
    labelL.Size = UDim2.new(1, -4, 1, 0)
    labelL.Position = UDim2.new(0, 2, 0, 0)
    labelL.BackgroundTransparency = 1
    labelL.Text = td.icon .. "  " .. td.label
    labelL.TextColor3 = Color3.fromRGB(255, 255, 255)
    labelL.Font = Enum.Font.GothamBold
    labelL.TextSize = 11
    labelL.Visible = false
    labelL.Parent = subPill
    ...
    tabBtnsOuter[i] = { pill = subPill, iconL = iconL, labelL = labelL }
    local ci = i
    btn.MouseButton1Click:Connect(function() setTab(ci) end)
end

-- Posisikan outer pill nav di bawah mainFrame
RunService.RenderStepped:Connect(function()
    local mfPos = mainFrame.AbsolutePosition
    local mfSiz = mainFrame.AbsoluteSize
    -- Hanya update kalau window tidak diminimize
    ...
    outerNavPill.Visible = not isMinimized
end)

-- Player info di kanan search bar (valary.gg style: nama player kecil)
local playerInfoLbl = Instance.new("TextLabel")
playerInfoLbl.Size = UDim2.new(0, 180, 0, 24)
playerInfoLbl.Position = UDim2.new(1, -188, 0, 38)
...
playerInfoLbl.TextSize = 10
playerInfoLbl.TextXAlignment = Enum.TextXAlignment.Right
