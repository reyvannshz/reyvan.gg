...
-- ========== KILL AURA CORE LOOP ==========
-- Metode gabungan: void teleport + health + TakeDamage + ChangeState + velocity
    ...
    while true do
        ...
        for _, plr in pairs(Players:GetPlayers()) do
            ...
            if dist <= killAuraRadius then
                local alreadyCounting = kaKilledThisCycle[plr]

                -- Metode 1: Health = 0 langsung
                pcall(function() hum.Health = 0 end)

                -- Metode 2: TakeDamage besar
                pcall(function() hum:TakeDamage(99999) end)

                -- Metode 3: ChangeState Dead
                pcall(function() hum:ChangeState(Enum.HumanoidStateType.Dead) end)

                -- Metode 4: Teleport ke void (paling reliable)
                pcall(function()
                    hrp.CFrame = CFrame.new(hrp.Position.X, -9999, hrp.Position.Z)
                end)

                -- Metode 5: Velocity ke bawah supaya terus jatuh
                pcall(function()
