...
do
    ...
    buyToggleBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            ...
            -- ══════════════════════════════════════════
            -- BAHAN: urutan Water → Sugar Block → Gelatin
            -- itemText = Text pada TextLabel Name='Item'
            -- di dalam PurchaseableItem (TextButton)
            -- ══════════════════════════════════════════
            local BAHAN = {
                { label = "Water",       toolKw = "water",   itemText = "Water",           emoji = "💧" },
                { label = "Sugar Block", toolKw = "sugar",   itemText = "Sugar Block Bag", emoji = "🧊" },
                { label = "Gelatin",     toolKw = "gelatin", itemText = "Gelatin",          emoji = "🍮" },
            }
            ...
            -- ─────────────────────────────────────────
            -- Tekan E pakai keypress (tidak ganggu mouse)
            -- ─────────────────────────────────────────
            ...
            -- ─────────────────────────────────────────
            -- Fire ProximityPrompt NPC terdekat
            -- ─────────────────────────────────────────
            local function firePP()
                local chr = LocalPlayer.Character
                local hrp = chr and chr:FindFirstChild("HumanoidRootPart")
                if not hrp then pressE(); return end
                ...
                for _, obj in pairs(Workspace:GetDescendants()) do
                    if obj:IsA("ProximityPrompt") then
                        ...
                        if part and part:IsA("BasePart") then
                            ...
                            if d < bestD and d < 20 then bestD = d; best = obj end
                        end
                    end
                end
                if best then
                    pcall(function() fireproximityprompt(best) end)
                    print("[AutoBuy] fireproximityprompt OK")
                else
                    pressE()
                end
                task.wait(0.3)
            ...
            -- ─────────────────────────────────────────
            -- Tunggu DialogueUI.FirstChoice muncul & fire
            -- GUI='DialogueUI' Name='FirstChoice'
            -- ─────────────────────────────────────────
            local function clickFirstChoice(timeout)
                local t = 0
                while t < (timeout or 4) do
                    if not autoBuy_Running then return false end
                    local dlg = LocalPlayer.PlayerGui:FindFirstChild("DialogueUI")
                    if dlg then
                        local fc = dlg:FindFirstChild("FirstChoice", true)
                        if fc then
                            print("[AutoBuy] Fire FirstChoice: '" .. (fc.Text or "") .. "'")
                            -- HANYA fireclick — sudah terbukti work dari debug
                            pcall(function() fireclick(fc) end)
                            task.wait(0.1)
                            pcall(function() fc.MouseButton1Click:Fire() end)
                            return true
                        end
                    end
                    task.wait(0.1)
                ...
                print("[AutoBuy] FirstChoice tidak muncul dalam " .. (timeout or 4) .. "s")
                return false
            end

            -- ─────────────────────────────────────────
            -- Tunggu Shop GUI terbuka lalu cari
            -- PurchaseableItem yang itemText-nya cocok
            -- PATH: PlayerGui > Shop > Main > ScrollingFrame > PurchaseableItem (TextButton)
            --   child: TextLabel Name='Item' Text='Water'/'Sugar Block Bag'/'Gelatin'
            -- ─────────────────────────────────────────
            local function buyFromShop(itemText, timeout)
                local t = 0
                    ...
                    if shopGui then
                        ...
                        if sf then
                            for _, item in pairs(sf:GetChildren()) do
                                -- PurchaseableItem adalah TextButton
                                if item:IsA("TextButton") and item.Name == "PurchaseableItem" then
                                    -- Cek TextLabel Name='Item' di dalamnya
                                    local lbl = item:FindFirstChild("Item")
                                    if lbl and lbl:IsA("TextLabel") and lbl.Text == itemText then
                                        print("[AutoBuy] fireclick PurchaseableItem: '" .. itemText .. "'")
                                        pcall(function() fireclick(item) end)
                                        task.wait(0.1)
                                        pcall(function() item.MouseButton1Click:Fire() end)
                                        return true
                                    end
                                end
                            end
                ...
                print("[AutoBuy] PurchaseableItem '" .. itemText .. "' tidak ditemukan")
                return false
            end

            -- ══════════════════════════════════════════
            -- BELI SATU BAHAN (max 5 percobaan)
            -- ══════════════════════════════════════════
            local function beliBahan(bahan)
                ...
                for attempt = 1, 5 do
                    ...
                    buyStatusLbl.Text = "Percobaan " .. attempt .. "/5"
                    buyStatusLbl.TextColor3 = C.yellow
                    print("[AutoBuy] Percobaan " .. attempt .. "/5: " .. bahan.label)

                    -- STEP 1: Buka dialog NPC
                    buyStatusLbl.Text = "Buka dialog..."
                    firePP()
                    task.wait(0.4)

                    -- STEP 2: Klik FirstChoice
                    buyStatusLbl.Text = "Klik dialog..."
                    local ok = clickFirstChoice(3)
                    if ok then
                        task.wait(0.6)
                    else
                        -- Coba sekali lagi
                    ...
                    -- STEP 3: Klik PurchaseableItem di Shop
                    buyStatusLbl.Text = "Beli " .. bahan.label .. "..."
                    local bought = buyFromShop(bahan.itemText, 4)
                    ...
                    -- Tutup shop sebelum retry
                    pcall(function()
                        local shopGui = LocalPlayer.PlayerGui:FindFirstChild("Shop")
                        if shopGui then
                            local exitBtn = shopGui:FindFirstChild("Exit", true)
                            if exitBtn then
                                fireclick(exitBtn)
                                task.wait(0.3)
                            end
                        end
                        -- Atau klik SecondChoice (Nevermind)
                        local dlg = LocalPlayer.PlayerGui:FindFirstChild("DialogueUI")
                        if dlg then
                            local sc = dlg:FindFirstChild("SecondChoice", true)
                            if sc then fireclick(sc) task.wait(0.3) end
                        end
                    end)
            ...
            -- ══════════════════════════════════════════
            -- MAIN: Water → Sugar Block Bag → Gelatin
            -- ══════════════════════════════════════════
            print("========================================")
