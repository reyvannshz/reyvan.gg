...
-- FreeAim: track mouse position untuk FOV circle & no-flick
local mousePos           = Vector2.new(0, 0)
local lastMousePos       = Vector2.new(0, 0)

-- FreeAim smooth accumulator (prevent flick)
local freeAimAccX        = 0
local freeAimAccY        = 0
...
minBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        mainFrame:TweenSize(UDim2.new(0,480,0,36),"Out","Quad",0.18,true)
        ...
        mainFrame:TweenSize(UDim2.new(0,480,0,390),"Out","Quad",0.18,true)
    end
end)
...
do
    ...
    mkRowLabel(modeRow, "Aimbot Mode")
    mkPairBtn(modeRow, "📷 Camera", "🖱️ FreeAim", 1, function(which)
        aimbotMode = which == 1 and "Camera" or "FreeAim"
    end)
    y = y + 40

    -- TARGET
    local targetRow = mkRow(pageAimbot, y, 34)
    mkRowLabel(targetRow, "Target Part")
    mkPairBtn(targetRow, "💀 Head", "🧍 Body", 1, function(which)
        aimbotTarget = which == 1 and "Head" or "HumanoidRootPart"
    end)
    y = y + 40
    ...
    UserInputService.InputBegan:Connect(function(input, gpe)
        if isBindingKey then
            ...
            elseif input.KeyCode == Enum.KeyCode.ButtonX
                or input.UserInputType == Enum.UserInputType.MouseButton1 then
                -- skip
                isBindingKey = true
                return
            elseif input.UserInputType == Enum.UserInputType.Keyboard then
                -- Cek apakah MB4 / MB5 via KeyCode (beberapa executor map MB4/MB5 ke KeyCode)
                local kName = tostring(input.KeyCode):gsub("Enum.KeyCode.", "")
                if kName == "MouseButton4" then
                    aimbotKeybindType  = "MB4"
                    aimbotKeybindIsKey = false
                    aimbotKeybindCode  = nil
                    aimbotKeybindLabel = "MB4"
                elseif kName == "MouseButton5" then
    ...
    pageAimbot.CanvasSize = UDim2.new(0, 0, 0, y + 10)
end
...
-- ========== AIMBOT FOV CIRCLE (mengikuti mouse/crosshair) ==========
...
RunService.RenderStepped:Connect(function()
    -- Circle selalu di posisi mouse (crosshair)
...
-- Velocity cache untuk prediction
local function getPredictedPosition(part, player)
    ...
    local timeAhead = predStrength * 0.5
    return currentPos + cache.lastVel * timeAhead
end

-- Cari target dalam FOV relatif ke posisi mouse
    ...
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            ...
            if char then
                ...
                -- Ambil part yang dipilih user (Head atau HumanoidRootPart)
...
-- Camera mode: putar kamera ke target
local function aimCamera(targetPart, targetPlayer)
    if not targetPart then return end
    local predPos = getPredictedPosition(targetPart, targetPlayer)
    local cf      = Camera.CFrame
    local dir     = (predPos - cf.Position).Unit
    local goal    = CFrame.new(cf.Position, cf.Position + dir)
    local t       = math.clamp(1 - (aimbotSmooth / 20), 0.05, 1)
    Camera.CFrame = cf:Lerp(goal, t)
end

-- FreeAim mode: gerak mouse ke target di layar
-- Fix atas/bawah: gunakan WorldToScreenPoint bukan WorldToViewportPoint
-- WorldToScreenPoint sudah memperhitungkan inset GUI sehingga koordinat Y akurat
local function aimFreeAim(targetPart, targetPlayer)
    if not targetPart then return end

    local predPos      = getPredictedPosition(targetPart, targetPlayer)

    -- WorldToScreenPoint: koordinat match dengan posisi mouse di layar penuh
    local sp, vis, _   = Camera:WorldToScreenPoint(predPos)
    if not vis then
        freeAimAccX = 0
        freeAimAccY = 0
        return
    ...
    -- Delta dari posisi mouse sekarang ke posisi target di layar
    local dx = sp.X - mousePos.X
    local dy = sp.Y - mousePos.Y

    -- Jika sudah sangat dekat, berhenti (prevent micro-drift)
    if math.abs(dx) < 0.8 and math.abs(dy) < 0.8 then
        freeAimAccX = 0
        freeAimAccY = 0
        return
    end

    -- Speed factor (smooth): 1=instant, 20=lambat
    local speed = math.clamp(1 - (aimbotSmooth - 1) / 20, 0.04, 1.0)

    -- Lerp accumulator menuju delta target — ini yg bikin smooth & no-flick
    freeAimAccX = freeAimAccX + (dx - freeAimAccX) * speed
    freeAimAccY = freeAimAccY + (dy - freeAimAccY) * speed

    -- Clamp max step per frame agar tidak loncat jauh
    local maxStep   = 60
    local moveX     = math.clamp(freeAimAccX, -maxStep, maxStep)
    local moveY     = math.clamp(freeAimAccY, -maxStep, maxStep)

    pcall(function() mousemoverel(moveX, moveY) end)
end

-- ========== KEYBIND HOLD CHECK (support MB4/MB5) ==========
-- MB4/MB5 di Roblox terdeteksi via KeyCode (beberapa executor) atau via
-- UserInputService InputBegan dengan custom input type.
-- Kita track state manual karena IsMouseButtonPressed tidak cover MB4/MB5.
local mb4Held = false
local mb5Held = false
...
UserInputService.InputBegan:Connect(function(input)
    if isBindingKey then return end
    local kName = tostring(input.KeyCode):gsub("Enum.KeyCode.", "")
    if kName == "MouseButton4" or input.UserInputType.Name == "MouseButton4" then
        mb4Held = true
    elseif kName == "MouseButton5" or input.UserInputType.Name == "MouseButton5" then
        mb5Held = true
    end
end)
...
local function isAimbotKeyHeld()
    ...
    if aimbotKeybindType == "KeyCode" then
        return aimbotKeybindCode and UserInputService:IsKeyDown(aimbotKeybindCode)
    elseif aimbotKeybindType == "MouseButton" then
    ...
    elseif aimbotKeybindType == "MB4" then
        return mb4Held
    elseif aimbotKeybindType == "MB5" then
        return mb5Held
    end
    return false
end
...
RunService.RenderStepped:Connect(function()
    ...
    -- Reset accumulator di frame pertama aktif (prevent first-frame flick)
    ...
    local target, targetPlayer = getBestTarget()
    if not target then return end

    if aimbotMode == "Camera" then
        aimCamera(target, targetPlayer)
    elseif aimbotMode == "FreeAim" then
        aimFreeAim(target, targetPlayer)
    end
end)
print("=== MAJESTY ONTOP v10.0 - VALARY THEME ===")
print("Fitur:")
print("- 📦 Inventory Tracker")
print("- ⚙️  Auto MS")
print("- 👁️  ESP + Item Dipegang")
print("- 🚀 Teleport")
print("- 🎯 AIMBOT: FreeAim no-flick | FOV ikut mouse | Prediction")
print("========================================")
