...
-- ===== PAGE 5: KALKULATOR =====
do
    sectionTitle(pageCalc, "🧮 KALKULATOR BAHAN", 8)

    -- Harga default per item
    local hargaWater   = 50
    local hargaSugar   = 120
    local hargaGelatin = 200
    local qtyWater     = 0
    local qtySugar     = 0
    local qtyGelatin   = 0

    -- Label hasil
    local subWaterLbl   = nil
    local subSugarLbl   = nil
    local subGelatinLbl = nil
    local totalLbl      = nil
    ...
    local function recalc()
        local sw = hargaWater   * qtyWater
        local ss = hargaSugar   * qtySugar
        local sg = hargaGelatin * qtyGelatin
        local total = sw + ss + sg
        if subWaterLbl   then subWaterLbl.Text   = formatRupiah(sw)    end
        if subSugarLbl   then subSugarLbl.Text   = formatRupiah(ss)    end
        if subGelatinLbl then subGelatinLbl.Text = formatRupiah(sg)    end
        if totalLbl      then totalLbl.Text       = "TOTAL  " .. formatRupiah(total) end
    end

    -- Helper: buat kartu item lengkap (harga input + qty control)
    local function makeItemCard(yPos, icon, label, colorBar, defaultHarga, onHargaChanged, onQtyChanged)
        local card = makeCard(pageCalc, yPos, 90)

        -- Garis warna kiri
        local bar = Instance.new("Frame")
        bar.Size = UDim2.new(0, 3, 1, -10)
        bar.Position = UDim2.new(0, 4, 0, 5)
        bar.BackgroundColor3 = colorBar
        bar.BorderSizePixel = 0
        bar.Parent = card
        mkCorner(bar, 3)
        ...
        priceLbl.Size = UDim2.new(0, 60, 0, 18)
        priceLbl.Position = UDim2.new(0, 14, 0, 44)
        priceLbl.BackgroundTransparency = 1
        priceLbl.Text = "HARGA/unit"
        priceLbl.TextColor3 = C.subtext
        ...
        priceBox.Size = UDim2.new(0, 70, 0, 22)
        priceBox.Position = UDim2.new(0, 80, 0, 40)
        priceBox.BackgroundColor3 = C.panel
        priceBox.BorderSizePixel = 0
        priceBox.Text = tostring(defaultHarga)
        priceBox.TextColor3 = C.yellow
        priceBox.Font = Enum.Font.GothamBold
        priceBox.TextSize = 12
        priceBox.PlaceholderText = "0"
        priceBox.Parent = card
        mkCorner(priceBox, 5)
        mkStroke(priceBox, 1, C.border)

        priceBox.FocusLost:Connect(function()
            local v = tonumber(priceBox.Text) or 0
            priceBox.Text = tostring(math.max(0, v))
            onHargaChanged(math.max(0, v))
            recalc()
        end)

        -- QTY label
        local qtyLbl = Instance.new("TextLabel")
        qtyLbl.Size = UDim2.new(0, 30, 0, 18)
        qtyLbl.Position = UDim2.new(0, 14, 0, 68)
        qtyLbl.BackgroundTransparency = 1
        qtyLbl.Text = "QTY"
        qtyLbl.TextColor3 = C.subtext
        qtyLbl.Font = Enum.Font.Gotham
        qtyLbl.TextSize = 9
        qtyLbl.TextXAlignment = Enum.TextXAlignment.Left
        qtyLbl.Parent = card

        -- QTY display
        local qtyDisp = Instance.new("TextLabel")
        qtyDisp.Size = UDim2.new(0, 36, 0, 24)
        qtyDisp.Position = UDim2.new(0, 80, 0, 62)
        qtyDisp.BackgroundColor3 = C.bg
        qtyDisp.BorderSizePixel = 0
        qtyDisp.Text = "0"
        qtyDisp.TextColor3 = C.text
        qtyDisp.Font = Enum.Font.GothamBold
        qtyDisp.TextSize = 13
        qtyDisp.Parent = card
        mkCorner(qtyDisp, 4)
        mkStroke(qtyDisp, 1, C.border)

        -- Minus button
        local minusBtn = Instance.new("TextButton")
        minusBtn.Size = UDim2.new(0, 24, 0, 24)
        minusBtn.Position = UDim2.new(0, 54, 0, 62)
        ...
        plusBtn.Position = UDim2.new(0, 120, 0, 62)
        ...
        minusBtn.MouseButton1Click:Connect(function()
            ...
            qtyDisp.Text = tostring(currentQty)
            onQtyChanged(currentQty)
            recalc()
        end)
        ...
        subLbl.Size = UDim2.new(0, 90, 1, 0)
        subLbl.Position = UDim2.new(1, -100, 0, 0)
        subLbl.BackgroundTransparency = 1
        subLbl.Text = "$0"
        subLbl.TextColor3 = colorBar
    ...
    -- Build 3 item cards
    subWaterLbl = makeItemCard(38,  "💧", "WATER",       Color3.fromRGB(56,189,248),  hargaWater,
        function(v) hargaWater = v end, function(v) qtyWater = v end)

    subSugarLbl = makeItemCard(138, "🧊", "SUGAR BLOCK", Color3.fromRGB(192,132,252), hargaSugar,
        function(v) hargaSugar = v end, function(v) qtySugar = v end)

    subGelatinLbl = makeItemCard(238, "🍮", "GELATIN",   Color3.fromRGB(251,146,60),  hargaGelatin,
        function(v) hargaGelatin = v end, function(v) qtyGelatin = v end)

    -- Separator line
    local sepLine = Instance.new("Frame")
    sepLine.Size = UDim2.new(1, -20, 0, 1)
    sepLine.Position = UDim2.new(0, 10, 0, 342)
    sepLine.BackgroundColor3 = C.border
    sepLine.BorderSizePixel = 0
    sepLine.Parent = pageCalc

    -- Total card
    local totalCard = Instance.new("Frame")
    totalCard.Size = UDim2.new(1, -20, 0, 50)
    totalCard.Position = UDim2.new(0, 10, 0, 352)
    totalCard.BackgroundColor3 = Color3.fromRGB(20, 20, 40)
    totalCard.BorderSizePixel = 0
    totalCard.Parent = pageCalc
    mkCorner(totalCard, 8)
    mkStroke(totalCard, 1, C.accent)

    -- Gradient accent di total
    local totalGrad = Instance.new("UIGradient")
    totalGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(25, 25, 55)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(15, 15, 30)),
    })
    totalGrad.Rotation = 135
    totalGrad.Parent = totalCard
    ...
    totalLbl.TextColor3 = C.accent
    totalLbl.Font = Enum.Font.GothamBlack
    totalLbl.TextSize = 18
    totalLbl.TextXAlignment = Enum.TextXAlignment.Center
    totalLbl.Parent = totalCard

    -- Reset button
    local resetBtn = Instance.new("TextButton")
    resetBtn.Size = UDim2.new(1, -20, 0, 36)
    resetBtn.Position = UDim2.new(0, 10, 0, 412)
    ...
    resetBtn.Parent = pageCalc
    mkCorner(resetBtn, 6)
    mkStroke(resetBtn, 1, C.red)
    ...
    pageCalc.CanvasSize = UDim2.new(0, 0, 0, 460)
    recalc()
end
...
local tabDefs = {
    ...
    {icon="🧮", label="Kalkulator",page=pageCalc},
}
