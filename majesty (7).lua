...
-- ========== FUNGSI TELEPORT SAFE (ANTI-KICK) ==========
-- Gerak bertahap dalam beberapa langkah kecil
-- supaya server anti-cheat tidak deteksi lompatan posisi tiba-tiba
local TP_STEPS    = 8      -- jumlah langkah intermediate
local TP_DELAY    = 0.18   -- jeda antar langkah (detik) — jangan terlalu cepat
...
local function doTeleport(targetCFrame)
    ...
    -- Gerak pelan-pelan dari posisi sekarang ke tujuan
    humanoid:ChangeState(Enum.HumanoidStateType.Physics)

    for i = 1, TP_STEPS do
        if not char or not char:FindFirstChild("HumanoidRootPart") then break end
        local alpha = i / TP_STEPS
        -- Interpolasi linear posisi & rotasi
        local lerpedCF = startCFrame:Lerp(targetCFrame, alpha)
        ...
        locBtn.MouseButton1Click:Connect(function()
            ...
            task.spawn(function()
                ...
                if ok then
                    ...
                    tpStatusValue.Text = "ERROR: No Char"
