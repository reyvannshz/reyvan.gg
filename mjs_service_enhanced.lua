...
-- SAFE MODE VARIABLES (ENHANCED)
local safeMode          = false
local safeModeActive    = false
local lastHealth        = 100
local safeModeStatusLbl = nil
local safeCoords        = nil  -- Koordinat safe yang bisa diatur
local isEscaping        = false
local escapeStartTime   = 0
local enemyCheckRadius  = 60   -- Radius cek musuh
local returnDelay       = 5    -- Delay sebelum balik ke apartemen

-- ENEMY DETECTION VARIABLES
local nearbyEnemies     = {}
local enemyDistanceThreshold = 50  -- Threshold jarak musuh dianggap dekat
local lastEnemyCheck    = 0
local enemyCheckInterval = 0.5

-- COOKING DETECTION VARIABLES
local uncookedFoodCache = {}
local stoveDetectionRadius = 15
local lastStoveCheck    = 0
local stoveCheckInterval = 1
...
-- ================================================================
-- ENEMY DETECTION SYSTEM (ESP + DISTANCE)
-- ================================================================
local function getNearbyEnemies(radius, position)
    local enemies = {}
    local myChar = LocalPlayer.Character
    local myPos = position or (myChar and myChar:FindFirstChild("HumanoidRootPart") and myChar.HumanoidRootPart.Position)
...
local function getClosestEnemy()
    local enemies = getNearbyEnemies(enemyDistanceThreshold)
    return enemies[1]
end

local function isEnemyNearby(radius, position)
    local enemies = getNearbyEnemies(radius or enemyDistanceThreshold, position)
    return #enemies > 0, enemies
end

-- ================================================================
-- STOVE/COOKING DETECTION SYSTEM
-- ================================================================
local function findStoves()
    local stoves = {}
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") or obj:IsA("Model") then
            local name = obj.Name:lower()
            if name:find("stove") or name:find("kompor") or name:find("cook") or name:find("pot") then
...
local function checkUncookedFood()
    local uncooked = {}
    local stoves = findStoves()
    local myChar = LocalPlayer.Character
    local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
    if not myRoot then return uncooked end
    
    for _, stove in ipairs(stoves) do
        local stovePos = stove:IsA("BasePart") and stove.Position or (stove.PrimaryPart and stove.PrimaryPart.Position)
        if stovePos then
            local dist = (stovePos - myRoot.Position).Magnitude
            if dist <= stoveDetectionRadius then
                -- Cek apakah ada masakan yang belum selesai
                local hasUncooked = false
                local status = "Unknown"
                
                -- Cek dari RPC queue atau UI
                for _, item in ipairs(rpcQueue) do
                    if item.type == "wait_boil" or item.type == "wait_cook" then
                        hasUncooked = true
                        status = "Cooking in progress"
                    end
                end
                
                -- Cek inventory untuk bahan yang belum digunakan
                local waterCount = countItem(CFG.ITEM_WATER)
                local sugarCount = countItem(CFG.ITEM_SUGAR)
                local gelCount = countItem(CFG.ITEM_GEL)
                
                if waterCount > 0 or sugarCount > 0 or gelCount > 0 then
                    hasUncooked = true
                    status = "Ingredients available"
                end
                
                if hasUncooked then
                    table.insert(uncooked, {
                        stove = stove,
                        position = stovePos,
                        distance = dist,
                        status = status,
...
local function doAutoSell(setStatus2)
    ...
    while countAllMS() > 0 do
        ...
        if not ok then
            fail += 1
            setStatus2("Gagal equip ("..fail.."/"..maxFail..")", Color3.fromRGB(210,40,40))
            task.wait(1)
        ...
        if terjual then
            ...
            fail += 1
            setStatus2("Tidak terjual, retry ("..fail.."/"..maxFail..")", Color3.fromRGB(255,155,35))
            task.wait(0.8)
            if fail >= maxFail then
                setStatus2("Gagal jual. Pastikan dekat NPC!", Color3.fromRGB(210,40,40))
                break
            end
        end
    end
...
local function doAutoBuy(setStatus2, overrideQty)
    ...
    for idx, item in ipairs(BUY_ITEMS) do
        ...
        if gained < qty then
            ...
            setStatus2(item.display.." x"..gained.." selesai!", Color3.fromRGB(80,220,130))
        end
        task.wait(0.2)
    end
...
-- Koordinat safe default (bisa diubah)
local SAFE_COORDS = {
    {name = "Safe Spot 1", x = -534.245971679687, y = 14.356728553771973, z = 189.90879821777344},
    {name = "Safe Spot 2", x = 120.85433197021484, y = 4.297231197357178, z = -587.6337280273438},
    {name = "GS Ujung", x = -464.5489501953125, y = 3.7371325492858887, z = 335.3158874511719},
}
local selectedSafeSpot = 1

local function getSafePosition()
    local spot = SAFE_COORDS[selectedSafeSpot]
    return Vector3.new(spot.x, spot.y, spot.z)
end
...
local function tpToSafe()
    ...
    if seat then
        ...
        if vModel and vModel.PrimaryPart then
            ...
            vModel:SetPrimaryPartCFrame(CFrame.new(safePos + Vector3.new(0, 2, 0)) * seatOff:Inverse())
            return
        end
    end
...
-- Fungsi untuk menunggu sampai tidak ada musuh dan balik ke apartemen
local function waitAndReturnToApartment(setStatus)
    if not fullySavedPos then return end
    
    local startWait = tick()
    setStatus("Menunggu musuh pergi...", Color3.fromRGB(255, 200, 50))
    
    while true do
        -- Cek apakah masih ada musuh di sekitar safe spot
        local hasEnemy, enemies = isEnemyNearby(FULLY_ENEMY_RADIUS, getSafePosition())
        
        if not hasEnemy then
            -- Tidak ada musuh, cek delay
            if tick() - startWait >= returnDelay then
                setStatus("Aman! Balik ke apartemen...", Color3.fromRGB(50, 210, 110))
                task.wait(0.5)
                fullyTeleport(fullySavedPos)
                setStatus("Sudah di apartemen", Color3.fromRGB(50, 210, 110))
                return true
            end
        else
            -- Masih ada musuh, reset timer
            startWait = tick()
            local closest = enemies[1]
            setStatus("Musuh terdeteksi! ("..math.floor(closest.distance).."m) - Menunggu...", Color3.fromRGB(255, 60, 60))
        end
        
        task.wait(0.5)
        
        -- Timeout setelah 60 detik
        if tick() - startWait > 60 then
            setStatus("Timeout! Balik ke apartemen...", Color3.fromRGB(255, 155, 35))
            fullyTeleport(fullySavedPos)
            return false
        end
    end
...
-- Fungsi untuk cek masakan yang belum selesai
local function checkAndCompleteCooking(setStatus)
    local uncooked = checkUncookedFood()
    
    if #uncooked > 0 then
        setStatus("Ada "..#uncooked.." masakan belum selesai!", Color3.fromRGB(255, 200, 50))
        
        for _, item in ipairs(uncooked) do
            setStatus("Menyelesaikan: "..item.status, Color3.fromRGB(100, 180, 255))
            
            -- Coba masukkan bahan yang tersedia
            if item.ingredients.water > 0 then
                cookInteract(CFG.ITEM_WATER)
                task.wait(0.5)
            end
            if item.ingredients.sugar > 0 then
                cookInteract(CFG.ITEM_SUGAR)
                task.wait(0.5)
            end
            if item.ingredients.gelatin > 0 then
                cookInteract(CFG.ITEM_GEL)
                task.wait(0.5)
            end
        end
...
local function doAutoFully(setFullyStatus)
    ...
    while fullyRunning do
        ...
        -- Cek apakah sedang dalam mode escape
        if isEscaping then
            setFullyStatus("Mode aman aktif - Menunggu...", Color3.fromRGB(255, 60, 60))
            waitAndReturnToApartment(setFullyStatus)
            isEscaping = false
        end

        -- Cek masakan yang belum selesai sebelum mulai loop baru
        if fullySavedPos then
            local myChar = LocalPlayer.Character
            local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
            if myRoot then
                local distToApart = (myRoot.Position - fullySavedPos).Magnitude
                if distToApart < 30 then
                    checkAndCompleteCooking(setFullyStatus)
                end
            end
        end
        ...
        -- Cek musuh sebelum mulai masak
        local hasEnemy, enemies = isEnemyNearby(FULLY_ENEMY_RADIUS)
        if hasEnemy then
            local closest = enemies[1]
            setFullyStatus("Musuh terdeteksi ("..math.floor(closest.distance).."m)! Kabur...", Color3.fromRGB(255, 60, 60))
            isEscaping = true
            tpToSafe()
            continue
        end
        ...
        while fullyRunning and hasAllIngredients() do
            -- Cek musuh setiap beberapa detik saat masak
            if tick() - lastEnemyCheck > enemyCheckInterval then
                lastEnemyCheck = tick()
                local enemyNear, enemyList = isEnemyNearby(FULLY_ENEMY_RADIUS)
                if enemyNear then
                    local closest = enemyList[1]
                    setFullyStatus("Musuh mendekat ("..math.floor(closest.distance).."m)! Kabur...", Color3.fromRGB(255, 60, 60))
                    isRunning = false
                    isEscaping = true
                    tpToSafe()
                    break
        ...
        -- Jika sedang escape, skip ke iterasi berikutnya
...
versionLabel.BackgroundTransparency = 1; versionLabel.Text = "v8.4.0"
...
do
    ...
    -- SAFE MODE (ENHANCED)
    sectionTitle(pageAuto,"SAFE MODE (ENHANCED)",522)
    ...
    makeLabel(smInfoCard,"Hit = TP Safe → Cek ESP musuh → Balik Apartemen",10,2,440,20,9,C.subtext)
    makeLabel(smInfoCard,"Auto deteksi masakan belum selesai",10,22,440,20,9,Color3.fromRGB(80,180,80))

    -- Pilih Safe Spot
    local safeSpotCard = makeCard(pageAuto,698,44)
    makeLabel(safeSpotCard,"Safe Spot",12,0,80,44,10,C.subtext)
    local safeSpotLbl = makeLabel(safeSpotCard,SAFE_COORDS[1].name,90,0,200,44,11,C.accent)
    
    local safePrevBtn = Instance.new("TextButton")
    safePrevBtn.Size = UDim2.new(0,26,0,22); safePrevBtn.Position = UDim2.new(1,-62,0.5,-11)
    safePrevBtn.BackgroundColor3 = C.card2; safePrevBtn.Text = "◀"; safePrevBtn.TextColor3 = C.text
    safePrevBtn.Font = Enum.Font.GothamBold; safePrevBtn.TextSize = 11; safePrevBtn.BorderSizePixel = 0
    safePrevBtn.Parent = safeSpotCard; mkCorner(safePrevBtn, 5)
    
    local safeNextBtn = Instance.new("TextButton")
    safeNextBtn.Size = UDim2.new(0,26,0,22); safeNextBtn.Position = UDim2.new(1,-30,0.5,-11)
    safeNextBtn.BackgroundColor3 = C.card2; safeNextBtn.Text = "▶"; safeNextBtn.TextColor3 = C.text
    safeNextBtn.Font = Enum.Font.GothamBold; safeNextBtn.TextSize = 11; safeNextBtn.BorderSizePixel = 0
    safeNextBtn.Parent = safeSpotCard; mkCorner(safeNextBtn, 5)
    
    local function updateSafeSpotDisplay()
        safeSpotLbl.Text = SAFE_COORDS[selectedSafeSpot].name
    end
    
    safePrevBtn.MouseButton1Click:Connect(function()
        selectedSafeSpot = selectedSafeSpot > 1 and selectedSafeSpot - 1 or #SAFE_COORDS
        updateSafeSpotDisplay()
    end)
    safeNextBtn.MouseButton1Click:Connect(function()
        selectedSafeSpot = selectedSafeSpot < #SAFE_COORDS and selectedSafeSpot + 1 or 1
        updateSafeSpotDisplay()
    end)
    ...
    safeTogBtn.MouseButton1Click:Connect(function()
        ...
        if safeMode then
            ...
            safeMode = false; safeModeActive = false; isEscaping = false
    ...
    -- AUTO SELL toggle
    sectionTitle(pageAuto,"AUTO SELL",750)
    local sellTogBtn = Instance.new("TextButton")
    sellTogBtn.Size = UDim2.new(1,-20,0,36); sellTogBtn.Position = UDim2.new(0,10,0,776)
    ...
    local ssc = makeCard(pageAuto,820,44); makeLabel(ssc,"SELL",12,0,80,44,10,C.subtext)
    sellStatusLbl_ref = makeLabel(ssc,"OFF",90,0,200,44,13,C.red)
    local sic = makeCard(pageAuto,872,44); makeLabel(sic,"ITEM",12,0,80,44,10,C.subtext)
    ...
    -- BUY BAHAN
    sectionTitle(pageAuto,"BUY BAHAN",918)
    ...
    makePlusMinusRow(944,  "Water",           function() return buyFullWater   end, function(v) buyFullWater   = v end)
    makePlusMinusRow(994,  "Gelatin",         function() return buyFullGelatin end, function(v) buyFullGelatin = v end)
    makePlusMinusRow(1044, "Sugar Block Bag", function() return buyFullSugar   end, function(v) buyFullSugar   = v end)

    local buyTogBtn=Instance.new("TextButton"); buyTogBtn.Size=UDim2.new(1,-20,0,36); buyTogBtn.Position=UDim2.new(0,10,0,1098)
    ...
    local bsc=makeCard(pageAuto,1142,44); makeLabel(bsc,"STATUS",12,0,80,44,10,C.subtext)
    local bsl=makeLabel(bsc,"OFF",90,0,180,44,13,C.red)
    local bpc=makeCard(pageAuto,1194,44); makeLabel(bpc,"ITEM",12,0,80,44,10,C.subtext)
    ...
    pageAuto.CanvasSize=UDim2.new(0,0,0,1250)
end

-- ================================================================
-- ESP (ENHANCED WITH DISTANCE)
-- ================================================================
local function createESP(player)
    ...
    local warnL = Drawing.new("Text");  warnL.Size=11; warnL.Font=2; warnL.Color=Color3.fromRGB(255,60,60); warnL.Outline=true; warnL.OutlineColor=Color3.fromRGB(0,0,0); warnL.Center=true; warnL.Visible=false; warnL.Text="⚠ ENEMY"
    espCache[player] = {box, nameL, hpBg, hpFl, dL, iL, warnL}
end
...
do
    ...
    sectionTitle(pageEsp,"ESP (ENHANCED)",144)
    ...
    makeLabel(eir,"Box | Username | HP Bar | Item Held | Distance | Warning",10,0,400,24,10,C.subtext)
    ...
    -- Enemy Distance Settings
    sectionTitle(pageEsp,"ENEMY DETECTION",248)
    local enemyDistCard = makeCard(pageEsp,274,44)
    makeLabel(enemyDistCard,"Warning Distance",10,0,150,44,11,C.text)
    local enemyDistLbl = makeLabel(enemyDistCard,tostring(enemyDistanceThreshold).."m",0,0,0,44,13,C.yellow,Enum.Font.GothamBold,Enum.TextXAlignment.Center)
    enemyDistLbl.Size = UDim2.new(0,60,1,0); enemyDistLbl.Position = UDim2.new(0.5,-30,0,0)
    
    local edMinus = Instance.new("TextButton")
    edMinus.Size = UDim2.new(0,30,0,28); edMinus.Position = UDim2.new(0.5,-30-36,0.5,-14)
    edMinus.BackgroundColor3 = Color3.fromRGB(40,15,15); edMinus.Text = "-"; edMinus.TextColor3 = C.red
    edMinus.Font = Enum.Font.GothamBold; edMinus.TextSize = 16; edMinus.Parent = enemyDistCard
    mkCorner(edMinus,5)
    
    local edPlus = Instance.new("TextButton")
    edPlus.Size = UDim2.new(0,30,0,28); edPlus.Position = UDim2.new(0.5,30+6,0.5,-14)
    edPlus.BackgroundColor3 = Color3.fromRGB(0,40,20); edPlus.Text = "+"; edPlus.TextColor3 = C.accent
    edPlus.Font = Enum.Font.GothamBold; edPlus.TextSize = 16; edPlus.Parent = enemyDistCard
    mkCorner(edPlus,5)
    
    edMinus.MouseButton1Click:Connect(function()
        enemyDistanceThreshold = math.max(10, enemyDistanceThreshold - 5)
        enemyDistLbl.Text = tostring(enemyDistanceThreshold).."m"
    end)
    edPlus.MouseButton1Click:Connect(function()
        enemyDistanceThreshold = math.min(200, enemyDistanceThreshold + 5)
        enemyDistLbl.Text = tostring(enemyDistanceThreshold).."m"
    end)
    
    local enemyInfo = makeCard(pageEsp,324,22)
    makeLabel(enemyInfo,"Musuh dalam radius = tanda ⚠ merah muncul",10,0,400,22,9,C.subtext)

    sectionTitle(pageEsp,"WHITELIST",356)
    local wlScroll=Instance.new("ScrollingFrame"); wlScroll.Size=UDim2.new(1,-20,0,80); wlScroll.Position=UDim2.new(0,10,0,382)
    ...
    refreshWL()
    local svrScroll=Instance.new("ScrollingFrame"); svrScroll.Size=UDim2.new(1,-20,0,100); svrScroll.Position=UDim2.new(0,10,0,472)
    ...
    local rfBtn=Instance.new("TextButton"); rfBtn.Size=UDim2.new(0.5,-14,0,30); rfBtn.Position=UDim2.new(0,10,0,580); rfBtn.BackgroundTransparency=1; rfBtn.Text="Refresh"; rfBtn.TextColor3=C.subtext; rfBtn.Font=Enum.Font.Gotham; rfBtn.TextSize=11; rfBtn.BorderSizePixel=0; rfBtn.Parent=pageEsp; rfBtn.MouseButton1Click:Connect(refreshSvr)
    local clBtn=Instance.new("TextButton"); clBtn.Size=UDim2.new(0.5,-14,0,30); clBtn.Position=UDim2.new(0.5,4,0,580); clBtn.BackgroundTransparency=1; clBtn.Text="Clear All"; clBtn.TextColor3=C.subtext; clBtn.Font=Enum.Font.Gotham; clBtn.TextSize=11; clBtn.BorderSizePixel=0; clBtn.Parent=pageEsp
    clBtn.MouseButton1Click:Connect(function() whitelist={}; refreshWL(); refreshSvr() end)
    
    sectionTitle(pageEsp,"VEHICLE FLY",622)
    local vftb=Instance.new("TextButton"); vftb.Size=UDim2.new(1,-20,0,34); vftb.Position=UDim2.new(0,10,0,648)
    ...
    local vfsc=makeCard(pageEsp,690,34); makeLabel(vfsc,"STATUS",12,0,80,34,10,C.subtext)
    local vfsl=makeLabel(vfsc,"Tidak di kendaraan",90,0,260,34,11,C.subtext)
    local vfSpCard=makeCard(pageEsp,732,44)
    ...
    local vfic=makeCard(pageEsp,784,40); makeLabel(vfic,"E = Naik | Q = Turun | WASD = Steer",10,4,380,16,10,C.subtext)
    ...
    pageEsp.CanvasSize=UDim2.new(0,0,0,840)
end
...
-- ================================================================
-- PAGE: VEHICLE TP + AUTO FULLY (ENHANCED)
-- ================================================================
do
...
-- ================================================================
-- PAGE: AUTO FULLY MS (ENHANCED)
-- ================================================================
do
    local fullyStatusLbl_af
    local fullyCoordLbl_af
    local fullyTargetLbl_af
    local enemyStatusLbl_af
    local cookingStatusLbl_af
    ...
    sectionTitle(pageAutoFully,"AUTO FULLY MS (ENHANCED)",8)
    local afInfoCard = makeCard(pageAutoFully,38,60)
    makeLabel(afInfoCard,"Loop: Beli bahan → Masak di Apart → Jual → Ulangi",10,2,400,18,9,C.subtext)
    makeLabel(afInfoCard,"Safe Mode: Hit = TP Safe → ESP Cek Musuh → Balik",10,22,400,18,9,C.accent)
    makeLabel(afInfoCard,"Auto deteksi & lanjutkan masakan yang belum selesai",10,42,400,18,9,Color3.fromRGB(80,180,80))
    ...
    -- Pilih Apart
    local afApartCard = makeCard(pageAutoFully,106,34)
    ...
    -- Target stepper
    local afTargetCard = makeCard(pageAutoFully,148,44)
    ...
    -- Status
    local afStatusCard = makeCard(pageAutoFully,200,30)
    fullyStatusLbl_af  = makeLabel(afStatusCard,"Belum dimulai",10,0,400,30,10,C.subtext)

    -- Enemy Detection Status
    local afEnemyCard = makeCard(pageAutoFully,238,30)
    makeLabel(afEnemyCard,"Musuh Terdekat",10,0,120,30,10,C.subtext)
    enemyStatusLbl_af = makeLabel(afEnemyCard,"Tidak ada",130,0,280,30,10,C.accent)
    
    -- Cooking Detection Status
    local afCookingCard = makeCard(pageAutoFully,276,30)
    makeLabel(afCookingCard,"Masakan",10,0,80,30,10,C.subtext)
    cookingStatusLbl_af = makeLabel(afCookingCard,"-",90,0,320,30,10,C.subtext)

    -- Update status musuh dan masakan
    task.spawn(function()
        while screenGui.Parent do
            if fullyRunning then
                -- Cek musuh terdekat
                local hasEnemy, enemies = isEnemyNearby(FULLY_ENEMY_RADIUS)
                if hasEnemy and enemies[1] then
                    enemyStatusLbl_af.Text = enemies[1].player.Name.." ("..math.floor(enemies[1].distance).."m)"
                    enemyStatusLbl_af.TextColor3 = C.red
                else
                    enemyStatusLbl_af.Text = "Tidak ada musuh"
                    enemyStatusLbl_af.TextColor3 = C.accent
                end
                
                -- Cek masakan
                local uncooked = checkUncookedFood()
                if #uncooked > 0 then
                    cookingStatusLbl_af.Text = #uncooked.." kompor belum selesai"
                    cookingStatusLbl_af.TextColor3 = C.yellow
                else
                    cookingStatusLbl_af.Text = "Tidak ada"
                    cookingStatusLbl_af.TextColor3 = C.subtext
                end
            end
            task.wait(0.5)
        end
    ...
    afStartBtn.Size=UDim2.new(0.48,-12,0,34); afStartBtn.Position=UDim2.new(0,10,0,316)
    ...
    afStopBtn.Size=UDim2.new(0.48,-12,0,34); afStopBtn.Position=UDim2.new(0.5,2,0,316)
    ...
    afStopBtn.MouseButton1Click:Connect(function()
        fullyRunning = false; isRunning = false; AutoMS_Running = false; isEscaping = false
    ...
    local afSafeModeCard = makeCard(pageAutoFully,356,44)
    ...
    local afRadiusCard = makeCard(pageAutoFully,408,44)
    ...
    local afRadiusInfo = makeCard(pageAutoFully,460,22)
    makeLabel(afRadiusInfo,"Musuh dalam radius ini = kabur dulu sebelum masak",10,0,430,22,9,C.subtext)
    
    -- Return Delay Setting
    local returnDelayCard = makeCard(pageAutoFully,490,44)
    makeLabel(returnDelayCard,"Delay Balik Apart",10,0,150,44,11,C.text)
    local returnDelayLbl = makeLabel(returnDelayCard,tostring(returnDelay).."s",0,0,0,44,13,C.yellow,Enum.Font.GothamBold,Enum.TextXAlignment.Center)
    returnDelayLbl.Size=UDim2.new(0,50,1,0); returnDelayLbl.Position=UDim2.new(0.5,-25,0,0)
    local rdMinus = Instance.new("TextButton")
    rdMinus.Size=UDim2.new(0,30,0,28); rdMinus.Position=UDim2.new(0.5,-25-36,0.5,-14)
    rdMinus.BackgroundColor3=Color3.fromRGB(40,15,15); rdMinus.Text="-"; rdMinus.TextColor3=C.red
    rdMinus.Font=Enum.Font.GothamBold; rdMinus.TextSize=16; rdMinus.Parent=returnDelayCard
    mkCorner(rdMinus,5)
    local rdPlus = Instance.new("TextButton")
    rdPlus.Size=UDim2.new(0,30,0,28); rdPlus.Position=UDim2.new(0.5,25+6,0.5,-14)
    rdPlus.BackgroundColor3=Color3.fromRGB(0,40,20); rdPlus.Text="+"; rdPlus.TextColor3=C.accent
    rdPlus.Font=Enum.Font.GothamBold; rdPlus.TextSize=16; rdPlus.Parent=returnDelayCard
    mkCorner(rdPlus,5)
    rdMinus.MouseButton1Click:Connect(function()
        returnDelay = math.max(1, returnDelay - 1)
        returnDelayLbl.Text = tostring(returnDelay).."s"
    end)
    rdPlus.MouseButton1Click:Connect(function()
        returnDelay = math.min(30, returnDelay + 1)
        returnDelayLbl.Text = tostring(returnDelay).."s"
    end)

    pageAutoFully.CanvasSize=UDim2.new(0,0,0,550)
end
...
do
    ...
    local creditData={{role="Investor & Owner",name="Hiro",color=Color3.fromRGB(255,215,0),initials="HI"},{role="Developer",name="V7x & Reyvan",color=Color3.fromRGB(100,180,255),initials="V7"},{role="Enhanced Features",name="AI Assistant",color=Color3.fromRGB(0,255,136),initials="AI"}}
    ...
    pageCredits.CanvasSize=UDim2.new(0,0,0,260)
end
...
local function triggerSafeEscape(newHP)
    ...
    -- Stop masak sementara
    if isRunning or fullyRunning then
        isRunning = false
        AutoMS_Running = false
        _setStatus("SAFE!", Color3.fromRGB(255,60,60))
        if phaseValue then phaseValue.Text = "Kabur..." end
    end
    
    -- Set escaping mode untuk Auto Fully
    if fullyRunning then
        isEscaping = true
    end
    ...
    task.spawn(function()
        task.wait(0.5)
        if safeModeStatusLbl then safeModeStatusLbl.Text = "DI SAFE SPOT - ESP AKTIF"; safeModeStatusLbl.TextColor3 = C.accent end
        
        -- Tunggu sampai aman dan balik ke apartemen jika Auto Fully
        if fullyRunning and fullySavedPos then
            waitAndReturnToApartment(function(msg, col)
                if safeModeStatusLbl then
                    safeModeStatusLbl.Text = msg
                    safeModeStatusLbl.TextColor3 = col or C.accent
                end
            end)
        end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    for player, drawings in pairs(espCache) do
        ...
        local hpFl=drawings[4]; local dL=drawings[5]; local iL=drawings[6]; local warnL=drawings[7]
        local function hideAll()
            box.Visible=false; nameL.Visible=false; hpBg.Visible=false
            hpFl.Visible=false; dL.Visible=false; warnL.Visible=false
        ...
        if not valid then hideAll()
            ...
            if myPos and espMaxDist>0 and dist3D>espMaxDist then hideAll()
                ...
                if not(hrpVis and headVis) then hideAll()
                    ...
                    -- Warning indicator for nearby enemies
                    if dist3D <= enemyDistanceThreshold then
                        warnL.Position = Vector2.new(hrpPos.X, boxY - 28)
                        warnL.Visible = true
                    else
                        warnL.Visible = false
                    end
...
print("=== MAJESTY STORE v8.4.0 ENHANCED LOADED ===")
print("Features: Enhanced Safe Mode + ESP Enemy Detection + Uncooked Food Detection")
