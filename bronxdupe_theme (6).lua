...
-- Hook CharacterAdded: saat karakter spawn, langsung set posisi
-- Ini 100% work karena posisi diset SEBELUM server bisa reset
local function onCharacterAdded(newChar)
    if pendingTP then
        local target = pendingTP
        pendingTP    = nil
        task.spawn(function()
            -- Tunggu semua part karakter siap
            local hrp = newChar:WaitForChild("HumanoidRootPart", 10)
            local hum = newChar:WaitForChild("Humanoid", 10)
            if not hrp or not hum then return end
            -- Tunggu physics siap (1 frame)
            RunService.Stepped:Wait()
            -- Anchor dulu, set posisi, baru lepas
            hrp.Anchored = true
            hrp.CFrame   = target
            task.wait(0.1)
            hrp.Anchored = false
...
-- ========== TELEPORT 100% WORK ==========
-- Strategi berlapis: coba semua metode dari yang paling simple sampai LoadCharacter
...
local function doTeleport(targetCFrame)
    task.spawn(function()
        ...
        -- ══ METODE A: Jarak dekat ≤ 100 studs ══
        -- Anchor + set posisi + unanchor (paling simple, paling clean)
        if dist <= 100 then
            hrp.Anchored = true
            ...
            task.wait(0.15)
            hrp.CFrame   = targetCFrame   -- set 2x konfirmasi
            task.wait(0.05)
            hrp.Anchored = false
            return
        end

        -- ══ METODE B: Jarak jauh > 100 studs ══
        -- Anchor SEMUA part → set posisi → unanchor
        -- Ini cegah server reset karena physics dikunci total

        -- Step 1: kumpulkan semua BasePart karakter
        local parts = {}
        ...
        -- Step 3: zero semua velocity
        pcall(function() hrp.AssemblyLinearVelocity  = Vector3.zero end)
        pcall(function() hrp.AssemblyAngularVelocity = Vector3.zero end)

        -- Step 4: pindah bertahap dengan anchor aktif
        ...
        for i = 1, steps do
            ...
            hrp.CFrame = CFrame.new(wPos)
            task.wait(0.05)
        end

        -- Step 5: set tepat di tujuan
        hrp.CFrame = targetCFrame
        task.wait(0.2)
        hrp.CFrame = targetCFrame  -- konfirmasi 2x

        -- Step 6: unanchor semua (karakter sudah ada di tujuan)
        for _, p in pairs(parts) do
            pcall(function()
        ...
        -- Step 7: zero velocity setelah unanchor supaya tidak terpelanting
        task.wait(0.05)
        pcall(function() hrp.AssemblyLinearVelocity  = Vector3.zero end)
        pcall(function() hrp.AssemblyAngularVelocity = Vector3.zero end)
    end)
...
local function safeTeleport(targetCFrame, statusLabel)
    ...
    -- Estimasi waktu: dekat ~0.3s, jauh ~steps*0.05 + 0.5s
    local steps   = math.max(1, math.ceil(dist / 80))
    local estTime = dist <= 100 and 0.4 or (steps * 0.05 + 0.5)
