...
do
    ...
    makeLabel(espInfo, "📦  Inventory player (tanpa peluru)", 10, 58, 280, 18, 11, Color3.fromRGB(255,230,100), Enum.Font.Gotham)
...
local function createESP(player)
    ...
    -- Fungsi scan inventory player lain (kecuali peluru/ammo)
    local BULLET_KEYWORDS = {"bullet", "ammo", "peluru", "ammunition", "shell", "round", "cartridge", "magazine", "mag", "9mm", "ar", "pistol ammo", "rifle ammo", "shotgun ammo"}
    local function isBullet(name)
        local low = string.lower(name)
        for _, kw in ipairs(BULLET_KEYWORDS) do
        ...
        pcall(function()
            ...
            if bp then
                for _, tool in pairs(bp:GetChildren()) do
                    if tool:IsA("Tool") and not isBullet(tool.Name) then
                        items[tool.Name] = (items[tool.Name] or 0) + 1
                    end
                end
            end
            -- Scan Character (item yang dipegang)
    ...
    local function buildInvText(items)
        local parts = {}
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if espEnabled and char
            ...
            if hrpVis and headVis then
                ...
                -- ===== INVENTORY ESP (update tiap 1.5 detik biar ringan) =====
                local now = tick()
                if now - lastInvUpdate >= 1.5 then
                    lastInvUpdate = now
                    local invData = getPlayerInventory(player)
                    cachedInvText = buildInvText(invData)
                end
...
print("- 👁️ ESP Box Outline + Name + Health Bar")
print("- 📦 ESP INVENTORY PLAYER (kecuali peluru/ammo)")
