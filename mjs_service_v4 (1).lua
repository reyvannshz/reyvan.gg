...
local function equipTool(name)
    -- Retry sampai 5x, tunggu setiap kegagalan
    for attempt = 1, 5 do
        local ch  = LocalPlayer.Character
        if not ch then task.wait(0.4); continue end
        local hum = ch:FindFirstChildOfClass("Humanoid")
        if not hum then task.wait(0.4); continue end
        -- Jika masih duduk di kendaraan, unequip dulu dan tunggu
        ...
        if not t then
            -- Cek juga di karakter (sudah ter-equip sebelumnya)
            local inChar = ch:FindFirstChild(name)
            if inChar and inChar:IsA("Tool") then return true end
            return false  -- item tidak ada sama sekali
        end
        hum:EquipTool(t)
        task.wait(0.3)
        -- Verifikasi: cek apakah tool sudah pindah ke karakter
...
local function doAutoSell(setStatus2)
    ...
    -- Pastikan tidak sedang equip apapun dulu
    unequipAll()
    task.wait(0.8)
    ...
    while countAllMS() > 0 do
        ...
        -- 1. Equip MS (equipTool sudah retry internal)
        ...
        if not ok then
            ...
            -- Jika sudah banyak gagal, coba teleport ulang ke NPC lalu retry
            if fail == 3 then
        ...
        -- 2. Coba jual: E + firePrompt (diulang 2x untuk antisipasi lag)
        ...
        for _attempt = 1, 2 do
            ...
            -- Cek apakah sudah terjual setelah percobaan ini
            local diffCheck = (bS - countItem(CFG.ITEM_MS_SMALL))
                            + (bM - countItem(CFG.ITEM_MS_MEDIUM))
                            + (bL - countItem(CFG.ITEM_MS_LARGE))
            if diffCheck > 0 then break end
            task.wait(0.5)
        end
        ...
        -- 4. Unequip sebelum iterasi berikutnya
        unequipAll()
        task.wait(0.4)
        ...
        if terjual then
            ...
            -- Re-teleport jika sudah 4 fail berturut-turut
            if fail == 4 then
                setStatus2("Re-teleport ke NPC...", Color3.fromRGB(255,160,40))
                fullyTeleport(NPC_MS_POS)
                task.wait(2)
...
local function doAutoFully(setFullyStatus)
    ...
    while fullyRunning do
        ...
        -- Tunggu player stabil setelah teleport (pastikan humanoid ready & tidak di kendaraan)
        task.wait(1.5)
        unequipAll()
        task.wait(0.5)
        -- Pastikan tidak masih duduk di kendaraan sebelum equip untuk jual
        local _waitSeat = 0
        repeat
            local _ch2 = LocalPlayer.Character
            local _hm2 = _ch2 and _ch2:FindFirstChildOfClass("Humanoid")
            if not _hm2 or not _hm2.SeatPart then break end
            task.wait(0.3); _waitSeat += 0.3
        until _waitSeat > 4
