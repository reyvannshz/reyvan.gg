...
local tpMethodValue  = nil
local walkStepLiveLabel  = nil
local walkDelayLiveLabel = nil
local walkSpeedLiveLabel = nil
local vehStepLiveLabel   = nil
local vehDelayLiveLabel  = nil
local vehSpeedLiveLabel  = nil
...
-- ========== AUTO RANGE CALCULATOR ==========
-- Jalan kaki: makin jauh -> step lebih besar & delay lebih kecil,
-- tapi tetap dalam batas aman anti-cheat
local function calcWalkAuto(dist)
    -- Target kecepatan: 6-10 studs/s (aman), naik perlahan sesuai jarak
    -- Rumus: speed = 6 + log10(dist+1) * 1.5 (max 12)
    local speed = math.min(12, 6 + math.log10(dist + 1) * 1.5)
    -- Step size: kecil untuk jarak dekat, lebih besar untuk jauh
    local step  = math.min(1.0, math.max(0.2, dist / 800))
    -- Delay dari step/speed
    local delay = math.max(0.016, step / speed)
    return step, delay
end

-- Kendaraan: step lebih besar, delay lebih aman untuk cegah void
local function calcVehAuto(dist)
    local speed = math.min(80, 20 + math.log10(dist + 1) * 8)
    local step  = math.min(40, math.max(10, dist / 50))
    local delay = math.max(0.05, step / speed)
    return step, delay
end

local function updateLiveLabels(dist)
    local ws, wd = calcWalkAuto(dist)
    if walkStepLiveLabel  then walkStepLiveLabel.Text  = string.format("%.2f st", ws) end
    if walkDelayLiveLabel then walkDelayLiveLabel.Text = string.format("%.0fms",  wd * 1000) end
    if walkSpeedLiveLabel then walkSpeedLiveLabel.Text = string.format("%.1f s/s", ws / wd) end
    local vs, vd = calcVehAuto(dist)
    if vehStepLiveLabel   then vehStepLiveLabel.Text   = string.format("%.0f st",  vs) end
    if vehDelayLiveLabel  then vehDelayLiveLabel.Text  = string.format("%.0fms",   vd * 1000) end
    if vehSpeedLiveLabel  then vehSpeedLiveLabel.Text  = string.format("%.0f s/s", vs / vd) end
end
...
local function getVehicleModel()
    ...
    if hum and hum.SeatPart then
        local m = hum.SeatPart:FindFirstAncestorOfClass("Model"); if m then return m end
    end
    local p = hrp.Parent
    while p and p ~= Workspace do
        if p:IsA("Model") and p ~= char then return p end
        p = p.Parent
    end
    for _, obj in pairs(Workspace:GetDescendants()) do
        if (obj:IsA("VehicleSeat") or obj:IsA("Seat")) and (obj.Position - hrp.Position).Magnitude < 8 then
            local m = obj:FindFirstAncestorOfClass("Model"); if m then return m end
        end
    end
    return nil
end

-- ========== TELEPORT: JALAN KAKI (MOONWALK AUTO) ==========
local function doTeleportWalk(targetCFrame)
    ...
    -- Hitung step & delay otomatis dari jarak
    local stepSize, stepDelay = calcWalkAuto(totalDist)

    -- Update live labels di GUI
    updateLiveLabels(totalDist)

    -- Kumpulkan semua part karakter
    local charParts = {}
    ...
    local function unanchorChar()
        for _, p in pairs(charParts) do p.Anchored = false end
    end

    -- PlatformStand mencegah ragdoll & server override posisi
    hum.PlatformStand = true
    task.wait(0.05)
    anchorChar()
    task.wait(0.05)

    if totalDist < stepSize then
    ...
    for i = 1, steps do
        ...
        local dirXZ    = Vector3.new((endPos - lerpPos).X, 0, (endPos - lerpPos).Z)
        -- MOONWALK: hadap berlawanan arah tujuan
        local lookDir
        ...
        hrp.CFrame = CFrame.new(lerpPos) * CFrame.fromMatrix(
            Vector3.zero,
            lookDir:Cross(Vector3.new(0,1,0)).Unit,
            Vector3.new(0,1,0),
            -lookDir
        )
        anchorChar()
...
-- ========== TELEPORT: KENDARAAN (ANTI VOID AUTO) ==========
local function doTeleportVehicle(targetCFrame)
    local char = LocalPlayer.Character; if not char then return false end
    local hrp  = char:FindFirstChild("HumanoidRootPart"); if not hrp then return false end
    local vModel = getVehicleModel()
    if not vModel then return doTeleportWalk(targetCFrame) end

    local allParts = {}
    for _, p in pairs(vModel:GetDescendants()) do if p:IsA("BasePart") then table.insert(allParts, p) end end
    for _, p in pairs(char:GetDescendants()) do if p:IsA("BasePart") then table.insert(allParts, p) end end
    ...
    local pivotStart = vModel:GetPivot()
    local pivotToHRP = pivotStart:ToObjectSpace(hrp.CFrame)
    local startPos   = pivotStart.Position
    local endPos     = targetCFrame.Position
    local totalDist  = (endPos - startPos).Magnitude

    -- Hitung step & delay otomatis
    local stepSize, stepDelay = calcVehAuto(totalDist)
    updateLiveLabels(totalDist)

    local steps    = math.max(1, math.ceil(totalDist / stepSize))
    local rotFinal = targetCFrame - targetCFrame.Position

    for i = 1, steps do
        local lerpPos = startPos:Lerp(endPos, i / steps)
        vModel:PivotTo((CFrame.new(lerpPos) * rotFinal) * pivotToHRP:Inverse())
        for _, p in pairs(allParts) do
            p.AssemblyLinearVelocity  = Vector3.zero
            p.AssemblyAngularVelocity = Vector3.zero
        end
    ...
    vModel:PivotTo((CFrame.new(endPos) * rotFinal) * pivotToHRP:Inverse())
    task.wait(0.15)
    for _, p in pairs(allParts) do p.Anchored = false end
    task.wait(0.1)
    for _, p in pairs(allParts) do
        p.AssemblyLinearVelocity  = Vector3.zero
        p.AssemblyAngularVelocity = Vector3.zero
...
local function doTeleport(targetCFrame)
    if tpMethod == "vehicle" then return doTeleportVehicle(targetCFrame)
    else return doTeleportWalk(targetCFrame) end
end
...
do
    ...
    local stb=Instance.new("TextButton"); stb.Name="SellToggleBtn"
    stb.Size=UDim2.new(1,-20,0,36); stb.Position=UDim2.new(0,10,0,548)
    stb.BackgroundColor3=C.card; stb.Text="AUTO SELL : OFF"; stb.TextColor3=C.red
    stb.Font=Enum.Font.Gotham; stb.TextSize=13; stb.BorderSizePixel=0; stb.Parent=pageAuto
    mkCorner(stb,5); mkStroke(stb,1,C.border)
    local ssc=makeCard(pageAuto,592,44); makeLabel(ssc,"STATUS",12,0,80,44,10,C.subtext)
    local ssl=makeLabel(ssc,"OFF",90,0,200,44,13,C.red)
    local scc=makeCard(pageAuto,644,44); makeLabel(scc,"TERJUAL",12,0,80,44,10,C.subtext)
    local scl=makeLabel(scc,"0",90,0,200,44,13,C.yellow)
    sellStatusLbl_ref=ssl; sellCountLbl_ref=scl
    stb.MouseButton1Click:Connect(function()
        autoSell_Running=not autoSell_Running
        if autoSell_Running then stb.Text="AUTO SELL : ON";stb.TextColor3=C.accent;ssl.Text="RUNNING";ssl.TextColor3=C.accent
        else stb.Text="AUTO SELL : OFF";stb.TextColor3=C.red;ssl.Text="OFF";ssl.TextColor3=C.red end
    end)
    -- ========== AUTO BUY ==========
-- ── SECTION: AUTO BUY (400-499) ──
makeSection(pageAuto, "🛒 Auto Buy Bahan", 400)
local autoBuyFull    = false
local buyFullWater   = 1
local buyFullSugar   = 1
local buyFullGelatin = 1
local buyFullStatusRef, _ = makeLabel(pageAuto, "STATUS  ›  OFF",  Color3.fromRGB(255, 80, 80),   401)
local buyFullPhaseRef,  _ = makeLabel(pageAuto, "PHASE   ›  Idle", Color3.fromRGB(0, 200, 255),   402)
local buyFullItemRef,   _ = makeLabel(pageAuto, "ITEM    ›  -",    Color3.fromRGB(160, 160, 180), 403)
makeAmountRow(pageAuto, "💧 Water", 404,
...
makeAmountRow(pageAuto, "🧂 Sugar Block Bag", 405,
...
makeAmountRow(pageAuto, "🟡 Gelatin", 406,
...
makeToggle(pageAuto, "🚀 Start Auto Buy", false, 408, function(v)
    autoBuyFull = v
    buyFullStatusRef.Text = "STATUS  ›  " .. (v and "ON" or "OFF")
    buyFullStatusRef.TextColor3 = v and Color3.fromRGB(0, 220, 100) or Color3.fromRGB(255, 80, 80)
    if not v then buyFullPhaseRef.Text = "PHASE   ›  Idle"; buyFullItemRef.Text = "ITEM    ›  -" end
end)

-- ════════════════════════════════════════════════════════
--  HELPER FUNCTIONS (Farm, Sell, Buy)
-- ════════════════════════════════════════════════════════
        ...
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("ProximityPrompt") and obj.Enabled then
                ...
                if part and part:IsA("BasePart") then
                    ...
                    if dist < 15 and dist < closestDist then closestDist = dist; closest = obj end
                end
            end
        end
...
local function hasInstruction(text)
    ...
    for _, ui in ipairs(pg:GetDescendants()) do
        if ui:FindFirstAncestor("KINGMJSFREE") then continue end
        if (ui:IsA("TextLabel") or ui:IsA("TextButton")) then
            local t = ui.Text
            if t and t:lower():find(text:lower()) then return true end
        end
    end
    return false
end
...
-- AUTO FARM LOOP — original Heartbeat version
RunService.Heartbeat:Connect(function(dt)
    ...
    if currentStep == 1 then
        if not hasWater then
            ...
            statusLblRef.Text = "STATUS  ›  BAHAN HABIS"; statusLblRef.TextColor3 = Color3.fromRGB(255, 180, 0)
            phaseLblRef.Text = "PHASE   ›  Idle"; stepLblRef.Text = "STEP    ›  1 / 4"; return
        end
        if selectInventoryItem("Water") then currentStep = 2; stepCooldown = 4; pressE() end
    elseif currentStep == 2 then
        ...
        if hasInstruction("sugar") and selectInventoryItem("Sugar Block Bag") then
            currentStep = 3; stepCooldown = 4; pressE()
        end
    elseif currentStep == 3 then
        ...
        if hasInstruction("gelatin") and selectInventoryItem("Gelatin") then
            currentStep = 4; stepCooldown = 4; pressE()
        end
    elseif currentStep == 4 then
        if not hasBag then
            local hasMS = countItem("Small Marshmallow Bag") > 0 or countItem("Medium Marshmallow Bag") > 0 or countItem("Large Marshmallow Bag") > 0
            if hasMS then currentStep = 1; marshmallowCount += 1; stepCooldown = 1
            else
                autoFarm = false; autoFarmRunning = false
                statusLblRef.Text = "STATUS  ›  BAHAN HABIS"; statusLblRef.TextColor3 = Color3.fromRGB(255, 180, 0)
                currentStep = 1
        ...
        if selectInventoryItem("Empty Bag") then stepCooldown = 2; pressE() end
    end
    if hasInstruction("bottle of water") then currentStep = 1; stepCooldown = 2 end
end)
...
task.spawn(function()
    while true do
        ...
        for _, name in ipairs(sellOrder) do
            if countItem(name) > 0 then targetItem = name; break end
        end
        if not targetItem then
            sellItemRef.Text = "ITEM  ›  Tidak ada"; sellItemRef.TextColor3 = Color3.fromRGB(120, 120, 140)
            continue
        end
        sellItemRef.Text = "ITEM  ›  " .. targetItem; sellItemRef.TextColor3 = Color3.fromRGB(0, 220, 120)
        if selectInventoryItem(targetItem) then
            task.wait(0.1)
            local hrp = char:FindFirstChild("HumanoidRootPart")
            ...
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if obj:IsA("ProximityPrompt") and obj.Enabled then
                    ...
                    if part and part:IsA("BasePart") then
                        local dist = (part.Position - myPos).Magnitude
                        if dist < 12 and dist < closestDist then closestDist = dist; closest = obj end
                    end
                end
            end
            if closest then
                local ok = pcall(fireproximityprompt, closest)
                if not ok then
                    pcall(function()
                        VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.E, false, game)
                        task.wait(0.05)
...
local cachedSeat = nil -- forward declare, diisi dari VTP section
...
local function clickGuiBtnFull(btn)
    ...
    local cy = pos.Y + size.Y / 2 + guiInsetBuy.Y
...
local function tpVehicleTo(x, y, z)
    local char = LocalPlayer.Character
...
task.spawn(function()
    while true do
        task.wait(0.5)
        if not autoBuyFull then continue end
        local char = LocalPlayer.Character
        local hum  = char and char:FindFirstChildOfClass("Humanoid")
        if not char or not hum or hum.Health <= 0 then continue end
        local hrp = char:FindFirstChild("HumanoidRootPart")
        ...
        if not cachedSeat then
            ...
            if hum2 and hum2.SeatPart then
                local s = hum2.SeatPart
                if s:IsA("Seat") or s:IsA("VehicleSeat") then cachedSeat = s end
            end
        end

        -- Set false di awal supaya tidak looping lagi selama proses
        autoBuyFull = false
        buyFullStatusRef.Text = "STATUS  ›  RUNNING..."
        buyFullStatusRef.TextColor3 = Color3.fromRGB(255, 180, 0)

        buyFullPhaseRef.Text = "PHASE   ›  TP ke NPC..."
        buyFullPhaseRef.TextColor3 = Color3.fromRGB(0, 180, 255)
        tpVehicleTo(LAMONT_X, LAMONT_Y, LAMONT_Z)
        task.wait(2)

        buyFullPhaseRef.Text = "PHASE   ›  Cari NPC..."
        local prompt = nil
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("ProximityPrompt") then
                ...
                if part and part:IsA("BasePart") then
                    ...
                    if mdl and mdl.Name:lower():find("lamont") then prompt = obj; break end
                end
            end
        end
        if not prompt then
            buyFullPhaseRef.Text = "PHASE   ›  NPC tidak ketemu!"
            buyFullPhaseRef.TextColor3 = Color3.fromRGB(255, 80, 80)
            task.wait(2); continue
        end

        buyFullPhaseRef.Text = "PHASE   ›  Interact NPC..."
        fireproximityprompt(prompt)

        local pg = LocalPlayer.PlayerGui
        local shopOpened = false
        local t = 0
        while t < 8 and not shopOpened do
            task.wait(0.2); t += 0.2
            local shopCheck = pg:FindFirstChild("Shop")
            ...
            if not shopOpened then
                local dlg = pg:FindFirstChild("DialogueUI")
                ...
                if fc and fc.Visible and fc.Text ~= "" and not fc.Text:lower():find("nevermind") then
                    buyFullPhaseRef.Text = "PHASE   ›  Klik dialog..."
                    local pos = fc.AbsolutePosition; local size = fc.AbsoluteSize
        ...
        if not shopOpened then
            buyFullPhaseRef.Text = "PHASE   ›  Gagal, ulangi..."
            buyFullPhaseRef.TextColor3 = Color3.fromRGB(255, 80, 80)
            task.wait(1); continue
        end

        buyFullPhaseRef.Text = "PHASE   ›  Beli item..."
        local shop2       = pg:FindFirstChild("Shop")
        local mainShop2   = shop2 and shop2:FindFirstChild("Main")
        local scrollFrame = mainShop2 and mainShop2:FindFirstChild("ScrollingFrame")

        if scrollFrame then
            local buyList = {
                {name="Water",           amount=buyFullWater,   color=Color3.fromRGB(0,180,255)},
                {name="Sugar Block Bag", amount=buyFullSugar,   color=Color3.fromRGB(255,180,50)},
                {name="Gelatin",         amount=buyFullGelatin, color=Color3.fromRGB(255,220,50)},
            }
            for _, entry in ipairs(buyList) do
                if entry.amount <= 0 then continue end
                local itemBtn = findShopItem(scrollFrame, entry.name)
                if itemBtn then
                    for i = 1, entry.amount do
                        buyFullPhaseRef.Text = "PHASE   ›  Beli item"
                        buyFullItemRef.Text  = "ITEM    ›  "..entry.name.." ("..i.."/"..entry.amount..")"
                        buyFullItemRef.TextColor3 = entry.color
                        clickGuiBtnFull(itemBtn)
                        task.wait(0.4)
                    end
                end
            end
            local exitBtn = mainShop2:FindFirstChild("Exit")
            if exitBtn then clickGuiBtnFull(exitBtn) end
        end

        task.wait(0.5)

        buyFullStatusRef.Text = "STATUS  ›  SELESAI ✅"; buyFullStatusRef.TextColor3 = Color3.fromRGB(0, 200, 255)
        buyFullPhaseRef.Text = "PHASE   ›  Done"; buyFullItemRef.Text = "ITEM    ›  Done ✅"
        task.wait(2)
        buyFullStatusRef.Text = "STATUS  ›  OFF"; buyFullStatusRef.TextColor3 = Color3.fromRGB(255, 80, 80)
        buyFullPhaseRef.Text = "PHASE   ›  Idle"; buyFullItemRef.Text = "ITEM    ›  -"
    end
end)
    pageAuto.CanvasSize=UDim2.new(0,0,0,1080)
end
...
--  TELEPORT TAB
-- ════════════════════════════════════════════════════════
makeSection(pageTeleport, "📍 Teleport", 1)
...
makeButton(pageTeleport, "🚗  Dealership",            Color3.fromRGB(0, 110, 220), 2,  function() tpTo(732.1171264648438, 3.3621320724487305, 406.0807189941406) end)
makeButton(pageTeleport, "🍡  Jual/Beli Marshmellow", Color3.fromRGB(180, 80, 0),  3,  function() tpTo(510.9961853027344, 3.5872106552124023, 598.3929443359375) end)
makeButton(pageTeleport, "⭐  Tier",                  Color3.fromRGB(120, 0, 200), 4,  function() tpTo(1094.7406005859375, 3.188796043395996, 158.09230041503906) end)
makeButton(pageTeleport, "🛣️  GS Ujung",             Color3.fromRGB(60, 60, 60),  5,  function() tpTo(-464.5489501953125, 3.7371325492858887, 335.3158874511719) end)
makeButton(pageTeleport, "🛣️  GS Mid",               Color3.fromRGB(60, 60, 60),  6,  function() tpTo(218.74879455566406, 3.729842185974121, -161.87036132812) end)
makeSection(pageTeleport, "ℹ️ Info", 7)
makeLabel(pageTeleport, "Kill → Respawn → TP ke tujuan", Color3.fromRGB(140, 140, 160), 8)

-- ════════════════════════════════════════════════════════
--  KENDARAAN TP TAB
-- ════════════════════════════════════════════════════════
makeSection(pageVehicleTP, "🚗 Teleport Kendaraan", 1)
...
local vehStatusLbl, _ = makeLabel(pageVehicleTP, "Kendaraan  ›  Tidak ditemukan", Color3.fromRGB(255, 80, 80), 2)
task.spawn(function()
    while true do
        task.wait(1)
        if cachedSeat then
            ...
            vehStatusLbl.TextColor3 = Color3.fromRGB(255, 80, 80)
        end
    end
end)
...
makeLabel(pageVehicleTP, "✅ Tidak perlu mati / respawn",   Color3.fromRGB(0, 220, 100), 3)
makeLabel(pageVehicleTP, "ℹ️ Bisa dipakai saat naik motor", Color3.fromRGB(0, 180, 255), 4)
makeButton(pageVehicleTP, "🚗  Dealership",            Color3.fromRGB(0, 110, 220), 5,  function() tpVehicle(732.1171264648438, 3.3621320724487305, 406.0807189941406) end)
makeButton(pageVehicleTP, "🍡  Jual/Beli Marshmellow", Color3.fromRGB(180, 80, 0),  6,  function() tpVehicle(510.9961853027344, 3.5872106552124023, 598.3929443359375) end)
makeButton(pageVehicleTP, "⭐  Tier",                  Color3.fromRGB(120, 0, 200), 7,  function() tpVehicle(1094.7406005859375, 3.188796043395996, 158.09230041503906) end)
makeButton(pageVehicleTP, "🎰  Casino",               Color3.fromRGB(180, 120, 0), 8,  function() tpVehicle(1154.863525390625, 4.289375305175781, -46.8486328125) end)
makeButton(pageVehicleTP, "💰  Jual Casino",           Color3.fromRGB(160, 100, 0), 9,  function() tpVehicle(1017.5814819335938, 4.545021533966064, -321.7923889160156) end)
makeButton(pageVehicleTP, "🛣️  GS Ujung",             Color3.fromRGB(60, 60, 60),  10, function() tpVehicle(-464.5489501953125, 3.7371325492858887, 335.3158874511719) end)
makeButton(pageVehicleTP, "🛣️  GS Mid",               Color3.fromRGB(60, 60, 60),  11, function() tpVehicle(218.74879455566406, 3.729842185974121, -161.87036132812) end)
makeSection(pageVehicleTP, "🏠 Teleport ke Kompor Apart", 12)
makeButton(pageVehicleTP, "🍳  Kompor Apart 1", Color3.fromRGB(0, 140, 100), 13, function() tpVehicle(897.308837890625,   10.105066299438477, 39.217220306396484) end)
makeButton(pageVehicleTP, "🍳  Kompor Apart 2", Color3.fromRGB(0, 140, 100), 14, function() tpVehicle(925.7492065429688,  10.105064392089844, 39.23077392578125) end)
makeButton(pageVehicleTP, "🍳  Kompor Apart 3", Color3.fromRGB(0, 140, 100), 15, function() tpVehicle(984.796875,         10.105064392089844, 248.2229461669922) end)
makeButton(pageVehicleTP, "🍳  Kompor Apart 4", Color3.fromRGB(0, 140, 100), 16, function() tpVehicle(984.8067626953125,  10.105064392089844, 219.7691192626953) end)
makeButton(pageVehicleTP, "🍳  Kompor Apart 5", Color3.fromRGB(0, 140, 100), 17, function() tpVehicle(1141.9276123046875, 10.105062484741211, 451.07061767578125) end)
makeButton(pageVehicleTP, "🍳  Kompor Apart 6", Color3.fromRGB(0, 140, 100), 18, function() tpVehicle(1141.8072509765625, 10.105064392089844, 422.4674987792969) end)
makeSection(pageVehicleTP, "ℹ️ Info", 19)
makeLabel(pageVehicleTP, "Jika tidak naik kendaraan,",        Color3.fromRGB(140, 140, 160), 20)
makeLabel(pageVehicleTP, "karakter langsung diTP tanpa mati.", Color3.fromRGB(140, 140, 160), 21)
...
-- ========== BOTTOM NAV ==========
local tabDefs={{label="AUTO MS",page=pageAuto},{label="GENERAL",page=pageEsp},{label="TELEPORT",page=pageTP},{label="AIMBOT",page=pageAimbot},{label="CREDITS",page=pageCredits}}
local tabBtns={}
...
-- ========== AUTO SELL ==========
local msKw={"small marshmellow","medium marshmellow","big marshmellow","marshmellow"}
local function getMSItems()
    local items={}
    local function chk(p) if not p then return end; for _,t in pairs(p:GetChildren()) do if t:IsA("Tool") then local n=t.Name:lower(); for _,kw in pairs(msKw) do if n:find(kw) then table.insert(items,t);break end end end end end
    chk(LocalPlayer:FindFirstChild("Backpack")); chk(LocalPlayer.Character); return items
end
local function holdE(dur)
    ...
    pcall(function() if keypress then keypress(0x45) end end)
    local elapsed=0; while elapsed<dur do if not autoSell_Running then break end; task.wait(0.05);elapsed=elapsed+0.05 end
    pcall(function() local vim=game:GetService("VirtualInputManager"); vim:SendKeyEvent(false,Enum.KeyCode.E,false,game) end)
    pcall(function() if keyrelease then keyrelease(0x45) end end); task.wait(0.1)
end
task.spawn(function()
    while true do task.wait(0.5)
        if autoSell_Running then
            local items=getMSItems()
            if #items==0 then if sellStatusLbl_ref then sellStatusLbl_ref.Text="MENUNGGU MS";sellStatusLbl_ref.TextColor3=C.yellow end
            else
                if sellStatusLbl_ref then sellStatusLbl_ref.Text="MENJUAL...";sellStatusLbl_ref.TextColor3=C.accent end
                for _,ms in ipairs(items) do
                    if not autoSell_Running then break end
                    pcall(function() if ms.Parent~=LocalPlayer.Character then ms.Parent=LocalPlayer.Character end end)
                    task.wait(0.3);holdE(1.5); autoSell_Count=autoSell_Count+1
                    if sellCountLbl_ref then sellCountLbl_ref.Text=tostring(autoSell_Count) end; task.wait(0.4)
                end
...
print("TP AUTO RANGE - tidak ada setting manual")
