...
local function moveVehicle(vehicle, targetPos)
    ...
    -- Hadap ke samping (+X) supaya posisi miring di samping kompor
    local newCF    = CFrame.new(spawnPos, spawnPos + Vector3.new(1,0,0))
...
local function doAutoFully(setFullyStatus)
    ...
    while fullyRunning do
        ...
        -- ── MASAK ─────────────────────────────────────────────────
        -- Pastikan karakter sudah settle di posisi apart dan dekat kompor
        task.wait(0.5)
        -- Trigger proximity prompt & tekan E supaya cooking session dimulai
        firePromptNearby(10)
        pcall(function()
            VIM:SendKeyEvent(true,  Enum.KeyCode.E, false, game)
            task.wait(0.15)
...
do
    ...
    local tpLocs = {
        ...
        {name="Apart 1 (Kompor)",      x=897.308837890625,   y=10.105066299438477, z=39.217220306396484},
        {name="Apart 2 (Kompor)",      x=925.7492065429688,  y=10.105064392089844, z=39.23077392578125},
        {name="Apart 3 (Kompor)",      x=897.308837890625,         y=10.105066299438477, z=39.217220306396484},
        {name="Apart 4 (Kompor)",      x=925.7492065429688,         y=10.105064392089844, z=39.23077392578125},
        {name="Apart 5 (Kompor)",      x=897.308837890625,         y=10.105066299438477, z=39.217220306396484},
        {name="Apart 6 (Kompor)",      x=925.7492065429688,         y=10.105064392089844, z=39.23077392578125},
    }
...
do
    ...
    -- isApart: true = pakai orientasi miring samping kompor (facing +X)
    local function tpVehicle(x, y, z, isApart)
        task.spawn(function()
            local char=LocalPlayer.Character; if not char then return end
            local hrp=char:FindFirstChild("HumanoidRootPart"); if not hrp then return end
            -- Orientasi: apart = hadap ke samping (+X), normal = hadap +Z
            local spawnPos = Vector3.new(x, y+2, z)
            local lookDir  = isApart and Vector3.new(1,0,0) or Vector3.new(0,0,1)
            local targetCF = CFrame.new(spawnPos, spawnPos + lookDir)
            if cachedSeat then
                local vehModel=cachedSeat:FindFirstAncestorWhichIsA("Model")
    ...
    for i, loc in ipairs(vtpLocs) do
        ...
        local ci=i; btn.MouseButton1Click:Connect(function()
            local l=vtpLocs[ci]
            local apart = l.name:find("Apart") ~= nil
            tpVehicle(l.x,l.y,l.z, apart)
        end)
    end
    ...
    local kompors = {
        ...
        {name="Kompor Apart 3", x=897.308837890625,         y=10.105066299438477, z=39.217220306396484},
        {name="Kompor Apart 4", x=925.7492065429688,         y=10.105064392089844, z=39.23077392578125},
        {name="Kompor Apart 5", x=897.308837890625,         y=10.105066299438477, z=39.217220306396484},
        {name="Kompor Apart 6", x=925.7492065429688,         y=10.105064392089844, z=39.23077392578125},
    }
    for i, k in ipairs(kompors) do
        ...
        btn.MouseEnter:Connect(function() btn.BackgroundColor3=Color3.fromRGB(20,30,45) end)
        btn.MouseLeave:Connect(function() btn.BackgroundColor3=C.card end)
        local ci=i; btn.MouseButton1Click:Connect(function() local kk=kompors[ci]; tpVehicle(kk.x,kk.y,kk.z, true) end)
    end
...
do
    ...
    local afApartList = {
        {name="Apart 1", x=897.308837890625,   y=10.105066299438477, z=39.217220306396484},
        {name="Apart 2", x=925.7492065429688,  y=10.105064392089844, z=39.23077392578125},
        {name="Apart 3", x=897.308837890625,         y=10.105066299438477, z=39.217220306396484},
        {name="Apart 4", x=925.7492065429688,         y=10.105064392089844, z=39.23077392578125},
        {name="Apart 5", x=897.308837890625,         y=10.105066299438477, z=39.217220306396484},
        {name="Apart 6", x=925.7492065429688,         y=10.105064392089844, z=39.23077392578125},
    }
