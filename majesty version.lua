...
do
    ...
    local function mkPairBtn(parent, label1, label2, active, callback)
        local function makeB(txt, xOff, isActive)
            ...
            b.Text=txt; b.TextColor3= isActive and Color3.fromRGB(255,255,255) or C.subtext
            b.Font=Enum.Font.Gotham; b.TextSize=10; b.BorderSizePixel=0; b.Parent=parent; mkCorner(b,5)
            if not isActive then mkStroke(b,1,C.border) end; return b
        end
        local b1=makeB(label1,-162,active==1); local b2=makeB(label2,-78,active==2)
        local function refresh(which)
            b1.BackgroundColor3=which==1 and C.accent or C.card2; b1.TextColor3=which==1 and Color3.fromRGB(255,255,255) or C.subtext
            b2.BackgroundColor3=which==2 and C.accent or C.card2; b2.TextColor3=which==2 and Color3.fromRGB(255,255,255) or C.subtext
        end
    ...
    local function mkSlider(parent, yPos, label, minV, maxV, defV, suffix, callback)
        ...
        local lbl=Instance.new("TextLabel"); lbl.Size=UDim2.new(0.6,0,0,20); lbl.Position=UDim2.new(0,10,0,2)
        lbl.BackgroundTransparency=1; lbl.Text=label; lbl.TextColor3=C.text; lbl.Font=Enum.Font.Gotham; lbl.TextSize=11; lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.Parent=row
        local valLbl=Instance.new("TextLabel"); valLbl.Size=UDim2.new(0.4,-10,0,20); valLbl.Position=UDim2.new(0.6,0,0,2)
        valLbl.BackgroundTransparency=1; valLbl.Text=defV..suffix; valLbl.TextColor3=C.accent2; valLbl.Font=Enum.Font.Gotham; valLbl.TextSize=11; valLbl.TextXAlignment=Enum.TextXAlignment.Right; valLbl.Parent=row
        local track=Instance.new("Frame"); track.Size=UDim2.new(1,-20,0,3); track.Position=UDim2.new(0,10,0,32)
        ...
        local fill=Instance.new("Frame"); fill.Size=UDim2.new(ratio0,0,1,0); fill.BackgroundColor3=C.accent; fill.BorderSizePixel=0; fill.Parent=track; mkCorner(fill,2)
        local knob=Instance.new("TextButton"); knob.Size=UDim2.new(0,10,0,10); knob.Position=UDim2.new(ratio0,-5,0.5,-5)
        ...
        UserInputService.InputChanged:Connect(function(i)
            if dragging and i.UserInputType==Enum.UserInputType.MouseMovement then
                ...
                valLbl.Text=v..suffix; if callback then callback(v) end
            end
        end)
    end
    local function mkColorRow(parent, yPos, label, onPick)
        local colorPresets={Color3.fromRGB(220,38,38),Color3.fromRGB(255,255,255),Color3.fromRGB(0,210,255),Color3.fromRGB(34,197,94),Color3.fromRGB(234,179,8),Color3.fromRGB(168,85,247)}
        local lbl=Instance.new("TextLabel"); lbl.Size=UDim2.new(1,-16,0,13); lbl.Position=UDim2.new(0,8,0,yPos)
        lbl.BackgroundTransparency=1; lbl.Text=label; lbl.TextColor3=C.subtext; lbl.Font=Enum.Font.Gotham; lbl.TextSize=9; lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.Parent=parent
        local sf=Instance.new("Frame"); sf.Size=UDim2.new(1,-16,0,20); sf.Position=UDim2.new(0,8,0,yPos+14)
        sf.BackgroundTransparency=1; sf.BorderSizePixel=0; sf.Parent=parent
        local cw=1/#colorPresets
        for ci,col in ipairs(colorPresets) do
            local sw=Instance.new("TextButton"); sw.Size=UDim2.new(cw,-3,1,0); sw.Position=UDim2.new(cw*(ci-1),0,0,0)
            sw.BackgroundColor3=col; sw.Text=""; sw.BorderSizePixel=0; sw.Parent=sf; mkCorner(sw,4)
            local capC=col
            sw.MouseButton1Click:Connect(function()
                onPick(capC)
    ...
    local statusTxt=Instance.new("TextLabel"); statusTxt.Size=UDim2.new(0.5,0,1,0); statusTxt.Position=UDim2.new(0,10,0,0)
    ...
    aimbotStatusLbl=Instance.new("TextLabel"); aimbotStatusLbl.Size=UDim2.new(0,40,1,0); aimbotStatusLbl.Position=UDim2.new(1,-88,0,0)
    ...
    mkToggle(statusRow,false,function(s)
        aimbotEnabled=s; aimbotStatusLbl.Text=s and "ON" or "OFF"; aimbotStatusLbl.TextColor3=s and C.green or C.red
        if aimbotFovCircle then aimbotFovCircle.Visible=s end
    end); y=y+40

    -- Mode
    mkSectionSep(pageAimbot,y,"MODE"); y=y+20
    local modeRow=mkRow(pageAimbot,y,34); mkRowLabel(modeRow,"Aim Mode")
    mkPairBtn(modeRow,"Camera","FreeAim",1,function(which) aimbotMode=which==1 and "Camera" or "FreeAim" end); y=y+40
    ...
    local targetIdx=1; local targetDropOpen=false
    local targetSelRow=mkRow(pageAimbot,y,30)
    local targetSelLbl=Instance.new("TextLabel"); targetSelLbl.Size=UDim2.new(0.42,0,1,0); targetSelLbl.Position=UDim2.new(0,10,0,0)
    ...
    local targetValLbl=Instance.new("TextLabel"); targetValLbl.Size=UDim2.new(0.36,0,1,0); targetValLbl.Position=UDim2.new(0.42,0,0,0)
    ...
    local chevronBtn=Instance.new("TextButton"); chevronBtn.Size=UDim2.new(0,26,0,20); chevronBtn.Position=UDim2.new(1,-32,0.5,-10)
    ...
    mkCorner(chevronBtn,4); mkStroke(chevronBtn,1,C.border); y=y+36
    local optionH=#targetLabels*28+4
    local dropContainer=Instance.new("Frame"); dropContainer.Size=UDim2.new(1,0,0,0); dropContainer.Position=UDim2.new(0,0,0,y)
    dropContainer.BackgroundTransparency=1; dropContainer.BorderSizePixel=0; dropContainer.ClipsDescendants=true; dropContainer.Parent=pageAimbot
    local targetOptBtns={}
    local function refreshTargetOpts()
        for li,btn in ipairs(targetOptBtns) do
            btn.BackgroundColor3=targetIdx==li and C.accent or C.card2; btn.TextColor3=targetIdx==li and Color3.fromRGB(255,255,255) or C.subtext
        end; targetValLbl.Text=targetLabels[targetIdx]
    end
    for li,lbl in ipairs(targetLabels) do
        local optBtn=Instance.new("TextButton"); optBtn.Size=UDim2.new(1,-16,0,24); optBtn.Position=UDim2.new(0,8,0,(li-1)*28+2)
        optBtn.BackgroundColor3=C.card2; optBtn.TextColor3=C.subtext; optBtn.Font=Enum.Font.Gotham; optBtn.TextSize=10
        optBtn.TextXAlignment=Enum.TextXAlignment.Left; optBtn.Text="    "..lbl; optBtn.BorderSizePixel=0; optBtn.Parent=dropContainer
        mkCorner(optBtn,4); targetOptBtns[li]=optBtn
        local capLi=li
        optBtn.MouseButton1Click:Connect(function() targetIdx=capLi; aimbotTarget=targetParts[capLi]; refreshTargetOpts() end)
    end
    refreshTargetOpts()
    local function toggleTargetDrop()
        targetDropOpen=not targetDropOpen
        TweenService:Create(dropContainer,TweenInfo.new(0.15,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Size=UDim2.new(1,0,0,targetDropOpen and optionH or 0)}):Play()
        chevronBtn.Text=targetDropOpen and "^" or "v"; chevronBtn.BackgroundColor3=targetDropOpen and C.accent or C.card2
    ...
    local rowClickBtn=Instance.new("TextButton"); rowClickBtn.Size=UDim2.new(1,-38,1,0); rowClickBtn.Position=UDim2.new(0,0,0,0)
    ...
    -- Priority
    local prioRow=mkRow(pageAimbot,y,34); mkRowLabel(prioRow,"Lock Priority")
    mkPairBtn(prioRow,"Crosshair","Distance",1,function(which) aimbotPriority=which==1 and "Crosshair" or "Distance" end); y=y+40

    -- Keybind
    mkSectionSep(pageAimbot,y,"KEYBIND"); y=y+20
    local kbRow=mkRow(pageAimbot,y,34); mkRowLabel(kbRow,"Hold Key")
    local kbBtn=Instance.new("TextButton"); kbBtn.Size=UDim2.new(0,80,0,22); kbBtn.Position=UDim2.new(1,-88,0.5,-11)
    kbBtn.BackgroundColor3=C.card2; kbBtn.Text="[ RMB ]"; kbBtn.TextColor3=C.accent2
    kbBtn.Font=Enum.Font.Gotham; kbBtn.TextSize=10; kbBtn.BorderSizePixel=0; kbBtn.Parent=kbRow
    mkCorner(kbBtn,5); mkStroke(kbBtn,1,C.accent); keybindBtnRef=kbBtn
    kbBtn.MouseButton1Click:Connect(function()
        if isBindingKey then return end; isBindingKey=true; kbBtn.Text="[ ... ]"; kbBtn.TextColor3=C.yellow
    end)
    UserInputService.InputBegan:Connect(function(input,gpe)
        ...
        if un=="MouseButton2" then aimbotKeybindType="MouseButton"; aimbotKeybindCode=nil; aimbotKeybind=Enum.UserInputType.MouseButton2; aimbotKeybindLabel="RMB"
        elseif un=="MouseButton3" then aimbotKeybindType="MouseButton"; aimbotKeybindCode=nil; aimbotKeybind=Enum.UserInputType.MouseButton3; aimbotKeybindLabel="MMB"
        elseif un=="MouseButton4" or kn=="MouseButton4" then aimbotKeybindType="MB4"; aimbotKeybindCode=nil; aimbotKeybindLabel="MB4"
        elseif un=="MouseButton5" or kn=="MouseButton5" then aimbotKeybindType="MB5"; aimbotKeybindCode=nil; aimbotKeybindLabel="MB5"
        ...
        kbBtn.Text="[ "..aimbotKeybindLabel.." ]"; kbBtn.TextColor3=C.accent2
    end); y=y+40

    -- Minimize Keybind
    mkSectionSep(pageAimbot,y,"MINIMIZE KEYBIND"); y=y+20
    local minKbRow=mkRow(pageAimbot,y,34)
    local minKbLabelTxt=Instance.new("TextLabel"); minKbLabelTxt.Size=UDim2.new(0.55,0,1,0); minKbLabelTxt.Position=UDim2.new(0,10,0,0)
    minKbLabelTxt.BackgroundTransparency=1; minKbLabelTxt.Text="Hide / Show GUI"; minKbLabelTxt.TextColor3=C.text
    minKbLabelTxt.Font=Enum.Font.Gotham; minKbLabelTxt.TextSize=11; minKbLabelTxt.TextXAlignment=Enum.TextXAlignment.Left; minKbLabelTxt.Parent=minKbRow
    local minKbBtn=Instance.new("TextButton"); minKbBtn.Size=UDim2.new(0,80,0,22); minKbBtn.Position=UDim2.new(1,-88,0.5,-11)
    minKbBtn.BackgroundColor3=C.card2; minKbBtn.Text="[ RShift ]"; minKbBtn.TextColor3=C.accent2
    minKbBtn.Font=Enum.Font.Gotham; minKbBtn.TextSize=10; minKbBtn.BorderSizePixel=0; minKbBtn.Parent=minKbRow
    mkCorner(minKbBtn,5); mkStroke(minKbBtn,1,C.accent); minKeybindBtnRef=minKbBtn
    minKbBtn.MouseButton1Click:Connect(function()
        if isBindingMin then return end; isBindingMin=true; minKbBtn.Text="[ ... ]"; minKbBtn.TextColor3=C.yellow
    end)
    UserInputService.InputBegan:Connect(function(input,_gpe)
        ...
        minKbBtn.Text="[ "..minKeyLabel.." ]"; minKbBtn.TextColor3=C.accent2
    end); y=y+40
    ...
    mkColorRow(pageAimbot,y,"Warna FOV Circle",function(c) fovColor=c; if aimbotFovCircle then aimbotFovCircle.Color=c end end); y=y+44
    ...
    local predLbl=Instance.new("TextLabel"); predLbl.Size=UDim2.new(0.55,0,1,0); predLbl.Position=UDim2.new(0,10,0,0)
...
do
    ...
    for i,cr in ipairs(creditData) do
        ...
        local accentBar=Instance.new("Frame"); accentBar.Size=UDim2.new(0,3,1,-10); accentBar.Position=UDim2.new(0,4,0,5)
        accentBar.BackgroundColor3=cr.color; accentBar.BorderSizePixel=0; accentBar.Parent=card; mkCorner(accentBar,2)
        local iconLbl=Instance.new("TextLabel"); iconLbl.Size=UDim2.new(0,34,1,0); iconLbl.Position=UDim2.new(0,14,0,0)
        iconLbl.BackgroundTransparency=1; iconLbl.Text=cr.icon; iconLbl.TextSize=22; iconLbl.Font=Enum.Font.Gotham; iconLbl.Parent=card
        local roleLbl=Instance.new("TextLabel"); roleLbl.Size=UDim2.new(1,-60,0,20); roleLbl.Position=UDim2.new(0,52,0,8)
        roleLbl.BackgroundTransparency=1; roleLbl.Text=cr.role; roleLbl.TextColor3=C.subtext; roleLbl.Font=Enum.Font.Gotham; roleLbl.TextSize=10; roleLbl.TextXAlignment=Enum.TextXAlignment.Left; roleLbl.Parent=card
        local nameLbl=Instance.new("TextLabel"); nameLbl.Size=UDim2.new(1,-60,0,24); nameLbl.Position=UDim2.new(0,52,0,26)
        nameLbl.BackgroundTransparency=1; nameLbl.Text=cr.name; nameLbl.TextColor3=cr.color; nameLbl.Font=Enum.Font.Gotham; nameLbl.TextSize=15; nameLbl.TextXAlignment=Enum.TextXAlignment.Left; nameLbl.Parent=card
    end
    local footerCard=makeCard(pageCredits,38+#creditData*68,40)
    local footerLbl=Instance.new("TextLabel"); footerLbl.Size=UDim2.new(1,-20,1,0); footerLbl.Position=UDim2.new(0,10,0,0)
    ...
    footerLbl.Font=Enum.Font.Gotham; footerLbl.TextSize=11; footerLbl.TextXAlignment=Enum.TextXAlignment.Center; footerLbl.Parent=footerCard
...
aimbotFovCircle.Thickness=1; aimbotFovCircle.Color=Color3.fromRGB(220,38,38); aimbotFovCircle.Filled=false
...
local _fovRadiusCur=250; local _fovPosX=0; local _fovPosY=0
...
RunService.RenderStepped:Connect(function(dt)
    ...
    local target,targetPlr=getBestTarget(); if not target then return end
...
print("=== MAJESTY ONTOP v20.0 ===")
print("Teleport: Vehicle Anchor Method (fix void)")
