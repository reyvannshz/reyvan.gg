...
local function doAutoFully(setFullyStatus)
    ...
    while fullyRunning do
        ...
        -- ── MASAK ─────────────────────────────────────────────────
        -- Beri jeda supaya karakter sudah benar-benar settle di apartemen
        task.wait(1.0)
        if not fullyRunning then break end

        -- Set flag running SEBELUM loop doOneCook dipanggil
        -- (doOneCook sudah handle cookInteract + firePromptNearby sendiri)
        ...
        while fullyRunning and hasAllIngredients() do
            ...
            if fullySafeEscaping then
                ...
                -- PERBAIKAN: Teleport balik setelah safe juga lurus
                fullyTeleport(apartPos)
                if not fullyRunning then break end
                task.wait(1.0)  -- jeda supaya karakter settle sebelum masak lagi
                AutoMS_Running = true; isRunning = true
