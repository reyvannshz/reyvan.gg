...
-- Claim network ownership HumanoidRootPart agar client bisa set CFrame
pcall(function()
    local hrp = LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if hrp then hrp:SetNetworkOwner(LocalPlayer) end
end)

-- Re-claim setiap respawn
...
-- ========== TELEPORT FINAL - TELEPORTPART METHOD ==========
-- South Bronx pakai TeleportParts di workspace untuk teleport resmi
-- Cara: cari TeleportPart terdekat ke tujuan, lalu "sentuh" part tersebut
-- Kalau tidak ada TeleportPart → fallback ke sethiddenproperty + CFrame

local function findTeleportPart(targetPos)
    -- Cari semua Part bernama "TeleportPart" atau "Teleport" di workspace
    for _, v in pairs(workspace:GetDescendants()) do
        if v:IsA("BasePart") then
            local n = v.Name:lower()
            if n:find("teleport") then
...
local function doTeleport(targetCFrame)
    task.spawn(function()
        ...
        -- ── METODE 1: pakai TeleportPart resmi game ──
        local tpPart = findTeleportPart(targetPos)
        if tpPart then
            -- Sentuh TeleportPart dengan cara pindah ke posisinya
            hrp.CFrame = tpPart.CFrame
            task.wait(0.2)
            return
        end

        -- ── METODE 2: sethiddenproperty SimulationRadius ──
        -- Perluas SimulationRadius ke sangat besar → client punya authority
        -- atas semua BasePart di area tujuan termasuk HRP
        pcall(function()
            sethiddenproperty(LocalPlayer, "SimulationRadius", math.huge)
        end)

        -- ── METODE 3: CFrame via Touched event trick ──
        -- Buat invisible Part di tujuan, teleport karakter ke sana via touch
        local fakePart = Instance.new("Part")
        fakePart.Size        = Vector3.new(4, 1, 4)
        fakePart.CFrame      = CFrame.new(targetPos + Vector3.new(0, 0.5, 0))
        fakePart.Anchored    = true
        fakePart.CanCollide  = false
        fakePart.Transparency = 1
        fakePart.Parent      = workspace

        -- Langsung set CFrame ke fake part tersebut
        hrp.CFrame = fakePart.CFrame + Vector3.new(0, 3, 0)
        ...
        -- Kembalikan SimulationRadius ke normal
        pcall(function()
