...
-- Interact kompor: equip tool, fire proximity prompt, tekan E, ulangi sampai berhasil
local function cookInteract(toolName, radius, maxRetry)
    radius   = radius   or 8
    maxRetry = maxRetry or 5

    -- Equip tool dulu kalau ada
    if toolName then
        local ok = equipTool(toolName)
        if not ok then
            -- Coba sekali lagi setelah jeda
            task.wait(0.3)
            equipTool(toolName)
        end
        task.wait(0.3)
    ...
    -- Tekan E + fire proximity sampai maxRetry kali
    for attempt = 1, maxRetry do
        -- Fire proximity prompt (kompor / interaksi)
        firePromptNearby(radius)
        task.wait(0.1)
        ...
        -- Fire proximity lagi untuk memastikan
        firePromptNearby(radius)
        task.wait(0.2)

        -- Jika tidak ada toolName, cukup 1 kali interact kompor
        if not toolName then break end

        -- Jika punya toolName: cek apakah tool sudah hilang dari inventory (berarti sudah masuk kompor)
        local stillHave = countItem(toolName) > 0
        if not stillHave then
            -- Sukses! Tool sudah masuk kompor
            break
        end

        -- Belum masuk, coba lagi setelah jeda
        task.wait(0.3)
    end

    -- Unequip setelah memasukkan bahan
    if toolName then
        task.wait(0.1)
        unequipAll()
        task.wait(0.2)
...
local function doAutoSell(setStatus2)
    ...
    while countAllMS() > 0 do
        ...
        -- Tekan E SEKALI lalu fire proximity — tunggu perubahan inventory
        ...
        -- Tunggu sampai item terjual (deteksi perubahan inventory), max SELL_TIMEOUT detik
        local elapsed = 0
        local terjual = false
        ...
        if terjual then
            ...
            -- Tidak terjual dalam timeout — coba sekali lagi dengan proximity saja
            failStreak += 1
            setStatus2("Tidak terjual, retry ("..failStreak.."/"..maxFail..")", Color3.fromRGB(255,155,35))
            unequipAll()
            task.wait(0.5)
...
local function _setStatusRunning()
...
local function doOneCook()
    ...
    -- ── INTERACT KOMPOR DULU (tekan E kompor tanpa tool) ──
    _setPhase("Interact Kompor...")
    unequipAll()
    task.wait(0.3)
    -- Tekan E ke kompor untuk membuka/memulai memasak
    for _i = 1, 3 do
        firePromptNearby(10)
        task.wait(0.1)
        pcall(function()
    ...
    cookInteract(CFG.ITEM_WATER, 10, 6)
    -- Verifikasi water masuk (tunggu max 3 detik)
    local wt = 0
    while countItem(CFG.ITEM_WATER) >= waterBefore and wt < 3 do
        task.wait(0.3); wt += 0.3
    end
    ...
    for i = boilSecs, 1, -1 do
        if not AutoMS_Running and not isRunning then isBusy = false; return false end
        _setStatusRunning()
        _setPhase("Mendidih...")
        _setTimer(i.."s")
        task.wait(1)
    ...
    cookInteract(CFG.ITEM_SUGAR, 10, 6)
    -- Verifikasi sugar masuk
    local st = 0
    while countItem(CFG.ITEM_SUGAR) >= sugarBefore and st < 3 do
        task.wait(0.3); st += 0.3
    end
    task.wait(0.3)
    ...
    cookInteract(CFG.ITEM_GEL, 10, 6)
    -- Verifikasi gelatin masuk
    local gt = 0
    while countItem(CFG.ITEM_GEL) >= gelBefore and gt < 3 do
        task.wait(0.3); gt += 0.3
    end
    task.wait(0.3)
    ...
    -- ── AMBIL MARSHMALLOW ──
    _setPhase("Ambil MS...")
    cookInteract(CFG.ITEM_EMPTY, 10, 5)
...
local function doAutoFully(setFullyStatus)
    ...
    while fullyRunning do
        ...
        -- PERBAIKAN: Teleport dengan orientasi lurus (bukan miring)
        fullyTeleport(apartPos)
        if not fullyRunning then break end
        -- Tambah jeda agar kendaraan/karakter stabil dulu
        task.wait(1.5)
        ...
        if enemy then
            ...
            -- PERBAIKAN: Teleport balik juga lurus
            fullyTeleport(apartPos)
            if not fullyRunning then break end
        end

        -- ── MASAK ─────────────────────────────────────────────────
        -- Unequip semua tool, beri jeda panjang agar karakter settle di kompor
        unequipAll()
        setFullyStatus("Tiba di Apart, bersiap masak...", Color3.fromRGB(148,80,255))
        task.wait(2.5)
        if not fullyRunning then break end

        -- Pastikan rpcQueue bersih sebelum mulai masak
        table.clear(rpcQueue)

        setFullyStatus("Mulai masak "..target.." MS...", Color3.fromRGB(82,130,255))
        -- Set KEDUA flag agar doOneCook berjalan dengan benar
        ...
        while fullyRunning and hasAllIngredients() do
            ...
            if fullySafeEscaping then
                ...
                setFullyStatus("Kena tembak! Kabur ke Safe...", Color3.fromRGB(255,60,60))
                -- Tunggu sampai safeModeActive selesai (TP selesai) dulu
                local waitEscape = 0
                while safeModeActive and waitEscape < 5 do
                    task.wait(0.3); waitEscape += 0.3
                end
                -- Tunggu musuh menjauh ATAU maksimal 6 menit (360 detik)
                local safeWaited = 0
                local MAX_SAFE_WAIT = 360
                while fullyRunning and safeWaited < MAX_SAFE_WAIT do
                    local stillEnemy, ename2 = isEnemyNearApart(apartPos, FULLY_ENEMY_RADIUS)
                    if not stillEnemy then
                        task.wait(2)
                        stillEnemy, ename2 = isEnemyNearApart(apartPos, FULLY_ENEMY_RADIUS)
                        if not stillEnemy then break end
                    end
                    safeWaited += 1
                    local remaining = MAX_SAFE_WAIT - safeWaited
                    setFullyStatus(
                        string.format("⚠ Musuh ada (%s)! Nunggu... %ds (max %ds)", ename2 or "?", safeWaited, remaining),
                        Color3.fromRGB(255, 80, 80)
                    )
                    task.wait(1)
                end
                if not fullyRunning then fullySafeEscaping = false; break end
                fullySafeEscaping = false
                ...
                unequipAll()
                table.clear(rpcQueue)
                setFullyStatus("Tiba kembali di Apart, bersiap masak...", Color3.fromRGB(148,80,255))
                task.wait(2.5)
                AutoMS_Running = true; isRunning = true
