...
-- Cek apakah masakan di kompor dekat apart belum diisi bahan (ProximityPrompt aktif)
local function detectUnfilledCooker()
    if not fullySavedPos then return false end
    local found = false
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("ProximityPrompt") and obj.Enabled then
            local part = obj.Parent
            if part and part:IsA("BasePart") then
                local dist = (part.Position - fullySavedPos).Magnitude
                if dist <= 20 then
                    found = true
                    break
                end
...
local function triggerSafeEscape(newHP)
    ...
    task.spawn(function()
        ...
        -- 4. Deteksi masakan yang belum diisi bahan
        local needCook = not hasAllIngredients() == false and detectUnfilledCooker()
        if safeModeStatusLbl then
            safeModeStatusLbl.Text = needCook and "KOMPOR BUTUH BAHAN – LANJUT MASAK" or "STANDBY"
            safeModeStatusLbl.TextColor3 = needCook and Color3.fromRGB(255,200,0) or C.accent
        end
        ...
        -- 6. Resume Auto Fully jika sebelumnya sedang jalan
        if wasFullyRunning and safeMode then
            if safeModeStatusLbl then safeModeStatusLbl.Text = "RESUME FULLY..."; safeModeStatusLbl.TextColor3 = C.accent end
            task.wait(0.5)
            -- resume masak dari awal loop (bahan cek dulu)
            if hasAllIngredients() then
                isRunning = true
                fullyRunning = true
                -- langsung masak tanpa beli ulang jika bahan masih ada
                task.spawn(function()
                    unequipAll()
                    table.clear(rpcQueue)
                    _setStatus("Resume masak...", Color3.fromRGB(82,130,255))
                    while fullyRunning and hasAllIngredients() do
                        local ok = doOneCook()
                        if ok then end
                        if fullyRunning then task.wait(0.3) end
                    ...
                    isRunning = false
                    -- setelah masak selesai, restart loop penuh
                    if fullyRunning then
                        task.spawn(function()
                            local function dummyStatus(msg, col)
                                _setStatus(msg, col)
                                if safeModeStatusLbl then safeModeStatusLbl.Text = msg; safeModeStatusLbl.TextColor3 = col or C.accent end
                            end
                            doAutoFully(dummyStatus)
                        end)
                    end
                end)
            else
                -- bahan habis, restart fully dari awal (beli bahan dulu)
                task.spawn(function()
                    local function dummyStatus(msg, col)
                        _setStatus(msg, col)
                        if safeModeStatusLbl then safeModeStatusLbl.Text = msg; safeModeStatusLbl.TextColor3 = col or C.accent end
