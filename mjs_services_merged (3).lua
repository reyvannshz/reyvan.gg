...
do
    ...
    makeLabel(buyInfoCard, "⚠  Berdiri dekat NPC lalu klik START", 10, 0, 440, 30, 10, C.yellow, Enum.Font.Gotham)
    ...
    buyToggleBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            ...
            local BAHAN = {
                { label = "Water",       toolKw = "water",   shopText = "Water",           emoji = "💧" },
                { label = "Sugar Block", toolKw = "sugar",   shopText = "Sugar Block Bag", emoji = "🧊" },
                { label = "Gelatin",     toolKw = "gelatin", shopText = "Gelatin",          emoji = "🍮" },
            }

            -- ══════════════════════════════════════════
            -- FIRE BUTTON: Tanpa sentuh mouse sama sekali
            -- Hanya fire event langsung ke button object
            -- ══════════════════════════════════════════
            local function fireBtn(btn)
                if not btn then return false end
                -- Fire event langsung ke signal button
                -- Ini tidak menggerakkan mouse sama sekali
                ...
                task.wait(0.05)
                -- Activated signal (untuk ImageButton / Frame clickable)
            ...
            -- ══════════════════════════════════════════
            -- TEKAN E: Pakai keypress API executor
            -- (bukan VirtualInputManager yang ganggu mouse)
            -- ══════════════════════════════════════════
            local function pressE()
                -- keypress/keyrelease: API executor, tidak ganggu mouse
            ...
            -- ══════════════════════════════════════════
            -- FIRE PROXIMITYPROMPT: Langsung ke object
            -- ══════════════════════════════════════════
            local function firePP()
                -- Cari ProximityPrompt di Workspace dekat player
                ...
                for _, obj in pairs(Workspace:GetDescendants()) do
                    if obj:IsA("ProximityPrompt") then
                        ...
                        if part and part:IsA("BasePart") then
                            ...
                            if d < closestDist and d < 20 then
                                closestDist = d
                                closest = obj
                            end
                        end
                ...
                if closest then
                    print("[AutoBuy] fireproximityprompt: " .. closest.Parent.Name)
                    pcall(function() fireproximityprompt(closest) end)
                    task.wait(0.3)
                    return true
                end
            ...
            -- ══════════════════════════════════════════
            -- CARI BUTTON: Tunggu by Name di GUI tertentu
            -- ══════════════════════════════════════════
            local function waitObjByName(guiName, objName, timeout)
                local t = 0
                while t < (timeout or 4) do
                    if not autoBuy_Running then return nil end
                    local gui = LocalPlayer.PlayerGui:FindFirstChild(guiName)
                    if gui then
                        local obj = gui:FindFirstChild(objName, true)
            ...
            -- Tunggu button by Text exact di seluruh PlayerGui
            local function waitObjByText(text, timeout)
                local t = 0
                while t < (timeout or 4) do
                    if not autoBuy_Running then return nil end
            ...
            local function beliBahan(bahan)
                ...
                for attempt = 1, 6 do
                    ...
                    if hasItem(bahan.toolKw) then
                        print("[AutoBuy] ✅ " .. bahan.label .. " sudah di Backpack!")
                        return true
                    end

                    buyStatusLbl.Text = "Percobaan " .. attempt
                    buyStatusLbl.TextColor3 = C.yellow
                    print("[AutoBuy] Percobaan " .. attempt .. "/6 - " .. bahan.label)

                    -- STEP 1: Fire ProximityPrompt / tekan E
                    buyStatusLbl.Text = "Tekan E..."
                    firePP()
                    task.wait(0.5)

                    -- STEP 2: Fire FirstChoice di DialogueUI
                    -- GUI='DialogueUI' Name='FirstChoice' Text='Yea.. you the guy?'
                    buyStatusLbl.Text = "Klik dialog..."
                    print("[AutoBuy] Tunggu DialogueUI.FirstChoice...")
                    local firstChoice = waitObjByName("DialogueUI", "FirstChoice", 3)
                    if firstChoice then
                        print("[AutoBuy] Fire FirstChoice: '" .. firstChoice.Text .. "'")
                        fireBtn(firstChoice)
                        task.wait(0.7)
                    else
                        -- Coba E lagi lalu tunggu
                        print("[AutoBuy] FirstChoice tidak muncul, coba E lagi...")
                        firePP()
                        task.wait(0.4)
                        firstChoice = waitObjByName("DialogueUI", "FirstChoice", 2)
                        if firstChoice then
                            fireBtn(firstChoice)
                            task.wait(0.7)
                        end
                    ...
                    -- STEP 3: Fire tombol item di Shop
                    -- Tombol: Text='Water' / 'Sugar Block Bag' / 'Gelatin'
                    buyStatusLbl.Text = "Beli " .. bahan.label .. "..."
                    print("[AutoBuy] Tunggu tombol '" .. bahan.shopText .. "'...")
                    local shopBtn = waitObjByText(bahan.shopText, 4)
                    if shopBtn then
                        print("[AutoBuy] Fire shop button: '" .. shopBtn.Text .. "'")
                        fireBtn(shopBtn)
                        task.wait(1.0)
                    else
                        -- Fallback scan partial
                        print("[AutoBuy] Tombol tidak ketemu, scan partial...")
                        for _, gui in pairs(LocalPlayer.PlayerGui:GetChildren()) do
                            for _, obj in pairs(gui:GetDescendants()) do
                                if obj:IsA("TextButton") and obj.Visible
                                   and obj.Text:lower():find(bahan.toolKw) then
                                    print("[AutoBuy] Partial match: '" .. obj.Text .. "'")
                                    fireBtn(obj)
                                    task.wait(1.0)
                                    break
                                end
                    ...
                    task.wait(0.5)
                    if hasItem(bahan.toolKw) then
                        print("[AutoBuy] ✅ BERHASIL! " .. bahan.label .. " masuk Backpack.")
                        updateInventory()
                        return true
                    end

                    -- Tutup dialog sebelum retry
                    local secondChoice = waitObjByName("DialogueUI", "SecondChoice", 0.5)
                    if secondChoice then fireBtn(secondChoice) task.wait(0.3) end

                    task.wait(0.8)
                end

                print("[AutoBuy] ❌ GAGAL: " .. bahan.label)
                return false
            end
            ...
            -- ══════════════════════════════════════════
            print("========================================")
            print("[AutoBuy] 🛒 MULAI: Water → Sugar Block → Gelatin")
            print("========================================")
            ...
            for i, bahan in ipairs(BAHAN) do
                ...
                if ok then
                    buyStatusLbl.Text = "✓ " .. bahan.label
                    buyStatusLbl.TextColor3 = C.green
                    print("[AutoBuy] ✅ Step " .. i .. " selesai!")
                else
                    allOk = false
                    buyStatusLbl.Text = "⚠ Gagal: " .. bahan.label
                    buyStatusLbl.TextColor3 = C.yellow
                end

                if i < #BAHAN and autoBuy_Running then task.wait(1.5) end
            end
            ...
            if allOk then
                ...
                buyStatusLbl.TextColor3 = C.yellow
                print("\n[AutoBuy] ⚠️ Ada bahan yang gagal.")
            end

            -- Ringkasan
            local bp = LocalPlayer:FindFirstChild("Backpack")
            if bp then
                print("[AutoBuy] 📦 Backpack:")
                local n = 0
