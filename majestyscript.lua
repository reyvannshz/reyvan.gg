...
do
    ...
    local espInfo = makeCard(pageEsp, 140, 96)
    ...
    makeLabel(espInfo, "📏  Jarak ke player (meter)", 10, 76, 280, 18, 11, Color3.fromRGB(180,220,255), Enum.Font.Gotham)

    pageEsp.CanvasSize = UDim2.new(0,0,0,256)
end
...
do
    ...
    -- Cyclic button: Head → UpperTorso → Torso → HumanoidRootPart
    local targetParts = {"Head", "UpperTorso", "Torso", "HumanoidRootPart"}
    local targetLabels = {"💀 Head", "🫁 UpperTorso", "🧍 Torso", "⬛ RootPart"}
    local targetIdx = 1
    local targetCycleBtn = Instance.new("TextButton")
    targetCycleBtn.Size = UDim2.new(0, 110, 0, 22)
    targetCycleBtn.Position = UDim2.new(1, -118, 0.5, -11)
    targetCycleBtn.BackgroundColor3 = C.card2
    targetCycleBtn.Text = targetLabels[targetIdx]
    targetCycleBtn.TextColor3 = C.accent2
    targetCycleBtn.Font = Enum.Font.GothamBold
    targetCycleBtn.TextSize = 10
    targetCycleBtn.BorderSizePixel = 0
    targetCycleBtn.Parent = targetRow
    mkCorner(targetCycleBtn, 5)
    mkStroke(targetCycleBtn, 1, C.accent)
    targetCycleBtn.MouseButton1Click:Connect(function()
        targetIdx = (targetIdx % #targetParts) + 1
        aimbotTarget = targetParts[targetIdx]
        targetCycleBtn.Text = targetLabels[targetIdx]
    end)
    y = y + 40
...
-- Xeno support mousemoverel tapi kadang perlu pcall ketat
local function safeMoveRel(x, y)
    -- Coba mousemoverel (Xeno, KRNL, Synapse)
    local ok = pcall(mousemoverel, x, y)
    if not ok then
        -- Fallback: mouse1 simulation tidak bisa, tapi setpos bisa dicoba
        pcall(function()
            local ms = game:GetService("UserInputService")
            -- Xeno fallback: tidak semua executor support ini
        end)
    end
end
...
local function getBestTarget()
    -- GetMouseLocation() mengembalikan viewport coords (sama dengan WorldToViewportPoint)
    local mpos     = getMouseViewportPos()
...
-- FreeAim mode: gerakkan mouse smooth ke target
-- KUNCI FIX:
--   GetMouseLocation()      → viewport coords
--   WorldToViewportPoint()  → viewport coords
--   Keduanya SAMA → tidak ada Y offset / mismatch
--   mousemoverel() memindahkan mouse → GetMouseLocation() di frame BERIKUTNYA
--   sudah reflect posisi baru → TIDAK ADA feedback loop / flick
local function aimFreeAim(part, plr)
    local predPos    = getPredictedPosition(part, plr)
    local sp, vis    = Camera:WorldToViewportPoint(predPos)
    if not vis then
        freeAimAccX, freeAimAccY = 0, 0
        return
    end

    -- Posisi mouse saat ini (viewport coords)
    local mpos    = getMouseViewportPos()
    local dx      = sp.X - mpos.X
    local dy      = sp.Y - mpos.Y

    -- Sudah locked → stop agar tidak micro-jitter
    if math.abs(dx) < 0.5 and math.abs(dy) < 0.5 then
        freeAimAccX, freeAimAccY = 0, 0
        return
    end

    -- Speed: smooth=1 → instant, smooth=20 → perlahan
    local speed = math.clamp(1 - (aimbotSmooth - 1) / 20, 0.04, 1.0)

    -- Lerp accumulator menuju delta (smooth, no-flick)
    freeAimAccX = freeAimAccX + (dx - freeAimAccX) * speed
    freeAimAccY = freeAimAccY + (dy - freeAimAccY) * speed

    -- Clamp step per frame supaya tidak loncat terlalu jauh
    local moveX = math.clamp(freeAimAccX, -50, 50)
    local moveY = math.clamp(freeAimAccY, -50, 50)

    safeMoveRel(moveX, moveY)
end
...
RunService.RenderStepped:Connect(function()
    -- Update posisi FOV circle setiap frame pakai GetMouseLocation
    -- GetMouseLocation() = viewport coords → circle center tepat di crosshair
    local mpos = getMouseViewportPos()
    aimbotFovCircle.Position = mpos         -- center circle = posisi mouse
    aimbotFovCircle.Radius   = aimbotFOV
    ...
    -- Reset accumulator di frame pertama aktif → cegah flick pertama
    ...
    elseif aimbotMode == "FreeAim" then
        aimFreeAim(target, targetPlr)
    end
end)

print("=== MAJESTY ONTOP v14.0 - VALARY THEME ===")
print("Fitur:")
print("- 📦 Inventory Tracker")
print("- ⚙️  Auto MS")
print("- 👁️  ESP + Item Dipegang + Distance")
print("- 🚀 Teleport")
print("- 🎯 AIMBOT: FOV center crosshair | FreeAim | Body Part Selector | Priority Mode")
print("=========================================")
