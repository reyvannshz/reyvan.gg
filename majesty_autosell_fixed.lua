...
do
    ...
    sellTogBtn.Size = UDim2.new(1,-20,0,36); sellTogBtn.Position = UDim2.new(0,10,0,548)
    sellTogBtn.BackgroundColor3 = C.card; sellTogBtn.Text = "AUTO SELL : OFF"
    sellTogBtn.TextColor3 = C.red; sellTogBtn.Font = Enum.Font.GothamBold
    sellTogBtn.TextSize = 13; sellTogBtn.BorderSizePixel = 0; sellTogBtn.Parent = pageAuto
    mkCorner(sellTogBtn,5); mkStroke(sellTogBtn,1,C.border)

    local ssc = makeCard(pageAuto,592,44); makeLabel(ssc,"SELL",12,0,80,44,10,C.subtext)
    sellStatusLbl_ref = makeLabel(ssc,"OFF",90,0,200,44,13,C.red)
    local sic = makeCard(pageAuto,644,44); makeLabel(sic,"ITEM",12,0,80,44,10,C.subtext)
    sellItemLbl_ref = makeLabel(sic,"-",90,0,280,44,13,C.text)
    local asC3 = makeCard(pageAuto,696,44); makeLabel(asC3,"FASE",12,0,80,44,10,C.subtext)
    local asPhaseLbl = makeLabel(asC3,"Idle",90,0,280,44,13,C.accent2)
    local asC4 = makeCard(pageAuto,748,40); makeLabel(asC4,"TERJUAL",12,0,80,40,10,C.subtext)
    local asSoldLbl  = makeLabel(asC4,"0",90,0,200,40,13,C.yellow)
...
-- Hitung item di Backpack + equipped
...
-- Equip item: coba EquipTool dulu, fallback Parent assign
local function equipMSItem(name)
    ...
    for _, t in pairs(bp:GetChildren()) do
        if t:IsA("Tool") and t.Name == name then
            ...
            if not ok then
                -- Fallback: parent langsung ke karakter
...
-- Cari ProximityPrompt dalam radius — telusuri parent chain untuk BasePart
local function findSellPrompt(pos, radius)
    local best, bestDist = nil, math.huge
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("ProximityPrompt") and obj.Enabled then
            -- Telusuri parent sampai ketemu BasePart
            local part = obj.Parent
            ...
            if part then
                ...
                elseif part:IsA("Model") then
                    -- Ambil posisi dari child BasePart pertama
...
-- Arahkan karakter ke posisi target
...
task.spawn(function()
    while true do
        ...
        -- Cari item yang ada di inventory
        ...
        if not targetItem then
            ...
            sellStatusLbl_ref.TextColor3 = C.yellow
            continue
        end

        -- Cari ProximityPrompt sell dalam radius 20 studs
        local prompt, dist = findSellPrompt(hrp.Position, 20)
        if not prompt then
            sellItemLbl_ref.Text = targetItem
            sellStatusLbl_ref.Text = "DEKATI NPC JUAL"
            sellStatusLbl_ref.TextColor3 = C.subtext
            continue
        end
        ...
        asSelling = true
        sellItemLbl_ref.Text = targetItem
        sellStatusLbl_ref.Text = "EQUIP..."
        sellStatusLbl_ref.TextColor3 = C.accent
        ...
        if promptPart and promptPart:IsA("BasePart") then
            faceTowards(promptPart.Position)
        end
        task.wait(0.2)

        -- 3. Trigger sell (3 metode fallback)
        sellStatusLbl_ref.Text = "MENJUAL..."
        local ok1 = pcall(fireproximityprompt, prompt)
        if not ok1 then
            -- Fallback 1: interact() (press & release E)
            interact()
        end
        task.wait(0.5)

        -- 4. Update sold count jika item berkurang
        if countMSItem(targetItem) == 0 then
            asSoldCount += 1
            asSoldLbl.Text = tostring(asSoldCount)
        end

        sellStatusLbl_ref.Text = "ON"
        sellStatusLbl_ref.TextColor3 = C.accent
        asSelling = false
    end
end)
